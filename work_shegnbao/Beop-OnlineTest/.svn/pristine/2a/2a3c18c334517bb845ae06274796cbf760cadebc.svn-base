...
url = "http://%s/diagnosis/fault/get/" % app.config['SERVICE_URL']
existId = [71]
wrongId = ['haha', -1, 100000000, True, None]


class Service012(unittest.TestCase):
    testCaseID = 'Service012'
    projectName = "BeopService"
    buzName = '获取诊断fault接口diagnosis/fault/get/'
    start = 0.0
    now = 0
    startTime = ""
    errors = []
        ...
        for projId in existId:
            ...
            try:
                if rv!=[]:
                    ...
                    self.errors.append(
                        "错误信息[%s]%s---访问%s接口,使用正确的projectId为%s返回结果无数据,结果为%s!" % (tool.getTime(),self.testCaseID, url + str(projId), projId,str(rv)))
            except Exception as e:
                print(e.__str__())
                self.writeLog(e.__str__())
                self.errors.append("错误信息[%s]%s---请求%s接口,返回的数据出错可能为空.!" % (tool.getTime(),self.testCaseID, url + str(projId)))
