...
url = "http://%s/diagnosis/zone/get/" % app.config['SERVICE_URL']
existId = [72, 76, 71]
wrongId = ['haha', -1, 100000000, True, None]


class Service014(unittest.TestCase):
    testCaseID = 'Service014'
    projectName = "BeopService"
    buzName = '获取诊断zone接口diagnosis/zone/get/'
    start = 0.0
    now = 0
    startTime = ""
    errors = []
        ...
        for projId in existId:
            ...
            try:
                if rv!=[]:
                    ...
                    self.errors.append(
                        "错误信息[%s]%s---访问%s接口,使用正确的projectId为%s返回结果无数据!" % (tool.getTime(), self.testCaseID,url + str(projId), projId))
        ...
        for projId in wrongId:
            ...
            try:
                if rv == []:
                    ...
                    self.errors.append(
                        "错误信息[%s]%s---访问%s接口,使用错误的projectId为%s返回结果有数据是%s!" % (tool.getTime(),self.testCaseID, url + str(projId), projId,str(rv)))
            except Exception as e:
                print(e.__str__())
                self.writeLog(e.__str__())
