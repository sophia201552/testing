...
site_url = 'http://%s/set_realtimedata_from_site' % app.config['SERVICE_URL']
histroy_url = "http://%s/get_history_data_padded_reduce" % serverip
realtime_url = "http://%s/admin/dataPointManager/search/" % serverip


class Service004(unittest.TestCase):
    testCaseID = 'Service004'
    projectName = "不针对项目"
    buzName = 'beopservice接口测试'
    start = 0.0
    now = 0
    startTime = ""
    errors = []
    ...
    def beopService(self):
        ...
        data1 = {"projId": "1", "point": ['test_123_456'], "value": ['1'],
                 'time': str(time.strftime("%Y-%m-%d %H:%M:00", time.localtime())), 'timePeriod': 'm1',
                 'waitForFinish': '0'}
        ...
        self.checkHistory(data4, result_history,histroy_data)
        # 验证周期为m5时候的值
        time_str = self.comHM()
        print(time_str)
        ...
        result_history = self.postURL(histroy_data, histroy_url)
        self.checkHistory(data5, result_history,histroy_data)
        # 验证周期为h1时候的值
        ...
        result_history = self.postURL(histroy_data, histroy_url)
        self.checkHistory(data6, result_history,histroy_data)
        # 验证周期为d1的值
        ...
        result_history = self.postURL(histroy_data, histroy_url)
        self.checkHistory(data7, result_history,histroy_data)
    ...
    # 验证历史接口返回的点值和预期的是否相等
    def checkHistory(self, data, result,histroy_data):
        tool = BeopTools()
        name = data['point'][0]
        value = data['value'][0]
        if (result is not None and result.get('data') is not None):
            ...
            tool.getTime(), self.testCaseID, histroy_url, str(histroy_data)))
    ...
    def checkRealTime(self, value1, value2):
        ...
        if value2 is not None and value2['list'] is not None:
            ...
            if (update_time):
                ...
                update = datetime.strptime(update_time, "%Y-%m-%d %H:%M")
