...
from beopWeb.mod_workflow.TransactionAttachment_v2 import TransactionAttachment
...
@bp_workflow.route('/v2/attachment/upload', methods=['POST'])
def wf_attachment_v2_upload():
...
@bp_workflow.route('/v2/attachment/getFiles/<taskId>', methods=['GET'])
def wf_attachment_v2_get_files_by_transId(taskId):
    user_id = AuthManager.get_userId()
    if not user_id:
        return Utils.beop_response_error(code=403)
...
@bp_workflow.route('/v2/attachment/delete', methods=['POST'])
def wf_attachment_v2_delete_file():
    user_id = AuthManager.get_userId()
    if not user_id:
        return Utils.beop_response_error(code=403)
...
@bp_workflow.route('/v2/attachment/deleteByName', methods=['POST'])
def wf_attachment_v2_delete_file_by_name():
    def delete_file_oss(file_name):
        oss = OssAPI(Utils.OSS_HOST, Utils.OSS_ACCESS_ID, Utils.OSS_SECRET_ACCESS_KEY)
        return oss.delete_object('beopweb', 'workflow/attachment/' + file_name)
...
@bp_workflow.route('/v2/attachment/download', methods=['POST'])
def wf_attachment_v2_download():
    user_id = AuthManager.get_userId()
    if not user_id:
        return Utils.beop_response_error(code=403)
    rq_data = request.get_json()
