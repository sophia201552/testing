...
class BEOPDataAccess:
    ...
    def getProject(self, user_id, project_code=None):
        # print('getProject')
        dbname = app.config['DATABASE']
        sql = '''select p.id, p.name_en, p.name_cn, p.latlng, p.address,p.name_english,p.pic, dtu.online,dtu.LastReceivedTime, p.is_advance, p.logo, dtu3.dtuname, ufp.user_id, p.time_format
        ...
        return [dict(id=x[0], name_en=x[1], name_cn=x[2], latlng=x[3], address=x[4], name_english=x[5], pic=x[6],
                     online=x[7], lastReceivedTime=x[8], isAdvance=x[9], logo=x[10], datadb=x[11], isFavorite=x[12], time_format=x[13]) for
                x in rv]
    ...
    def createProject(self, name_en, name_cn, s3dbname, mysqlname, latlng, address, name_english, userid, projImg, time_format,
        ...
        if projImg is not None:
            imgStr = projImg
        sql = "insert into project(id, name_en, name_cn, s3dbname, mysqlname,update_time,latlng,address,name_english,weather_station_id,pic, time_format, collectionname, is_advance, logo) values " \
              "(%d, \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', %d,\'%s\', %d, \'%s\',\'%s\', \'%s\')" % \
              (newid, name_en, name_cn, s3dbname, mysqlname, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), latlng,
               address, name_english, weatheaStationId, imgStr, time_format, mysqlname, is_advance, logo)
        if self._mysqlDBContainer.op_db_update(dbname, sql, ()):
            ...
            if self._mysqlDBContainer.op_db_update(dbname, sql, params):
                ...
                if self._mysqlDBContainer.op_db_update(dbname, sql, params):
                    tStart = datetime.strptime('2014-01-01 00:00:00', Utils.datetime_format_full)
    ...
    def updateProject(self, projId, name_en, name_cn, latlng, address, name_english, projImg, time_format, real_time_db_name=None,
                      is_advance=0, logo_img=None, dtu_name_id=None):
        dbname = app.config['DATABASE']
    ...
    def getHistoryData(self, proj, pointName, timeStart, timeEnd, timeFormat, raw=False):
        ...
        return self.mergeHisData(connList, dbname, pointName, timeStart, timeEnd, timeFormat, raw)

    def mergeHisData(self, connList, dbname, pointName, timeStart, timeEnd, timeFormat, raw=False):
        rt = []
        try:
            if isinstance(connList, list):
                        ...
                        if item.get('st') and item.get('et'):
                            ...
                            if conn:
                                item_rt = conn.getHistoryDataByFormat(dbname, pointName,
                                                                      ...
                                                                      et.strftime('%Y-%m-%d %H:%M:%S'), timeFormat, raw)
                                if item_rt:
                                    if not rt:
                                        rt = item_rt
                                    else:
                ...
                if connList:
                    rt = connList.getHistoryDataByFormat(dbname, pointName, timeStart, timeEnd, timeFormat, raw)
        except Exception as e:
    ...
    def get_project_info_by_projId(self, projId):
        ...
        try:
            ...
            #import pdb;pdb.set_trace()
            if dbrv:
                rt = {'id':dbrv[0], 'name_en':dbrv[1], 'name_cn':dbrv[2], 's3dbname':dbrv[3], 'mysqlname':dbrv[4],
    ...
    def addUserRoles(self, roleIds, userIds):
        rt = False
        dbname = app.config['DATABASE']
        sql = 'insert ignore into user_role values '
        arr = []
