...
class BEOPMongoDataAccess:
    ...
    def getHistoryDataByFormat(self,dbname, pointList, timeStart, timeEnd, timeFormat, raw=False):
            ...
            if length > 0:
                ...
                if timeFormat == 'm1':
                    ...
                    if timeFormat == 'h1':
                        ...
                        tl = []
                        startObject = self.get_nearest_start_m5_obj(startObject)
                        curTime = startObject
                        while curTime <= endObject:
                            tl.append(curTime)
                            while curTime <= endObject:
    ...
    def get_nearest_data_from_table(self, _dbname, _time, _point_name, tDistanceMinutesLimit):
        rt = None
        try:
            print(_dbname, _time, _point_name, sep=',')
...
#### 温控 app 接口 ####
    ...
    def AddNewProject(self, userId, proName, proZhName, proEnName, bindId, timeFormat):
        id = ObjectId()
        try:
            # 修改 项目名称重复，直接返回提示
    ...
    def set_iot_things(self, data):
        ...
        try:
            ...
            dbrv = self.mdbBb[g_table_iot_thing].update({'_id':data.get('_id')}, {'$set':data}, True)
            if dbrv.get('ok'):
    ...
    def del_iot_things(self, tId):
        ...
        try:
            ...
            dbrv = self.mdbBb[g_table_iot_thing].delete_many({'_id':{'$in':[ObjectId(Id) for Id in tId if ObjectId.is_valid(Id)]}})
    ...
    def FacImportProject(self, userId, project, navs, projIdMerge='', navMergeList = []):
        navIdResult =  request.get_json().get('navIdResult');
        newProjId = projIdMerge
        try:
            # 将项目信息插入到 Fac_Project 表中
            if projIdMerge == '':
                newProjId = self.AddNewProject(userId, project.get('name_en'), project.get('name_cn'), project.get('name_english'), project.get('id'), project.get('time_format'))
    ...
    def SaveToFactoryReportWrap(self, pageId, reportList, isFactory = 1):
        ...
        try:
            ...
            print('facSaveReportWrapInfo error:'+e.__str__())
            logging.error('facSaveReportWrapInfo error:'+e.__str__())
        return rt
    ...
    def FacSaveReportData(self, data):
        rs = False
        try:
            if not ('dataId' in data.keys()):
                print('存储报表数据失败：缺少 dataId')
                return False
            if not ('creatorId' in data.keys()):
                print('存储报表数据失败：缺少 creatorId')
                return False
            if not ('date' in data.keys()):
                print('存储报表数据失败：缺少 date')
                return False
            if not ObjectId.is_valid( data.get('dataId') ):
                print('存储报表数据失败：dataId 不是有效的 ObjectId 类型')
                return rs
            ...
            rs = self.mdbBb[g_table_fac_report_data].insert_one(value)
            rs = True
        except Exception as e:
