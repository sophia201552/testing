...
def analysisGetModel():
    try:
        headers = {'content-type': 'application/json'}
        data = request.get_json()
        rt = requests.post('http://121.40.188.158:5111/correlationanalysis/analysis/analysisModel', data=json.dumps(data), headers=headers)
    except Exception as e:
...
def benchmarkGetdiagnosis(projectId):
    ...
    try:
        ...
        if dbname:
            strSQL = "SELECT f.Name, f.Description, SUM(n.Energy) FROM %s_diagnosis_faults as f " \
                     "INNER JOIN (SELECT FaultId, SUM(Energy) as Energy FROM `%s_diagnosis_notices` " \
                     "where Time <= '%s' and Time >= '%s' group by FaultId ) as n on f.Id = n.FaultId " \
                     "GROUP BY f.Name ORDER BY SUM(n.Energy) DESC"%(dbname, dbname, timeStart, timeEnd)
