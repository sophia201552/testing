...
def getDumpStringByBool(b, msg='', data=None):
    if b:
        rt = {"status": 'OK'}
    else:
        rt = {"status": 'ERROR'}
...
def facPublishToNewProject():
    ...
    if facCheckProjName(projName) == False:
        return jsonify(getDumpStringByBool(False, 'Project name exited'))
    if sourceId:
        # 获取离线项目数据
        sourceProjDetailList = MongoConnManager.getConfigConn().GetProjectFromIdLists([sourceId])
        if len(sourceProjDetailList) == 0:
            return jsonify(getDumpStringByBool(False, 'failed to get offline project data'))
        ...
        if targetId:
            ...
            return jsonify(getDumpStringByBool(False, 'error occurs when inserting new project'))
        ...
        if MongoConnManager.getConfigConn().RemoveProjectPublish(targetId):
            ...
            if MongoConnManager.getConfigConn().SaveToObNavigation(targetId, page_id_list):
                ...
                return jsonify(getDumpStringByBool(False, 'failed to save offline project page list to online list'))
            # 将 id 转换成 ObjectId
            for item in pageList:
                item.update({'_id':ObjectId(item.get('_id'))})

            if not MongoConnManager.getConfigConn().SaveToObPage(pageList):
                return jsonify(getDumpStringByBool(False, 'failed to save Factory page data'))

            # 绑定关联项目
            if not MongoConnManager.getConfigConn().factoryBindOnlineProject(sourceId, targetId):
                return jsonify(getDumpStringByBool(False, 'failed to match online project ID'))
        else:
            return jsonify(getDumpStringByBool(False, 'error occurs when clearing existing project data'))
    else:
        return jsonify(getDumpStringByBool(False, 'offline project ID can not be blank'))

    return jsonify(getDumpStringByBool(True, 'publish project and match to online project successfully', {'bindId': targetId}))
...
def factoryGetPageInfo(pageId):
    ...
    if page is None:
        return jsonify(getDumpStringByBool(False, 'failed to get page data'))
    page.update({'_id': page.get('_id').__str__()})

    return jsonify(getDumpStringByBool(True, '', page))
...
def factoryPublishWithMerging():
    ...
    if validate is None:
        return jsonify(getDumpStringByBool(False, 'Wrong password!'))
    ...
    if targetId == None or targetId == '':
        ...
        return jsonify(getDumpStringByBool(False, 'Target online project is not exist'))
    ...
    if sourceId:
        ...
        if roleIds is None:
            return jsonify(getDumpStringByBool(False, 'There isn\'t a "factory publish role group" in target online project!'))
        ...
        if rt == False:
            return jsonify(getDumpStringByBool(False, 'Occur an error when add users to online project role group.'))
...
def factoryGetPageList(projId, isFactory=0):
    ...
    return jsonify(getDumpStringByBool(rt, msg, data))
...
def factoryMaterialSave():
    ...
    #     if len(findList) > 0:
    #         return jsonify(getDumpStringByBool(False, 'Template name already exists'))
...
def factoryMaterialGetByGroupId(type=None, groupId=None):
    ...
    return jsonify(getDumpStringByBool(True, '', data))
...
def factoryMaterialSearch():
    ...
    return jsonify(getDumpStringByBool(True, '', rs))
...
def factoryRecycleGet(status=1):
    ...
    if data:
        rt = True
    return jsonify(getDumpStringByBool(rt, 'get recycle success!', data))
...
def factoryGetReportDataList():
    ...
    return jsonify(getDumpStringByBool(True, '', rt))
...
def factoryGetReportDataById(dataId, date):
    ...
    if rt:
        return jsonify(getDumpStringByBool(True, '', rt))
    return jsonify(getDumpStringByBool(False, 'failed to acquire data！'))
...
@bp_factory.route('/report/saveDynamicData', methods=['POST'])
def factoryReportSaveDynamicData():
    rt = None
    # 保存动态报表数据
    rt = MongoConnManager.getConfigConn().FacSaveReportData(date)
    if rt:
        return jsonify(getDumpStringByBool(True, '', rt))
    return jsonify(getDumpStringByBool(False, 'failed'))
...
def factoryTest():
   ...
   return jsonify(getDumpStringByBool(True))
