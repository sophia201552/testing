(function (beop) {
    var configMap = {
            htmlURL: '/static/views/workflow/memberSelected.v2.html',
    ...
    init = function ($container, options) {
        ...
        if (typeof options === 'object') {
            ...
            if (typeof configModel === 'object') {
                ...
                var validUsers = [];
                for (var i = 0; i < configMap.userHasSelected.length; i++) {
                    var selectedUser = configMap.userHasSelected[i];
                    if (selectedUser) {
                        validUsers.push(selectedUser);
                    }
                }
                configMap.userHasSelected = validUsers;
                configMap.userHasSelected.forEach(function (item, index, array) {
                    if (item) {
                        configModel.userMemberMap.forEach(function (members) {
                            if (item == members.id || item.id == members.id) {
        ...
        WebAPI.get(configMap.htmlURL + '?=' + new Date().getTime().toString(16)).done(function (html) {
            ...
            if (options && options.configModel) {
                options.configModel.hasFrame ? stateMap.hasFrame = true : stateMap.hasFrame = false;
                if (options.configModel.userReadOnly) {
                    stateMap.userReadOnly = true;
                    $title.attr('i18n', 'workflow.memberSelected.TITLE1');
                }
            } else {
                stateMap.hasFrame = false;
                stateMap.userReadOnly = false;
    ...
    bindEvents = function () {
        ...
        jqueryMap.$add_user_dialog.off().on('show.bs.modal', function () {
            ...
            if (stateMap.hasFrame) {
                jqueryMap.$wf_check_result.empty().html(beopTmpl('tpl_wf_dialog_added_member', {members: stateMap.userHasSelectedDefault}, document.getElementById("tpl_wf_dialog_added_member").innerHTML))
            } else {
    ...
    renderAddedUsers = function () {
        if (stateMap.hasFrame) {
            jqueryMap.$wf_added_user_list.html(beopTmpl('tpl_wf_dialog_added_member', {members: stateMap.userHasSelectedDefault}, document.getElementById("tpl_wf_dialog_added_member").innerHTML));
        } else {
    ...
    renderCheckedUsers = function (data) {
        ...
        if (stateMap.hasFrame) {
            jqueryMap.$wf_check_result.html(beopTmpl('tpl_wf_dialog_added_member', {members: data}, document.getElementById("tpl_wf_dialog_added_member").innerHTML))
        } else {
    ...
    progressResultData = function () {
        ...
        if (stateMap.hasFrame) {
            ...
            }, document.getElementById("tpl_wf_member").innerHTML));
        } else {
            jqueryMap.$wf_search_result.html(beopTmpl('tpl_wf_member', {
                members: stateMap.allUserList,
                addUserMap: addedUserMap
    ...
    onClickUser = function () {
        if (stateMap.userReadOnly) {
            alert(I18n.resource.workflow.memberSelected.INFO_READONLY);
            return;
        }
