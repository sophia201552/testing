...
var WorkflowInsert = (function () {
    ...
    WorkflowInsert.prototype = {
        ...
        _loadDataNormal: function () {
            ...
            this.jqueryMap.$modalBody.empty().html(beopTmpl('tpl_fault_new_order', {
        ...
        _loadDataNormalSpecial: function () {
            ...
            this.jqueryMap.$modalBody.empty().html(beopTmpl('tpl_fault_group_add'));
            $dfd.resolve();
            return $dfd;
        },
        //noop
        ...
        _addUserSelectDialog: function (ev) {
            ...
            if (this.isUserHasGroup) {
                $modalBody.empty().html(beopTmpl('tpl_workInsert_add_person'), {});
        ...
        _openUserSelectDialog: function (type) {
            ...
            if (this.isUserHasGroup) {
                ...
                beop.view.memberSelected.init(_this.jqueryMap.$container, beop.model.stateMap.groupId, 'fault_order');
            } else {
                beop.view.memberSelected.configModel({
                ...
                beop.view.memberSelected.init(_this.jqueryMap.$container, null, 'fault_order');
            }
        },
        //没有任务组的添加人物回调
        _renderAddedUsersSpecial: function () {
            return function (addedUserList) {
                ...
                this.jqueryMap.$container.find("#wf-workflow-memberList").html(beopTmpl('tpl_wf_added_member', {members: this.stateMap.userSelectGroupList}));
                //恢复modal类型
                this.modalType = 'default';
            }.bind(this);
        },
        ...
        _renderAddedUsersNormal: function (type) {
            return function (addedUserList) {
                ...
                this.jqueryMap.$modalBody.find(".wf-detail-userInfoWrapper").each(function (index, item) {
                    ...
                    $item.html(beopTmpl('tpl_wf_added_member_personal', {
        ...
        _uploadFile: function () {
            Spinner.spin(ElScreenContainer);
            var _this = this;
            var fileUploadInstance = new WfFileUpload(false, this.pendingFiles.length);
        ...
        _newTaskGroupSubmit: function () {
            ...
            WebAPI.post('/workflow/group/new/' + AppConfig.userId, formObject).done(function (result) {
                ...
                Alert.danger(_this.jqueryMap.$container.get(0), err.msg).showAtTop(1000);
            })
        },
        //新的工单提交事件
                    ...
                    if (_this.pendingFiles && _this.pendingFiles.length) {
                        ...
                        _this._submitFileUpload(result.data).done(function (uploadResult) {
                            if (uploadResult && uploadResult.success) {
                                ...
                                _this.eventCbList.submitSuccess && _this.eventCbList.submitSuccess.call(_this, _this.pendingFiles);
                            } else {
                                Alert.danger(_this.jqueryMap.$container.get(0), I18n.resource.workflow.task.ATTACHMENT_FILE_SUCCESS_INFO).showAtTop(2000);
                            }
                        }).fail(function () {
                        ...
                        _this.eventCbList.submitSuccess && _this.eventCbList.submitSuccess.call(_this, null);
                    }
                } else {
        ...
        _submitData: function () {
            var data = this._getSubmitData(), _this = this;
            return WebAPI.post('/workflow/transaction/new/', data);
        },
        ...
        _getSubmitData: function () {
            var formObject = this.jqueryMap.$container.find('#wf-detail-form').serializeObject();
            var param = {
                chartPointList: this.insertData.points,
                ...
                noticeId: this.insertData.id
            };
            return $.extend(param, formObject);
        },
        //如果用户没有任务组的情况下绑定事件
