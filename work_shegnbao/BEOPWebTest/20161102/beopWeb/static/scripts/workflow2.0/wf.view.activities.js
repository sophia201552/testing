(function (beop) {
    var configMap = {
        ...
        renderActivities,
        onChangeActivities,
        onReplyFocusout, onReplyFocusin, onReplySubmit, onTaskClick, onUserClick, onGetMoreActivitiesClick, bindImgZoom, imgZoom, getLimitData;
    ...
    renderActivities = function () {
        if (beop.model.stateMap.activities.length > 0) {
            ...
            jqueryMap.$wf_activity_container.html(beopTmpl('tpl_wf_activity_empty', {
                stateMap: stateMap,
                showPosition: beop.model.stateMap.activity_type >= 4,
                noData: beop.model.stateMap.activity_type < 4
            }));
            Spinner.stop();
        }
...
//加载更多
    onGetMoreActivitiesClick = function () {
        configMap.activities_model.getActivities(beop.model.stateMap.activity_type, configMap.activities_number).done(function (result) {
...
//---------方法---------
    getLimitData = function (limt) {
        var activities = beop.model.stateMap.activities;
        if (!limt) {
            return activities;
        }
        if (activities && activities.length) {
            if (activities.length <= configMap.activities_number) {
                return activities
            } else {
                jqueryMap.$wf_activity_getMore.parent().show();
                return activities.splice(0, configMap.activities_number);
            }
        }
        return [];
    };
