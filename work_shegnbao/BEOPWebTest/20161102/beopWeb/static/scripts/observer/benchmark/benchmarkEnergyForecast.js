    ...
    attEvents(){
        ...
        $('.timeSelsect').off('change').change(function(){
            ...
            if ($this.val() === 'M1') {
                ...
                start = new Date().timeFormat(timeFormatStr);
                end = new Date().timeFormat(timeFormatStr);
                minView = 3;
                startView = 3;
                
            }else if($this.val()==='h1'){
                timeFormatStr = timeFormatChange('yyyy-mm-dd hh')+' :00';
                start = new Date().format('yyyy-MM-dd').toDate().timeFormat(timeFormatStr);
                end = new Date().timeFormat(timeFormatStr);
                minView = 1;
                startView = 2;
            } else {
                timeFormatStr = timeFormatChange('yyyy-mm-dd');
                start = new Date(new Date().valueOf() - 86400000).timeFormat(timeFormatStr);
                end = new Date().timeFormat(timeFormatStr);
                minView = 2;
                startView = 2;
            }
        ...
        function historyForest(startTimes,endTimes,isRepeat){
            ...
            WebAPI.post('/analysis/startWorkspaceDataGenHistogram',postData).done(result=>{
                if(new Date(judgeStartTime).valueOf()>new Date().valueOf()){
                    ...
                    if(result.timeShaft.length===0){
                        ...
                        for (var i = 0; i < result.timeShaft.length; i++) {
                            ...
                            if (period == 'd1') {
                                monthArr.push(mothArrRep[i].split(' ')[0]);
                            } else if (period == 'M1') {
                                rmonthArr.push(mothArrRep[i].split('-')[0] + '-' + mothArrRep[i].split('-')[1]);
                            } else {
                                monthArr.push(mothArrRep[i].split(':')[0] + ':' + mothArrRep[i].split('-')[1]);
                            }
                        }
                        if (isRepeat) {
                            monthArr = monthArrCount;
                ...
                if(!isRepeat){
                    ...
                    $('#modalConfigStartTime').val(startTimeCogVal);
                    $('#modalConfigEndTime').val(endTimeCogVal);
        ...
        $('#forestConfig').off('click').click(function(){
            ...
            var startTimeCogVal = timeFormat($('#startTimeCog').val(),'yyyy-mm-dd hh:ii');
            var endTimeCogVal = timeFormat($('#endTimeCog').val(),'yyyy-mm-dd hh:ii');
            //var startYear = parseInt(startTimeCogVal.split('-')[0]);
            //var startMonth = parseInt(startTimeCogVal.split('-')[1]);
            //var startDay = parseInt(startTimeCogVal.split(' ')[0].split('-')[2]);
            //var endYear = parseInt(endTimeCogVal.split('-')[0]);
            ...
            //}
            if(currentModal) {
                if($('#configForestModal').length!==0){$('#configForestModal').remove()}
                if($('#configForestModal').length===0) {
                    ...
                    historyForest(startTimeCogVal,endTimeCogVal);
        ...
        $('#savaParams').off('click').click(function(){
            ...
            if(period ==='d1'){
                modalPeriod = 'd';
                if(_this.importSuccess){
                    startTime = startTimeCogVal;
                    endTime = endTimeCogVal;
            ...
            for(var i = 0;i<tableCell.length;i++){
                ...
                var currentVal = $current.find('.paramsShow').html().trim();
                if(currentVal===''){
            ...
            for(var i = 0;i<tableCell.length;i++){
                var $current = tableCell.eq(i);
                var currentVal = parseInt($current.find('.paramsShow').html().trim());
