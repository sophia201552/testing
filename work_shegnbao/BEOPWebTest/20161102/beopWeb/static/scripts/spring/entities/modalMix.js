    ...
    ModalMix.prototype.renderModal = function (e) {
        ...
        if (displaySlider) {
            ...
            if(AppConfig.isMobile){
                $(_this.container).append('<style>.carousel-indicators .active{background-image: -webkit-radial-gradient(center,#fff 0%,#F7B702 80%,#fff 85%);border: none;}.carousel-indicators li{border-color:#aaa}</style>');
            }
        }
            ...
            for (var i = 0, item; i < _this.screen.store.layout.length; i++) {
                if (displaySlider) {
                    ...
                    for (var z = 0; z < _this.screen.store.layout[i].length; z++) {
                        ...
                        var $sliderDot = $('<li data-target="#carousel_' + carouselTime.getTime() + '" data-slide-to="' + index + '" style="border-color:#aaa;box-shadow:1px rgba(0,0,0,.05);margin:1.5px 3px;"></li>')
    ...
    ModalMix.prototype.modalInit = function(){
        var configModalOpt = {
            ...
            "area" : [
                {
                    "type": 'option',
                    "widget":[{type:'text',name:'组合图轮播(s)', data: this.entity.modal.option.displayInterval,opt:{attr:{class:'inlineBlock iptInterval',placeholder:'时间间隔'}}}]
                },{
                    "module" : "colorConfig",
                    "data": this.entity.modal.option.bgColor
                },
                {
                    'type':'footer',
            ...
            result:{func: data => {
                this.entity.modal.option.displayInterval = data.displayInterval;//轮播时间间隔
                this.entity.modal.option.bgColor = data.bgColor;//背景颜色
                this.renderModal();
            }}
        }
