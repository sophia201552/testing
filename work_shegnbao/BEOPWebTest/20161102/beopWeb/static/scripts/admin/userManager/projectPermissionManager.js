...
var ProjectPermissionManager = (function () {
    ...
    ProjectPermissionManager.prototype = {
        ...
        attachEvents: function () {
            ...
            }).off('click', '.delete-btn').on('click', '.delete-btn', function () { //点击删除项目按钮
                var $this = $(this);
            ...
            $('#saveBenchmarkMenu').click(function () {
                ...
                Spinner.spin($benchmarkSet.find('.modal-content').get(0));
        ...
        loadProjectPermission: function () {
            ...
            return WebAPI.post('/admin/loadProjectPermission', {'userId': AppConfig.userId}).done(function (result) {
                if (result.success) {
                    ...
                    if (!_this.currentProjectId) {
                        _this.renderProject(_this.pageViewModel.projectList[0]);
                        _this.currentProjectId = _this.pageViewModel.projectList[0].id;
                    } else if (_this.currentProjectId == 'all') {
                        _this.renderAllProject();
                    } else {
                        _this.renderProject(_this.getProjectById(_this.currentProjectId));
                    }
                    _this.renderSelectProject();
        ...
        renderSelectProject: function () {
            $('#selectProject').html(beopTmpl('selectProjectListTmpl', {
                projectList: this.pageViewModel.projectList,
