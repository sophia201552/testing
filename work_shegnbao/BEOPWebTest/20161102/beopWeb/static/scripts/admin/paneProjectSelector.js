    ...
    PaneProjectSelector.prototype = {
        show: function (selectorType) {
            ...
            if (AppConfig.skipProjectSelector) {//仅有一个项目,不仅地图直接打开项目
                ...
                this.initProject(project.id, project.name_en, StringUtil.getI18nProjectName(project), project.logo).done(function (result) {
        ...
        setLogo: function () {
            ...
            if (localStorage.getItem('systemSkin_' + AppConfig.userId) == 'dark') {
        ...
        init: function () {
            ...
            var showNextMsg = function (msg) {
                ...
                html +=
                    '</div>' +
                    '<div class="fl tr wdr3 messageRight"><div class="messageTime">' + timeFormat(msg.messageInfo.time, timeFormatChange('yyyy-mm-dd hh:ii:ss')) + '</div>' +
            ...
            $(document).off('click.infoBoxMessage').on("click.infoBoxMessage", function (e) { //点击页面其它地方，隐藏消息列表
                var $target = $(e.target);
        ...
        initProject: function (projectId, projectEnName, projectName, pageId, logo) {
            var _this = this;
            return WebAPI.get("/get_plant_pagedetails/" + projectId + "/" + AppConfig.userId).done(function (result) {
                ...
                if (loginCode && loginCode.toLowerCase() == 'hzbt') {//华滋奔腾需要把项目名显示在左侧
                    ...
                    if (logo && logo != 'None') {
                        $('#navHomeLogo span').attr("style", "width: 78px; height: 32px;background-image: url(" + logo + ") !important;");
                    }
                } else {
                    $('#projectMenu').text(projectName).attr('title', projectName).show();
                }
        ...
        initNav: function () {
            ...
            function showSubDropDownEvent(e) {
                var $this = $(this), $ul = $(this).closest('.dropdown-submenu').find("ul");
