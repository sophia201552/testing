                ...
                //}else {
                    ...
                    WebAPI.post('/workflow/transaction/complete/', postData).done(function (result) {
                        if (result.success) {
                            ...
                            window.plugins && window.plugins.toast.show(I18n.resource.appDashboard.workflow.COMPLETE_SUCCESS, 'short', 'center');
                            ScreenManager.show(WorkflowDetail, {id: WkConfig.Id})
                        } else {
                            //new Alert($(AlertContainer), "danger", I18n.resource.appDashboard.workflow.COMPLETE_FAIL).show().close();
                            window.plugins && window.plugins.toast.show(I18n.resource.appDashboard.workflow.COMPLETE_FAIL, 'short', 'center');
                        }
                    }).fail(function () {
                        //new Alert($(AlertContainer), "danger", I18n.resource.appDashboard.workflow.COMPLETE_FAIL).show().close();
                        window.plugins && window.plugins.toast.show(I18n.resource.appDashboard.workflow.COMPLETE_FAIL, 'short', 'center');
