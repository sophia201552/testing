...
from beopWeb.mod_tag.Tag import Tag
...
class ProjectTags:
    ...
    def get_project_tags_detail(self):
        tag_tree = self.get_project_tags()
        tag_id_map = Tag.get_tags_id_map()

        for item in tag_tree:
            item['name'] = tag_id_map.get(str(item.get('tag_id'))).get('name')
