        ...
        try:
            ...
            while n >= 0:
                ...
                if num == limit:
                    ...
                    n = -1
            if ids_excess:
                dbrv = cnn.mdbBb[collection].insert_many(ids_excess)
            rt = True
        except Exception as e:
        ...
        try:
            ...
            while n >= 0:
                ...
                if num == limit:
                    ...
                    n = -1
            if ids_excess:
                dbrv = cnn.mdbBb[collection].delete_many({'_id':{'$in':ids_excess}})
            if data_diff:
                for i in data_diff:
                    dbrv = cnn.mdbBb[collection].update({'_id':i.get('_id')}, {'$set':{'name':i.get('name')}})
            rt = True
        except Exception as e:
        ...
        try:
            ...
            for item in cursor:
                rt.append({'_id': item.get('_id').__str__(), 'name': item.get('name').__str__(),
                           'tag': item.get('tag'),
                           'Prt': item.get('Prt').__str__(), 'path': self._switch_path_in_thingsTree(item.get('path')), 'type': item.get('type')})
        except Exception as e:
            ...
            if ObjectId.is_valid(things_id):
                ...
                if dbrv:
                    ...
                    if cls.name_disambiguation(projId, name_new, Prt):
                        res = cnn.mdbBb[collection].update({'_id':ObjectId(things_id), 'type':'group'}, {'$set':{'name':name_new}})
                        if res:
                            if res.get('ok') and res.get('nModified') == 1:
                                dbres = cnn.mdbBb[collection].update({que:name_old}, {'$set':{que:name_new}}, multi=True)
                                if dbres:
                                    if dbres.get('ok'):
                                        rt = name_new
                            else:
        ...
        try:
            ...
            while n >= 0:
                ...
                if len(res) == limit:
                    ...
                    n = -1
            cnn = MongoConnManager.getConfigConn()
            dbrv = cnn.mdbBb[collection].update({'_id':{'$in':thing_list}}, {'$set':{'path':path, 'Prt':ObjectId(Prt)}}, multi = True)
            dbrv = cnn.mdbBb[collection].update({'_id':{'$in':group_list}}, {'$set':{'path':path, 'Prt':ObjectId(Prt)}}, multi = True)
