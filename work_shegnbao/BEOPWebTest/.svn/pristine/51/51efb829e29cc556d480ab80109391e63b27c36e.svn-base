(function (beop) {
    ...
    var filterCheckedUser, searchByFirstName, getPYLocalStorage, getZH_EN, addActiveSearchBtn, setPYMap, searchByInput, showSearchByID, resetData,
        renderAddedUsers, onClickUser, onClickRemoveUser, removeMember, addMember, isMemberAdded, checkUserSelectedAmount, restoreSearchResult;
    ...
    isPYStored = function () {
        return !!localStorage.getItem(configMap.KEY_PY_STORY);
    };
    ...
    progressResultData = function (result) {
        ...
        getPYLocalStorage().done(function (pin_yin) {
            setPYMap(pin_yin);
        });
    ...
    setPYMap = function (Pin_Yin) {
        var format, key;
        for (key in stateMap.usersMap) {
            if (stateMap.usersMap.hasOwnProperty(key)) {
                //stateMap.PYMap[key] = {};
                format = getZH_EN(Pin_Yin, stateMap.usersMap[key].userfullname.charAt(0));
                //stateMap.PYMap[key].pinyin = format.pinyin;
                //stateMap.PYMap[key].acronym = format.acronym;
                stateMap.PYMap.push({
                    id: key,
    ...
    getPYLocalStorage = function () {
        if (!isPYStored()) {
            return $.getJSON('/static/scripts/workflow2.0/wf.pinyin.json').done(function (result) {
                if (result) {
                    window.localStorage.setItem(configMap.KEY_PY_STORY, JSON.stringify(result));
                }
            })
        } else {
            var pin_yin = JSON.parse(window.localStorage.getItem(configMap.KEY_PY_STORY));
            return $.Deferred().resolve(pin_yin);
        }
    };
    //获取拼音
    getZH_EN = function (PinYin, first_letter) {
        if (typeof first_letter != 'string') return false;
        var _name = '',
            _char = '',
            _firstChar = '',
            _reg = /[a-z0-9A-Z\- ]/;

        for (var i = 0, len = first_letter.length; i < len; i++) {
...
//取得单个汉字拼音
    ...
    checkUserSelectedAmount = function () {
        ...
        } else if (configMap.maxSelected == null) {
            return true;
        }
    };
    ...
    searchByFirstName = function () {
        ...
        if ($this.index() == 0) {
            ...
            for (i = 0; i < stateMap.PYMap.length; i++) {
                if (stateMap.PYMap[i].pinyin.charAt(0).toLowerCase() == searchIndex || stateMap.PYMap[i].acronym.charAt(0).toLowerCase() == searchIndex) {
