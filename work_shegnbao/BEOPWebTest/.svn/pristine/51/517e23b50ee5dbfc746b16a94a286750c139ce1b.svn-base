(function (exports) {
    ...
    // 报表模板参数接口 - 开始
    // 获取模板中的模板参数
    PagePanel.prototype.getTplParams = function () {
        //_gTplContent
        ...
        options.forEach(function(row){
            var str;
            if(row.type === 'HtmlText' || row.type === 'CanvasText' || row.type === 'HtmlButton'){
                str = row.option.text;
            }else if(row.type === 'HtmlContainer'){
                str = row.option.html +  row.option.css + row.option.js;
            }
    ...
    // 应用
    PagePanel.prototype.applyTplParams = function (params) {
        try{
            var widgets,reg,strNew;
            var pages = this._gTplContent.page.length;
            if(pages === 1){
                ...
                for(var i in params){
                    ...
                    widgets.forEach(function(row){
                        if(row.type === 'HtmlText' || row.type === 'CanvasText' || row.type === 'HtmlButton'){
                            row.option.text = row.option.text.replace(reg, strNew);
                        }else if(row.type === 'HtmlContainer'){
                            row.option.html = row.option.html.replace(reg, strNew);
                            row.option.css = row.option.css.replace(reg, strNew);
                            row.option.js = row.option.js.replace(reg, strNew);
                        }
                    })
                }
            }else if(pages > 1){
                ...
                for(var i in params){
                    ...
                    strNew.forEach(function(row,j){
                        var index = (len/pages)*(j+1);
                        if(index > widgets.length){
                            return;
                        }
                        for(var k = index - len/pages; k<index;k++){
                            if(widgets[k].type === 'HtmlText' || widgets[k].type === 'CanvasText' || widgets[k].type === 'HtmlButton'){
                                widgets[k].option.text = widgets[k].option.text.replace(reg, strNew[add]);
                            }else if(widgets[k].type === 'HtmlContainer'){
                                widgets[k].option.html = widgets[k].option.html.replace(reg, strNew[add]);
                                widgets[k].option.css = widgets[k].option.css.replace(reg, strNew[add]);
                                widgets[k].option.js = widgets[k].option.js.replace(reg, strNew[add]);
                            }
                        }
                        add++;
                    });
                    if(add*(len/pages) < len){
                        for(var m = add*(len/pages); m<len;m++){
                            if(widgets[m].type === 'HtmlText' || widgets[m].type === 'CanvasText' || widgets[m].type === 'HtmlButton'){
                                widgets[m].option.text = widgets[m].option.text.replace(reg, strNew[add-1]);
                            }else if(widgets[m].type === 'HtmlContainer'){
                                widgets[m].option.html = widgets[m].option.html.replace(reg, strNew[add-1]);
                                widgets[m].option.css = widgets[m].option.css.replace(reg, strNew[add-1]);
                                widgets[m].option.js = widgets[m].option.js.replace(reg, strNew[add-1]);
                            }
                        }
                    }
                }
            ...
            var data = {
                pages: this._gTplContent.page,
                layers: this._gTplContent.layers,
                widgets: this._gTplContent.widgets
            };
            this.replacePageSave(data);
        }catch (e){
            console.log('应用模板失败');
        }
    };

    PagePanel.prototype.replacePageSave = function (data){
        // 同步pageList
        this.treeView.syncPageList();
        // 保存
        if(data){
            this.save(data);
        }else{
    ...
    PagePanel.prototype.entity = {modal: {}};
...
(function () {
    ...
    TreeView.prototype.operatePage = function (url,data) {
        WebAPI.post(url + this.projectId, data).done(function(rs){
            if (rs.status === 'OK'){
    ...
    TreeView.prototype.callbackAddPage = function (pageCfg) {
        if (_this.treeObj) {
            ...
            switch (pageCfg.type) {
                case 'PageScreen':
                    newNode = $.extend(false, newNode, {layerList:[], width: pageCfg.width, height: pageCfg.height, display:pageCfg.display, option:pageCfg.option});
                    pageData = $.extend(false,Data,{layerList:[], width: pageCfg.width, height: pageCfg.height, display:pageCfg.display, option:pageCfg.option});
                    break;
            ...
            _this.operatePage('/factory/page/save/',saveDate);
    ...
    TreeView.prototype.callbackAddPageBatch = function (arrPageCfg,isCopy) {
        ...
        _this.screen._gTplContent = {
            page:arrPage,
            layers:arrLayer,
            widgets:arrWidget
        };
        ReportTplParamsConfigModal.setOptions({
            modalIns: _this.screen,
            container: 'panels'
        });
        ...
        function renderPage(pageCfg){
            if (_this.treeObj) {
                ...
                var newNode = {
                    ...
                    layerList: pageCfg.page.layerList,
    ...
    TreeView.prototype.importTemplate = function () {
        ...
        MaterialModal.show([{'title':'Template',data:['Page.PageScreen','Page.EnergyScreen']}], function (data,isCopy) {
            if (data.length) {
    ...
    TreeView.prototype.exportTemplate = function () {
        ...
        } else if (node.type === 'EnergyScreen') {
            data.type = 'page.EnergyScreen';
