...
var Internationalization = (function () {
    ...
    Internationalization.prototype = {
        ...
        setDefautFont: function (strFont) {
            if (!strFont) {
                if (/OS/.test(window.jscd.os)) strFont = 'Helvetica';
                if (window.jscd.os == 'windows') strFont = '微软雅黑';
            }
            if(strFont) document.getElementsByTagName('body')[0].style.fontFamily = strFont;
        },
