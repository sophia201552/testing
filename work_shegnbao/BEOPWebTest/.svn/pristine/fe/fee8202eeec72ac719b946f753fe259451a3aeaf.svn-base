    ...
    EnergyScreen.prototype = {
        ...
        refreshData: function (e) {
            ...
            if (!e.data.error) {
                ...
                for (var key in _this.listEntity) {
                    ...
                    if(entity.entity.modal.type =="ModalNone")continue;
                    arrUpdataData = [];
                    for (var i = 0, iLen = e.data.length; i < iLen; i++) {
        ...
        saveLayout: function () {
            ...
            for (var i = 0; i < this.arrEntityOrder.length; i++) {
                if (this.listEntity[this.arrEntityOrder[i]].entity.modal.type != 'ModalNone' && (!this.listEntity[this.arrEntityOrder[i]].entity.modal.points || this.listEntity[this.arrEntityOrder[i]].entity.modal.points.length == 0))continue;
