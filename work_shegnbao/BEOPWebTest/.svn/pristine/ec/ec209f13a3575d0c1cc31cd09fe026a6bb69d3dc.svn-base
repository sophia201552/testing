...
class TransactionEmail:
    ...
    def get_trans_info(self, trans_id):
        ...
        if trans:
            trans_info["creatorName"] = user.query_user_by_id(trans.get('creatorID'), 'userfullname').get(
                "userfullname")
            trans_info["createTime"] = trans.get("createTime").strftime('%Y-%m-%d %I:%M:%S %p')
            ...
            trans_info["due_date"] = trans.get("dueDate").strftime('%Y-%m-%d')
        return trans_info
    ...
    def get_send_status_email_subject(self, i18n, status, user_email, verifier_email_list, trans_title, trans_id,
        ...
        if status == 'complete':
            if user_email in verifier_email_list:
                return str(i18n.trans('complete_email_subject_need_verify')) + user_name + str(
                    i18n.trans("action_status_" + str(
                        status))) + trans_id + TransactionEmailUtils.emailSubjectPrefix + trans_title
            else:
                return user_name + str(
                    i18n.trans(
                        "action_status_" + trans_id + str(
                            status))) + str(
                    i18n.trans('work_order')) + TransactionEmailUtils.emailSubjectPrefix + trans_title
        else:
            return user_name + str(
                i18n.trans("action_status_" + str(status))) + trans_id + str(
                i18n.trans('work_order')) + TransactionEmailUtils.emailSubjectPrefix + trans_title

    # 发送状态邮件
    # 审核人，创建人，相关人员，执行人，不给操作本人发送邮件
    def send_status_email(self, trans, tm, trans_id, user_id, status, lang='zh-CN'):
        verifiers_id_list = []
        watchers_id_list = []
        tr = Transaction()
    ...
    # 发送给执行人和审核人和相关人员
    def send_task_email(self, task_id, request_json, pending_files, lang="zh-CN", email_type='NEW'):
        user = User()
        ...
        executor_email_subject = email_tip + TransactionEmailUtils.emailSubjectPrefix + title
        if executor_email:
            try:
        ...
        for watcher in watcher_list:
            if watcher is not None:
                ...
                watcher_email_list.append(watcher_info.get('useremail'))
        ...
        for watcher_mail in list(set(watcher_email_list)):
            # 如果是空或者和执行人，创建人的邮箱重复
            if not watcher_mail or watcher_mail == executor_email or watcher_mail == creator_email:
                continue
            try:
                email_html = render_template('email/workflowEmail.html', configMap=watcher_email_map,
                                             trans=i18n.trans)
                Utils.EmailTool.send_email(watcher_email_subject, watcher_mail, email_html)
            except Exception as e:
                continue
        return True
    ...
    def send_edit_email(self, executor_old, verifier_id_db_list, watcher_id_db_list, request_json, record_detail,
        ...
        i18n = I18n()
        i18n.set_lang(lang)
        edit_email_subject = str(i18n.trans("edit_email_subject", edit_user=user_name)) + \
                             TransactionEmailUtils.emailSubjectPrefix + \
    ...
    # 回复工单任务
    # 审核人，创建人，相关人员，执行人
    def send_replay_email(self, req_data, trans_id, detail, lang='zh-CN'):
        trans = Transaction()
        ...
        replay_email_subject = i18n.trans("replay_email_subject", reply_user=reply_user_info.get(
            'userfullname')) + TransactionEmailUtils.emailSubjectPrefix + transaction_info.get('title')
        # 检查是否是本人添加
    ...
    def send_forward_email(self, user_id, trans_id, old_executor, current_executor,
        ...
        send_forward_email_subject = str(
            i18n.trans("send_forward_email_subject", forward_user=user_info.get(
                'userfullname'))) + TransactionEmailUtils.emailSubjectPrefix + trans_info.get(
            'title')
    ...
    def send_week_report(self, user_info, lang='zh-CN'):
        user = User()
