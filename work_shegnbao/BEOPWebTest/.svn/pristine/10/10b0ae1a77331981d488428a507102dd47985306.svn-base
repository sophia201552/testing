...
var AnlsWorkspace = (function () {
    ...
    AnlsWorkspace.prototype = {
                    ...
                    if (temp.modalList && temp.modalList.length > 0) {
                        for (var j in temp.modalList) {
                            ...
                            if(tempData.imagebin){
                                ...
                                //var tempImg = parse(localStorage.getItem('ws_'+ this.screen.store.workspaces[i].id));
                                //tempImg = tempImg['img_'+tempData.id];
                                $divPage.css({'background-image':'url('+ localStorage.getItem('ws_' + this.screen.store.workspaces[i].id + 'modal_' + tempData.id) +')'});
                                if (!localStorage.getItem('ws_' + this.screen.store.workspaces[i].id + 'modal_' + tempData.id)){
                                    localStorage.setItem('workSpacePicStorageEnable_' + AppConfig.userId,false);
                                }
                            }
        ...
        //divPage上移
        pageUp: function () {
            var $thisPage = $('.divPageCt').not(':hidden').find('.divPage.selected');
        ...
        addSliderDom: function (isExternalCall) {
            ...
            $divPage.append($divPageTitle).append($btnRemove).append($effectLayer).append($cbSlider);
            $divPageCt.append($divPage);
            _this.setToolsBtnStatus($divPage);
            $divPage.click(function (e) {
                var modalId;
                var isSelected = $(this).hasClass('selected');
                // prevent repeat click
        ...
        deleteDivPage: function (obj) {
            if (confirm(I18n.resource.analysis.workspace.CONFIRM_PAGE_DELETE)) {
                ...
                var workspaceTotal = _this.screen.store.workspaces;
                if (modalId) {
                    WebAPI.get('/analysis/modal/remove/' + AppConfig.userId + '/' + WSId + '/' + modalId).done(function (result) {
                        ...
                        $thisDivPage.remove();
                        _this.setToolsBtnStatus($('.divPage.selected'));
                        ...
                        //
                        for (var i = 0, workspace; i < workspaceTotal.length; i++) {
                            workspace = workspaceTotal[i];
                            for (var j = 0; j < workspace.modalList.length; j++) {
                                if (workspace.modalList[j]['id'] == modalId) {
                                    workspaceTotal[i].modalList.splice(j,1);
                                }
                            }
                        }
                    }).always(function (e) {
                    ...
                    //var tempLocalStorage = JSON.parse(localStorage.getItem('ws_'+ WSId));
                    //delete tempLocalStorage['img_'+ result.modalId];
                    //localStorage.setItem('ws_'+ WSId,JSON.stringify(tempLocalStorage));
                    localStorage.removeItem('ws_' + WSId +'modal_' + modalId);
                } else {
            ...
            function domdrop(e) {
                ...
                if (dragEl != this) {
                    ...
                    WebAPI.post('/analysis/workspace/saveLayout/' + AppConfig.userId, {
                        ...
                        var newWsModalList = [];
                        for (var i = 0, len = workspaceTotal.length; i < len; i++) {
                            if(workspaceTotal[i]['id'] === wsId) {
                                var wsModalList = workspaceTotal[i]['modalList'];
                                ...
                                workspaceTotal[i]['modalList'] = newWsModalList;
                            }
                        }
                    });
                }
        ...
        modalNameInput: function (value) {
            ...
            var $input = $('<textarea class="newDivPageTitle" value="" i18n="placeholder=analysis.workspace.INPUT_PLACEHOLDER_PAGE_NAME"/>')
                .blur(function () {
                    ...
                    //_this.screen.targetSlide.push($(this).closest('.divPage'));
                    saveName(this);
                })
                .keypress(function(e){
                    if(e.keyCode == 13){
            ...
            function saveName(obj){
                ...
                if(id == $('.selected').closest('.divPage').attr('id') && id != 'divPageTemp'){
                ...
                if (id != 'divPageTemp') {
                    ...
                    _this.screen.saveModal();
                    _this.screen.saveModalJudge.resolveWith(_this.screen, [_this.screen, _this.chart,$(obj).closest('.divPage'),_this.screen.curModal,false]);
                } else {
                    modalName = inputVal;
                    $(thisInput).val(inputVal);
                }
        ...
        replaceNewModal: function (workspaceId, modalId, data,targetSlide,_chartImageBin) {
            //var $divPage = $('#divPageTemp');
            targetSlide.attr('id', modalId).attr('modal-id', modalId);
            if(_chartImageBin){
                targetSlide.css({'background-image': 'url('+ _chartImageBin +')'});
            }
        ...
        synchronizeToSliderMatrix: function ($obj){
             if($obj.length > 0 && $obj.not(':hidden').length > 0){
                _this.viewSliderMatrix();
            }
        },
        // 中间配置面板同步到左边管理面板
