class BenchmarkEnergyBenchmarking{
    ...
    projectListShow(){
        ...
        for (var i = 0, len = projectList.length; i < len; i++) {
            ...
            var projSingle = '<div class="areaSingleBox" data_projId="'+item.id+'" title="'+nameLan+'">'+
                             ...
                             '<b class="showOrHide"><span class="glyphicon glyphicon-eye-open"></span></b></div>';
    ...
    echartsData(){
        ...
        //}else {
        var dsItemIds = [];
        var timeFormat = _this.$bechingBox.find('.timeSelsect').val();
    ...
    sortArrBigToSmall(data){
        data.sort(function(a,b){
            if(a.data!=undefined){
                ...
                return b.dataList.data-a.dataList.data
            }
        });
        return data;
    }
