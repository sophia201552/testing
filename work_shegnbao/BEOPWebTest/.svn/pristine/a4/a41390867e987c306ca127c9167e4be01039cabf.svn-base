...
from beopWeb.mod_workflow import bp_workflow, Task
...
from beopWeb.mod_workflow.Activity import ActivityType
...
@bp_workflow.route('/transaction/get_history_complete/<int:page_num>/<int:page_sie>', methods=['POST'])
def transaction_history_complete(page_num, page_sie):
    user_id = AuthManager.get_userId()
    if not user_id:
        return Utils.beop_response_error(code=403)
    team = Team()
    if not team.user_has_team(user_id):
        return Utils.beop_response_error(msg='user has no team', code=40001)
    ...
    task = Task()
    all_user_map = User().get_all_user_map()
    total, task_list = task.get_history_complete_tasks(user_id, page_num, page_sie)
    for item in task_list:
        user = all_user_map.get(item.get('executor'))
        item["executor"] = user.get('userfullname') if user else ''
        user = all_user_map.get(item.get('creator'))
        item["creator"] = user.get('userfullname') if user else ''
...
@bp_workflow.route('/transaction/joined_by/<user_id>/<int:page_num>/<int:page_size>', methods=['POST'])
def transaction_joined_by(user_id, page_num, page_size):
    ...
    start_num = (page_num - 1) * page_size
    task = Task()
    total, trans_result = task.get_joined_tasks(user_id, start_num, page_size)
    result['total'] = total
    result['records'] = trans_result
    all_user_map = User().get_all_user_map()
    trans_group = TransactionGroup()
...
def transaction_working(user_id, page_num, page_size):
    ...
    start_num = (page_num - 1) * page_size
    request_user_id = AuthManager.get_userId()
    if not request_user_id:
        abort(403)
    task = Task()
    total, task_list = task.get_working_tasks(user_id, start_num, page_size)
    result['total'] = total
    result['records'] = task_list

    # task.wrap_task_list(task_list)

    return Utils.beop_response_success(result)


@bp_workflow.route('/group/transactionProcess/<group_id>/<int:page_num>/<int:page_size>', methods=['POST'])
def trans_process_in_group(group_id, page_num, page_size):
    user_id = AuthManager.get_userId()
    if not user_id:
        return Utils.beop_response_error(code=403)
    result = {
    ...
    task = Task()
    # 如果查询的 group_id 为0 的时候，就拿所有的group
    count = task.task_count_by_group_id(user_id, group_id)

    if not count and str(group_id) != '0':
        return Utils.beop_response_success(result)

    if str(group_id) == '0':
        tg = TaskGroup()
        group_list = tg.get_task_group_by_user_id(user_id)
        group_id_list = [str(item.get('_id')) for item in group_list]
        task_list, count = task.get_by_group_id_list(user_id, group_id_list, page_num, page_size)
    else:
        task_list = task.get_by_group_id(user_id, group_id, page_num, page_size)
    ...
    for task in task_list:
        ...
        if executor and user_map.get(executor):
            task['executor'] = user_map.get(task.get('executor')).get('userfullname')
        if creator and user_map.get(creator):
            task['creator'] = user_map.get(task.get('creator')).get('userfullname')
...
def add_transaction():
    ...
    if group_id == 'default' or not group_id:
        tg = TransactionGroup()
        default_group = tg.get_default_group_by_user_id(user_id)
        group_id = default_group.get('id') if default_group else None
        if not group_id:
            group_id = tg.create_default_group(user_id)
    ...
    try:
        ...
        message = Message()
        mu = MessageUser()
        msg_id = message.add_new_workflow_message_with_return_id(user_id, str(title), detail, {
            "id": trans_id,
            "op": ActivityType.NEW
        }, isMachine=True)
        if msg_id:
            message_receiver = []
            if user_id:
                message_receiver.append(int(user_id))
            if watcher_list:
                for watcher in watcher_list:
                    message_receiver.append(int(watcher))
            if executor_list:
                for executor in executor_list:
                    message_receiver.append(int(executor))
            if verifier_list:
                for verifier in verifier_list:
                    message_receiver.append(int(verifier))

            mu.add_workflow_users_message(list(set(message_receiver)), msg_id)
    except Exception as e:
        logging.error("新建工单给执行人添加message失败" + e.__str__())
        pass

    try:
        te = TransactionEmail()
        te.send_task_email(trans_id, request_json, pending_files)
    except Exception as e:
...
def update_transaction():
    ...
    detail = request_json.get('detail')
    group_id = request_json.get('groupId')
    executor_list = request_json.get('executor[]') if request_json.get('executor[]') else [None]
    verifier_list = request_json.get('verifiers[]') if request_json.get('verifiers[]') else None
    watcher_list = request_json.get('watchers[]')
    tags_list = request_json.get('tags[]')
    critical = request_json.get('critical')
    ...
    if result:
        ...
        try:
            if verifiers:
                ...
                message = Message()
                mu = MessageUser()
                msg_id = message.add_new_message_with_return_id(user_id,
                                                                i18n.trans('wait_verify') + trans.get("title", ""),
                                                                trans.get("detail", ""), isMachine=True)
                if msg_id:
                    mu.add_workflow_users_message(verifiers, msg_id)
        except Exception as e:
            logging.error("工单完成后给审核人添加message失败" + e.__str__())
            pass

        try:
            te = TransactionEmail()
            status = "complete"
            te.send_status_email(trans, tm, trans_id, user_id, status)
        except Exception as e:
            logging.error("完成工单发送邮件失败" + e.__str__())
            pass
            # end
...
def pass_verify():
    ...
    if result:
        ...
        try:
            ...
            logging.error("审核通过发送邮件失败" + e.__str__())

        try:
            executor = trans.get('executor')
            if executor:
                i18n = I18n()
                i18n.set_lang("zh-CN")
                message = Message()
                mu = MessageUser()
                msg_id = message.add_new_message_with_return_id(user_id,
                                                                i18n.trans('verify_success') + trans.get("title", ""),
                                                                trans.get("detail", ""), isMachine=True)
                if msg_id:
                    mu.add_workflow_users_message(executor, msg_id)
        except Exception as e:
            logging.error("工单审核通过添加message失败" + e.__str__())
            pass
...
def not_pass_verify():
    ...
    if result:
        ...
        try:
            ...
            logging.error("工单审核不通过发送邮件失败" + e.__str__())
            pass
        # end
        ...
        try:
            ...
            if executor:
                ...
                mu = MessageUser()
                msg_id = message.add_new_message_with_return_id(user_id,
                                                                i18n.trans('verify_failed') + trans.get("title", ""),
                                                                trans.get("detail", ""), isMachine=True)
                if msg_id:
                    mu.add_workflow_users_message(executor, msg_id)
        except Exception as e:
            logging.error("工单审核不通过添加message失败" + e.__str__())
            pass
...
@bp_workflow.route('/task/taskListFilter/', methods=['POST'])
def task_list_filter():
    ...
    task_type_query = rq_data.get("taskTypeQuery", {})
    filter = rq_data.get("filter", {})
    page_num = rq_data.get("page", None)
    page_size = rq_data.get("limit", None)
    groupId = rq_data.get("groupId", None)

    task_list_query = {"$and": []}
    task_list_query.get("$and").append(task_type_query)

    for key in filter:
        if key == "dueDate":
            try:
                dueDate = filter.get(key)
                if dueDate:
                    dueDate = dueDate.get("$and")
                    if dueDate:
                        for date in dueDate:
                            create_time = date.get("createTime")
                            if create_time.get("$gte"):
                                time = create_time.get("$gte")
                                create_time["$gte"] = datetime.strptime(time,
                                                                        "%Y-%m-%d %H:%M:%S")
                                continue
                            if create_time.get("$lte"):
                                time = create_time.get("$lte")
                                create_time["$lte"] = datetime.strptime(time,
                                                                        "%Y-%m-%d %H:%M:%S")
                                continue
                task_list_query.get("$and").append(filter.get(key))
            except Exception as e:
                continue
        else:
            task_list_query.get("$and").append(filter.get(key))

    trans_result, total = tk.task_filter(task_list_query, page_num=page_num, page_size=page_size)
...
def add_task_comment():
    ...
    msg_id = message.add_new_workflow_message_with_return_id(user_id, '评论', content, {
        "id": task_id,
        "op": "reply",
