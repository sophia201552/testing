...
var PaneProjectSelector = (function () {
    ...
    PaneProjectSelector.prototype = {
        ...
        initDropDownItem: function (dropdownItem) {
            ...
            if (dropdownItem.type === 'ReportScreen') {
                aTag.attributes['report-type'] = dropdownItem.reportType;
                aTag.attributes['report-folder'] = dropdownItem.reportFolder;
            }

            aTag.textContent = dropdownItem.text;
            aTag.onclick = function (e) {
                Spinner.spin(ElScreenContainer);
                var pageId = e.currentTarget.attributes['pageId'];
                if (screen === ReportScreen) {
                    var reportType = e.currentTarget.attributes['report-type'],
                        reportFolder = e.currentTarget.attributes['report-folder'];
                    ScreenManager.show(screen, pageId, reportType, reportFolder);
                } else {
                    ScreenManager.show(screen, pageId);
                }
            };
            ...
            liTag.setAttribute('pageid', dropdownItem.id);
            if (dropdownItem.type === 'ReportScreen') {
                liTag.setAttribute('report-folder', dropdownItem.reportFolder);
                liTag.setAttribute('report-type', dropdownItem.reportType);
            }
        ...
        initNavigationPane: function (navItems) {
            ...
            var strLiReport = '<li>\
                    <a class="nav-btn-a" pageId="<%menuId%>" page-type="report" report-type="<%reportType%>" report-folder="<%reportFolder%>">\
                        <span>\
                            <svg class="nav-icon-svg" version="1.1" x="0px" y="0px" width="30px" height="30px" viewBox="0 0 30 30" enable-background="new 0 0 30 30" xml:space="preserve">\
                            <g class="nav-icon-svg-g">\
