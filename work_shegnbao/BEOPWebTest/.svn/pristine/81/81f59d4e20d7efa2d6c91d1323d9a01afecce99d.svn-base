...
def analysisDataSourceSaveMulti(userId):
    ...
    userId = int(userId)
    if isinstance(data, dict):
        rt = MongoConnManager.getMongoConnByName().analysisDataSourceSaveMulti(userId, data)
    return json.dumps(rt, ensure_ascii=False)
...
def analysisDataSourceRemoveSingle(datasourceId, datasourceItemId, userId):
    ...
    userId = int(userId)
    rt = MongoConnManager.getMongoConnByName().analysisDataSourceRemoveSingle(datasourceId, datasourceItemId, userId)
    return json.dumps(rt, ensure_ascii=False)
...
def getDataSourceDirect(datasourcelist=[]):
    ...
    rtList = MongoConnManager.getMongoConnByName().analysisDataSourceGetByIdList(dsList)
    return json.dumps(rtList, ensure_ascii=False)
        ...
        for datasource in datasourceList:
            ...
            for item in dsl:
                ...
                if ObjectId.is_valid(parentId):
                    if parentId == groupId:
                        rt = MongoConnManager.getMongoConnByName().analysisDataSourceRemoveSingle('', item.get('id'),
                                                                                                  userId)
