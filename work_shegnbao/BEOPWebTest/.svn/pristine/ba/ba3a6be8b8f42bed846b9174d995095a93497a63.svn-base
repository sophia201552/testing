...
def workflow_transaction_edit():
    ...
    if trans_id is None:
        ...
        return BEOPDataAccess.getInstance().editTransaction(trans_id, user_id, title=title, dueDate=due_date,
                                                            executorID=executor_id, critical=critical, detail=detail)
...
def workflow_detail_reply(user_id, record_id):
    record_detail = BEOPDataAccess.getInstance().workflow_get_status_by_transaction_id(user_id, record_id)
    reply_list = BEOPDataAccess.getInstance().workflow_get_transaction_reply(record_id)
    operations = BEOPDataAccess.getInstance().workflow_get_transaction_operations(record_id)
    is_editable = record_detail.get('executorId', None) == int(user_id)

    return jsonify(replyList=reply_list, record_detail=record_detail,
                   isEditable=is_editable, operations=operations,
                   success=True)
