...
var IndexScreen = (function () {
    ...
    IndexScreen.prototype = {
        ...
        initRegister: function () {
            ...
            $("#messageAll").click(function () { // 添加消息
                if ($('.infoBoxMessage').length) {
                    ...
                    WebAPI.get('/getAllMessages/' + AppConfig.userId).done(function (result) {
                        if (result.success) {
                            var messageListTest = [
                                {
                                    'id': 111,
                                ...
                                },
                                {
                                    ...
                                    'user': '管理员',
                                    'userPic': '/static/images/avatar/default/7.png',
                                    'message': '冷凝器进水温度无规律波动冷凝器进水温度无规律波动',
                                    'time': '2016-05-03'
                                },
                                {
                                    'id': 114,
                            ...
                            //messageListTest = result.data.workflow_list;
                            var defaultMessageNum = 5;
                            var messageHtml = '<ul>';
                            for (var i = 0; i < messageListTest.length; i++) {
                                if (i < defaultMessageNum) {
                                    var mes = messageListTest[i];
                                    var isReadClass = mes.unread ? 'unread' : '';
                                    messageHtml += '<li id="infoBoxMessage-' + mes.id + '" mesId="' + mes.id + '">' +
                                        '<div class="fl wdr1"><img class="userImg" src="' + mes.userPic + '"/></div>' +
                                            '<div class="fl wdr2 messageText">' +
                                                '<span class="messageUserName mr5">' + mes.user + '</span><span>' + mes.message + '</span>' +
                                            '</div>' +
                                            '<div class="fl tr wdr3 messageRight"><div class="messageTime">' + mes.time + '</div>' +
                                            '<div class="' + isReadClass + '"></div>' +
                                        '</div>' +
                                    '</li>';
                                }
                            }
                            messageHtml += '</ul>';
                            if (messageListTest.length > defaultMessageNum) {
                                messageHtml += '<div id="messageShowAll" class="tc cp messageShowAll"><a href="#page=AllMessages"><span>查看所有消息</span><span>&gt;&gt;</span></a></div>';
                            }
            ...
            $(document).on("click.infoBoxMessage", function (e) { //点击页面其它地方，隐藏消息列表
                var $target = $(e.target);
        ...
        login: function () {
            ...
            var loginFail = function (msg) {
                $('.alert').remove();
                new Alert($("#divLogin"), "danger", msg).show();
                    ...
                    var after_login = function () {
                        if (login_result.status != undefined && login_result.status == true) {
                            ...
                            if (login_result.projects && login_result.projects.length > 0) {
                                ...
                                AppConfig.skipProjectSelector = AppConfig.projectList.length == 1;

                                AppConfig.permission = login_result.permission;
                                //添加版本历史
                                if (login_result.versionNotification) {
                                    _this.setVersionHistory(login_result.versionNotification);
                                } else {
