...
(function (beop) {
    ...
    BeopMap.prototype.attachPanelEvent = function () {
        ...
        $('.map-card').on('click', '.media', function () {
            ...
            var $this = $(this);
            var projectIdStr = $this.closest('.media').attr("id");
            var projectId = parseInt(projectIdStr.split('-')[1]);
            var $projectsCard = $('#projectsCard');
            var $currentEleAll = $projectsCard.find('.media[id=' + projectIdStr + '] .favorite');
            if ($this.hasClass('active')) {
    ...
    BeopMap.prototype.initSingleProject = function (project) {
        ...
        return this.paneProjectSelector.initProject(project.id, project.name_en, StringUtil.getI18nProjectName(project), project.logo).done(function (result) {
    ...
    BeopMap.prototype.addToVersionHistory = function () {
        var versionHistory = beop.data.versionHistory, $controlBtn;
        if (!(!!versionHistory) || versionHistory == 'undefined') {
            $controlBtn = this.createVersionHistory('');
        } else {
            $controlBtn = this.createVersionHistory(versionHistory.version);
        }
        this.addControl($controlBtn)
    };
    BeopMap.prototype.createVersionHistory = function (text, isShowVersionInfo) {
        return $('<div class="map-beop-version-history" title="beop版本历史记录"><a href="#page=VersionHistory"> beop version: ' + text + ' </a></div>');
    };
    BeopMap.prototype.addNewVersionInfo = function () {
        if (configMap.isFirstLoadVersion) {
            configMap.isFirstLoadVersion = false;
            var versionHistory = beop.data.versionHistory;
            if (!!versionHistory && versionHistory != 'undefined' && !!versionHistory.log) {
                beop.data.versionHistory.newest = true;
                beop.data.versionHistory.readUsers.push(AppConfig.userId);
                window.localStorage.setItem('versionHistory', JSON.stringify(beop.data.versionHistory));
                versionInfoBox(versionHistory.version, versionHistory.log);
            }
        }
    };
...
function mapLoadCallBack() {
    try {
        ...
        if (window.localStorage.getItem('language') == 'zh') {
            //var version = beop.data.versionHistory,
            //    isShowVersionInfo = !!(version && !version.newest && version.readUsers && version.readUsers.indexOf(AppConfig.userId) == -1);
            mapIns.addToVersionHistory(beop.data.versionHistory.readUsers);
            // 隐藏版本通知窗 2016-07-12
            //if (version && isShowVersionInfo) {
            //    if ($('#new-version-info').length == 0) {
            //        mapIns.addNewVersionInfo();
            //    }
            //}
        }
