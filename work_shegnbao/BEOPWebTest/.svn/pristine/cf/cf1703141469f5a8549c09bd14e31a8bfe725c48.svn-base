...
class Patrol(object):
    ...
    def savePath(cls, projId, data):
        ...
        try:
            ...
            #missionlog_list = get_missionlog_list_by_pathId(projId, data.get('_id'))
            for key in data:
                value = data.get(key)
            ...
            if 'timeRange' in data.keys():
                del data['timeRange']
                data.update({'time_up':time_up, 'time_down':time_down})
            dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_path].update({'_id':data.get('_id')}, {'$set':data}, True)
            if dbrv.get('ok'):
                pathId = data.get('_id').__str__()
        except Exception as e:
            print('savePath failed:'+e.__str__())
            logging.error('savePath failed:'+e.__str__())
        return pathId
    ...
    def get_missionlog_list_by_pathId(cls, projId, pathId):
        ...
        cursor = None
        try:
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_missionlog].find({'pathId':ObjectId(pathId), 'projId':int(projId)})
            for item in cursor:
                rt.append(item)
        except Exception as e:
    ...
    def removePath(cls, projId, pathId):
        ...
        # missionId = []
        try:
            # missionId = get_patrol_mission_by_path(projId, pathId)
            # if len(missionId)==0:
            if ObjectId.is_valid(pathId):
                # data.update({'_id':ObjectId(pathId)})
                dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_path].update({'_id':ObjectId(pathId), 'projId':int(projId)}, {'$set':{'status':0}}, True)
                if dbrv.get('ok'):
                    rt = True
        except Exception as e:
            print('savePath failed:'+e.__str__())
        ...
        try:
            ...
            for key in data:
                ...
                if key == 'type' or key == 'projId':
                    ...
                    value = ObjectId(value)
                data.update({key:value})
            dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_point].update({'_id':data.get('_id')}, {'$set':data}, True)
            if dbrv.get('ok'):
                pointId = data.get('_id').__str__()
        except Exception as e:
            print('savePoint failed:'+e.__str__())
            logging.error('savePoint failed:'+e.__str__())
        return pointId
    ...
    def removePoint(cls, projId, pointId):
        ...
        try:
            ...
            if ObjectId.is_valid(pointId):
                dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_point].update({'_id':ObjectId(pointId), 'projId':int(projId)}, {'$set':{'status':0}}, True)
                if dbrv.get('ok'):
                    rt = True
        except Exception as e:
            print('savePoint failed:'+e.__str__())
    ...
    def saveExecutor(cls, projId, data):
        ...
        try:
            ...
            for key in data:
                ...
                data.update({key:value})
            dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_executor].update({'_id':data.get('_id')}, {'$set':data}, True)
            if dbrv.get('ok'):
                executorId = data.get('_id').__str__()
        except Exception as e:
            print('saveExecutor failed:'+e.__str__())
            logging.error('saveExecutor failed:'+e.__str__())
        return executorId
    ...
    def removeExecutor(cls, projId, executorId):
        ...
        try:
            if ObjectId.is_valid(executorId):
                # missionId = get_patrol_mission_by_executor(projId, executorId)
                # if len(missionId) == 0:
                # data.update({'_id':ObjectId(executorId)})
                dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_executor].update({'_id':ObjectId(executorId), 'projId':int(projId)}, {'$set':{'status':0}}, True)
                if dbrv.get('ok'):
                    rt = True
        except Exception as e:
            print('saveExecutor failed:'+e.__str__())
    ...
    def get_patrol_mission_by_executor(cls, projId, executorId):
        ...
        try:
            ...
            mission_list = get_patrol_mission_list_by_projId(projId, pTime, pTime)
    ...
    def get_patrol_point_list(cls, projId):
        ...
        try:
            if not isinstance(projId, int):
                projId = int(projId)
            cursor_1 = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_point].find({'projId':projId})
            for item in cursor_1:
                try:
                    cursor_2 = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_path].find({'path':item.get('_id').__str__(), 'projId':projId})
    ...
    def get_patrol_path_by_pointId(cls, projId, pointId):
        ...
        try:
            ...
            cursor_1 = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_path].find({'projId':projId, 'path':pointId})
            list_1 = list(cursor_1)
            for item in list_1:
                cursor_2 = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_point].find({'_id':{'$in':[ObjectId(x) for x in item.get('path')]}})
    ...
    def get_patrol_path_by_projId(cls, projId):
        ...
        try:
            if not  isinstance(projId, int):
                projId = int(projId)
            cursor_1 = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_path].find({'projId':projId}, sort=[('sort', 1)])
            for item in cursor_1:
                try:
                    cursor_2 = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_point].find({'_id':{'$in':[ObjectId(x) for x in item.get('path') if ObjectId.is_valid(x)]}})
                    point_dict = {}
                    for i in cursor_2:
    ...
    def get_patrol_executor_list_by_projId(cls, projId):
        ...
        try:
            if not isinstance(projId, int):
                projId = int(projId)
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_executor].find({'projId':projId})
            for item in cursor:
                try:
    ...
    def get_patrol_mission_list_by_projId(cls, projId, startTime, endTime):
        ...
        try:
            ...
            limit = 100
            while n >= 0:
                cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_mission].find({'projId':projId, 'startTime':{'$lte':endTime}}, skip = n * limit, limit = limit, sort = [('startTime', -1)])
                list_1 = list(cursor)
                for item in list_1:
                        ...
                        if item.get('startTime') + timedelta(days = int(item.get('interval'))) <= startTime:
                            if startTime - timedelta(days = int(item.get('interval'))) < datetime.now():
                                ...
                                if len(rt) == 0:
                                    ...
                                    dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_mission].update({'_id':new_mission.get('_id')}, {'$set':new_mission}, True)
                                    if dbrv.get('ok'):
    ...
    def save_patrol_mission(cls, projId, data):
        ...
        try:
            ...
            for key in data:
                ...
                data.update({key:value})
            data = get_patrol_mission_cycle(projId, data)
            data.update({'lastModified': datetime.now(), 'lastModifiedUser': request.cookies.get('userId', None)})

            if data is not None:
                dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_mission].update({'_id':data.get('_id')}, {'$set':data}, True)
                if dbrv.get('ok'):
    ...
    def get_patrol_mission_cycle(cls, projId, data):
        ...
        try:
            if isinstance(data.get('startTime'), datetime):
                dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_mission].find_one({'projId':int(projId), 'startTime':{'$lte':data.get('startTime')}}, sort = [('startTime', -1)])
                if dbrv is not None:
    ...
    def save_patrol_missionlog(cls, data):
        ...
        try:
            ...
            dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_missionlog].update({'_id':data.get('_id')}, {'$set':data}, True)
            if dbrv.get('ok'):
                rt = data.get('_id').__str__()
        except Exception as e:
    ...
    def get_patrol_report_baseon_man(cls, projId, startTime, endTime):
        ...
        try:
            ...
            #获取段时间内的mission列表
            mission_list = get_patrol_mission_list_by_projId(projId, startTime, endTime)
            mission_list.reverse()
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_missionlog].find({'missionId':{'$in':[ObjectId(x.get('_id')) for x in mission_list
                        ...
                        if option:
                            ...
                            for missionlog in missionlog_list:
                                ...
                                if ptime.date() == planDate.date():
                                    ...
                                    if judge_missionlog_path(missionlog_path):
            ...
            if cursor:
                cursor.close()
            #获取人员信息
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_executor].find({'_id':{'$in':[ObjectId(t) for t in executor_data.keys() if ObjectId.is_valid(t)]}},
                                                   sort=[('sort', 1), ('name', 1)])
            ...
            for item in executor_list:
                if item.get('_id').__str__() in executor_data.keys():
                    rt.get('data').append({'exectorId':item.get('_id').__str__(), 'name':item.get('name'), 'partment':item.get('department'),
                                           ...
                                           'percent':calculate_the_percentage_from_list(executor_data.get(item.get('_id').__str__()).get('data'))})
        except Exception as e:
    ...
    def get_patrol_report_baseon_path(cls, projId, startTime, endTime):
        ...
        try:
            ...
            mission_list = get_patrol_mission_list_by_projId(projId, startTime, endTime)
            mission_list.reverse()
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_missionlog].find({'missionId':{'$in':[ObjectId(x.get('_id')) for x in mission_list if ObjectId.is_valid(x.get('_id'))]}})
            #段时间内的missionlog列表
            missionlog_list = list(cursor)
            ...
            if cursor:
                cursor.close()
            #获取报表中每条路线的路线名
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_path].find({'_id':{'$in':[ObjectId(p) for p in path_data.keys() if ObjectId.is_valid(p)]}},
                                               sort=[('sort', 1), ('name', 1)])
            for item in cursor:
    ...
    def get_patrol_log_by_pathId(cls, projId, pathId, pTime):
        ...
        try:
            ...
            path = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_path].find_one({'_id':ObjectId(pathId)})
            #获取段时间内的mission
            mission_list = get_patrol_mission_list_by_projId(int(projId), pTime, pTime)
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_missionlog].find({'projId':int(projId), 'pathId':ObjectId(pathId),
            ...
            #
            point_data = {}
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_point].find({'_id':{'$in':[ObjectId(po) for po in path.get('path') if ObjectId.is_valid(po)]}})
            for item in cursor:
            ...
            if cursor:
                cursor.close()
            executor_data = {}
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_executor].find({'projId':int(projId)})
            for ei in cursor:
    ...
    def get_patrol_executorName_by_idlist(cls, idlist):
        ...
        cursor = None
        try:
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_executor].find({'_id':{'$in':[ObjectId(i) for i in idlist if ObjectId.is_valid(i)]}})
            for item in cursor:
    ...
    def get_patrol_path_by_pathid(cls, pathid):
        ...
        try:
            ...
            path = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_path].find_one({'_id':ObjectId(pathid)})
            if path:
                cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_point].find({'_id':{'$in':[ObjectId(i) for i in path.get('path') if ObjectId.is_valid(i)]}})
                point = []
                for item in cursor:
    ...
    def get_patrol_log_by_executorId(cls, projId, executorId, pTime):
        ...
        try:
            pTime = datetime.strptime(pTime, '%Y-%m-%d')
            #人员信息
            executor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_executor].find_one({'_id':ObjectId(executorId)})
            #mission信息
            mission_list = get_patrol_mission_list_by_projId(projId, pTime, pTime)
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_missionlog].find({'projId':int(projId), 'executorId':ObjectId(executorId),
                                                      'missionId':{'$in':[ObjectId(m.get('_id')) for m in mission_list if ObjectId.is_valid(m.get('_id'))]},
                                                      '$or':[{'planDate':{'$gte':pTime, '$lt':pTime + timedelta(days = 1)}},{'startTime':{'$gte':pTime, '$lt':pTime + timedelta(days = 1)}}]})
            #日志列表
            missionlog_list = list(cursor)
            ...
            #巡更路线信息
            path_dict = {}
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_path].find({'projId':int(projId)})
            for p in cursor:
            ...
            #巡更点信息
            point_dict = {}
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_point].find({'projId':int(projId)})
            for pp in cursor:
    ...
    def get_patrol_abnormity(cls, projId, startTime, endTime):
        ...
        try:
            ...
            if startTime is None or len(startTime) == 0:
                cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_missionlog].find({'projId':projId})
            else:
                startTime = datetime.strptime(startTime, '%Y-%m-%d %H:%M:%S')
                endTime = datetime.strptime(endTime, '%Y-%m-%d %H:%M:%S')
                cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_missionlog].find({'projId':projId, 'startTime':{'$gte':endTime, '$lte':startTime}})
            for item in cursor:
                try:
                    if item.get('state') == 2:
    ...
    def get_patrol_path_complete_num(cls, projId, date = datetime.now()):
        ...
        try:
            ...
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_missionlog].find({'projId':int(projId),
            ...
            if cursor:
                cursor.close()
            dict_path = {}
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_patrol_path].find({'_id':{'$in':[ObjectId(x) for x in path_info.keys()]}})
            for item in cursor:
