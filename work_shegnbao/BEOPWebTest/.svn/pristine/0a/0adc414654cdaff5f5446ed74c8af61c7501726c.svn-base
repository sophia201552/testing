...
class MongoConnManager:
    ...
    def getHisConn(projId):
        ...
        try:
            ...
            if MongoConnManager.useMongoServerOffline():
                ...
                ins = BEOPMongoDataAccess(configIP)
                if ins:
                    ins.setTimeAttrStart(datetime(year=2014, month=1, day=1, hour=0, minute=0, second=0))
                    ins.setTimeAttrEnd(datetime(year=2018, month=1, day=1, hour=0, minute=0, second=0))
                    rt.append(ins)
            else:
                if locateMap:
                    ...
                    if projectLocate:
                        ...
                        if locateList:
                            for l in locateList:
                                ...
                                ins = BEOPMongoDataAccess(addr)
                                if ins:
                                    ins.setTimeAttrStart(st)
                                    ins.setTimeAttrEnd(et)
                                    rt.append(ins)
                        if rt:
    ...
    def getConfigConn():
        ...
        try:
            ...
            rt = BEOPMongoDataAccess(configIP)
        except Exception as e:
