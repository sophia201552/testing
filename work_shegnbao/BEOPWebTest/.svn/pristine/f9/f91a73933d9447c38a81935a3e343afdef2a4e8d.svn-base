...
class Utils:
    ...
    class DbHelper:
        ...
        def query(self, db_name, table, fields=(), where=None, order=None, limit=None):
            ...
            rv = self.db.op_db_query(db_name, sql, where[1] if where and len(where) > 1 else None)
    ...
    class EmailTool:
        ...
        def send_email(subject, recipients, html):
            ...
            try:
                Mail(app).send(Message(subject=subject, recipients=recipients, charset='utf-8', html=html))
                return True
