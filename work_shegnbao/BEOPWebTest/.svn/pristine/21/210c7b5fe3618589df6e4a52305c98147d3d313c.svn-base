...
def check_workflow_auth(f):
    ...
    def decorated(*args, **kwargs):
        if request.headers.get('algorithm') == 'rnbtech2015':
...
def update_transaction():
    ...
    if due_date != old_trans.get('dueDate').strftime('%Y-%m-%d') if isinstance(old_trans.get('dueDate'),
                                                                               datetime) else old_trans.get('dueDate'):
        record_detail['dueDate'] = {
            "new": due_date,
            "old": old_trans.get('dueDate').strftime('%Y-%m-%d') if isinstance(old_trans.get('dueDate'),
                                                                               datetime) else old_trans.get('dueDate')
        }
    ...
    if len(record_detail) > 0:
        ...
        if trans.update_trans(trans_id, model):
            ...
            try:
                current_trans = trans.get_transaction_by_id(trans_id)

                message = Message()
                mu = MessageUser()

                title = current_trans.get("title", "")
                msg_id = message.add_new_workflow_message_with_return_id(user_id, title,
                                                                         current_trans.get("detail", ""), {
                                                                             "id": trans_id,
                                                                             "op": ActivityType.EDIT
                                                                         }, isMachine=True)
                if msg_id:
                    mu.add_workflow_users_message(trans.get_trans_all_people_id_by_id(trans_id), msg_id)
            except Exception as e:
                logging.error("工单编辑任务添加message失败" + str(e))
                pass
...
def get_trans(trans_id):
    ...
    trans['createTime'] = trans.get('createTime').strftime(Utils.datetime_format_full) if trans.get(
        'createTime') and isinstance(trans.get('createTime'), datetime) else trans.get('createTime')
    trans['dueDate'] = trans.get('dueDate').strftime(Utils.datetime_format_date) if trans.get('dueDate') and isinstance(
        trans.get('dueDate'), datetime) else trans.get('dueDate')
    tg = TransactionGroup()
    ...
    if result:
        ...
        try:
            if verifiers:
                ...
                msg_id = message.add_new_workflow_message_with_return_id(user_id, title, trans.get("detail", ""), {
                    "id": trans_id,
                    "op": ActivityType.COMPLETE
                }, isMachine=True)
                if msg_id:
                    mu.add_workflow_users_message(trans_db.get_trans_all_people_id_by_id(trans_id), msg_id)
        except Exception as e:
...
def start_trans(user_id, trans_id):
    if not user_id or not trans_id:
        return Utils.beop_response_error(code=403)
    trans_db = Transaction()
    result = trans_db.start_trans(trans_id)
    if result:
        try:
            trans = trans_db.get_transaction_by_id(trans_id)
            ...
            msg_id = message.add_new_workflow_message_with_return_id(user_id, title, trans.get("detail", ""), {
                "id": trans_id,
                "op": ActivityType.START
            }, isMachine=True)
            if msg_id:
                mu.add_workflow_users_message(trans_db.get_trans_all_people_id_by_id(trans_id), msg_id)
        except Exception as e:
            logging.error("工单开始任务添加message失败" + str(e))
            pass
...
def update_executor(user_id, trans_id):
    ...
    current_trans = trans.get_transaction_by_id(trans_id)
    current_status = current_trans.get('statusId')
    ...
    if result:
        try:
            ...
            title = current_trans.get("title", "")
            msg_id = message.add_new_workflow_message_with_return_id(user_id, title, current_trans.get("detail", ""), {
                "id": trans_id,
                "op": ActivityType.FORWARD
            }, isMachine=True)
            if msg_id:
                mu.add_workflow_users_message(trans.get_trans_all_people_id_by_id(trans_id), msg_id)
        except Exception as e:
            logging.error("工单转发任务添加message失败" + str(e))
            pass
...
def close_task(user_id, trans_id):
    ...
    if result:
        ...
        try:
            ...
            msg_id = message.add_new_workflow_message_with_return_id(user_id, title, current_trans.get("detail", ""), {
                "id": trans_id,
                "op": ActivityType.CLOSE
            }, isMachine=True)
            if msg_id:
                mu.add_workflow_users_message(trans.get_trans_all_people_id_by_id(trans_id), msg_id)
        except Exception as e:
            logging.error("工单关闭任务添加message失败" + str(e))
            pass
...
def task_type_filter():
    ...
    tk = Task()
    result = {}
    trans_result, total = tk.task_filter(query, page_num=pageNumber,
                                         page_size=pageSize)
