            ...
            $.when(this.saveModalJudge).done(function (_this, _mode, _chart,_targetSlider,_curModal,imgSaveJudge) {
                ...
                if (imgSaveJudge) {
                    if (!_chart) {
                        ...
                        if(_chart.getOption().toolbox){
                            isShowToolbox = _chart.getOption().toolbox.show;
                            _chart.setOption({toolbox:{show: false}});
                        }
                        if(_chart.getOption().dataZoom){
                            isShowDataZoom = _chart.getOption().dataZoom.show
                            _chart.setOption({dataZoom:{show: false}});
                        }
                        //设置缩略图背景颜色
                        ...
                        //还原toolbox和dataZoom
                        if(isShowToolbox){
                            _chart.setOption({toolbox:{show: isShowToolbox}});
                        }
                        if(isShowDataZoom){
                            _chart.setOption({dataZoom:{show: isShowDataZoom}});
                        }
