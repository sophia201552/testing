...
var DragPanel = (function(){
        ...
        if(!this.isShow || !arrPanel || arrPanel.length < 1){//显示历史面板
            ...
            if(arrPanel && (arrPanel instanceof Array) && arrPanel.length > 0){
                arrPanel.forEach(function(data){
                    ...
                    if(isAdd){
                        new HistoryDragPanel(data);
                    }
                });
            }else{
                new HistoryDragPanel();
            }
            this.isShow = true;
        }else{//隐藏历史面板
            this.hide();
    ...
    DragPanel.prototype.hide = function(){
        this.arrInstance.forEach(function(instance){
            instance.close();
        });
        this.arrInstance.length = 0;
    }
...
var HistoryDragPanel = (function(){
    ...
    HistoryDragPanel.prototype.renderChartByData = function(){
        var _this = this;
        var postData;
        this.$panel.css({top: this.data.top*screen.height, left: this.data.left*screen.width, width: this.data.width, height: this.data.height});
