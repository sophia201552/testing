var TaskPool = (function () {
    ...
    function TaskPool() {
        ...
        this.apiMap = {
            ...
            getDiagnosisAll: 'diagnosis/getAll/'
        };
        this.configMap = {
    ...
    TaskPool.prototype = {
        ...
        show: function ($container, projectID) {
            ...
            this.init($currentContainer).done(function () {
                ...
                $.getJSON(_this.apiMap.getDiagnosisAll + projectID).done(function (getDiagnosisAllData) {
                    //处理数据
                    if (getDiagnosisAllData.notices !== undefined && getDiagnosisAllData.notices.length !== 0) {
                        _this.renderFaultsList(getDiagnosisAllData).done(function () {
                            Spinner.stop();
                        });
                    } else {
        ...
        renderFaultsList: function (getDiagnosisAllData) {
            //这里是处理数据。根据 workflowCalendar
            var _this = this, data = getDiagnosisAllData;
            ...
            //给faults 设置 data 属性来进行筛选
            return this.setDataOnFaults().done(function () {
                _this.attachEvent();
            });
        },
        setDataOnFaults: function () {
            ...
            for (i; i < data.length; i++) {
                notice = data[i];
                $faultsItem = $('<div class="ellipsis faults">' +
                    '<span class="zoneInfo">' +
