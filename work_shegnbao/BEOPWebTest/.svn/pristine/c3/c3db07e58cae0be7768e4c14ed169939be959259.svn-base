    ...
    ModalHistoryChartNormal.prototype.initOption = function(dataSrc){
        ...
        if ('month' == this.entity.modal.option.timeType) {
            for (var i=0; i<xLen; i++) {
                arrXAxis.push(new Date(dataSrc.timeShaft[i]).format("MM-dd"));
            }
        }
        else if ('week' == this.entity.modal.option.timeType) {
            for (var i=0; i<xLen; i++) {
                arrXAxis.push(new Date(dataSrc.timeShaft[i]).format("yyyy-MM-dd"));
            }
        }
        else if ('day' == this.entity.modal.option.timeType) {
            for (var i=0; i<xLen; i++) {
                arrXAxis.push(new Date(dataSrc.timeShaft[i]).format("MM-dd HH:00"));
            }
        }
        xAxis = [
            {
    ...
    ModalHistoryChartEnergyConsume.prototype.initOption = function(dataSrc){
        ...
        if ('week' == this.entity.modal.option.timeType) {
            for (var i=0; i<xLen -1; i++) {
                var dayNum = (new Date(dataSrc.timeShaft[i])).getDay();
            ...
            for (var i=0; i<xLen - 1; i++) {
                var monthNum = (new Date(dataSrc.timeShaft[i])).getMonth();
    ...
    ModalHistoryChartYearOnYearLine.prototype.renderModal = function () {
        ...
        WebAPI.post('/analysis/startWorkspaceDataGenHistogram', {
            ...
            for (flagCount = 0; flagCount < len; flagCount++) {
                if (new Date(dataSrc.timeShaft[flagCount]) >= tmFlag) {
                    break;
                }
            }
            var fixNum = 0;
            ...
            for (var k = 0, lenK = dataSrc.timeShaft.length; k < lenK; k++) {
                if (k < flagCount) {
                    arrXAxis.push(new Date(dataSrc.timeShaft[k]).format("HH:mm"));
                }
            }
            ...
            for (var n = 0; n < dataSrc.list.length; n++) {
                ...
                for (var j = 0, len = dataSrc.list[n].data.length; j < len; j++) {
                    setVal = Number(dataSrc.list[n].data[j]);
                    if (setVal < 100) {
                        fixNum = 2;
                    }
                    setVal = setVal.toFixed(fixNum);
...
var ModalHistoryChartYearOnYearBar = (function () {
    ...
    ModalHistoryChartYearOnYearBar.prototype.renderModal = function () {
        ...
        WebAPI.post('/analysis/startWorkspaceDataGenHistogram', {
            ...
            var len = dataSrc.timeShaft.length;
            for (var i = 0; i < len; i++) {
                if (new Date(dataSrc.timeShaft[i]) >= startDate) {
                    flag1 = i;
                    break;
                }
            }
            for (var i = 0; i < len; i++) {
                if (new Date(dataSrc.timeShaft[i]) >= endDateZero) {
                    flag2 = i;
                    break;
                }
            }
            ...
            for (var i = 0; i < dataSrc.list.length; i++) {
                ...
                for (var j = 0, len = dataSrc.list[i].data.length; j < len; j++) {
                    if (j == flag1) {
                        val1 = Number(dataSrc.list[i].data[j]) - Number(dataSrc.list[i].data[0]);
                    }
                    else if (j == flag2) {
                        val2 = Number(dataSrc.list[i].data[len-1]) - Number(dataSrc.list[i].data[j]);
                    }
                }
                if (val1 < 100) {
                    fixNum1 = 2;
                ...
                arrVal1.push(val1.toFixed(fixNum1));
                arrVal2.push(val2.toFixed(fixNum2));
                break;
            }
