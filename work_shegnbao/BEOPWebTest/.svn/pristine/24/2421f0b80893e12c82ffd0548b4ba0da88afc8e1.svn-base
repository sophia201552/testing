var WfFileUpload = (function ($) {
    ...
    fileUpload.prototype = {
        ...
        __showUploadModal: function () {
            var self = this;
            WebAPI.get('/static/views/workflow/upload.v2.html' + '?=' + new Date().getTime()).done(function (result) {
                ...
                if (self.store.isCreateNewTask) {
                    ...
                    WebAPI.get('/workflow/v2/attachment/getFiles/' + window.parent.beop.model.stateMap.cur_trans.id).done(function (fileResult) {
        ...
        __removeUploadFileItem: function (ev, dom, isRemove) {
            ...
            if (window.parent.beop.model.stateMap.cur_trans) {
                data.taskId = window.parent.beop.model.stateMap.cur_trans.id;
            }
            data.fileName = $this.data('filename');
            if (isRemove) {
                WebAPI.post('/workflow/v2/attachment/delete', data).done(function (result) {
                    if (result.success) {
        ...
        __uploadFile: function () {
            ...
            xhr.open("POST", "workflow/v2/attachment/upload");
        ...
        _setFormData: function () {
            ...
            if (window.parent.beop.model.stateMap.cur_trans) {
                this.formData.append('taskId', window.parent.beop.model.stateMap.cur_trans._id);
            }
