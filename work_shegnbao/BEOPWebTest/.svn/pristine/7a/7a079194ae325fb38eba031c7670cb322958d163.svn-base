                        ...
                        for row0Index,row0Content in enumerate(row0_data):
                            ...
                            if row0Index > 1 and row0Index < len(row0_data)-2 and row0Content == '':
                                if len(str(row0_data[row0Index-1])) > 0 and len(str(row0_data[row0Index-2])) > 0 and \
                                    ...
                                    centerBlankCol += 1
                            if len(str(row0Content)) > 0 and 'time' not in str(row0Content).lower():
...
def readCSV(filePath):
    ...
    try:
        ...
        for index, item in enumerate(dataFile):
            ...
            rowInfoList.append({'colnum':len(item), 'rowdata':item, 'delimiterno':delimiterNo})
...
def getweatherinfo(projId):
    ...
    try:
        ...
        update_permit = MemcacheManager.get('weatherupdatetime%s'%str(projId))
        if update_permit is not None or isinstance(update_permit, int) or isinstance(update_permit, float):
            if time() - update_permit <= 60*60*2:
                rt = MemcacheManager.get(memkey)
        else:
            MemcacheManager.set('weatherupdatetime%s'%str(projId), 0)
    except Exception as e:
...
def get_weather_info_by_cityname(city):
    ...
    try:
        ...
        update_permit = MemcacheManager.get('weatherupdatetime%s'%str(city))
        if update_permit is not None or isinstance(update_permit, int) or isinstance(update_permit, float):
            if time() - update_permit <= 60*60*2:
                rt = MemcacheManager.get(memkey)
        else:
            MemcacheManager.set('weatherupdatetime%s'%str(city), 0)
    except Exception as e:
...
def updateweatherinfo(projId, info):
    ...
    try:
        memkey = 'weatherinfo' + str(projId)
        if MemcacheManager.set(memkey, info):
            MemcacheManager.set('weatherupdatetime%s'%str(projId), time())
            rt = True
    except Exception as e:
...
def update_weather_info_by_cityname(city, info):
    ...
    try:
        memkey = 'weatherinfo' + str(city)
        if MemcacheManager.set(memkey, info):
            MemcacheManager.set('weatherupdatetime%s'%str(city), time())
            rt = True
    except Exception as e:
