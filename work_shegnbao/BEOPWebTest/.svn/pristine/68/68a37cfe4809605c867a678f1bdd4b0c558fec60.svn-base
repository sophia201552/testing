    ...
    ModalAppButton.prototype.renderModal = function (e) {
        ...
        for (var i = 0 ; i < len; i++) {
            ...
            if (appButtonArr[i].backColor == "") {
                divIcon.style.background = staticColor[i];//"-webkit-gradient(radial, 184 -25, 161, 220 -257, 465, from(#fabd3e), to(#f4ae32))";
            } else {
    ...
    ModalAppButton.prototype.attachMonitorEvent = function () {
        ...
        $(".divMonitor").on('click', '.divIcon', function (e) {
            ...
            if (_this.$modal) {
                ...
                if (_this.tempOpt.option.appButton[index].iconColor) {
                    $iptColor.val(_this.tempOpt.option.appButton[index].iconColor);
                }
                if (_this.tempOpt.option.appButton[index].icon) {
                    _this.$modal.find('.bs-glyphicons-list .' + _this.tempOpt.option.appButton[index].icon.split(' ')[1]).parent().addClass('selected');
                }
            } else {
                WebAPI.get('static/scripts/spring/entities/modalMonitor.html').done(function (resultHTML) {
                    ...
                    _this.$modal.modal('show');
                    if (_this.tempOpt.option.appButton[index].iconColor) {
                        _this.$modal.find('#iptColorSel').val(_this.tempOpt.option.appButton[index].iconColor);
                    }
                    if (_this.tempOpt.option.appButton[index].icon) {
                        _this.$modal.find('.bs-glyphicons-list .' + _this.tempOpt.option.appButton[index].icon.split(' ')[1]).parent().addClass('selected');
                    }
                    ...
                    _this.$modal.find('.btnSure').click(function (e) {
                        ...
                        _this.tempOpt.option.appButton[index].icon = icon;
                        _this.tempOpt.option.appButton[index].iconColor = $iptColor.val();
...
var ModalAppHistory = (function () {
    ...
    ModalAppHistory.prototype.optionDefault = {
        title:{
            text:"1"
        },
        color: ['rgb(250,214,74)'],
        tooltip : {
            trigger: 'axis',
            axisPointer : {            // 坐标轴指示器，坐标轴触发有效
    ...
    ModalAppHistory.prototype.renderPreviewModal = function (points) {
        var _this = this;
        var pointsId = [];
        var pointsData = [];
        for(var i=0,length=points.length;i<length;i++){
            pointsId.push(points[i].dsItemId);
            pointsData.push(points[i].data);
        }
        //拿到数据源的备注
        WebAPI.post('/analysis/datasource/getDsItemsById',pointsId).done(function(result){
            
            if($(_this.container).find(".listDataBox").html() === ""){
                for(var j=0,jLength=result.length;j<jLength;j++){
                    if(j===0){
                        var singlePointer='<div class="singleContent activeSingle" data-id='+result[j].id+'>\
                                            <div class="singleTitle">'+result[j].alias+'</div>\
                                            <div class="singleValue">'+Number(pointsData[j]).toFixed(2)+'</div>\
                                        </div>';
                    }else{
                        var singlePointer='<div class="singleContent" data-id='+result[j].id+'>\
                                            <div class="singleTitle">'+result[j].alias+'</div>\
                                            <div class="singleValue">'+Number(pointsData[j]).toFixed(2)+'</div>\
                                        </div>';
                    }
                ...
                var $list = $(_this.container).find(".singleContent");
                $list.each(function(){
                    var index = $list.index($(this)); 
                    $(this).find(".singleValue").text(Number(pointsData[index]).toFixed(2));
                })
            }
            var $allSingleContent = $(_this.container).find(".singleContent");
            var sumWidth = 0; 
            $allSingleContent.each(function(){
                sumWidth += $(this).width()+5;
            })
            $(_this.container).find(".listDataBox").width(sumWidth);
        })
     //底部选择时间的容器
        if($(_this.container).find("#bottomPoints").length === 0){
            ...
            var endTime = nowDate.timeFormat(timeFormatChange('yyyy-mm-dd hh:ii:ss'));
            var startTime = nowDate.timeFormat(timeFormatChange("yyyy-mm-dd"))+' 00:00:00';
            ...
            $datetimepickerStart.datetimepicker({
                format: timeFormatChange("yyyy-mm-dd")+' 00:00:00',
            ...
            $datetimepickerEnd.datetimepicker({
                format: timeFormatChange('yyyy-mm-dd hh:ii:ss'),
    ...
    ModalAppHistory.prototype.updateModal = function (points) {
        this.renderPreviewModal(points);
    },
    ModalAppHistory.prototype.renderModal = function (id,startTime,endTime,tPeriod) {
        if($(this.container).find(".middlePointers").length === 0 && $(this.container).find(".bottomBox").length === 0 && $(this.container).find("#chartBox").length === 0){
            ...
            var $bottomBox = $('<div class="bottomBox" style="height:40%"></div>');
            $bottomBox.appendTo(this.container);
            var $chartBox = $('<div id="chartBox" style="height:235px"></div>');
            $middlePointers.before($chartBox);
        }
        var _this = this;
        //当天
        var todayEndTime = new Date().format('yyyy-MM-dd HH:mm:ss');
        ...
        WebAPI.post('/analysis/startWorkspaceDataGenHistogram', {
            ...
            if($("#chartBox").html()===""){//第一次
                WebAPI.post('/analysis/datasource/getDsItemsById',id).done(function(result){
                    for(var i=0,length=dataSrc.list.length;i<length;i++){
                        dataArr = dataSrc.list[0].data;
                        _this.optionDefault.title.text = result[0].alias;
                        _this.optionDefault.series[0].name = result[0].alias;
                        _this.optionDefault.legend.data[0] = result[0].alias;
                    }
                    _this.optionDefault.series[0].data = dataArr;
                    _this.optionDefault.xAxis[0].data = time;
                    
                    var $chartContainer = $(_this.container).find("#chartBox");
                    !_this.chart && (_this.chart = echarts.init($chartContainer[0], AppConfig.chartTheme));
                    _this.chart.setOption(_this.optionDefault);
                    _this.spinner && _this.spinner.stop();
                })
            }else{
                //默认显示第一个x选中的数据  点击下面的
                var $firstSelected = $(_this.container).find(".activeSingle");//当前选中的点
                var singlePointerId = $firstSelected.attr("data-id");
                var singlePointerTitle = $firstSelected.find(".singleTitle").text();
                for(var i=0,length=dataSrc.list.length;i<length;i++){
                    if(dataSrc.list[i].dsItemId === singlePointerId){
                        var dataArr = dataSrc.list[i].data;
                        _this.optionDefault.title.text = singlePointerTitle;
                        _this.optionDefault.series[0].name = singlePointerTitle;
                        _this.optionDefault.legend.data[0] = singlePointerTitle;
                    }
                }
                _this.optionDefault.series[0].data = dataArr;
                _this.optionDefault.xAxis[0].data = time;
    ...
    ModalAppHistory.prototype.attachEvent = function () {
       ...
       $(this.container).off('click').on("click","#btnQuery",function () {
            ...
            var id = $(_this.container).find(".activeSingle").attr("data-id");
            _this.renderModal(id,startTime,endTime,tPeriod);
        })
       //中间各项点击事件
        ...
        WebAPI.post('/analysis/startWorkspaceDataGenPieChart',{dsItemIds: dsItemIds}).done(function(result){
            ...
            if(result.dsItemList[0].data.indexOf('MonthRankList') !== -1){
                ...
                for(var j=0,jLength=data.length;j<jLength;j++){
                    var sonContent = '<tr class="diagnosisTr borderColorTemp3">\
                                    <td>'+(j+1)+'</td>\
                ...
                $(_this.container).find('.diagnosisTr').off('click').click(function(){
                    ...
                    WebAPI.post('/diagnosis/getFaultDetails',postData).done(function(faultDetail){
                        ...
                        faultInfos['containerScreen'] = $(_this.container).parents('.indexContent');
...
var ModalAppPie = (function(){
    ...
    ModalAppPie.prototype.updateModal = function (point) {
        ...
        for(var i = 0;i<data.EnergyList[0].children.length;i++){
            ...
            if(colorJ===data.EnergyList[0].children.length-1) colorJ = 0;
