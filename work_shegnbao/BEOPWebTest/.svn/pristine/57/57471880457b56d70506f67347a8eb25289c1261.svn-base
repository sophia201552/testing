...
var ReportScreen = (function () {
    ...
    ReportScreen.prototype = {
        ...
        registerExportWordEvent: function () {
            $(ElScreenContainer)
                .off('click.report.exportWord', '#exportWord')
                .on('click.report.exportWord', '#exportWord', this.requestWordDownload.bind(this));
        },
        registerExportPDFEvent: function () {
            var _this = this;
            $(ElScreenContainer).off('click.report.exportPDF', '#exportPDF').on('click.report.exportPDF', '#exportPDF', function () {
                var chart = null,
                    $this = $(this),
                    $report = $('.step-play-list').clone(),
                    reportFolderName = $this.attr('reportFolderName'),
                    version = $this.attr('version'),
                    pdfName = AppConfig.projectShowName + '.' + _this.getReportName() + '.' + timeFormat(version, timeFormatChange('yyyy-mm-dd')) + '.pdf',
                    uploadPromises = [];
                        ...
                        $.when(pdfTemplatePromise, coverPagePromise).done(function (template, result) {
                            WebAPI.get("/project/getinfo/" + AppConfig.projectId).done(function (rs) {
                                ...
                                result = result[0].formatEL({ // 根据返回的封面页组装数据
                                    ...
                                    date: timeFormat($("#beopReport").find(' .input-append input').val(), timeFormatChange('yyyy-mm-dd')),
