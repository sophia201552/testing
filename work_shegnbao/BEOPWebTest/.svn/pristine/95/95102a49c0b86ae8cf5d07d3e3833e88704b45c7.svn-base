...
def project_create():
    ...
    if len(beopDBList) > 0:
        ...
        if dbName in beopDBList:
            ...
            newProjId = BEOPDataAccess.getInstance().createProject(projName, projName, projName+'.s3db', dbName, latlng, address, projName, createUserId, int(weatherStationId), projImg)
            if datafileDir != None and os.path.exists(datafileDir):
...
@app.route('/analysis/modal/saveThumb/<userId>', methods=['POST'])
def analysisModalSaveThumb(userId):
    rt = {}
    userId = int(userId)
    data = request.get_json()
    if isinstance(data, dict):
        rt = BEOPMongoDataAccess.getInstance().analysisModalSaveThumb(userId, data)
    return json.dumps(rt)
...
def analysisModalRemove(userId, workspaceId, modalId):
    rt = {}
    userId = int(userId)
    if isinstance(workspaceId, str) and isinstance(modalId, str):
...
def analysisWorkspaceSaveLayout(userId):
    ...
    if isinstance(data, dict):
        keys = data.keys()
        if 'workspaceId' in keys and 'modalList' in keys:
...
@app.route('/mongodb/rename', methods=['GET'])
def renameMongoCollection():
    rt = BEOPMongoDataAccess.getInstance().renameCollection()
    return json.dumps(rt)
