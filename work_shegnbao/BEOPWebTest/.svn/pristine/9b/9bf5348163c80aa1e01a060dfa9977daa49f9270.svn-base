...
from flask import request, url_for, redirect, render_template, json, jsonify, make_response, send_file
import requests
...
@app.route('/diagnosis/downloadHistoryFault/<projectId>/<startTime>/<endTime>')
def download_file_of_historyFault(projectId, startTime, endTime):
...
def get_historyFault(projectId, startTime, endTime):
    ...
    try:
        ...
        dbname = dbAccess.getProjMysqldb(projectId)
        if dbname != None:
            strSQL = 'SELECT n.Time, n.`Status`, n.CheckTime, n.Operator, f.Id, f.NAME, f.DefaultGrade, f.IsUserDefined, f.UserFaultGrade, e.`Name`, ' \
                     'z.SubBuildingName FROM %s_diagnosis_notices AS n ' \
                    ...
                    for item in dbrv:
                        try:
                            if item[n] in fault_dict.keys():
                                fault_dict.get(item[n]).get('arrNoticeTime').append({'NoticeId': item[6],
                                                                                     'Time':item[7].strftime('%Y-%m-%d %H:%M:%S')})
                            else:
                                fault_dict.update({item[n]:{'arrNoticeTime':[{'NoticeId': item[6],
                                                                              'Time':item[7].strftime('%Y-%m-%d %H:%M:%S')}]}})
                                k = n
                                for i in keys:
        ...
        if post_data:
            ...
            if dbname:
                strSQL = 'SELECT f.`Name` as FaultName, f.Description, group_concat(f.EquipmentId) as EquipmentId, ' \
                         ...
                         'COUNT(IF(`Status` = 1, TRUE, NULL)) AS weichuli, COUNT(IF(`Status` = 0, TRUE, NULL)) AS yichuli ' \
                         ...
                         'GROUP BY f.`Name`, f.TargetExecutor'%(dbname, dbname, dbname, endTime, startTime)
                dbrv = dbAccess._mysqlDBContainerReadOnly.op_db_query('diagnosis', strSQL)
                if dbrv:
                    group_dict = {}
                    for item in dbrv:
                        try:
                            ...
                            if item[4] in group_dict.keys():
                                group_dict.get(item[4]).get('list').append({'FaultName':item[0], 'Grade':grade, 'Desc':item[1],
                                                                            'Status':[item[11], item[12]], 'ExecuterId':item[5],
                                ...
                                group_dict.update({item[4]:{'GroupName':item[7], 'GroupId':item[4],
                                                            'list':[{'FaultName':item[0], 'Grade':grade, 'Desc':item[1],
                                                                     'Status':[item[11], item[12]], 'ExecuterId':item[5],
                                                                     'ExecuterName':item[6], 'EquipmentId':item[2],
                                                                     'EquipmentName':item[3], 'Target':get_target_by_equipment(item[3], item[2])}]}})
                        except Exception as e:
