...
class TransactionEmail():
    ...
    # 发送给执行人和审核人和相关人员
    def send_new_email(self, trans_id, request_json, pending_files, lang="zh-CN"):
        user = User()
        email_list = []
        ...
        if executor_email is not None and not not executor_email:
            try:
                ...
                pass
        # 审核人和相关人员的邮箱列表
        verifier_list = verifier_list + watcher_list
        ...
        for verifier_mail in list(set(email_list)):
            # 如果是空或者和执行人，创建人的邮箱重复
    ...
    def send_forward_email(self, user_id, trans_id, old_executor, current_executor,
        ...
        for watchers_email in watcher_email_list:
            if watchers_email is None or not watchers_email or verifiers_email == current_executor_email or verifiers_email == old_executor_email or watchers_email in verifiers_email_list:
                continue
            else:
                try:
                    email_html = render_template('email/workflowEditEmail.html', configMap=forward_config_map,
