...
var DiagnosisScreen = (function () {
    ...
    DiagnosisScreen.prototype = {
        ...
        insertLogItem: function (item, itemNum, className) {
            ...
            //添加反馈;
            $('<span class="glyphicon glyphicon-comment grow span-hover-pointer span-btn" title="feedback">')
                .click(function (e) {
                    WebAPI.get("/static/views/workflow/temp_user_comments.html").done(function (resultHtml) {
                        var $dialog = $('#dialogModal').find('#dialogContent').empty().html(resultHtml).end();
                        $dialog.find('#feedbackTitle').text(item.name);
                        $dialog.find('#feedbackCritical').text(I18n.resource.workflow.urgencyLevel[item.grade]);
                        $dialog.find('#feedbackDetail').text(item.description);
                        $dialog.modal();
                        var curveInfo = {
                            projectId: Number(item.project),
                            chartPointList: item.points,
                            chartQueryCircle: 'm5',
                            chartStartTime: new Date(new Date(item.time.toDate()).getTime() - 43200000 /*12 * 60 * 60 * 1000*/).format(DateUtil.DATA_FORMAT.FULL_DATETIME),
                            chartEndTime: new Date(new Date(item.time.toDate()).getTime() + 43200000 /*12 * 60 * 60 * 1000*/).format(DateUtil.DATA_FORMAT.FULL_DATETIME)
                        };
                        if (item.points) {
                            beop.view.faultCurve.configModel({
                                transactions_model: beop.model.transactionsModel
                            });
                            beop.view.faultCurve.init($dialog.find('.fault-feedback-curve'), curveInfo);
                        } else {
                            $dialog.find('.fault-feedback-curve').hide();
                        }
                        ...
                        $dialog.find('form').submit(function () {
                            ...
                            var formMap = {
                                'executor[]': [1],
                                detail: I18n.resource.workflow.main.ORDER_EQUIPMENT_NAME + currentEquipmentName + "\n" +
                                        I18n.resource.workflow.main.ORDER_FAULT_INFO + item.description + "\n" +
                                        I18n.resource.workflow.main.ORDER_FEEDBACK_MSG + $('#commentsContent').val(),
                                userId: AppConfig.userId,
                                title: I18n.resource.workflow.main.FEEDBACK_TITLE + item.name,
                                critical: item.grade,
                                dueDate: new Date((new Date().getTime() + 2592000000 /*1000*60*60*24*30*/)).format(DateUtil.DATA_FORMAT.FULL_DATETIME)
                            };

                            $.extend(formMap, curveInfo);
                            Spinner.spin(document.body);
                            WebAPI.post('/workflow/transaction/new/', formMap).done(function (result) {
                                if (result.success) {
                                    $dialog.modal('hide');
                                    alert(I18n.resource.workflow.main.FEEDBACK_SUCCESS);
                                } else {
                                    alert.danger('feedback failed');
                                }
                            }).always(function () {
                                Spinner.stop();
                            })
