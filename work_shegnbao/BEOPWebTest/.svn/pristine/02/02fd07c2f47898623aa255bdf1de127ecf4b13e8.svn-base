...
var ModelTempDistribution = (function () {
    ...
    function calcAllTempData(rawData, x, y, w, h) {
        ...
        configMap.TEMP_RANGE.max = getArrayMax(handledTempArray);
        return allTempPoint;
    }
    ...
    ModelTempDistribution.prototype.update = function (bound, value) {
        ...
        for (var m = 0, len = this.data.length; m < len; m++) {
            if (this.data[m].idCom === bound) {
                this.data[m].value = Number(value) ? Number(value) : 0;
            }
        }
        var allHeatData = calcAllTempData(this.data, this.x, this.y, this.width, this.height);
