...
var ReportScreen = (function () {
    ...
    ReportScreen.prototype = {
        ...
        initReportList: function () {
            ...
            $reportNavList.css({
                left: parseInt($(".step-container").offset().left) - parseInt($reportNavList.width()) + 1 - 2 + 'px'
            });
        ...
        getReport: function (year, month, day, isWeekly) {
            ...
            var getReportFailed = function () {
                ...
                WebAPI.get('/static/projectReports/emptyReport.html').done(function (resultHtml) {
                    ...
                    if (_this.dateTimePickerInline) {
                        BEOPUtil.setRelativePosition($(".step-container"), $(".reportNavigation"), 0, -10);
                    } else {
                        BEOPUtil.setRelativePosition($(".step-container"), $(".reportNavigation"), 0, 50);
                    }
                    $(window).resize(function () {
                        if (_this.dateTimePickerInline) {
                            BEOPUtil.setRelativePosition($(".step-container"), $(".reportNavigation"), 0, 39);
                        } else {
                            BEOPUtil.setRelativePosition($(".step-container"), $(".reportNavigation"), 0, 50);
                        }
                    });
            ...
            return WebAPI.get("/report/getReport/" + AppConfig.projectName + "/" + reportFolderName + '/' + version).done(function (resultHtml) {
                ...
                var $pdfContent = '<a id="exportPDF" class="pdf_download exportPDF pa"><span class="add-on"><img class="pdf_img" src="/static/images/pdf_download.png"　alt="PDF" /></span><span class="pdf_text" i18n="observer.reportScreen.PDF_DOWNLOAD"></span></a>';
        ...
        renderCharts: function ($chartContainer) {
            ...
            if (jscd.mobile) {
                ...
                window.addEventListener('load', function () {
                    ...
                    var m = 0;
                    ReportScreen = null;
                    for (m = 0; m < _this.chartList.length; m++) {
