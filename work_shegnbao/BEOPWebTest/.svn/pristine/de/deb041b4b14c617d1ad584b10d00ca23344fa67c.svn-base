...
class BEOPMongoDataAccess:
    ...
    def __init__(self):
        ...
        if not bOk:
            logging.error('mongodb connect failed')
    ...
    def analysisDataSourceRemoveSingle(self, datasourceId, datasourceItemId, userId):
        assert(isinstance(datasourceId, str))
        assert(isinstance(datasourceItemId, str))
    ...
    def analysisModalSave(self, userId, data):
        ...
        try:
            if self.mdbBb[g_tableWorkspace].find_one({'userId':userId}) == None:
                assert('workspaceId' not in data.keys())
                assert('id' not in data['modal'].keys())
                assert('workspaceName' in data.keys())
                oj_workspaceId = ObjectId()
                oj_modalId = ObjectId()
                assert(isinstance(data['modal']['option'], dict))
                assert('imagebin' in data['modal'].keys())
                self.mdbBb[g_tableWorkspace].save({'userId':userId, 'workspaceList':[{'id':oj_workspaceId, 'name':str(data['workspaceName']),
                                                ...
                                                'note':str(data['modal']['note']), 'imagebin':bytes(data['modal']['imagebin'].encode()), 'option':data['modal']['option']}]}]})
                ...
                if 'workspaceId' in data.keys() and 'workspaceName' in data.keys() and 'modal' not in data.keys():
                    ...
                    if 'workspaceId' not in data.keys():
                        ...
                        if 'id' not in data['modal'].keys():
                            ...
                            assert(isinstance(data['modal']['option'], dict))
                            doc = {}
                            if len(data['modal']['imagebin']) > 0:
                                doc = {'$addToSet':{'workspaceList.$.modalList':{'id':oj_modalId, 'name':str(data['modal']['name']),
                                    'type':str(data['modal']['type']), 'note':str(data['modal']['note']), 'imagebin':bytes(data['modal']['imagebin'].encode()), 'option':data['modal']['option']}}}
                            else:
                                doc = {'$addToSet':{'workspaceList.$.modalList':{'id':oj_modalId, 'name':str(data['modal']['name']),
                            ...
                            if index >= 0:
                                ...
                                doc = {}
                                if len(data['modal']['imagebin']) > 0:
                                    doc = {'$set':{arrayIndex:{'id':ObjectId(str(data['modal']['id'])), 'name':str(data['modal']['name']),
                                           'type':str(data['modal']['type']), 'note':str(data['modal']['note']), 'imagebin':bytes(data['modal']['imagebin'].encode()), 'option':data['modal']['option']}}}
                                else:
                                    doc = {'$set':{arrayIndex:{'id':ObjectId(str(data['modal']['id'])), 'name':str(data['modal']['name']),
                                           'type':str(data['modal']['type']), 'note':str(data['modal']['note']), 'option':data['modal']['option']}}}
                                self.mdbBb[g_tableWorkspace].update(post, doc)
                                rt = {'workspaceId':oj_workspaceId.__str__(), 'modalId':oj_modalId.__str__()}
        except Exception as e:
    ...
    def analysisWorkspaceSaveLayout(self, userId, workspaceId, modalList):
        assert(isinstance(userId, int))
        assert(isinstance(workspaceId, str))
        assert(isinstance(modalList, list))
        ...
        for newitem in modalList:
            for oldItem in oldList:
                if newitem == oldItem['id'].__str__():
                    #newList.append({'id':oldItem['id'].__str__(), 'type':oldItem['type'], 'name':oldItem['name'], 'option':oldItem['option'], 'note':oldItem['note'], })
    ...
    def saveHistoryData(self, pointName, pointValue, pointTime, dbNameMongo):
        assert(len(pointName) > 0)
        assert(isinstance(pointTime, datetime))
    ...
    def saveDataSourceGroupLayout(self, userId, newGroupIdList):
        assert(isinstance(userId, int))
        assert(isinstance(newGroupIdList, list))
