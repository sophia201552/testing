...
#         'status': 0,
#     }]
@bp_strategy.route('/item/getList/<projId>', methods=['GET'])
def getStrategyItemList(projId):
    ...
    return formatResult(True, 'Success', {'data': rt})


# 根据策略ID，获取策略详情
# return.data: struct_DB_Strategy.Strategy_Item
@bp_strategy.route('/item/get/<strategyId>', methods=['GET'])
...
def saveStrategyItem():
    # 1、更新到数据库
    # 2、写入LOG
    return formatResult(True, 'Success', {'_id': '565558adf18a890a2448075b'})


# 根据策略ID，删除策略
# post.data: null
@bp_strategy.route('/item/remove/<strategyId>', methods=['POST'])
...
@bp_strategy.route('/item/setStatus', methods=['POST'])
def setStrategyItemsStatus():
    # 1、更新到数据库
    # 2、写入LOG
    return formatResult(True, 'Success')
...
#         'type': 0,
#     }]
@bp_strategy.route('/template/getList/<userId>', methods=['GET'])
...
@bp_strategy.route('/template/get/<templateId>', methods=['POST'])
