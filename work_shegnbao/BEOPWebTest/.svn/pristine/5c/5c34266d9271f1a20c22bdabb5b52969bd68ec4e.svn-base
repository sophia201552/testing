...
class Transaction(DbEntity):
    ...
    def get_working(self, user_id, start_num, offset, order=('id', 'DESC')):
        ...
        return self.query_distinct(self.fields, where=(
            ...
            '(SELECT DISTINCT  transId FROM transaction_member WHERE userId=%s and type="2") or executorID=%s or creatorID=%s)',
    ...
    def get_trans_all_people_id_by_id(self, trans_id):
        trans = self.get_transaction_by_id(trans_id)
        tm = TransactionMember()
        rv = []
        verifier_list = []
        watcher_list = []
        if trans:
            creator = trans.get("creatorID")
            executor = trans.get("executorID")
            watcher_map = tm.get_watcher(trans_id)
            verifier_map = tm.get_verifier(trans_id)

            creator = [creator] if creator else  []
            executor = [executor] if executor else  []
            if watcher_map:
                for watcher_item in watcher_map:
                    watcher_list.append(watcher_item.get("userId"))

            if verifier_map:
                for verifier_item in verifier_map:
                    watcher_list.append(verifier_item.get("userId"))

            rv = list(set(creator + executor + watcher_list + verifier_list))
        return rv
