...
def workflow_transaction_newdiagnosealarm():
    return json.dumps(BEOPDataAccess.getInstance().workflowTransactionNewDiagnoseAlarm(request.get_json()),ensure_ascii=True)
...
@app.route('/getTransactionGroup')
def getTransactionGroup():
    return json.dumps(BEOPDataAccess.getInstance().getTransactionGroup(), ensure_ascii=False)

@app.route('/workflow/complete/', methods=['POST'])
def workflow_complete():
    request_data = request.get_json()
    trans_id = request_data['id']
    user_id = request_data['userId']
    if trans_id is None or user_id is None:
        return 'transaction id or user id is empty.'
    else:
        return BEOPDataAccess.getInstance().completeWorkflowTransaction(trans_id, user_id)
...
def workflow_transaction_edit():
    ...
    if trans_id is None:
        ...
        return BEOPDataAccess.getInstance().editTransaction(trans_id, user_id, title=title, dueDate=due_date,
...
def workflow_detail_reply(user_id, record_id):
    list_record = BEOPDataAccess.getInstance().workflow_get_information_by_record_id(user_id, record_id)
    list_reply = BEOPDataAccess.getInstance().workflow_get_transaction_reply(record_id)
    list_idname_maping = BEOPDataAccess.getInstance().get_user_id_name_mapping()
    #is_editable = BEOPDataAccess.getInstance().is_notice_editable(user_id, record_id)
    my_dict = {'detail_reply': list_reply, 'list_record': list_record,
               'list_idname_maping': list_idname_maping, 'isEditable': False}
    return json.dumps(my_dict)
...
def workflow_transaction_add():
    ...
    if user_id is None:
        ...
        return json.dumps(BEOPDataAccess.getInstance().addTransaction(user_id, due_date, title, detail, group_id, executor_id, critical),
                          ensure_ascii=True)
...
def insert_one_notice_detail():
    ...
    detail = request_json.get('detail')
    replyToId = request_json.get('replyToId')
    if ofTransactionId is None:
        ...
        return 'error: detail is empty'
    elif replyToId is None:
        return 'error: replyToId is empty'
    else:
        result = BEOPDataAccess.getInstance().add_transaction_reply(ofTransactionId, replyUserId, replyTime, detail, replyToId)
        return 'success' if result else 'failed'
