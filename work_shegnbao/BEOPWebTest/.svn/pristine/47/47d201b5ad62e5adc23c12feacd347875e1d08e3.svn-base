    ...
    DiagnosisLogHistory.prototype = {
        show: function () {
            ...
            WebAPI.get("/static/views/observer/diagnosis/paneHistory.html").done(function (resultHtml) {
                ...
                //页面上没有dialog摸态框
                if (dialog.length < 1) {
                    $('body').append('<div class="modal fade" id="dialogModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"><div class="modal-dialog modal-lg" id="dialogContent"></div></div>');
                    dialog = $('#dialogModal');
                };
        ...
        init: function () {
            ...
            $("#btnDownload").off('click').click(function () {
                ...
                window.open('/diagnosis/downloadHistoryFault/' + AppConfig.projectId + '/' + startTime + '/' + endTime);
            });
            $("#btnDownloadPDF").off('click').click(function () {
            ...
            for (var i = 0, len = data.length; i < len; i++) {
                ...
                if (!item.resTime || item.resTime == null) {
                    ...
                    if (!this.isModal) {
                        ...
                        $(tr).append($('<td class="removeNode"></td>').append($spanPoint));
                    }
                } else {
