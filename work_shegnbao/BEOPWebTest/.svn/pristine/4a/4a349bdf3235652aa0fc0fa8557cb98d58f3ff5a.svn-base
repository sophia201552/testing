var DiagnosisConfig = (function () {
    ...
    DiagnosisConfig.prototype = {
        show: function () {
            ...
            WebAPI.get("/static/views/observer/diagnosis/diagnosisConfig.html").done(function (resultHtml) {
                ...
                $dialog.off('hidden.bs.modal').on('hidden.bs.modal', function (e) {
                    _this.close();
                    Spinner.stop();
                    $dialog.find('#dialogContent').empty();//when modal is closed, dom is removed
                }).modal({});
        ...
        init:function(){
            _this.isInit = true;
            WebAPI.get('/diagnosis/config/get/' + AppConfig.projectId).done(function (rs) {
                if(rs.limit){
        ...
        getGroupsMembers: function(){
            WebAPI.get('/diagnosis/getGroupsMembers/' + AppConfig.userId).done(function (data) {
                if(!data) return;
                var $group = $('#group');
