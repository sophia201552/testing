            ...
            $.get('/workflow/group/' + AppConfig['userId'] + '/' + this.group.id).done(function (result) {
                ...
                WorkflowTool.getWorkflowTemplate('#wf-transaction-task').done(function (temp) {
                    for (var n = 0, nlen = trans.length; n < nlen; n++) {
                        ...
                        inner_temp.find('.task-title').html('<span class="taskNoWrapper"><em i18n="workflow.common.NUMBER"></em><em class="taskNo">'+item.id+'</em></span>：'+item.title)
                                   .attr('title', I18n.resource.workflow.common.NUMBER +item.id+":"+item.title);
                        ...
                        inner_temp.find('.timer').children("span").removeClass().addClass("glyphicon ic"+item.critical).attr("i18n","title="+_this.i18urgencyLevelArr[item.critical]);
