...
var MemberManager = (function () {
    ...
    MemberManager.prototype = {
        ...
        attachEvents: function () {
            ...
            $userTreeCon.on('click', '.setting', function (event) {//点击管理面板-员工管理图片设置
                ...
                WebAPI.post('/admin/loadUsersSetting', {
                    ...
                    if (result.success) {
                        result.data.managers.sort(function(a, b){ return a.userfullname > b.userfullname })
            ...
            }).on('click', '.icon-minus-sign', function () {//点击管理面板-minus图标
                var $this = $(this);
                var $opSpan = $this.parents("span");
                var $ul = $opSpan.next("ul");
                var ulL = $ul.length;
                if (ulL) {
                    $ul.slideUp(300);
                    $this.removeClass("icon-minus-sign").addClass("icon-plus-sign");
                }
            }).on('click', '.icon-plus-sign', function () {//点击管理面板-minus图标
                var $this = $(this);
                var $opSpan = $this.parents("span");
                var $ul = $opSpan.next("ul");
                var ulL = $ul.length;
            ...
            $userSettingWin.on("click", "#userDel", function () {//点击页面管理-浮动窗口删除按钮
                ...
                if (supervisor != oldSupervisor || isManager != oldIsManager) {
                ...
                var userRecord = _this.managerCtrl.renderRecordsEvent.call(_this.managerCtrl,
                    userId,
                    $('#recordLogDateStart'),
                    $('#recordLogDateEnd'),
                    $('#recordInfoDate'),
            ...
            $('#addPersonWin').on('change', '#selectSupervisor', function () {
                ...
                WebAPI.post('/admin/getProjectPermissionByUserId',
                    {userId: selectedUserId}).done(function (result) {
                        if (result.success) {
        ...
        searchUserObj: function (val, $tree) {//员工管理页面-得到查询用户列表
            ...
            var userNameAllL = $userNameAll.length;
            for (var i = 0; i < userNameAllL; i++) {
                userNameArr.push($userNameAll.eq(i).text());
                userPicArr.push($userPicAll.eq(i).attr("src"));
                userIdArr.push($userLiAll.eq(i).attr("userid"));
            }
            var userNameArrL = userNameArr.length;
            for (var i = 0; i < userNameArrL; i++) {
            ...
            for (var i = 0; i < arrNameInfo.length; i++) {
                var userNameStr = arrNameInfo[i];
                var userPicUrl = arrPicInfo[i];
                var userId = arrIdInfo[i];
                li += '<li userId="' + userId + '" class="pr"><input type="text" value="' + userNameStr + '" class="pa" readonly /><img src="' + userPicUrl + '" class="pa" style="z-index:55" /></li>';
            }
        ...
        selectedUserObj: function (userid, searchText) {//员工管理页面-选中用户对象
            ...
            var $targetI = $targetUl.prev("span").find(".icon-plus-sign");
            $span.removeClass("selected");
            $userSpan.addClass('selected');
            $targetI.removeClass("icon-plus-sign").addClass("icon-minus-sign");
