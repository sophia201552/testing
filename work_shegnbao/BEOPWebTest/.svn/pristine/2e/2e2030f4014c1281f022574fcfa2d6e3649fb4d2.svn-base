...
class BEOPDataAccess:
    ...
    def getOperationRecordByUserId(self, userId, rownumber):
        ...
        if userId == '-1':
            sql = '''select u.userfullname, u.userpic, op.id, op.opTime, op.userId, op.op, op.title, op.detail, op.linkToTransactionId
            ...
            sql = '''select u.userfullname, u.userpic, op.id, op.opTime, op.userId, op.op, op.title, op.detail, op.linkToTransactionId
                      FROM operation_record op left join beopdoengine.user u on op.userId=u.id
        ...
        return [{'userName': x[0], 'userpic': x[1], 'id': x[2],
                 'opTime': x[3].strftime('%Y-%m-%d %H:%M:%S'), 'userId': x[4], 'op': x[5],
                 'title': x[6], 'detail': x[7], 'linkToTransactionId': x[8]} for x in rvQuery]
    ...
    def addTransaction(self, user_id, due_date, title, detail, group_id, executor_id, critical, dbName=None,
        ...
        sql_transaction_insert = '''insert into transaction
                                 (id, creatorID, executorID, dueDate, title, detail, groupid, createTime,critical, dbName, chartPointList, chartQueryCircle, chartStartTime, chartEndTime)
        ...
        self._mysqlDBContainer.op_db_update(app.config['WORKFLOW_DATABASE'], sql_transaction_insert, param)
        self.addTransactionOperation(app.config['WORKFLOW_DATABASE'], user_id, idNew, 'new')
        self.sendWorkflowEmail(user_id, due_date, title, detail, executor_id, now, critical)
        return idNew

    def updateNoticeWithTransaction(self, db_name, notice_id, trans_id):
        if not db_name or notice_id is None or trans_id is None:
            return False
        sql = 'update diagnosis_notices set OrderID=%s where ID=%s'
        return self._mysqlDBContainer.op_db_update(db_name, sql, (trans_id, notice_id))
    ...
    def workflowProfile(self, user_id):
        ...
        result = rvQuery[0]
        return dict(id=result[0], username=result[1], userfullname=result[2], useremail=result[3], userpic=result[4])

    def createProject(self, name_en, name_cn, s3dbname, mysqlname, latlng,address,  name_english, userid, weatheaStationId=0, projImg='default.jpg'):
        dbname = app.config['DATABASE']
        sql = ('select max(id) from project')
        dbrv = self._mysqlDBContainer.op_db_query(dbname, sql, ())
        maxid = dbrv[0][0]
        newid = int(maxid)+1
        sql = "insert into project(id, name_en, name_cn, s3dbname, mysqlname,update_time,latlng,address,name_english,weather_station_id,pic) values " \
              "(%d, \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', %d, \'%s\')" % \
              (newid, name_en, name_cn, s3dbname, mysqlname, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), latlng, address, name_english, weatheaStationId, projImg)
    ...
    def workflow_get_transaction_reply(self, transaction_id):
        ...
        for item in rv:
            result.append({'id': item[0], 'ofTransactionId': item[1], 'replyUserId': item[2],
                           ...
                           'userpic': item[6], 'userfullname': item[7]})
        return result
    ...
    def workflow_get_transaction_operations(self, transaction_id):
        ...
        return [{'opTime': item[0].strftime('%Y-%m-%d %H:%M:%S'), 'op': item[1], 'title': item[2],
                 'detail': item[3], 'userfullname': item[4], 'userpic': item[5]} for item in rv]
    ...
    def workflow_get_fault_curve_data(self, dbName, chartPointList, chartQueryCircle, chartStartTime, chartEndTime):
        list_value = []
        list_time = []
        list_description = []
        ...
        for i in range(len(name_description_outer_list)):
            ...
            for j in range(len(name_description_inner_list)):
                if j % 2 == 0:
                    data = self.get_history_data_padded(dbName, [name_description_inner_list[j]], chartStartTime,
    ...
    def workflow_get_status_by_transaction_id(self, user_id, trans_id):
        ...
        sql = '''select ts.value as statusValue, dueDate, dbName, chartPointList,
                        ...
                        t.statusId, t.critical, t.executorId, u.userfullname creatorName
                 from transaction t
        ...
        return {'statusValue': transaction[0], 'due_date': due_date, 'list_description': list_description,
                ...
                'executorId': transaction[14], 'creator': transaction[15]}
    ...
    # mango added
    def add_transaction_reply(self, ofTransactionId, replyUserId, replyTime, detail):
        dbname = app.config['WORKFLOW_DATABASE']
        newID = self.getMaxIDOfTable('transaction_reply') + 1
        sql = 'insert into transaction_reply (id, ofTransactionId, replyUserId, replyTime, detail) values (%s, %s, %s, %s, %s)'
        param = (newID, ofTransactionId, replyUserId, replyTime, detail)
        return self._mysqlDBContainer.op_db_update(dbname, sql, param)
        ...
        if bSuccess:
            ...
            for index in range(len(pointNameList)):
                ...
                if isinstance(pointValueList[index], str) or isinstance(pointValueList[index], float) or isinstance(pointValueList[index], int):
                    ...
                    parameters = (pointNameList[index], '')
    ...
    def getWarningRecord(self, projId, startTime, endTime, unconfirmed=0):
        ...
        rv = self._mysqlDBContainer.op_db_query(app.config.get('DATABASE'), sql)
        for item in rv:
