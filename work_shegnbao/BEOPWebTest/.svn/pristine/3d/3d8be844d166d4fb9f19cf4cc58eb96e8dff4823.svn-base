...
class MenuConfigure:
    db = MongoConnManager.getConfigConn()
    ...
    def save_menu_model(self, project_id, new_model, indexed_ids=None):
        ...
        mongo_client_CustomNavItem = self.db.mdbBb[g_tableCustomNavItem]
        # table g_tableCustomNav
        mongo_client_CustomNav = self.db.mdbBb[g_tableCustomNav]
        current_customNav = mongo_client_CustomNav.find_one({'projectId': project_id})
        ...
        if all_add:
            ...
            try:
                for item in all_add:
                    ...
                    mongo_client_CustomNav.update({'projectId': project_id}, {'$addToSet': {'nav': new_id}})
                ...
                for item in all_delete:
                    ...
                    if ObjectId.is_valid(item_id):
                        ...
                        mongo_client_CustomNav.update({'projectId': int(project_id)},
                                                      {"$pull": {"nav": item_id}})
                        mongo_client_CustomNavItem.update({"$pull": {"_id": ObjectId(item_id)}})
            except Exception as e:
                success = False
        ...
        try:
            ...
            for child_item_nav in child_customNav_role:
                ...
                for child_item_nav_list_id in child_item_nav_list:
                    try:
                        result = mongo_client_CustomNavItem.find_one({"_id": ObjectId(child_item_nav_list_id)})
                        if result:
                            continue
                        else:
    ...
    def get_project_roles(self, project_id):
        mongo_client_CustomNav = self.db.mdbBb[g_tableCustomNav]
        nav = mongo_client_CustomNav.find_one({'projectId': project_id})
        roleNav = nav.get('roleNav', {})
    ...
    def update_all_page_menu(self, project_id, top_nav_list):
        table_custom_nav = self.db.mdbBb[g_tableCustomNav]
        try:
            page_menu = table_custom_nav.find_one({'projectId': int(project_id)})
            ...
            table_custom_nav.update_one({'projectId': int(project_id)}, {'$set': {'roleNav': new_role_nav}})
            return True
        except Exception as e:
            logging.error(e)
