...
class BEOPDataAccess:
        ...
        try:
            ...
            if ret:
                ...
                if dbrv == None or len(dbrv) < 1:
                    ...
                    projectInfoList = MemcacheManager.get('projectInfoList') if MemcacheManager.get(
                        'projectInfoList') != None else []
                ...
                if len(item.get('record')) == 1:
                    try:
                        temp.append(dict(time=item.get('record')[0].get('time'),
                                         value=float('%.2f' % float(item.get('record')[0].get('value'))), error=False))
                    except Exception as e:
    ...
    def validate_user(self, login):
        ...
        dbname = app.config['DATABASE']
        q = 'select id, unitproperty01, expiryDate from `user` where username=%s or usermobile=%s'
    ...
    def createProject(self, name_en, name_cn, s3dbname, mysqlname, latlng, address, name_english, userid, projImg,
                      weatheaStationId=0):
        dbname = app.config['DATABASE']
        ...
        if not (projImg is None):
            ...
            imgStr = 'default.jpg'
        sql = "insert into project(id, name_en, name_cn, s3dbname, mysqlname,update_time,latlng,address,name_english,weather_station_id,pic, collectionname) values " \
              "(%d, \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', %d,\'%s\',\'%s\')" % \
              (newid, name_en, name_cn, s3dbname, mysqlname, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), latlng,
               address, name_english, weatheaStationId, imgStr, name_en)
        if self._mysqlDBContainer.op_db_update(dbname, sql, ()):
            ...
            if self._mysqlDBContainer.op_db_update(dbname, sql, params):
                ...
                if self._mysqlDBContainer.op_db_update(dbname, sql, params):
                    tNow = datetime.now()
                    plm = MemcacheManager.get('projectLocateMap')
                    if plm:
                        ...
                        for key in mi:
                            ...
                            if item:
                                if item.get('writable') and item.get('is_default'):
                                    if self.InsertDefaultMonogoAdd(newid, key, tNow):
                                        return newid
                else:
                    return -2
        else:
    ...
    def InsertDefaultMonogoAdd(self, projId, mongoServerId, startTime):
        ...
        endTime = startTime + timedelta(days=700)
    ...
    def get_details(self, userId):
        ...
        sql = "SELECT id, username, usermobile, useremail, userfullname FROM `user` WHERE id IN (%s)" % ",".join(
            arrUserTemp)
        rvQuery = self._mysqlDBContainerReadOnly.op_db_query(dbname, sql, ())
        listMember = []
        for item in rvQuery:
    ...
    def getProjectNameById(self, projId):
        ...
        return dict(id=rs[0], name_en=rs[1], name_cn=rs[2], name_english=rs[3])
    ...
    def get_history_data_padded(self, projectId, pointList, strTimeStart, strTimeEnd, strTimeFormat):
        ...
        if startTime > endTime:
            print(
                'error params get_history_data_padded:[startTime:%s],[endTime:%s],[timeFormat:%s],[projectId:%d],[pointList:%s]' % (
                    strTimeStart, strTimeEnd, strTimeFormat, int(projectId)), str(pointList))
            logging.error(
                'error params get_history_data_padded:[startTime:%s],[endTime:%s],[timeFormat:%s],[projectId:%d],[pointList:%s]' % (
                    strTimeStart, strTimeEnd, strTimeFormat, int(projectId)), str(pointList))
    ...
    def getHistoryData(self, proj, pointName, timeStart, timeEnd, timeFormat):
        ...
        return self.mergeHisData(connList, dbname, pointName, timeStart, timeEnd, timeFormat)

    def mergeHisData(self, connList, dbname, pointName, timeStart, timeEnd, timeFormat):
        rt = []
        try:
    ...
    def getWarningConfig(self, projId):
        ...
        for item in rv:
            result.append(
                {'HHEnable': item[0], 'HEnable': item[1], 'LEnable': item[2], 'LLEnable': item[3], 'HHLimit': item[4],
    ...
    def getWarningRecord(self, projId, startTime, endTime, unconfirmed=0):
        ...
        if dbname != None:
            ...
            for item in rv:
                result.append(
                    {'time': item[0].strftime('%Y-%m-%d %H:%M:%S'), 'code': item[1], 'info': item[2], 'level': item[3],
                     'endtime': item[4].strftime('%Y-%m-%d %H:%M:%S'), 'confirmed': item[5], 'confirmeduser': item[6],
            ...
            if isinstance(rtTableName, str):
                if len(rtTableName) > 0:
                    sql = "CREATE TABLE IF NOT EXISTS %s (`time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,pointname varchar(64) NOT NULL DEFAULT '',\
                        pointvalue varchar(256) NOT NULL DEFAULT '',flag int(11) DEFAULT NULL,PRIMARY KEY (pointname)) ENGINE=InnoDB DEFAULT CHARSET=utf8;" % (rtTableName,)
    ...
    def getProjectLocateMap(self):
        ...
        if (not projectLocateMap) and (not MongoConnManager.isLocalEnv()):
            print('projectLocateMap is None for non_local!!!')
            logging.error('projectLocateMap is non_local!!!')
    ...
    def get_maintain_from_transaction(self, id):
        ...
        try:
            ...
            rt.update({'transactionId':dbrv[0], 'name':dbrv[1], 'endTime':dbrv[2], 'createTime':dbrv[3], 'critical':dbrv[4],
    ...
    def get_user_by_phonenum(self, phonenum):
        ...
        try:
            dbname = app.config['DATABASE']
            strSql = 'SELECT id, username, usermobile FROM `user` WHERE usermobile = "%s"'%phonenum
            rt = self._mysqlDBContainerReadOnly.op_db_query(dbname, strSql)
        except Exception as e:
    ...
    def add_user_by_phonenum(self, phonenum, password, sex=0):
        ...
        try:
            ...
            strSql = 'INSERT INTO `user` SET username = "%s", usermobile = "%s", usersex = %d, usercreatedt = "%s", ' \
                     'unitproperty01 = "%s", userstatus = "%s"'%(phonenum, phonenum, int(sex), createdt, pwhash, 'registered')
            rt = self._mysqlDBContainer.op_db_update_with_id(dbname, 'user', strSql)
        except Exception as e:
    ...
    def updateUserPhone(self, data):
        ...
        try:
            ...
            dbname = app.config.get('DATABASE')
            sql = 'update user set usermobile = %s where id = %s'
            if self._mysqlDBContainer.op_db_update(dbname, sql, (phone, id)):
                rt = True
        except Exception as e:
