(function () {
    ...
    LayerPanel.prototype.getModelByTreeNode = function (treeNode) {
        ...
        }else if(treeNode.type !== "layer" && typeof(treeNode.pId) == "string"){
            if(treeNode.pId == ""){
                var model = this.store.widgets.findByProperty('_id', treeNode.modelId); 
            }else{
                var model = this.store.layers.findByProperty('_id', treeNode.pId);
            }
        }
        return model;
    }
    ...
    LayerPanel.prototype.addZTreeNode = function(model, isSilent){
        ...
        if(value.type == 'layer'){
            ...
            parentNode = this.treeObj.getNodeByParam('modelId',parentId);
            if(!this.treeObj.getNodeByParam('modelId',value._id)){
                newNode = { modelId: value._id, pId: parentId, name: value.name ? value.name : value.type, type: value.type, 'isHide': 0, 'isLock': 0, dropInner: false };
    ...
    function zTreeOnDrop(event, treeId, treeNodes, targetNode, moveType) {
        ...
        for (var i = 0, len = treeNodes.length; i < len;i++){
            ...
            //1,旧的父节点对应的list更新
            _this.getLayerList(_this.treeObj.getNodeByParam('modelId', oldParentId));

            //2,新的父节点对应的list更新
            parentNode = treeNodes[i].getParentNode();
            _this.getLayerList(parentNode);
