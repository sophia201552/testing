var WorkflowCalendar = (function () {
    ...
    WorkflowCalendar.prototype = {
        ...
        fetchNoticesData: function (force) {
            ...
            if (!noticePromise) {
                noticePromise = $.getJSON('/diagnosis/getRealtimeFault/' + projectID);
                this.noticePromiseMap[projectID] = noticePromise;
            }
            var dfd = $.Deferred();
            //返回一个dfd对象
            noticePromise.done(function (faults) {
                if (Array.isArray(faults)) {
                    if (faults.length) {
                        //1、如果有faults的话再去请求另外一个接口返回equipment和zones，没有则返回
                        $.getJSON('/diagnosis/getStruct/' + projectID).done(function (data) {
                            var noticesList = [];
                            //2、遍历所有的faults
        ...
        renderNoticeList: function () {
            ...
            for (i; i < this.noticeList.length; i++) {
                ...
                $eventItems = $('<div class="fc-event ui-draggable ui-draggable-handle ellipsis" title="fault info">' +
                    '<span class="zoneInfo">' +
        ...
        setCalendarHeight: function () {
            ...
            $("#taskPoolContent").css("height", $("#events").height() - $(".event-selector").height() - 40);
