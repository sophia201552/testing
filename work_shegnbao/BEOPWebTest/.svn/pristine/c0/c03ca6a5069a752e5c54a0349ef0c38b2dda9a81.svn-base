    ...
    AccountManager.prototype = {
        ...
        attachEvents: function () {
            ...
            this.$container.on("click", "#confrimEditName", function () {
                ...
                var sexVal = $('#basicSetInfoWrapper input:radio[name="userSex"]:checked').val();
                ...
                Spinner.spin(ElScreenContainer);
                _this.userVM.updateUserfullname(userfullname.trim(), sexVal.trim()).done(function (result) {
                    if (result.success) {
                        //_this.userVM.usersex = sexVal.trim();
                        _this.refreshManageUserInfo();
                        AppConfig.userProfile.fullname = userfullname.trim();
                        AppConfig.userProfile.sex = sexVal.trim();
                    }
                }).always(function () {
                    I18n.fillArea($(ElScreenContainer));
                    Spinner.stop();
