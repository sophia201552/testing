...
var WorkflowReport = (function () {
    ...
    WorkflowReport.prototype = {
        show: function () {
            var _this = this;
            WebAPI.get("/static/views/workflow/report.html").done(function (resultHtml) {
        ...
        renderData: function (data) {
            ...
            for (var key in userdata) {
                if (userdata[key]['done'] != 0 || userdata[key]['undone'] != 0) {
                    ...
                    var userpic = userdata[key]['userpic']
                    var done = userdata[key]['done']
                    var undone = userdata[key]['undone']
                    var total = undone + done;
                    var divList = $('<div class="col-md-10 col-md-offset-1 pb20" id = "week-report-member"><div class="row"><div class="description col-md-11">'
                    + '<img class="avatar-icon" src='+userpic+' />'
                    + '<div class="text">' + name
                    ...
                    for (var i = 0; i < doneList.length; i++) {
                        ...
                        + '</div><div class="col-md-2 text item-text-color" title="resolved date">' + (item['complete_time'] ? (item['complete_time'].toDate().format('yyyy-MM-dd')).toLocaleString(I18n.type ? I18n.type : 'zh') : '')
                    ...
                    for (var i = 0; i < undoneList.length; i++) {
                        ...
                        + '<div class="col-md-3 name" title="' + item['title'] + '">' + item['title']
                        + '</div><div class="col-md-5 text item-text-color" title="' + item['detail'] + '">' + item['detail']
                        + '</div><div class="col-md-2 text item-text-color" title="deadline">' + (item['duedate'] ? (item['duedate'].toDate().format('yyyy-MM-dd')).toLocaleString(I18n.type ? I18n.type : 'zh') : '')
