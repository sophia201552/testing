var modalConfigurePane = (function(){
    ...
    modalConfigurePane.prototype = {
        ...
        initConfigData: function(){
            ...
            for (var i = 0; i < _this.option.rowDataType.length; ++i) {
                strConfigModalBody += '<div class="divConfigData" dataType="' + _this.option.rowDataType[i] + '">\
                                                ...
                                                strDataTypeShowName[i] + arrTempDataMaxNum[i] +
                                                '</div>\
                    ...
                    function insertHtmlAtCaret(html,targetId) {
                        ...
                        if (window.getSelection) {
                            ...
                            if (sel.getRangeAt && sel.rangeCount) {
                                ...
                                _this.$editor.find('#'+targetId)[0].addEventListener('DOMNodeRemoved', _this.domNodeRemoved, false);
        ...
        initConfigStart: function(){
            ...
            document.getElementById('startConfig').onclick = function () {
                ...
                if(_this.modalType == "dataAnalysis"){
                    ...
                    if(_this.templateObj.entity.modal.type == 'ModalNote' && _this.$editor && !_this.$editor.is(':hidden')){
                        _this.templateObj.entity.modal.modalText = _this.$editor.html();
                        var $divModalTextSpan = $('#editor').find('.pointValue');
        ...
        initEditor: function(){
            var _this = this;
            var $noteEditor = $('#noteEditor');
            if($noteEditor.find('#editor').length == 0){
                $noteEditor.append(getWysiwyg());
            }
            this.editorData = _this.templateObj.entity.modal.modalTextUrl?_this.templateObj.entity.modal.modalTextUrl:[];
            this.$editor = $noteEditor.find('#editor').css({height: 'calc(100% - 72px)'});
            this.$editor.wysiwyg();
            this.$editor[0].ondrop = function(e){
                e.preventDefault();
                this.focus();
                var $divConfigData = $('.divConfigData');
                $divConfigData.trigger('drop',[e]);
                var $editorPoint = $noteEditor.find('#editor .pointValue');
            ...
            this.$editor[0].ondragover = function (e) {
                e.preventDefault();
                $(e.currentTarget).find('.dataDragTip').addClass('addData');
            }
            this.$editor[0].ondragleave = function (e) {
                e.preventDefault();
                $(e.currentTarget).find('.dataDragTip').removeClass('addData');
            }

            this.$editor[0].onmouseup = function(e){
            ...
            this.$editor[0].onkeydown = function(e){
        ...
        domNodeRemoved: function(){
            ...
            setTimeout(function(){
                var newDom = _this.$editor.find('#'+id);
                if(newDom.length < 1){
        ...
        initNotePtCfg: function(index){
            var modalTextUrl;
            var $editorPoint = $('#editor .pointValue');
