(function (beop) {
    var configMap = {
            ...
            timeFormatMap: {
                startView: 'month',
                autoclose: true,
                format: timeFormatChange('yyyy-mm-dd hh:ii'),
                forceParse: false
            },
            dataType: {
                HISTORY: 'history',
        ...
        jqueryMap = {},
        setJqueryMap, configModel, importSheet, init,
        ...
        loadApiTree, zTreeOnDrag, zTreeOnDrop, zTreeOnDblClick, changeToZTreeNodes, showNodeTitle, insertContent, setTreeNodeIcon, enlargeFullScreen, treeSearch, batchAllPointsChange, cloudPoint_SetUp, btnPreservation;
    ...
    setJqueryMap = function () {
        ...
        jqueryMap = {
            ...
            $cloudPointSetUp: $container.find('#cloudPointSetUp'),
    ...
    loadAutoCompleteList = function () {
        ...
        $.ajax({
            url: stateMap.ExpertContainerUrl + urlSuffix,
            type: "GET"
        }).done(function (result) {
    ...
    loadPointTable = function (page) {// 加载点表
        ...
        jqueryMap.$container.off('click.cloudPoint_SetUp').on('click.cloudPoint_SetUp', '#cloudPoint_SetUp', cloudPoint_SetUp);
        jqueryMap.$container.off('click.btnPreservation').on('click.btnPreservation', '#btnPreservation', btnPreservation);
            ...
            confirm(i18n_resource.dataManage.CONFIRM_STOP_REPAIR, function () {
                stopRepairing($this.closest('tr').attr('id')).done(function (result) {
                    if (result && result.success) {
                        $this.closest('tr').find('.history-progress').text('已取消');
                    }
                })
            });
        });
    ...
    cloudPoint_SetUp = function () {
        jqueryMap.$cloudPointSetUp.modal();
        WebAPI.post("/getCalcSettings/" + AppConfig.projectId, {projId: AppConfig.projectId}).done(function (returnData) {
            $("#fixedTimeMode").prop("checked", !!returnData.data.triggerReal.fixed_time);
            $("#rawDataMode").prop('checked', !!returnData.data.triggerReal.raw_data);
        })
    };

    btnPreservation = function () {
        var data = {
            projId: AppConfig.projectId,
            triggerReal: {
        ...
        WebAPI.post("/setCalcSettings/" + AppConfig.projectId, data).done(function (result) {
            if (result.success) {
                jqueryMap.$cloudPointSetUp.modal("hide");
            } else {
    ...
    loadPointDetailPage = function (isEdit) {
        ...
        configMap.timeFormatMap.format = timeFormatChange('yyyy-mm-dd hh:ii');
...
//判断字符串是否可以转变成json格式
    ...
    joinCurve = function () {
        ...
        if (!selectedPoints.length) {
            ...
            selectedPoints.forEach(function (item) {
                if (stateMap.pointType == configMap.cloudPointType.MAPPING_POINT) {
                    ...
                    var structure = tryParseJSON(item.pointValue);
                    if (structure) {
                        var attrList = [];
                        for (var attr in structure) {
                            attrList.push(attr);
                        }
                        structure_list[item.value] = attrList;
                    }
        ...
        var getHistoryDataReduce = function (hidePointList) {
            ...
            WebAPI.post("/get_history_data_padded_reduce", {
                projectId: AppConfig.projectId,
                pointList: point_list.concat(hidePointList),
    ...
    loadSheet = function (page) {
        ...
        var dataTableOptions = {
            ...
            rowsNums: [25, 50, 100, 200, 500, 1000],
    ...
    changeToZTreeNodes = function () {
        ...
        for (var i = 0; i < stateMap.requestTreeList.length; i++) {
            stateMap.treeList.push({
                id: ObjectId(),
                name: stateMap.requestTreeList[i].name,
                sample: stateMap.requestTreeList[i].sample,
                pId: stateMap.requestTreeList[i].api_type == '0' ? 9 : stateMap.requestTreeList[i].api_type
            });
        }
        for (var j = 0; j < configMap.folderList.length; j++) {
            stateMap.treeList.push({
                id: configMap.folderList[j].id,
                name: configMap.folderList[j].name,
                pId: 10,
                open: false,
                isFolder: true
            });
