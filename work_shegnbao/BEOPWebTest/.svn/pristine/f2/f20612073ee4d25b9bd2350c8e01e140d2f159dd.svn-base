...
def home():
    ...
    if rt.get('status') != AuthManager.token_valid:
        return render_template('indexObserver.html', title='BEOP Energy Leader')
    else:
        rv = login_after(user_id)
        return render_template('indexObserverLoad.html', title='BEOP Energy Leader', AppConfig=rv)
...
def get_history_data_raw():
    ...
    try:
        ...
        if data:
            ...
            timeEnd = data.get('timeEnd')
            timeFormat = data.get('timeFormat')
            rt = BEOPDataAccess.getInstance().getHistoryData(projId, pointList, timeStart, timeEnd, timeFormat, True)
    except Exception as e:
