    ...
    DiagnosisScreen.prototype = {
            ...
            $('#btnChangeAlarm').click(function (e) {
                ...
                WebAPI.post('/diagnosis/fault/customUpdate/' + AppConfig.projectId, postData).done(function (result) {
                    ...
                    if (data) {
                        ...
                        if(oldLevel != level && 0 == delayVal){//等级更改 依旧是实时
                            ...
                            $divFault.clone(true).prependTo($newRows);
                        }
                        $divFault.remove();
        ...
        initPaneBuilding: function () {
            ...
            for (var i = 0, len = arrZone.length; i < len; i++) {
                ...
                div.style.backgroundImage = "url('http://images.rnbtech.com.hk/static/images/diagnosis/" + AppConfig.projectName + "/" + item.image + "')";
        ...
        initPaneNav: function () {
            ...
            //隐藏/显示概览小图
            //$('#showSurveyChart').off('click').click(function () {
            //    var $spanIconTotal;
            //    if ($(this).hasClass('glyphicon-eye-open')) {
            //        for (var i = 0; i < $('.div-nav-row').length; i++) {
            //            $($('.div-nav-row')[i]).css({'line-height':'40px','border-bottom':'0px'});
            ...
            //            }
            //        }
            //        //$('#floorCt').removeClass('col-sm-2').addClass('col-sm-1');
            //        $('#floorCt').find('.panel-default').css('background-color', 'rgba(0,0,0,0)');
            //        $('.floorTop .div-row-icon-badge').css('background-color', 'rgba(146, 143, 143, 0.72)');
            //    } else if ($(this).hasClass('glyphicon-eye-close')) {
            //        for (var i = 0; i < $('.div-nav-row').length; i++) {
            //            $($('.div-nav-row')[i]).css({ 'line-height': 'inherit', 'border-bottom': '1px inset #ccc' });
            //            $spanIconTotal = $($('.div-nav-row')[i]).find('span:not(:first)');
            //            if ($spanIconTotal) {
            ...
            //            }
            //        }
            //        //$('#floorCt').removeClass('col-sm-1').addClass('col-sm-2');
            //        if ($('#btnFloorsSP').hasClass('top')) {
            //            $('#floorCt').find('.panel-default').css('background-color', 'rgba(0,0,0,0.45)');
            //        } else {
            //            $('#floorCt').find('.panel-default').css('background-color', 'rgb(255,255,255)');
            //        }
            //        $('.floorTop .div-row-icon-badge').css('background-color', 'rgba(194, 194, 194, 0.7)');
            //   }
            //});
        },
        //切换模板
