...
var ModalWiki = (function(){
    ...
    ModalWiki.prototype.init = function(){
        ...
        WebAPI.post('/getAllWiki', data).done(function(result){
            var rlst = JSON.parse(result);
            if(rlst.length > 0){
    ...
    ModalWiki.prototype.showWikiSearch = function(){
        ...
        this.$modalContent.find('#btnSearchWiki').click(function(){
            ...
            WebAPI.post('/getWikiByKeywordsAndProjectId', queryCondition).done(function(result){
                var rslt = JSON.parse(result), temp = '';
    ...
    ModalWiki.prototype.showWikiCreate = function(){
        ...
        this.$modalContent = this.$modal.find('.modal-content');
        this.$modalContent.html(tempHtml);
        this.$modalContent.find('#editor').wysiwyg().css({height: 'calc(100% - 79px)'});
    ...
    ModalWiki.prototype.showWikiEdit = function(wiki){
        var _this = this, $tagStrAdd, $tagProjectAdd, $divWikiEditor, $editor;
        var tempHtml = '\
            <div class="modal-header">\
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>\
        ...
        this.$modalContent.find('#btnRemoveWiki').off().on('click',function(e){
            if(confirm('Are you sure to unbind this wiki ?')){
                if(_this.parent.entity.modal.type == 'ModalPointKPI'){
        ...
        this.$modalContent.find('#confirm').off().on('click',function(){
            ...
            var content = $editor[0].innerHTML;
            if(title.trim() == ''){
    ...
    ModalWiki.prototype.createWiki = function(){
        ...
        var content = this.$modalContent.find('#editor')[0].innerHTML;
        ...
        //spin
        WebAPI.post('/createWiki',wiki).done(function(result){
