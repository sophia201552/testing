...
def update_user():
    ...
    if supervisor:
        ...
        if success:
            logging.info('用户直属上级修改成功:old:{},new:{}', format(user_profile.get('supervisor'), supervisor))
        else:
            logging.error('用户直属上级修改失败:{}'.format(json.dumps(data, ensure_ascii=False)))
...
def set_color_setting():
    ...
    if user and user.get('unitproperty02'):
        ...
        user_setting['color'] = color_setting
    else:
        user_setting = {'color': color_setting}
...
def get_color_setting():
    ...
    user = User.query_user_by_id(user_id, 'unitproperty02')
    if user and user.get('unitproperty02'):
        user_setting = json.loads(user.get('unitproperty02')).get('color')
    else:
        user_setting = [{'value': '', 'color': 'blue', 'type': 'min'},
                        {'value': '', 'color': 'red', 'type': 'max'}]
