...
@bp_admin.route("/checkProjDataSource", methods=["POST"])
def check_project_mark_value():
    req = request.get_json()
    dtu_name = req.get("dtuName")
    proj_id = req.get("projectId")

    if not dtu_name or not proj_id:
        return Utils.beop_response_error('data source or project id is empty')
    ...
    dtu_to_proj = DtusertToProject().get_by_dtu(dtu.get('id'))

    if dtu_to_proj and dtu_to_proj.get('projectid') != proj_id:  # 已经被使用过
        rv['used'] = True
        return Utils.beop_response_success(rv)
    if dtu.get('dbname'):
        ...
        return Utils.beop_response_error('dbname 为空')
