...
class Algorithm(object):
    ...
    def AddAlgorithmTemplateFolder(cls, data):
        ...
        try:
            ...
            if data:
                ...
                if insert:
                    rt = str(MongoConnManager.getConfigConn().mdbBb[g_table_algorithm_template].save(insert))
    ...
    def GetAlgorithmTemplateContent(cls, algorithm_id):
        rt = {}
        try:
            rt = MongoConnManager.getConfigConn().mdbBb[g_table_algorithm_template].find_one({'_id': ObjectId(algorithm_id)})
            if rt:
    ...
    def LoadTemplateTree(cls, parent_id):
        ...
        try:
            ...
            try:
                ...
                cursor = MongoConnManager.getConfigConn().mdbBb[g_table_algorithm_template].find(query)
                for item in cursor:
                    if 'content' in item:
                        item.pop('content')
    ...
    def SearchTemplate(cls, search_text):
        ...
        try:
            ...
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_algorithm_template].find({'$or': [{'note': regDict}, {'name': regDict}]})
            for item in cursor:
                if 'content' in item:
                    item.pop('content')
                item.update({'_id': str(item.get('_id'))})
    ...
    def TraceTreeItem(cls, parent_id, arr):
        cursor = None
        try:
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_algorithm_template].find({'parent_id': parent_id})
            for item in cursor:
                Algorithm.set_creator_name(item)
                arr.append(item)
    ...
    def delete_template(cls, template_id):
        if not template_id or not ObjectId.is_valid(template_id):
            return False
        return MongoConnManager.getConfigConn().mdbBb[g_table_algorithm_template].remove({'_id': ObjectId(template_id)})