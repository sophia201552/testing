...
@app.route('/spring/get/<menuId>/<int:isFactory>')
def springGetMenu(menuId, isFactory=0):
    return json.dumps(MongoConnManager.getConfigConn().springGetMenu(menuId, int(isFactory)), ensure_ascii=False)

@app.route('/spring/saveLayout', methods=['POST'])
def saveSpringLayout():
    data = request.get_json()
    assert(isinstance(data, dict))
    MongoConnManager.getConfigConn().saveSpringLayout(data)
