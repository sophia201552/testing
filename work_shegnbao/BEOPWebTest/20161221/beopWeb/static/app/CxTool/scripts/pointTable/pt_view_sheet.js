(function (beop) {
    ...
    var configMap = {
        ...
        init, loadSheet, onNewPoint, onEditPoint, onDeletePoint, onClearPoint, renderSheet, onJumpPointSet,
        onUndoPoint, onRedoPoint, onSearchTextKeyDown, onEditPointConfirm, onPointUpdateAll,
        ...
        onDeletePointType, onUpdateRowConfirm, onDeletePartialPoints, onChangeProject, synPointList, paginationRefresh;
    ...
    setJqueryMap = function () {
        ...
        jqueryMap = {
            ...
            $point_update_all: $('#point_update_all')
        };
    };
    ...
    onUpdateRowConfirm = function () {
        ...
        if (stateMap.command === 'update') {
            var postData = {
                data: stateMap.update_list,
                project_id: stateMap.project_id
            };
            WebAPI.post('/point_tool/updatePartialToCore', postData).done(function (result) {
                if (result.success) {
                ...
                jqueryMap.$isUpdatePointWin.modal('hide');
                if (200 == e.status) {
                    Alert.success(document.body, i18n_info.UPDATE_SUCCESS).showAtTop(2000);
                }
                spinner.stop();
            })
            ...
            WebAPI.post('/point_tool/deletePartialPoint', postData).done(function (result) {
                if (result.success) {
                    jqueryMap.$isUpdatePointWin.modal('hide');
                    Alert.success(document.body, i18n_info.UPLOAD_POINT_SUCCESS.format(result.data.resp)).showAtTop(2000);
                } else {
                ...
                jqueryMap.$isUpdatePointWin.modal('hide');
                if (200 == e.status) {
                    Alert.success(document.body, i18n_info.DELETE_SUCCESS).showAtTop(2000);
                }
                spinner.stop();
            });
...
//---------事件---------
    ...
    loadPointTable = function (page) {// 加载点表
        var initPointTable = function (resultHtml) {
            ...
            jqueryMap.$point_update_all.on('click', onPointUpdateAll);
    ...
    onClearPoint = function () {
        confirm('确认清空所有点么?', function () {
            WebAPI.post('/point_tool/clearEnginePoints/', {projectId: stateMap.project_id}).done(function (result) {
                if (result) {
                    location.reload();
                }
            })
    ...
    onPointUpdateAll = function () {
        // do download table
        if (!confirm(I18n.resource.debugTools.btnNav.DOWNLOAD_TABLE_CONFIRM)) {
            return;
        }
