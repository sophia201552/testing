    ...
    WorkflowRemind.prototype = {
        ...
        _createBox: function () {// 有数据创建消息盒子
            ...
            for (var i = 0; i < this.remindData.length; i++) { // 添加消息列表
                var item = this.remindData[i];
                messageLi += '<li class="wfMessageLi oh" trans_id = "' + item.linkToTransactionId + '">' +
                    '<img src="' + item.userpic + '" class="userImg fl">' +
                    '<div class="fl oh">' +
                    '<span class="db wfMessageInfo">' +
                    '<span class="wfMessageUserName">' + item.userfullname + '</span>' +
                    '<span class="wfMessageOp">' + this._translateOp(item.op) + '</span>' +
                    '</span>' +
                    '<span class="db wfMessageInfo wfMessageIdAndTitle ellipsis" title="' + item.linkToTransactionId + ' ' + item.transTitle + '">' +
                    '<span>' + item.linkToTransactionId + ' </span>' +
                    '<span>' + item.transTitle + '</span>' +
                    '</span>' +
                    '<span class="db wfMessageTime">' + item.opTime + '</span>' +
                    '</div>' +
                    '</li>';
            }
            messageLi += '</ul>';
