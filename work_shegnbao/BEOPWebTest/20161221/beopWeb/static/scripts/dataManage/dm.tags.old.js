var PointManagerImportTags = (function () {
    ...
    function PointManagerImportTags(projectId) {
        ...
        this.htmlUrl = '/static/scripts/dataManage/views/dm.importTags.html';
        _this = this;
    ...
    var PointManagerImportTagsFunc = {
        ...
        _dr_dropTree2Dom: function (event, treeId, treeNodes, targetNode, moveType, isCopy) {
            var thingIds = [];
            for (var i = 0, iLen = treeNodes.length; i < iLen; i++) {
                thingIds.push(treeNodes[i]._id);
            }
            //如果targetNode为最外层的thing类型节点,释放treeNodes到最外层
            if (!targetNode.Prt && targetNode.type == 'thing') {
                targetNode = null;
            }
            targetNodeId = targetNode ? targetNode._id : "";
            ...
            WebAPI.post('/tag/moveThings', {
                ...
                if (result.status) {
                    ...
                    _this.moveTreeNodes(targetNodeId, thingIds);
                    if (_this.currentNode._id == targetNodeId) {
                        _this.$container.find('#' + _this.currentNode.tId + '_span').click();
                    }
                } else {
        ...
        _beforeDrop: function (treeId, treeNodes, targetNode, moveType) {
            return targetNode.type == 'group' || (targetNode.type == 'thing' && !targetNode.Prt);
        },
        /***
        ...
        _dr_itemToTree: function (event, treeId, treeNode) {
            ...
            if (treeNode.type == ThingType.group) {
                ...
                if (_this.stateMap.isDragging && treeNode.type == 'thing' && !treeNode.Prt) {
                    _this.moveOutmostLayer();
                }
                $('#drag_box').remove();
        ...
        asyncSuccess: function (event, treeId, treeNode, msg) {
            if (_this.isRenderRight) {
                ...
                _this.jqueryMap.$tagsPath.find('.breadcrumb').empty().append(I18n.resource.common.ROOT_DIRECTORY + beopTmpl('tpl_tags_path', {paths: _this.path}));
                _this._showRightSlowly();
        ...
        movePointsToGroup: function (id) {
            ...
            if ($dragBox.length) {
                ...
                if (!_this.selectedIds || !_this.selectedIds.length) {
                    return false;
                }
                if ($.inArray(id, _this.selectedIds) != -1) {
                    return false;
                }
                Spinner.spin(_this.jqueryMap.$tagContent.get(0));
                WebAPI.post('/tag/moveThings', {
                    Prt: id,
                    thingsId: _this.selectedIds,
                    projId: AppConfig.projectId
                }).done(function (result) {
                    if (result.status) {
                        for (var i = 0; i < _this.selectedIds.length; i++) {
                            if (_this.selectedIds[i]) {
                                $('.tag-list-item[data-id=' + _this.selectedIds[i] + ']', _this.jqueryMap.$tagList).remove();
                            }
                        }
                        _this.jqueryMap.$tagsRightContent.find('.tag-list-item').removeClass('dropTarget');
                        _this.moveTreeNodes(id, _this.selectedIds);
                    } else {
                        alert.danger('move failed');
                    }
                }).always(function () {
        ...
        moveOutmostLayer: function () {
            var $selectedFiles = _this.jqueryMap.$tagsRightContent.find('.active'), thingIds = [];
            for (var i = 0, iLen = $selectedFiles.length; i < iLen; i++) {
                thingIds.push($selectedFiles[i].dataset.id);
            }
            WebAPI.post('/tag/moveThings', {
                Prt: '',
                thingsId: thingIds,
        ...
        attachEvents: function () {
            ...
            this.jqueryMap.$tagsRightContent.on('contextmenu', '.tag-list-item', function (e) {
                ...
                if ($firstSelectedFile.hasClass('tag-folder')) {
                    $contentMenu.removeClass('not_group_file').addClass('has_group_file');
                } else {
                    $contentMenu.addClass('not_group_file').removeClass('has_group_file');
                }
                    ...
                    if ($selected_files && $selected_files.length) {
                        ...
                        for (var i = 1, iLen = $selected_files.length; i < iLen; i++) {
                            if ($($selected_files[i]).hasClass('tag-folder')) {
                                $contentMenu.addClass('has_group_file');
                            } else {
            ...
            //移到最外层
            this.jqueryMap.$container.off('click.menu-move-outermost-layer').on('click.menu-move-outermost-layer', '.menu-move-outermost-layer', function () {
                _this.moveOutmostLayer();
            });
