    ...
    var PointManagerExportDataFunc = {
        ...
        attachEvents: function () {
            ...
            $exportDataBox.off('click.exportData').on('click.exportData', '#exportData', function () {
                ...
                _this.data.startTime = new Date(startTime).format(DateUtil.DATA_FORMAT.FULL_DATETIME_NO_SEC_CHANGE);
                _this.data.endTime = new Date(endTime).format(DateUtil.DATA_FORMAT.FULL_DATETIME_NO_SEC_CHANGE);
                ...
                var startAlert = timeFormat(_this.data.startTime, timeFormatChange(DateUtil.DATA_FORMAT.FULL_DATETIME_NO_SEC));
                var endAlert = timeFormat(_this.data.endTime, timeFormatChange(DateUtil.DATA_FORMAT.FULL_DATETIME_NO_SEC));
                confirm(I18n.resource.debugTools.exportData.IS_EXPORT_DATA_ALARM.format(startAlert, endAlert, pointType), function () {
                    ...
                    _this.expertContainerUrlPromise.done(function () {
                        $.ajax({
                            ...
                            if (result) {
                                ...
                                alert(I18n.resource.debugTools.exportData.REQUEST_FAILURE);
                            }
                        }).always(function () {
                            Spinner.stop();
                        });
            ...
            $exportDataBox.off('click.record-stop').on('click.record-stop', '.record-stop', function () {
                ...
                _this.expertContainerUrlPromise.done(function () {
                    $.ajax({
                        url: _this.ExpertContainerUrl + 'dataManage/exportData/task/stop',
                        type: "POST",
                        data: {
                            taskId: $tr.attr('taskId'),
                            force: true
        ...
        searchExportList: function () {
            ...
            _this.expertContainerUrlPromise.done(function () {
                _this.exportDataTimer && clearInterval(_this.exportDataTimer);
                $("#exportDataList").modal();

                $('#exportDataList').on('shown.bs.modal', function () {
                    Spinner.spin(document.body);
                });
            ...
            $.ajax({
                ...
                if (result) {
                    ...
                    $exportDataListTBody.html(beopTmpl('tpl_exportData_List', {
                        list: result,
                        url: _this.ExpertContainerUrl.substr(0, _this.ExpertContainerUrl.length - 1)
                    }));
        ...
        loadSheet: function () {
            ...
            var dataTableOptions = {
                ...
                colModel: [
                    ...
                    {index: '', isCheckColumn: true, width: '30px'},
