    ...
    var PointManagerRealTimeDataFunc = {
        ...
        attachEvents: function () {
            ...
            $("#joinCurve").click(function () { //点击加入曲线按钮
                ...
                if ($selectedItem.length == 1) {
                    var isStructure = _this.tryParseJSON($selectedItem.data('value'));
                    showHistoryTable = false;
                    if (isNaN($selectedItem.data('value'))) {
                        showHistoryTable = true;
                    }
                }

                $point_name_list = $selectedItem.map(function (index, item) {
                    valueType($(item).data('point'), $(item).data('value'));
        ...
        refreshSearchTable: function (keyWordVal) { //刷新查询分页列表
            ...
            for (var i = 0; i < this.pointInfoList.length; i++) {
                ...
                pointListHtml += '<tr data-point="' + (point.pointname || point.name) + '" data-value = "' + point.pointvalue + '">' +
                    '<td>' + keyWordName + '</td><td>' + point.pointvalue + '</td>' +
                    '<td>' + timeFormat(point.time, timeFormatChange(DateUtil.DATA_FORMAT.FULL_DATETIME_NO_SEC)) + '</td>' +
