...
def delIotInfo():
    rt = False
    data = request.get_json()
    arrItem = data.get('itemList')
    try:
        for item in arrItem:
            if item.get('id'):
                if item.get('type') == 'things':
                    rt = MongoConnManager.getConfigConn().del_iot_things({'_id': item.get('id'), 'projId': data.get('projId'), 'userId': request.cookies.get('userId', None)})
                elif item.get('type') == 'groups':
                    rt = MongoConnManager.getConfigConn().del_iot_groups({'_id': item.get('id'), 'projId': data.get('projId'), 'userId': request.cookies.get('userId', None)})
...
# 获取IOT相关的操作记录
@bp_iot.route('/getIOTRecords', methods=['POST'])
def getIOTRecords():
    data = request.get_json()
    rt = {'status':'fail'}
    try:
        rt = MongoConnManager.getConfigConn().get_asset_operation_log(data)
    except Exception as e:
        print('mod_iot.setIotProject error:' + e.__str__())
        logging.error('mod_iot.setIotProject error:' + e.__str__())
    return jsonify(data = rt)
