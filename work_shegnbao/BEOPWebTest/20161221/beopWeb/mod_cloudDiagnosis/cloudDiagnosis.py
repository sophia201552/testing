...
class CloudDiagnosis:
    ...
    def save_cloudDiagnosis_customDiagnosis(cls, data):
        try:
            ...
            if data_id:
                MongoConnManager.getConfigConn().mdbBb[g_table_clouddiagnosis].update({'_id': ObjectId(data_id)}, {'$set': data})
            else:
                isexit = cls.isexist_cloudDiagnosis(moduleName, projId)
                if isexit:
                    raise Exception(moduleName + ' is existed.')
                else:
                    data_id = MongoConnManager.getConfigConn().mdbBb[g_table_clouddiagnosis].insert(data)
            data['_id'] = data_id
    ...
    def isexist_cloudDiagnosis(cls, moduleName, projId):
        projId = int(projId)
        isexit = MongoConnManager.getConfigConn().mdbBb[g_table_clouddiagnosis].find_one(
    ...
    def copy_cloudDiagnosis_customDiagnosis(cls, diagonsis_idList, parent_id=None):
        try:
            ...
            if len(result) > 0:
                MongoConnManager.getConfigConn().mdbBb[g_table_clouddiagnosis].insert_many(result)
        except Exception as e:
    ...
    def find_need_cpoy_customDiagnosis(cls, diagonsis_idList, parent_id=None):
        ...
        result_temp = MongoConnManager.getConfigConn().mdbBb[g_table_clouddiagnosis].find({'_id': {'$in': obid_list}})
        if result_temp:
    ...
    def remove_cloudDiagnosis_customDiagnosis(cls, IdList):
        ...
        try:
            ...
            if isinstance(IdList, list):
                dbrv = MongoConnManager.getConfigConn().mdbBb[g_table_clouddiagnosis].update(
                    {'_id': {'$in': [ObjectId(x) for x in IdList if ObjectId.is_valid(x)]}},
                    {'$set': {'isDelete': bool(1)}}, multi=True)
                if dbrv.get('ok'):
                    rt = True
        except Exception as e:
    ...
    @classmethod
    def get_cloudDiagnosis_customDiagnosisId_by_parentId(cls, IdList):
        rt = []
        cursor = None
        try:
            cursor = MongoConnManager.getConfigConn().mdbBb[g_table_clouddiagnosis].find({'Parent': {'$in': [x.__str__() for x in IdList]}})
            for item in cursor:
                rt.append(item.get('_id').__str__())
            if rt:
                rt.extend(cls.get_cloudDiagnosis_customDiagnosisId_by_parentId(rt))
        except Exception as e:
            print('get_cloudDiagnosis_customDiagnosisId_by_parentId error:' + e.__str__())
            logging.error(e.__str__())
        finally:
            if cursor:
                cursor.close()
    ...
    def get_cloudDiagnosis_customDiagnosis(cls, filter):
        ...
        try:
            if isinstance(filter, dict):
                ...
                cursor = MongoConnManager.getConfigConn().mdbBb[g_table_clouddiagnosis].find(filter)
                for item in cursor:
