...
(function (beop) {
    ...
    BeopMap.prototype.destroy = function () {
    ...
    BeopMap.prototype.openProjectsCard = function () {
        ...
        if (AppConfig.projectList.length === 1) {//仅有一个项目
            ...
            if (configMap.isFirstComeIn) {
                $(".project-media-container").find(".media").click();
            }
        } else {
    ...
    BeopMap.prototype.attachPanelEvent = function () {
        ...
        PYFormat.getPYLocalStorage().done(function (result) {
            AppConfig.projectList.forEach(function (item) {
                var pinyin = ' ';
                PYItem = PYFormat.getPYMap(result, item.name_cn);
                ...
                if (searchType == "pms-project" || searchType == "" || searchType == undefined) {
                    if (isPullEnglish) {
                        ...
                        projectList.forEach(function (item) {
                            if ((item.name_cn && $.trim(item.name_cn && item.name_cn.indexOf(value) !== -1))) {
                                result.push({
                                    pic: '/static/images/project_img/' + item.pic,
                                    name: item[language],
                                    address: item.address,
    ...
    GoogleMap.prototype.isMapLoaded = function () {        try {
