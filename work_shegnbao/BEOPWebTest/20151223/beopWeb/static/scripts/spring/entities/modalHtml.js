...
(function() {
    ...
    var ModalHtmlConfig = (function() {
        ...
        ModalHtmlConfig.prototype.recoverForm = function(form) {
            ...
            form.points.forEach(function (row) {
                var $ele = _this.addDsFormGroup({
                    ...
                    cls: ' dropped',
                    value: row,
                    name: AppConfig.datasource.getDSItemById(row).alias,
                    display: 'inline'
                });
            });
            // 额外新增一行空白行
        ...
        ModalHtmlConfig.prototype.attachEvents = function () {
            ...
            // drop EVENTS //
            /////////////////
            this.$wrap.on('drop', '.drop-area', function (e) {
                var $this = $(this);
    ...
    this.ModalHtml = (function() {
        ...
        ModalHtml.prototype.initCustomVaribles = function () {
            ...
            // 初始化一些内置方法
            // linkTo: 跳转到指定页面
            this.customVaribles.linkTo = function (menuId, ctnSelector, linkType) {
                var $ev, ctn;
                ...
                // 如果是链接到容器
                if(ctnSelector) {
                    ctn = container.querySelector(ctnSelector);
                    // 不存在此容器，不做任何事
                    if(!ctn) return;
        ...
        ModalHtml.prototype.initCustomAttrs = function () {
            ...
            $container.on('click', '[data-link-to]', function (e) {
                ...
                // 跳转
                _this.customVaribles.linkTo(menuId, ctnSelector, linkType);
                e.preventDefault();
            });
        };
