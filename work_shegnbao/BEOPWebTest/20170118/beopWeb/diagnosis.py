        ...
        if post_data:
            ...
            if dbname:
                strSQL = 'SELECT f.Id, f.`Name`, f.EquipmentId, e.`Name` AS EquipmentName, e.ZoneId, ' \
                         'z.SubBuildingName, n.Id, n.Time, n.Num, n.CheckTime FROM %s_diagnosis_faults AS f INNER JOIN (' \
                         'SELECT Id, FaultId, `Time`, COUNT(*) as Num, CheckTime FROM %s_diagnosis_notices WHERE Time <= "%s" AND Time >= "%s" ' \
                        ...
                        try:
                            if item[0] in fault_dict.keys():
                                fault_dict.get(item[0]).get('arrNoticeTime').append({'NoticeId': item[6],
                                                                                     'CheckTime': item[9].strftime('%Y-%m-%d %H:%M:%S') if isinstance(item[9], datetime) else item[9],
        ...
        if type == 'workhours':
            ...
            if strTime_list:
                rt = 'SELECT f.Id, f.`Name`, f.EquipmentId, e.`Name` AS EquipmentName, e.ZoneId, ' \
                     'z.SubBuildingName, n.Id, n.Time, n.Num, n.CheckTime FROM %s_diagnosis_faults AS f INNER JOIN (' \
                     'SELECT Id, FaultId, `Time`, COUNT(*) as Num, CheckTime FROM %s_diagnosis_notices WHERE '%(dbname, dbname)
