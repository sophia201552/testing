 ...
 */
var MessageFactoryReport = (function(){
    var _this = this;
    function MessageFactoryReport(reportDate){
        _this = this;
        this.reportDate = reportDate;
    }
    MessageFactoryReport.navOptions = {
        top: '<div class="topNavTitle" i18n="appDashboard.message.MY_REPORT"></div>',
        bottom:true,
        backDisable:false,
        module:'message'
    };
    MessageFactoryReport.prototype = {
        reportTypeMap: {
            day: '0',
            month: '1',
            week: '2'
        ...
        initReportList:function(){
            ...
            WebAPI.get('/factory/reportWrap/' + AppConfig.isFactory + '/' + ProjectConfig.reportPageId).done(function(index){
                ProjectConfig.reportList = index.list;
                for (var i = 0 ; i< index.list.length ;i++){
                    ...
                    var icon = _this.initReportIcon(reportName);
                    strPanelReport.append('<div class="panel panel-primary ">');
                    ...
                    if(icon.add != ''){
                        strPanelReport.append('<span class="spAddIcon">' + _this.initReportIcon(reportName).add + '</span>');
                    }
                    strPanelReport.append('        </div>');
                    strPanelReport.append('        <h4 class="panel-title">');
                    strPanelReport.append('            <a class="aCollapse collapsed" role="button" data-toggle="collapse" data-parent="#containerMessageReport" href="#'+ index.list[i].reportId +'" aria-expanded="false" aria-controls="' + index.list[i].reportId + '">');
                    strPanelReport.append('                <span class="reportType">' + reportName + '</span>');
                    ...
                    strPanelReport.append('    </div>');
                    strPanelReport.append('    <div id="'+ index.list[i].reportId + '" class="panel-collapse collapse " role="tabpanel" aria-labelledby="reportTitle_'+ i +'">');
                    strPanelReport.append('        <div class="panel-body">');
                    strPanelReport.append('        </div>');
                    strPanelReport.append('        <div class="btnDetail zepto-ev additionText" report-to="' + index.list[i].reportId + '">' + I18n.resource.appDashboard.message.MORE);
                    strPanelReport.append('        </div>');
                    strPanelReport.append('    </div>');
                    strPanelReport.append('</div>');
            ...
            this.initReportSummary();
            //var strReportType = '';
            //api.report.renderReport(document.getElementById('containerMessageFactoryReport'),ProjectConfig.reportList[0].id,{onlySummary: true});
        },
        initReportSummary:function(){
            var $container = $('#containerMessageReport');
            $container.on('show.bs.collapse',function(e){
                if($(e.target).find('.report-container-wrap').length > 0)return;
                var spin = new LoadingSpinner({color: '#00FFFF'}).spin(e.target.querySelector('.panel-body'));
                api.report.renderReport(e.target.querySelector('.panel-body'),e.target.id,{onlySummary: true}).done(function(){
                    spin.stop();
                });
            })
        },
        initReportDetail:function(){
            ...
            btnReportDetail.off('tap').on('tap',function(e){
                e.preventDefault();
                e.stopPropagation();
                strReportTime = $(e.currentTarget).parentsUntil('#containerMessageFactoryReport','.panel').find('.reportTime').html();
                                        
                for (var i = 0;i<ProjectConfig.reportList.length; i++){
                    if(ProjectConfig.reportList[i].reportId == $(e.currentTarget).attr('report-to')) {
        ...
        getReportVersion:function(reportPeriod){
            var thisDay,version;
            var reportType = reportPeriod;
            var year,month,day;
