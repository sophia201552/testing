    ...
    function() {
        ...
        this.ready = function(model) {
            ...
            model.modules.forEach((module, i) => {
                ...
                //输入点绑定后的名字
                var valueArr = model.strategy.value = model.strategy.value || [],
                    nameMap = valueArr[i] = valueArr[i] || {};
        ...
        this.add = function(model) {
            ...
            if (data.option.input.length > 0 || data.option.output.length > 0) { //模板
                this.renameInput(data.option.input, model);
                model.modules.push(data);
                model.strategy.value.push({});
                model.selectIds = [];
                model.update = null;
                this.ready(model);
            } else { //非模板
        ...
        this.destroy = function(model) {
            ...
            var target = model.modules,
                index = target.findIndex(v => v['_id'] == id);

            if (index == -1) { //TODO 删除input或output
                model.modules.forEach(module => {
                ...
                target.splice(index, 1);
                target.forEach(module => { //删除其他模块对于该模块输出的引用
                    var i = module.option.input.findIndex(input => input.refId == id);
                    if (i > -1) {
                        module.option.input.splice(index, 1);
                    }
                });
                model.strategy.value.splice(index, 1);
            }
        ...
        this.update = function(model) {
            ...
            if (selectId != undefined) {
                var index = model.selectIds.indexOf(selectId.id);
                if (index > -1) { //已经是选中状态
                    model.selectIds.splice(index, 1);
                } else { //不是选中状态
                    if (selectId.ctrlKey) {
                        model.selectIds.push(selectId.id);
                    } else {
                        model.selectIds = [selectId.id];
                    };
                }
                this.noSelect(model);
        ...
        this.renameInput = function(inputs, model) {
            var nameSet = new Set();
