(function () {
        ...
        this.loadMap('AMap').done(function () {
            ...
            if (_this.projectId) {
                WebAPI.get("/project/getinfo/" + _this.projectId).done(function (rs) {
                    ...
                    if (rs.projectinfo) {
                        var dsType = 'real';
                        if (!rs.projectinfo.datadb && (rs.projectinfo.mysqlname || rs.projectinfo.collectionname)) {
                            dsType = 'virtual';
                        }

                        _this.$DSType.filter('[value=' + dsType + ']').click();
                    }
    ...
    AddProModal.prototype.checkRealTimeDb = function (dbName) {
        ...
        WebAPI.post('/admin/checkRealTimeDb', {realTimeDb: dbName}).done(function (result) {
            if (result.success && (!result.data || !result.data.length)) {
                ...
                _this.$VP_RealTimeDS.text('实时数据标识 {0} 已经被占用'.format(dbName));
    ...
    AddProModal.prototype.checkDtuName = function (dtuName) {
        var _this = this;
        WebAPI.post('/admin/checkProjDataSource', {
            dtuName: dtuName,
            ...
            if (!result.data.valid) {
                _this.isDTUValid = false;
                _this.setFormFieldError(_this.$projDataSource);
                _this.setFormFieldHelpText('项目标识 {0} 不存在。'.format(dtuName));
                return;
            }
            ...
            if (result.data.used) {
                _this.isDTUValid = false;
                _this.setFormFieldError(_this.$projDataSource);
                _this.setFormFieldHelpText('项目标识 {0} 已经被其他项目正在使用。'.format(dtuName));
                return;
            }
    ...
    AddProModal.prototype.attachEvents = function () {
        ...
        this.$projDataSource.off().on('keyup', function () {
            var $this = $(this), dtuName = $this.val().trim();
        ...
        this.$btnSubmit.off('click').click(function (ev) {
            ...
            defer.then(function (form) {
                ...
                if (_this.$DSType.filter(':checked').val() == 'real') {
                    if (!_this.isDTUValid) {
                        alert.danger('项目标识不合法, 无法提交');
                        return;
                    }
                    formData.append("dtuName", _this.$projDataSource.val());
                } else {
                    //虚拟项目
                    if (!_this.isRealDbValid) {
                        alert.danger('实时数据标识已经被占用, 无法提交');
                        return;
                    }
