    ...
    TBase.prototype.dropActionPerformed = function (e) {
        ...
        if (data.type) {
            entity = {
                ...
                option: {
                    type: data.type,
                    display: (data.type === 'ModalColdHotAreaSummary'||data.type === 'ModalEquipmentPerfectRate'||data.type === 'ModalWorkOrderStatistics'||data.type === 'ModalPriorityHandlingFaultList'||data.type === 'ModalEnergyTrendAnalysis') ? 0 : 1,
                    // display:1,
                    bg: 'blackBg',//默认 黑色字体
                    titleName: '',
                    titleCss: 'width:100%;height:50px;color:#111111;font-size:25px;',
                    containerCss: 'background:rgba(255,255,255,.7);border-radius:5px;padding:10px 14px;'
                },
                isHide:0
            };
