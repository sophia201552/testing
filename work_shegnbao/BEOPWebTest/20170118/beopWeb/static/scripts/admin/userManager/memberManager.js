        ...
        attachEvents: function () {
            ...
            $userSettingWin.on("click", "#userDel", function () {//点击页面管理-浮动窗口删除按钮
                ...
                WebAPI.post('/admin/getRecords', postData).done(function (result) {
                    if (result.success) {
                        $userSettingWin.find(".userOperationContent").children().empty().append(beopTmpl('accountOperationRecord', {obj: result.data}));
                ...
                WebAPI.post('/admin/userProjRecords/', {
                    ...
                    if (result.success) {
                        _this.data = result;
                        $userSettingWin.find(".userOperationContent").children().empty().append(beopTmpl('distributionProject', {obj: result.data}));
                        I18n.fillArea($("#userOperationRecords"));
                    } else {
                        $userSettingWin.find(".userOperationContent").css('background', '#ffffff').children().empty();
                    }
                });
