var SharpViewScreen = (function ($, window, undefined) {
    ...
    SharpViewScreen.prototype = {
        ...
        show: function () {
            WebAPI.get('/static/views/observer/sharpViewScreen.html').done(function (resultHtml) {
                // use a wrap
                $('<div class="sharpview-wrap sharpview" id="sharpViewWrap">').appendTo('body').html(resultHtml);
                _this.init();
            });
        },
        init: function () {
    ...
    SharpViewScreen.DEFAULTS = {
        itemTmpl: '<div class="play-slider-ctn" data-index="{0}"><div style="padding: 20px;z-index: 100;position: absolute;left:0;top:0;right:0;bottom:0;overflow-x: hidden;overflow-y: auto;">This page has no data.</div></div>'
    };
    ...
    LinearLayout.prototype.to = function (pos) {
        var _this = this;
        var direction = pos < this.curPos ? 'left' : 'right';
        var pos = pos === undefined ? 0 : Math.min(Math.max(pos, 0), this.total-1);
        if(pos === this.curPos) {this.doBoundAnimation(direction);return;}
        this.curPos = pos;
    ...
    LinearLayout.prototype.doBoundAnimation = function (direction) {
        direction = direction || 'left';
        $slider = this.$sliders.eq(this.curPos);
        if(direction === 'left') {
            $slider.addClass('bound-left');
        } else {
            $slider.addClass('bound-right');
        }
        if(this.curPos === 0) ; 
