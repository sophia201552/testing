var shareLogging = (function () {
    ...
    shareLogging.prototype = {
        ...
        init: function () {
            ...
            WebAPI.get('/analysis/getShareLog/' + AppConfig.userId).done(function (result) {
                _this.shareLogList = result.shareLogList;
                var totalShareLog = '';
                ...
                for (var i = 0; i < _this.shareLogList.length; ++i) {
                    ...
                    var shareTime = new Date(_this.shareLogList[i].shareDate.slice(0, _this.shareLogList[i].shareDate.indexOf('GMT'))).timeFormat(timeFormatChange('yyyy-mm-dd hh:ii'));//:ss
                    var strShareLog = new StringBuilder();
                    ...
                    strShareLog.append('    <div class="shareDescribe shareCon"><span style="text-align:left;vertical-align:middle;display:inline-block;width:90%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" class="shareVal" title="">' +
                        _this.shareLogList[i].shareDesc + '</span></div>');
                    strShareLog.append('    <div class="shareDate shareDate">' + shareTime + '</div>');
                    strShareLog.append('    <div class="shareOperate"><span class="glyphicon glyphicon-pencil shareDesc grow" style="vertical-align:middle;" title="' + I18n.resource.analysis.shareLog.EDIT_DESCRIPTION + '"></span><span class="glyphicon glyphicon-edit btnSharePageEdit grow" title="' + I18n.resource.analysis.shareLog.EDIT_PAGE + '"></span><span class="glyphicon glyphicon-remove btnShareRemove grow" title="' + I18n.resource.analysis.shareLog.DELETE_SHARE_LOG + '"></span></div>');
        ...
        initEdit: function () {
            ...
            $('.shareDesc').click(function (e) {
                ...
                e.stopPropagation();
                //获取描述文字
                var shareOldVal = $(e.currentTarget).parent('.shareOperate').siblings('.shareCon').children('span').text();
                //描述文字和铅笔隐藏
                $(e.currentTarget).parent('.shareOperate').siblings('.shareCon').children('span').css('display', 'none');
                ...
                //将输入框添加到此处
                $(e.currentTarget).parent('.shareOperate').siblings('.shareCon').append(inputDescEdit);
                inputDescEdit.focus();
                ...
                inputDescEdit.onblur = function (e) {
                    ...
                    $(e.target).parent('.shareCon').children('.shareVal').css('display', 'inline-block').html(e.target.value);
                    $(e.target).parent('.shareCon').siblings('.shareOperate').children('.shareDesc').html('');
                    e.target.remove();
                }
            });
        },
        initDelete: function () {
            $('.btnShareRemove').click(function (e) {
                e.stopPropagation();
                //TODO 测试confirm
                confirm(I18n.resource.analysis.shareLog.REMOVE_ALERT, function () {
        ...
        initDashboardPageEdit: function () {
            $('.btnSharePageEdit').click(function (e) {
                ...
                var menuItemId = _this.shareLogList[index].menuItemId.split('?')[0];
                var postData = {
