...
class BEOPSqliteAccess:
    ...
    def getPlantPageDetails(self, s3dbname, isOnlyStickPage = True):
        ...
        try:
            ...
            cu = con.cursor()
            if isOnlyStickPage:
                cu.execute("SELECT id, name, pagetype, width, hight, unitproperty02, groupid FROM list_page WHERE pagetype = 'fullscreen' AND unitproperty01 = '1' ORDER BY showOrder")
            else:
                cu.execute("SELECT id, name, pagetype, width, hight, unitproperty02, groupid FROM list_page ORDER BY showOrder")
            for x in cu.fetchall():
                url = ''
                if x[5] is not None: url = x[5].decode('gbk')
                data.append({ 'id': x[0], 'name': x[1].decode('gbk'), 'type': x[2].decode('gbk'), 'width': x[3], 'height': x[4], 'url': url, 'groupid': x[6] })
        except Exception as e:
            print('getPlantPageDetails error:' + e.__str__())
            logging.error('getPlantPageDetails error:' + e.__str__())
            logging.error('getPlantPageDetails error s3dbname:' + s3dbname.__str__())
        finally:
            if cu is not None:
                cu.close()
            if con is not None:
