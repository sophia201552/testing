...
class Reply(DbEntity):
    ...
    fields = ["id", "ofTransactionId", "replyUserId", "replyTime", "title", "detail", "replyToId"]
    ...
    def get_reply_after(self, date):
        return self.query(('id', 'ofTransactionId', 'replyUserId', 'replyTime', 'detail', 'replyToId'),
                          where=('replyTime>=%s and replyTime<=%s', [date.get("start"), date.get("end")]),
                          order=["replyTime DESC"])
    ...
    def delete_reply_by_id(self, reply_id):
        return self.delete(where=('id=%s', [reply_id]))

    def get_reply_by_id(self, reply_id):
        return self.query_one(self.fields, where=('id=%s', [reply_id]))
