...
class Transaction(DbEntity):
        ...
        for type in filter:
            ...
            if type == "tags":
                ...
                if isinstance(param, dict):
                    ...
                    if param == 1:
                        time_duration = DateUtils.get_today_duration(date_now)
                    elif param == 2:
                        time_duration = DateUtils.get_tomorrow_duration(date_now)
                    elif param == 3:
                        time_duration = DateUtils.get_weekly_duration(date_now)
                    elif param == 4:
                        time_duration = DateUtils.get_monthly_duration(date_now)
                    else:
                        time_duration = DateUtils.get_monthly_duration(date_now)
    ...
    def get_by_group_id(self, user_id, group_id, start_num, offset, order=('id', 'DESC')):
        ...
        return self.query(self.fields, where=(
            '''groupid = %s and (creatorId=%s or executorID=%s)
            or id in (select tm.transId from TRANSACTION_MEMBER tm left join TRANSACTION t on tm.transId = t.id
    ...
    def trans_message(self, user_id):
        record = Record()
        progress = record.get_records_by_user_id(user_id)
        if progress:
            all_user = User().get_all_user_map()
            for item in progress:
                item['transTitle'] = item.get('title')
                ...
                user = all_user.get(user_id)
                if user:
                    item['user'] = user.get('userfullname')
                    item['userPic'] = user.get('userpic')
                    item['useremail'] = user.get('useremail')
                item['time'] = item.get('opTime').strftime(Utils.datetime_format_full)
            return progress
        else:
            return []
