...
var ModelText = (function () {
    var textEditorTemplate = '<div class="popover observer-text-editor" style="padding: 10px;"><button type="button" class="close"><span>×</span></button>' +
        ...
        '</div>' +
        '</div>';
    var textTooltipTemplate = '<div class="tooltip observer-text-tooltip observer-text-editor" draggable="true" data-h5-draggable-node="true" data-ds-id=""><div class="tooltip-inner"><div id="pointName"></div><div><a class="lkAddToDS" href="javascript:;">' +
        '{0}</a></div></div></div>';
    ...
    ModelText.prototype.createTextEditor = function () {
        ...
        $template.find('#pointName').text(this.idCom);
        $template[0].dataset.dsId = '@'+AppConfig.projectId+'|'+this.idCom;
        return $template;
    };
