(function (beop) {
    ...
    wfDeleteTeam = function () {
        confirm("一旦团队解散将会删除相关数据， 是否确认解散该团队？", function () {
    ...
    wfTeamCreateConfirm = function () { // 创建团队提交
        ...
        if (!formData.name && !formData.desc && !isCreatorInTeamMembers && !tags) {
            alert.danger("您没有填写任何信息，无法添加团队");
            return false;
        }
        if (!formData.name || !formData.desc) {
            alert.danger("名称或者创建人为空");
            return false;
        }
        if (!isCreatorInTeamMembers) {
            alert.danger("你必须在新的团队架构当中");
            return false;
        }
        ...
        configMap.teamModel.addTeam(data).done(function (result) {
            if (result.success) {
                ...
                infoBox.alert("创建团队失败:" + result.data)
            }
        });
    };
    ...
    hasCreateTeamPermission = function () {
        return Permission.hasPermission('DCGrant') || Permission.hasPermission('SCGrant') || Permission.hasPermission('PGrant');
    };
