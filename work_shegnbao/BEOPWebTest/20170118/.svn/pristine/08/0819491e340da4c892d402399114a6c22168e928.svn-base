(function (beop) {
    ...
    loadServerStatus = function () {
        ...
        stateMap.currentPage = stateMap.currentPage ? stateMap.currentPage : 1;
    ...
    remarkSave = function () {
        return WebAPI.post("./updateServerRemark/" + stateMap.projectId, {
    ...
    enterProject = function () {
        ...
        stateMap.projectId = $parents.attr('project-id');
        location.hash = '#server/' + stateMap.projectId + '/';
    };

    itemRecordsWin = function () { // 点击详情文字弹出记录窗口，加载相应数据
        var $this = $(this);
        stateMap.selectRecordDom = $this.parents('td').find('.record_text');
        stateMap.projectId = $this.parents('tr').attr('dtu-id');
        jqueryMap.$remarksBox.hide();
        return WebAPI.get("./getServerRecords/" + stateMap.projectId).done(function (result) {
            if (result.success) {
    ...
    addRecords = function () { // 列表页添加记录接口
        ...
        if (info) {
            ...
            return WebAPI.post("/addServerRecords/" + stateMap.projectId, {
                'info': info
            }).done(function (result) {
                if (result.success) {
    ...
    loadServerDetail = function (project_id, $container) {
        stateMap.projectId = project_id;
    ...
    onClickSureIcon = function () {
        ...
        if ((stateMap.unit01[key] + '') === (value + '')) {
            ...
            $.ajax({
                type: 'POST',
                url: '/updateUnit/' + stateMap.dtu_name,
                crossDomain: true,
    ...
    onUploadCore = function (event) {
        ...
        spinner.spin(document.body);
        $.ajax({
            url: '/updateCore/' + stateMap.dtu_name,
            crossDomain: true,
            type: "POST",
            data: formData,
    ...
    getServerDetail = function () {
        return $.ajax({
            type: 'GET',
            url: '/serverDetail/' + stateMap.projectId
        }).done(function (result) {
            if (result.success) {
                if (!result.data) {
                    return;
    ...
    getCoreVersion = function () {
        return $.ajax({
            type: 'GET',
            url: '/getCoreVersion/' + stateMap.projectId
        }).done(function (result) {
            if (result.success) {
