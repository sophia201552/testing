...
(function(root, factory) {
    ...
    function() {
        ...
        this.getItem = function() {
            return this.model.getStore().strategy;
        }
        ...
        this.sync = function(dataset) {
            ...
            if (diffData) {
                ...
                var $saveBtn = $('#toolBarWrap #saveBtn').hide(),
                    $animateDiv = $('#toolBarWrap #saveAnimate').show(),
                    animateHtml = $animateDiv.html();
                ...
                $.when(saveApi, syncApi).done(function() {
                    $animateDiv.text('成功');
                }).fail(function() {
                    $animateDiv.text('失败');
                }).always(function() {
                    setTimeout(function() {
                        $animateDiv.html(animateHtml);
                        $animateDiv.hide();
                        $saveBtn.show();
                    }, 1000);
                });
        ...
        this.modelBLCProcessing = function(store, dataset) {
            ...
            store.strategy = dataset.strategy || store.strategy;
            return store;
        };
