...
@bp_workflow.route('/plan/addTaskToPool', methods=['POST'])
def plan_add_task_to_pool():
    ...
    task_pool = TaskPool()
    if task_pool.add_task(task_type, content):
...
@bp_workflow.route('/plan/listGroupTasks', methods=['POST'])
def plan_list_group_tasks():
    req = request.get_json()
    group_id = req.get('groupId')
    if group_id is None:
        return Utils.beop_response_error()
    scheduler = Scheduler()
    result = scheduler.get_group_scheduler_by_group_id(group_id)
...
@bp_workflow.route('/plan/listMonthTasks', methods=['POST'])
def plan_list_month_tasks():
    req = request.get_json()
    user_id = req.get('userId')
    year = req.get('year')
    month = req.get('month')
    if int(month) > 12 or int(month) < 1:
        return Utils.beop_response_error()
    scheduler = Scheduler()
    result = scheduler.get_group_scheduler_by_time(user_id, year, month)
    return Utils.beop_response_success(result)
