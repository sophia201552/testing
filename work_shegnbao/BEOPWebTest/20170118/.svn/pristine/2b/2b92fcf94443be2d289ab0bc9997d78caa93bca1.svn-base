...
def company_index(code):
    ...
    if rt.get('status') == AuthManager.token_valid:
        ...
        if code.lower() == 'cbre':
            project_id = 80
        return render_template('indexObserverLoad.html', projectId=project_id, icon=icon, title=title,
    ...
    if code.lower() == 'hzbt':
        title = 'Tricyclic'
        return render_template('login/HZBT.html', projectId=186, icon='project_img/tricyclic', title=title)

    if not project:
        return render_template('indexObserver.html')

    try:
        return render_template('login/' + code + '.html', projectId=project_id, code=code, title='BeOP')
    except:
        return render_template('indexObserver.html')
...
def loginTask(login,requestType=0):
    ...
    return dict(status=True, id=userId, userProfile=userProfile,
                projects=BEOPDataAccess.getInstance().getProject(userId, login.get('loginCode', None),requestType))
...
def project_create():
    ...
    dtu_name_id = None
    dtu_name = data.get('dtuName')  # 项目标识
    ...
    if dtu_name:  # 实际项目
        dtu = DtuServerProject().get_by_dtu_name(dtu_name)
        dtu_name_id = dtu.get('id')
        real_time_db = dtu.get('dbname')
        history_db = dtu.get('dbname')
    else:
        BEOPMySqlDBContainer().op_db_update(app.config['DATABASE'], "DELETE FROM dtusert_to_project WHERE projectid= '%s'" % (projId))
...
def importUserData(userId):
    ...
    try:
        ...
        for fileName in dataFileList:
            ...
            if bRes1 and bRes2:
                with open(fullFilePath, 'rt', encoding='utf8') as csvFile:
                    row_count = 0
                    data_count = 0
                    pointList = []
                    timeList = []
                                ...
                                for index, t in enumerate(timeList):
                                    if len(valueList[index]) == len(pointList):
                                        for index1, v in enumerate(valueList[index]):
                                            if v != 'null':
                                                insert_data_list.append((t.strftime('%Y-%m-%d %H:%M:%S'), pointList[index1].strip(), v.strip()))
                                                if len(insert_data_list) > 0 and len(insert_data_list) % 30000 == 0:
                                                    ret = requests.post(url = app.config.get('BEOP_SERVICE_ADDRESS') + '/sync_data_to_mongodb/smart',#app.config.get('BEOP_SERVICE_ADDRESS')
                                                                        headers={'content-type': 'application/json'},
