...
class diagnosisEngine:
    ...
    _collection_thing = MongoConnManager.getConfigConn().mdbBb[g_table_diagengine_thing]
    _collection_template = MongoConnManager.getConfigConn().mdbBb[g_table_diagengine_template]
    _collection_algorithm = MongoConnManager.getConfigConn().mdbBb[g_table_diagengine_template]
    ...
    def update_diagengine_thing(cls, data):
        ...
        try:
            ...
            dbrv = cls._collection_thing.update({'_id':data.get('_id')}, {'$set':data}, True)
            if dbrv.get('ok'):
    ...
    def update_dictVariable_in_diagEngine_thing(cls, thingId, dictVariable):
        ...
        try:
            if ObjectId.is_valid(thingId):
                confdata = cls._collection_thing.find_one({'_id':ObjectId(thingId)})
                if isinstance(confdata, dict):
                    confdata.get('dictVariable', {}).update(dictVariable)
                    dbrv = cls._collection_thing.update({'_id':ObjectId(thingId)}, {'$set':confdata})
                    if dbrv.get('ok'):
                        rt = True
        except Exception as e:
    ...
    def remove_diagengine_thing(cls, IdList):
        rt = False
        try:
            cls._collection_thing.delete_many({'_id':{'$in':[ObjectId(i) for i in IdList if ObjectId.is_valid(i)]}})
            rt = True
        except Exception as e:
    ...
    def get_diagengine_thing(cls, projId):
        ...
        cursor = None
        try:
            cursor = cls._collection_thing.find({'projId':int(projId)})
            for item in cursor:
                try:
    ...
    def update_diagengine_template(cls, data):
        ...
        try:
            ...
            for key in data:
                ...
                data.update({key:value})
            dbrv = cls._collection_template.update({'_id':data.get('_id')}, {'$set':data}, True)
            if dbrv.get('ok'):
                rt = data.get('_id').__str__()
        except Exception as e:
    ...
    def remove_diagengine_template(cls, IdList):
        rt = False
        try:
            cls._collection_template.delete_many({'_id':{'$in':[ObjectId(i) for i in IdList if ObjectId.is_valid(i)]}})
            rt = True
        except Exception as e:
    ...
    def get_diagengine_template(cls):
        ...
        cursor = None
        try:
            cursor = cls._collection_template.find({})
            for item in cursor:
                try:
                    rt.append({'_id':item.get('_id').__str__(), 'name':item.get('name'), 'type':item.get('type'),
                               'srcPageId':item.get('srcPageId'), 'dictVariable':item.get('dictVariable'),
    ...
    def update_diagengine_Algorithm(cls, data):
        ...
        try:
            ...
            dbrv = cls._collection_algorithm.update({'_id':data.get('_id')}, {'$set':data}, True)
            if dbrv.get('ok'):
                rt = data.get('_id').__str__()
        except Exception as e:
    ...
    def remove_diagengine_Algorithm(cls, IdList):
        rt = None
        try:
            dbrv = cls._collection_algorithm.update({'_id':{'$in':[ObjectId(x) for x in IdList if ObjectId.is_valid(x)]}}, {'$set':{'status':0}}, False)
            if dbrv.get('ok'):
                rt = True
        except Exception as e:
    ...
    def get_diagengine_Algorithm_by_IdList(cls, IdList):
        ...
        cursor = None
        try:
            cursor = cls._collection_algorithm.find({'_id':{'$in':[ObjectId(x) for x in IdList if ObjectId.is_valid(x)]},
                                                     'status':1})
            for item in cursor:
                try:
    ...
    def switch_algorithm_status_of_thing(cls, thingId, algorithmId, status, data):
        ...
        try:
            if ObjectId.is_valid(thingId):
                dbrv = cls._collection_thing.update({'_id':ObjectId(thingId)}, {'$set':{'dictAlgorithm':{algorithmId.__str__():int(status)}}})
                if dbrv.get('ok'):
    ...
    def get_notice_occurrence_statistics(cls, thingId, startTime, endTime, format):
        ...
        try:
            if ObjectId.is_valid(thingId):
                ...
                dbrv = cls._collection_thing.find_one({'_id':ObjectId(thingId)})
