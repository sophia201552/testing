        ...
        if dbname:
            ...
            if system_dict:
                strSQL = 'SELECT n.Id, n.Time, f.`Name`, f.Description, f.Points, '\
                         ...
                         'LEFT JOIN %s_diagnosis_equipments AS e ON f.EquipmentId = e.Id '\
                         'WHERE n.Time <= "%s" AND n.Time >= "%s" '\
                         'GROUP BY e.`Name` ORDER BY f.`Name`' % (dbname, dbname, dbname, endTime, startTime)
                dbrv = dbAccess._mysqlDBContainerReadOnly.op_db_query('diagnosis', strSQL, ())
                if dbrv:
                    for item in dbrv:
                        ...
                        def group_fault():
                            if len(detail) == 0:
                                ...
                                detail[-1].get('Detail').append({'NoticeId': item[0],
                                                                 'NoticeTime': item[1].strftime('%Y-%m-%d %H:%M:%S'),
                                                                 'EquipmentId': item[6], 'EquipmentName': item[8]})
                            else:
                                detail.append({'FaultName': item[2], 'FaultDescription': item[3],
                        ...
                        if len(sys.get('Subitem')) > 1:
                            ...
                            for k in i:
                                ...
                                if keys != sensor:
                                    ...
                                    gNum = tNum - sum([len(x.get('Detail')) for x in sys.get('Subitem').get(k).get('Detail')])
