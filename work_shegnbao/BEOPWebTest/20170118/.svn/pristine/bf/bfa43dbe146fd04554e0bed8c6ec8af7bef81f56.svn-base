        ...
        try:
            ...
            if dbname:
                if isinstance(connList, list):
                        ...
                        for item in find_list:
                            if item.get('st') and item.get('et'):
                                ...
                                if conn:
                                    ...
                                    print('getHistoryDataByFormat cost: %.1f'%(time.time()-tStart))
                                    if item_rt:
                                        if not rt:
                                            rt = item_rt
                                        else:
                    ...
                    if connList:
                        rt = connList.getHistoryDataByFormat(projId, dbname, pointName, timeStart, timeEnd, timeFormat, bSearchNearest)
        except Exception as e:
