                                ...
                                if len(rowItem[1]) == len(point_list):
                                    for colIndex in range(len(point_list)):
                                        ...
                                        try:
                                            ...
                                            if len(str(rowItem[1][colIndex])) > 0:
                                                record.append((rowItem[0], point_list[colIndex], rowItem[1][colIndex]))
                                            else:
                                                record.append((rowItem[0], point_list[colIndex], 0.0))
                                    ...
                                    if len(point_list_array[j]) == len(blockList[i][j]):
                                        for k in range(len(blockList[i][j])):
                                            ...
                                            if bNum:
                                                ...
                                                if len(str(blockList[i][j][k])) > 0:
                                                    record.append((timeList[i][j], point_list_array[j][k], blockList[i][j][k]))
                                                else:
                                                    record.append((timeList[i][j], point_list_array[j][k], 0.0))
                    elif leftBlankCol == 1:
                        for rowIndex in range(1,nrows):
                            row_data = sh.row_values(rowIndex)
                            if len(row_data) > 1:
...
def checkUploadFile(fileFullPath):
    ...
    if resEXCEL.get('erorr') != 'successful':
