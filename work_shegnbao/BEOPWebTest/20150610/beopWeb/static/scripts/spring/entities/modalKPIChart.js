...
var ModalKPIConfig = (function ($, window, undefined) {
    ...
    ModalKPIConfig.prototype.init = function () {
        ...
        this.$btnPreHistoryValUsage    = $('#btnPreHistoryValUsage', '#formWrap');
        this.$ulPreHistoryValUsage     = $('#ulPreHistoryValUsage', '#');
        // form groups
...
var ModalKPIChart = (function ($) {
    ...
    ModalKPIChart.prototype._render = function () {
        ...
        WebAPI.post('/analysis/startWorkspaceDataGenHistogramMulti', postData)
            .done(function (rs) {
                ...
                _this.chart = echarts.init(_this.container, 'macarons').setOption(_this.options);
                _this.reloadChart();
    ...
    // initialize history chart modal
    ModalKPIChart.prototype.initHistoryModal = function () {
        var _this = this;
        var $modalKPI = $('#modalKPIHistory');
        var $btnShowHistory = $('<a href="javascript:;" title="Show History" class="lk-show-history"><span class="glyphicon glyphicon-stats"></span></a>');
        var $panel = $(this.container).parents('.panel');
        ...
        // add "show history data" button
        $btnShowHistory.click(function () {
            $modalKPI.one('shown.bs.modal', function () {
                var option = _this.entity.modal.option;
                var targetPointId = option.targetPointId;
                var list = _this.store.list[targetPointId];
                var lastTime = _this.store.timeShaft[_this.store.timeShaft.length-1];
                // render line charts
                _this.historyChartOptions.series[0].data = _this.store.list[option.targetPointId];
                _this.historyChartOptions.series[0].markPoint.data = [
                    {name: lastTime, value: parseFloat(list[list.length-1]).toFixed(PRECISION), xAxis: list.length-1, yAxis: list[list.length-1]}
                ];
                
                _this.historyChartOptions.series[0].markLine.data.push([
                    {name: 'Average', value: _this.indicators.average.toFixed(PRECISION), xAxis: 0, yAxis: _this.indicators.average, itemStyle:{normal:{lineStyle:{type:'solid'}}}},
                    {xAxis: _this.store.deadline, yAxis: _this.indicators.average}
                ]);
                _this.historyChartOptions.xAxis[0].data = _this.store.fullTimeShaft;

                if(option.warnMode === 1) {
                    // series
                    _this.historyChartOptions.series[1] = {
                        name: 'Historic Comparison',
                        type: 'line',
                        symbolSize: 0,
                        data: _this.store2.list[option.targetPointId]
                    }
                    // legend
                    _this.historyChartOptions.legend.data[1] = 'Historic Comparison';
                    // tooltip
                    _this.historyChartOptions.tooltip.formatter = function (p) {
                        var name;
                        if(p.length === 2) {
                            name = new Date(new Date(p[0].name).valueOf() - _this.period.lag).format('yyyy-MM-dd HH:mm:ss');
                            return '{0}<br/>{1}: {2}<br/>{3}<br/>{4}: {5}'
                                .format(p[0].name, p[0].seriesName, p[0].value, name, p[1].seriesName, p[1].value);
                        } else {
                            if(p[0].seriesName === 'Historic Comparison') {
                                name = new Date(new Date(p[0].name).valueOf() - _this.period.lag).format('yyyy-MM-dd HH:mm:ss');
                                return '{0}<br/>{1}: {2}'.format(name, p[0].seriesName, p[0].value);
                            } else {
                                return '{0}<br/>{1}: {2}'.format(p[0].name, p[0].seriesName, p[0].value);
                            }
                        }
                        
                    };//'{b0}<br/>{a0}: {c0}<br/>{b1}<br/>{a1}: {c1}';
                }
                ...
                // in case pushing the "average mark line" repeatly
                _this.historyChartOptions.series[0].markLine.data.pop();
            });
    ...
    ModalKPIChart.prototype.update = function (rs) {
        ...
        lastTick = new Date(lastTick).valueOf() + 60000;
    ...
    ModalKPIChart.prototype.getTimePeriod = function () {
        ...
        if(circleMode === 0) {
            ...
            if(warnTimeMode === 0) {
                period.refStart = new Date(warnStart).format('yyyy-MM');
                period.refEnd = period.refStart + '-' + DateUtil.daysInMonth(new Date(period.refStart)) + ' 23:59:59';
            ...
            period.lag = new Date(period.nowStart).valueOf() - new Date(period.refStart).valueOf();
        }
        // quarterly
        else {
    ...
    ModalKPIChart.prototype.appendData = function (tValue, rValue) {
        ...
        if(isNaN(rValue) || this.isPointRuled(rValue, condVal, cond)) {
            ...
            if(lastTick !== nowStr) {
                timeStamp = new Date(lastTick).valueOf();
                // deal with the suitation when there is no data in last hour
                if((new Date(nowStr).valueOf() - timeStamp) > 3600000) {
    ...
    ModalKPIChart.prototype.setTimeParams = function () {
        ...
        if(option.dataCycleMode === 0) {
            this.store.deadline = new Date(now.format('yyyy-MM-'+DateUtil.daysInMonth(now)+' 23:55:00'));
