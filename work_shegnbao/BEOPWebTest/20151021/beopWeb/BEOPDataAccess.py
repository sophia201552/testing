...
class BEOPDataAccess:
    ...
    def getProjS3db(self,id):
        rt = None
        if BEOPDataAccess._lockProjectInfo.acquire():
            try:
                index = self.findProjectInfoById(id)
                if index >= 0:
                    rt = self._projectInfoList[index]['s3dbname']
                else:
                    ...
                    if dbrv == None or len(dbrv)<1:
                        ...
                        self._projectInfoList.append(dict(name_en= dbrv[0][2],  s3dbname= dbrv[0][0],  mysqlname=dbrv[0][1], id = id, collectionname=dbrv[0][3]))
                        rt = dbrv[0][0]
            except:
                logging.error('getProjS3db error')
            finally:
                BEOPDataAccess._lockProjectInfo.release()
        return rt
    ...
    def getProjMysqldb(self,id):
        ...
        if BEOPDataAccess._lockProjectInfo.acquire():
            try:
                index = self.findProjectInfoById(id)
                if index >= 0:
                    rt = self._projectInfoList[index]['mysqlname']
                else:
                    dbname = app.config.get('DATABASE')
                    ...
                    if dbrv is None or len(dbrv)==0:
                        ...
                        self._projectInfoList.append(dict(name_en= dbrv[0][1],  s3dbname= dbrv[0][2],  mysqlname=dbrv[0][0], id = id, collectionname=dbrv[0][3]))
                        rt = dbrv[0][0]
            except:
                logging.error('getProjMysqldb error')
            finally:
                BEOPDataAccess._lockProjectInfo.release()
        return rt
    ...
    def getProjIdByName(self,name_en):
        ...
        if BEOPDataAccess._lockProjectInfo.acquire():
            try:
                ...
                if index >= 0:
                    rt = self._projectInfoList[index]['id']
                else:
                    dbname = app.config.get('DATABASE')
                    ...
                    if dbrv is None or len(dbrv)==0:
                        ...
                        self._projectInfoList.append(dict(name_en= name_en, id = dbrv[0][1],  s3dbname= dbrv[0][2],  mysqlname=dbrv[0][0], collectionname=[0][3]))
                        rt = dbrv[0][1]
            except:
                logging.error('getProjIdByName error')
            finally:
                BEOPDataAccess._lockProjectInfo.release()
        return rt
    ...
    def getProjNameById(self, id):
        ...
        if BEOPDataAccess._lockProjectInfo.acquire():
            try:
                index = self.findProjectInfoById(id)
                if index >= 0:
                    rt = self._projectInfoList[index]['name_en']
                else:
                    dbname = app.config.get('DATABASE')
                    ...
                    if len(dbrv)==0:
                        ...
                        self._projectInfoList.append(dict(name_en= dbrv[0][0],  s3dbname= dbrv[0][1],  mysqlname=dbrv[0][2], id = id, collectionname=dbrv[0][3]))
                        rt = dbrv[0][0]
            except:
                logging.error('getProjNameById error')
            finally:
                BEOPDataAccess._lockProjectInfo.release()
        return rt
    ...
    def getCollectionNameById(self, id):
        ...
        if BEOPDataAccess._lockProjectInfo.acquire():
            try:
                index = self.findProjectInfoById(id)
                if index >= 0:
                    rt = self._projectInfoList[index]['collectionname']
                else:
                    dbname = app.config.get('DATABASE')
                    strQ = 'select name_en, s3dbname,mysqlname,collectionname from project where id=%s'
                    dbrv = self._mysqlDBContainerReadOnly.op_db_query(dbname,strQ, (id,))
                ...
                logging.error('getCollectionNameById error')
            finally:
                BEOPDataAccess._lockProjectInfo.release()
        return rt
    ...
    def findProjectInfoByNameEn(self, name_en):
        rt = -1
        try:
            for index, item in enumerate(self._projectInfoList):
                if item['name_en'] == name_en:
                    rt = index
                    break
        except:
            logging.error('findProjectInfoByNameEn error')
        return rt
    ...
    def findProjectInfoById(self, id):
        ...
        try:
            ...
            logging.error('findProjectInfoById error')
        return rt
            ...
            try:
                ...
                if index >= 0:
                    ...
                    if len(dbrv)==0:
                        ...
                        self._projectInfoList.append(dict(name_en= name_en,  s3dbname= dbrv[0][0],  mysqlname=dbrv[0][1], id = dbrv[0][2], collectionname = dbrv[0][3]))
                        rt = dbrv[0][1]
            except:
                logging.error('getProjMysqldbByName error')
            finally:
                BEOPDataAccess._lockProjectInfo.release()
        return rt
    ...
    def genMemProjectLocateMap(self):
        if BEOPDataAccess._lockLocateMap.acquire():
            try:
                BEOPDataAccess._projectLocateMap.clear()
                dbname = app.config.get('DATABASE')
                ...
                for item in dbrv:
                    if 'beopdata_' in item[1]:
                        ...
                        key = tuple(tempList)
                        BEOPDataAccess._projectLocateMap[key] = (item[0], item[2], item[3])
                        keyList.append(key)
                        addrNetList.append(item[2])
                        addrNalList.append(item[3])
                BEOPDataAccess._projectLocateMap['internetAddrList'] = list(set(addrNetList))
                BEOPDataAccess._projectLocateMap['internalAddrList'] = list(set(addrNalList))
                BEOPDataAccess._projectLocateMap['keyList'] = keyList
            except:
                logging.error('genMemProjectLocateMap error')
            finally:
                BEOPDataAccess._lockLocateMap.release()

    def getProjectLocateMap(self):
        if isinstance(app.config['Condition'], str) or\
            app.config['Condition'] >= 2:
            if not BEOPDataAccess._projectLocateMap:
                self.genMemProjectLocateMap()
        return BEOPDataAccess._projectLocateMap

    def UpdateProjectLocateMap(self):
        self.genMemProjectLocateMap()
        if BEOPDataAccess._projectLocateMap:
            return True
        return False
    ...
    def UpdateProjectInfo(self):
        ...
        try:
            if BEOPDataAccess._lockProjectInfo.acquire():
                if self._projectInfoList:
                    self._projectInfoList.clear()
                dbname = app.config.get('DATABASE')
                ...
                if dbrv == None:
                    ...
                    for item in dbrv:
                        self._projectInfoList.append(dict(name_en= item[3],  s3dbname= item[1],  mysqlname=item[2], id = item[0], collectionname=item[4]))
                if self._projectInfoList:
                    rt = True
        except:
            logging.error('UpdateProjectInfo error')
        finally:
            BEOPDataAccess._lockProjectInfo.release()
        return rt