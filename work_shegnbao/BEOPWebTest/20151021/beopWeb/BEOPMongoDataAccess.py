...
class BEOPMongoDataAccess:
    ...
    def analysisDataSourceSaveMulti(self, userId, data):
        itemIdList = []
        if isinstance(userId, int) and isinstance(data, dict) and len(list(data.keys())) > 0:
            bDataSourceId = False
            ...
            try:
                ...
                for item in data['itemList']:
                    if 'id' not in item.keys():
                        ...
                        val = item['value']
                        itemIdList.append({'alias':str(item['alias']), 'id':geneId.__str__(), 'value':val, 'note':str(item['note']), 'groupId':str(item['groupId']), 'projId':item['projId'], 'type':item['type']})
                    else:
                        if 'projId' in item.keys():
                        ...
                        val = item['value']
                        itemIdList.append({'alias':str(item['alias']), 'id':item['id'],  'value':val, 'note':str(item['note']), 'groupId':str(item['groupId']), 'projId':item['projId'], 'type':item['type']})

                rv = self.mdbBb[g_tableDataSource].find_one({'userId':userId})
                if rv == None:
                    if 'itemList' in data.keys():
                        self.mdbBb[g_tableDataSource].save({'userId':userId, 'list':listArray})
                else:
                    post = {}
                    doc = {}
                    ...
                    for item in listArray:
                        if ObjectId.is_valid(item.get('groupId')):
                            self.mdbBb[g_tableDataSourceGroupItem].update({'_id':ObjectId(item.get('groupId'))}, {'$addToSet':{'dsList':item.get('id').__str__()}})
            except Exception as e:
    ...
    def analysisDataSourceRemoveSingle(self, datasourceId, datasourceItemId, userId):
        ...
        if isinstance(datasourceId, str) and isinstance(datasourceItemId, str):
            ...
            if ObjectId.is_valid(datasourceId):
                ...
                spec = {'userId':userId}
            try:
                self.mdbBb[g_tableDataSource].update(spec, {'$pull':{'list':{'id':ObjectId(datasourceItemId)}}})
                self.mdbBb[g_tableDataSourceGroupItem].update({'dsList.id':datasourceItemId}, {'$pull':{'dsList':datasourceItemId}})
            except Exception as e:
    ...
    def analysisDataSourceGet(self, userId):
        ...
        if isinstance(userId, int):
            ...
            try:
                ...
                for item in cursor:
                    itemList = []
                    for index, subItem in enumerate(item['list']):
                        if subItem is not None and len(list(subItem.keys())) > 0:
                            val = subItem['value']
                            strNote = ''
    ...
    def getItemContentById(self, itemId, itemList):
        if isinstance(itemList, list):
            try:
                ...
                print('getItemContentById failed')
                print(e.__str__())
                logging.error(e.__str__())
                logging.exception(e)
        return {}
            ...
            try:
                ...
                if cursor.count() > 0:
                    for item in cursor:
                        ...
                        #     menuItemId = url[url.rfind('/') + 1:]
                        # menuItem = self.springGetMenu(menuItemId)
                        # layout = menuItem.get('layout')
                        # dashboardId = menuItem.get('id')
    ...
    def shareLogSave(self,userId,menuItemId,data):
        ...
        if isinstance(userId, int) and isinstance(menuItemId, str):
            if not mode:
                ...
                shareId = ObjectId(data.get('shareLogId'))
                post = {'_id': shareId}
    ...
    def shareLogEdit(self, data, editMode):
        rt = {'status':'unprocess'}
        shareId = ObjectId(data.get('shareId'))
        menuItemId = data.get('menuItemId')
        desc = data.get('desc')
        post = {'_id': shareId}
        doc = {'$set': {'desc': desc}}
    ...
    # def getOperationRecord(self, userId, startTime, endTime):
    #     rt = []
    #     try:
    #         start_time_obj = datetime.strptime(startTime, '%Y-%m-%d')
    #         end_time_obj = datetime.strptime(endTime, '%Y-%m-%d') + timedelta(hours=23, minutes=59, seconds=59)
    #         post = {'userId':userId, 'time':{'$gte':start_time_obj, '$lte':end_time_obj}}
    #         doc = {'_id':0}
    #         cursor = self.mdbBb[g_tableOperationRecord].find(post, doc).sort([('time',pymongo.ASCENDING)])
    #         for item in cursor.batch_size(300):
    #             for item_type in self ._pageTypeMapping:
    #                 if item.get('pageType') == item_type.get('id'):
    #                     item.update({'pageType':item_type.get('pageType')})
    #                     break
    #             rt.append(item)
    #     except Exception as e:
    #         print('getOperationRecord failed')
    #         print(e.__str__())
    #         logging.error(e.__str__())
    #         logging.exception(e)
    #     return rt
    ...
    def analysisGetDatasourceByItemId(self, itemId):
        ...
        if isinstance(itemId, str):
            ...
            cursor = None
            try:
                cursor = self.mdbBb[g_tableDataSource].find(post,dic)
                for item in cursor:
                    for index, subItem in enumerate(item['list']):
                        if subItem is not None and len(list(subItem.keys())) > 0:
                            val = subItem['value']
                            ...
                            if 'groupId' in subItem.keys():
                                strGroupId = subItem['groupId']
                            rt = {'type': subItem['type'], 'projId':subItem['projId'], 'alias': subItem['alias'], 'id': subItem['id'].__str__(), 'value':val, 'note':strNote, 'groupId':strGroupId}
                            break
            except Exception as e:
...
#analysis reconstruction from 2015-07-29
    ...
    def analysisGetDataSourceByIdList(self, userId, idList):
        ...
        for userDs in userDsList:
            for ds in userDs.get('list', []):
                if ds['id'].__str__() in idList:
                    dsList.append(ds)
