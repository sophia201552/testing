...
var WorkflowNotice = (function () {
    ...
    function render_template(notice) {
        WorkflowTool.getWorkflowTemplate('#wf-notice-latest-info').done(function (template) {
            template.find('.avatar-icon').attr("src",notice.userpic);
            template.find('.workflow-name').text(notice.trans_title);
            template.find('.avatar-name').text(notice.userName);
            template.find('.info-date-row .date').text(notice.opTime);

            template.find('.latest-info-title').html(WorkflowTool.translate(notice.title));
            try{
                var detail=JSON.parse(notice.detail);
            }catch(e){
                var detail=notice.detail;
            }
            if(typeof(detail)=="object"){//为编辑内容时
                if(notice.op=="new"){
                    template.find('.media-heading').html(WorkflowTool.translate(notice.op));
                }else{
                    template.find('.media-heading').html(WorkflowTool.translate(notice.op));
                    template.find('.notice-item-content').html(WorkflowTool.translate(notice.detail));
                }
            }else{
                template.find('.media-heading').html(WorkflowTool.translate(notice.op));
                template.find('.notice-item-content').remove();
            }
    ...
    WorkflowNotice.prototype = {
        show: function () {
            var _this = this;
            WebAPI.get("/static/views/workflow/notice.html").done(function (resultHtml) {
