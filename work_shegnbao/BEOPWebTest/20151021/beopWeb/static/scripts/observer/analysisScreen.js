...
var AnalysisScreen = (function () {
    ...
    AnalysisScreen.prototype = {
        ...
        close: function () {
            ...
            if(isExistSpinner) {
                if( !confirm('Exit now will interrupt the current operation，continue？') ) {
                    return false;
                }
            }
            ...
            this.wsPanel && this.wsPanel.close && this.wsPanel.close();
            this.tplPanel && this.tplPanel.close && this.tplPanel.close();
            ...
            // 停止 数据同步 定时器
            // 停止前需要同步一次
            if(this.syncWorker.status === 'processing') {
        ...
        initItemTools: function(){
            ...
            FullScreenManager.init(function (e) {
                ...
                if(e.isFullScreen) {
                    // find slider sets
                    sliders = _this.paneCenter.ws.modalList;
        ...
        renderModal: function () {
            ...
            if (modalClass) {
                var param2 = ('AnlzTendency' == option.type) ? option : null;
                this.refreshPaneCenter(new modalClass(this.container, param2, this));
            }
            ...
            $.when(this.saveModalJudge).done(function (_this, _mode, _chart,_targetSlider,_curModal,imgSaveJudge) {
                ...
                if (imgSaveJudge) {
                    if (!_chart) {
                        ...
                        //设置缩略图背景颜色
                        if(AppConfig.chartTheme == theme.Dark){
                            _chart.setOption({backgroundColor: "#fff"});
                            imageSm = _chart.getImage('jpeg');
                            _chart.setOption({backgroundColor: "transparent"});
                        }else{
                            imageSm = _chart.getImage('jpeg');
                        }
        ...
        shareTo: function(){
            ...
            workAndReport = new Date().format('yyyy-MM-dd') + ' ' + this.paneCenter.ws.name + ' <i style="font-weight:600;">' + selectedNames + '  </i>  ' + I18n.resource.analysis.shareLog.DESC_TIP_ETC + '  ' + selectedIds.length + '  ' + I18n.resource.analysis.shareLog.DESC_TIP_TABLE;
        ...
        preview: function () {
            ...
            promise.done(function () {
                ...
                ScreenCurrent.dsInfolist = dsInfoList;
