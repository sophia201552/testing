    ...
    DataCalcFilter.prototype = {
        ...
        initCloseControl: function() {
            ...
            this.$btnFilter.off().click(function (e) {
                var filterCondition = _this.$filterCodition.mathquill('latex');     //_this.$filterCodition.val();
                if (!filterCondition) {
                    alert('Please input filter condition !');
                    _this.$filterCodition.select();
                    return;
                }

                var strPtId = _this.$selPtList.val();
                var radioFilterVal = _this.$page.find('input:radio[name=radioFilterType]:checked').val();
                var radioShowVal = _this.$page.find('input:radio[name=radioShowType]:checked').val();
...
*/
                ...
                // post
                var condition = _this.$filterCodition.mathquill('latex');
                ...
                for (var i = 0, len1 = _this.m_arrDsItem.length; i < len1; i++) {
                    ...
                    if (-1 != condition.indexOf(alph)) {
                        var id = _this.m_arrDsItem[i].id;
                        dataObj[alph] = [];
                        for (var j = 0, len2 = _this.m_chartOption.series.length; j < len2; j++) {
                            if (id == _this.m_chartOption.series[j].id) {
                                dataObj[alph] = _this.m_chartOption.series[j].data;
                                break;
                            }
                        }
                    }
                ...
                var dataTarget;
                var tarId = _this.$selPtList.val();
                for (var i = 0, len = _this.m_arrDsItem.length; i < len; i++) {
                    if (tarId == _this.m_arrDsItem[i].id) {
                        dataTarget = _this.m_arrDsItem[i].alphabet;
                        break;
                    }
                }
                var postData = {
                ...
                WebAPI.post('/expressAnalysis/calc', postData).done(function (result) {
                    ...
                    for (var j = 0; j < dataLen; j++) {
                        if (Boolean(res.data[j]) && !filterVal[j]) {
                            item.appendData.push(res.data[j]);
                        }
                        else {
                    ...
                    for (var i= 0,len=_this.m_curModal.arrFilter.length; i<len; i++) {
                        if (strPtId == _this.m_curModal.arrFilter[i].pointId) {
                            bFind = true;
                            break;
                        }
                    }
        ...
        initFilterCondition: function() {
            ...
            if (codition) {
                ...
                //var arrValName = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'];
                //for (var i= 0,len=this.m_arrDsId.length; i<len; i++) {
                //    codition = codition.replace(new RegExp('<%' + this.m_arrDsId[i] + '%>','g'), arrValName[i]);
                //}
                //this.$filterCodition.mathquill('editable');
                this.$filterCodition.mathquill('latex', codition);

                //for (var i= 0,len=this.m_arrDsId.length; i<len; i++) {
                //    this.m_curModal.dictFilter[arrValName[i]] = this.m_arrDsId[i];
                //}
            }
        },
