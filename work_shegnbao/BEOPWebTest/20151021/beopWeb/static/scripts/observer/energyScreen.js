...
var EnergyScreen = (function () {
    function EnergyScreen(id, store, shareSaveChange,dsInfoList) {
        if (!id) {
    ...
    EnergyScreen.prototype = {
        ...
        close: function () {
            if (this.isConfigMode) {
                if (this.isScreenChange) {
                    var saveConfirm = confirm('There are some changes haven\'t been saved, do you want to save them before quit? ');
                    if (saveConfirm) {
        ...
        init: function () {
            ...
            if (this.store) {
                ...
                WebAPI.get("/spring/get/" + this.id + '/' + AppConfig.userId).done(function (result) {
            ...
            if (this.dsInfolist){
                _this.store.dsInfoList = this.dsInfolist;
            }
            function initByData() {
        ...
        initToolBar: function () {
            ...
            if (this.isForReport) {
                ...
                var $liConfig = $('<li class="iconWrap" id="btnConfigDashboard"><span class="glyphicon glyphicon-cog"></span><span class="dropdownNav">'+ I18n.resource.observer.widgets.CONFIG_MODE +'</span> </li>');
                var btnMode = $('<span>')
                .addClass('badge grow')
            ...
            $('#btnConfigDashboard').remove();
            $('#btnChangeProject').before($liConfig);
        },
        ...
        saveLayout: function () {
            ...
            WebAPI.post('/spring/saveLayout', data).done(function (result) {
                if (_this.isForReport) {
                    ...
                    _this.initShareIframe("/share/db/" + AppConfig.userId + "/" + _this.id);
        ...
        rebornEntity: function (entityParams, tragetType,targetTitle) {
            ...
            if ((!entityParams.isNotRender) && entityParams.modal.type !== 'ModalMix') {
        ...
        initReplay: function() {
            var _this = this;
            var $ele = $('#right-nav').find('li:first ul>.toolBacktrace');
            if($ele.length > 0) {
                //$ele.prev().remove();
                $ele.remove();
            }
