...
def plan_list_today_tasks():
    ...
    trans = Transaction()
    reminds_result = trans.trans_remind(user_id)
    ...
    return Utils.beop_response_success({'scheduler': scheduler_result,
                                        'transaction': trans_result, 'reminds': reminds_result})
...
def activities(user_id, activity_time_type, limit):
    ...
    record_list = activity.get_record_by_date_activityType(last_activity_id, time_interval_dict, group_ids,
    ...
    if not record_list_count:
        ...
        record_list = activity.get_record_by_date_activityType(last_activity_id, time_interval_dict,
                                                               group_ids,
                                                               int(activity_time_type), limit=limit_list)
        record_list_count = len(record_list.get('activities'))
        # 如果还查不到数据就直接返回出去
...
# 团队 加载
@bp_workflow.route('/team/', methods=['POST'])
def wf_team_show():
    user_id = AuthManager.get_userId()
    team = Team()
    try:
...
# 任务组 加载
@bp_workflow.route('/taskGroup/', methods=['POST'])
def wf_task_group():
    user_id = AuthManager.get_userId()
...
# 任务组 项目 加载
@bp_workflow.route('/taskGroupProject/', methods=['POST'])
def wf_task_group_project():
    ...
    for task_group_item in task_group_result:
        ...
        for process_id in task_group_item.get("process"):
            process = team_process_map.get(process_id)
            process_result.append(process if process else [])
        result.append({
            '_id': str(task_group_item.get('_id')),
            'name': task_group_item.get('name'),
            'desc': task_group_item.get('desc'),
...
def wf_task_save():
    ...
    task_group_id = rq_data['taskGroup']
...
def wf_task_group_new():
    ...
    rq_data = request.get_json()
    tg = TaskGroup()
    result = tg.save_task_group(rq_data)
    if result:
        return Utils.beop_response_success()
    else:
        return Utils.beop_response_error()
