(function (beop) {
    ...
    var _structureShow, _wfGroupNameEdit, _wfGroupNameSave, _wfAddTeamGroup, _wfTeamGroupAddInput, _wfTeamPeopleAddWin,
        _renderStructure, _resetData,
        getStructureItem, addStructureItem, changeStructureItemName, changeStructureItemMember, deleteStructureItem,
        getStructure, _getTeamId;
    var initTeamProcess;
    ...
    init = function ($box, structure, viewType) { // viewType 为团队页面的view 类型
        _resetData();
            ...
            structure = [
                {
                    ...
                    name: '超级管理员',
                    members: [],
                    type: stateMap.groupUserRoleType.superAdmin
                },
                {
                    id: ObjectId(),
                    //name: 'admin',
                    name: '管理员',
                    members: [],
                    type: stateMap.groupUserRoleType.admin
                }
            ]
        } else {
            structure[0].name = '超级管理员';
            structure[1].name = '管理员';
        }
    ...
    addStructureItem = function (item) {
        ...
        stateMap.structure.push({
            ...
            name: item.name,
            members: [],
            type: stateMap.groupUserRoleType.member
        });
    };
    ...
    changeStructureItemMember = function (id, members) {
        var item = getStructureItem(id);
        if (!item) {
    ...
    deleteStructureItem = function (id) {
        for (var m = 0, n = stateMap.structure.length; m < n; m++) {
            if (stateMap.structure[m].id == id) {
                stateMap.structure.splice(m, 1);
            }
        }
    };
    ...
    _wfTeamPeopleAddWin = function () {
        var selectNum = null, $this = $(this);
        var teamId = _getTeamId($this);
        var structureItem = getStructureItem(teamId);


        if (structureItem.type == stateMap.groupUserRoleType.superAdmin) {
            selectNum = 1;
        } else if (structureItem.type == stateMap.groupUserRoleType.admin) {
            selectNum = 5;
        } else if (structureItem.type == stateMap.groupUserRoleType.member) {
            selectNum = null;
        }
    ...
    _renderStructure = function (teamId) { // 刷新成员结构
        $('[wf-team-id=' + teamId + ']').find('.wf-group-people-box').html(beopTmpl('tpl_wf_added_member', {members: getStructureItem(teamId).members}));
    };
