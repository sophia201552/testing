...
PaneProjectConfigure = (function () {
	...
	PaneProjectConfigure.prototype = {
		show: function () {
			var _this = this;
			$.get('/static/views/admin/paneProjectConfigure.html').done(function (resultHtml) {
		...
		init: function () {
			...
			$('#returnMain').click(function(){
				if ($('#ulLogin').text() != '')ScreenManager.show(IndexScreen);
				else ScreenManager.show(ProjectManager);
			});
		...
		//根据图片制表
		chartDraw: function () {
			var myChart1 = echarts.init(document.getElementById('step4Chart1'));
			var myChart2 = echarts.init(document.getElementById('step4Chart2'));
			var myChart3 = echarts.init(document.getElementById('step4Chart3'));
			var myChart4 = echarts.init(document.getElementById('step4Chart4'));
		...
		finalDataUpload: function () {
			...
			WebAPI.post('/project/create', {
				...
				if (data.error) alert(I18n.resource.admin.projectConfigure.UPLOAD_PROJECT_FAILED);
					...
					WebAPI.post('/project/update',{userId: AppConfig.userId}).done(function(pro_result){
						var proList = JSON.parse(pro_result);
