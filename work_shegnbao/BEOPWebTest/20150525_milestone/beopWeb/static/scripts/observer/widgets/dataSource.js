var DataSource = (function () {
    ...
    DataSource.prototype = {
        ...
        initContain: function () {
            ...
            $('#inputAddGroup').attr('placeholder', _this.m_lang.ADD_GROUP);
                    ...
                    WebAPI.post('/datasource/saveDataSourceGroup', postData).done(function (result) {
                        ...
                        if (24 == groupId.length) {
                            ...
                            if (!bIsFind) {
                                ...
                                _this.insertTreeGroup(data.groupId, data.groupName, 0, '');
...
*/
        ...
        initToolTips: function (parent, customName, projectName, pointName, pointDesc) {
            ...
            show.append('        <p class="customName" style="word-break:break-all;">').append(_this.m_lang.CUSTOM_NAME).append(': ').append(customName).append('</p>');
            show.append('        <p class="projectName" style="word-break:break-all;">').append(_this.m_lang.PROJECT_NAME).append(': ').append(projectName).append('</p> ');
            show.append('        <p class="pointName" style="word-break:break-all;">').append(_this.m_lang.POINT_NAME).append(': ').append(pointName).append('</p> ');
            show.append('        <p class="pointDesc" style="word-break:break-all;">').append(_this.m_lang.POINT_DESC).append(': ').append(pointDesc).append('</p> ');
            show.append('    </div>');
...
*/
        ...
        initFormulaToolTips: function (parent, customName, formula, desc) {
            ...
            show.append('    <div class="tooltipTitle tooltip-inner">GeneralRegressor</div>');
            show.append('    <div class="tooltipContent">');
            show.append('        <p class="customName" style="white-space:nowrap;">').append(_this.m_lang.CUSTOM_NAME).append(': ').append(customName).append('</p>');
            show.append('        <p class="formula" style="white-space:nowrap;">').append(_this.m_lang.FORMULA_NAME).append(': ').append('</p>');
            show.append('        <p class="pointDesc" style="white-space:nowrap;">').append(_this.m_lang.POINT_DESC).append(': ').append(desc).append('</p>');
            show.append('    </div>');
            show.append('    <div class="tooltip-arrow"></div>');
            show.append('</div>');
        ...
        getDSItemData: function (target, arrDSItemIds) {
            ...
            var sessionFind = sessionStorage.getItem(ItemKey);
            ...
            if (null == sessionFind) {
                ...
                var jsonSes = JSON.parse(sessionFind);
                var tmStartSes = new Date(jsonSes.timeShaft[0]);
                var tmLen = jsonSes.timeShaft.length;
                var tmEndSes = new Date(jsonSes.timeShaft[tmLen - 1]);
                if (tmStartSes <= tmStart && tmEndSes >= tmEnd) {
                    var dataSrc = sessionStorage.getItem(ItemKey);
                    target.renderModal(JSON.parse(dataSrc));
                }
                else {
