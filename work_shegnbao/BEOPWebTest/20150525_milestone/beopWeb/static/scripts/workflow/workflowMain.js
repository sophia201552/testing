...
var WorkflowTool = (function () {
    var pic_path = '/static/images/workflow/', getTemplatePromise, templateStorage;
...
$(function () {
    ...
    function FetchGroups() {
        if (!_fetchGroupPromise) {
            _fetchGroupPromise = WebAPI.get('/getTransactionGroup');
        }
    ...
    $(ElScreenContainer).on('click.filter-toggle', '.member-filter-dialog-icon', function (ele) {
        ...
        if (mini_dialog.hasClass('open') && !mini_dialog.attr('done')) {
            ...
            FetchGroups().done(function (result) {
                ...
                for (var n = 0, nlen = result.length; n < nlen; n++) {
                    var item = result[n];
                    groups.append('<a class="btn btn-info task-btn" data-id="' + item.id + '">' + item.nm + '</a>');
                }
                mini_dialog.attr('done', true);
            }).always(function () {
                Spinner.stop();
