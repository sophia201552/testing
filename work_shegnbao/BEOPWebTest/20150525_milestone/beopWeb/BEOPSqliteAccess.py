...
class BEOPSqliteAccess:
    ...
    def prepareResouceFromS3db(self, name, s3db, updatetime, clean):
        ...
        if os.path.exists(dbfileDec):
            ...
            print("%s is not exist!" % (dbfileDec));
    ...
    def getPlantPageDetails(self, s3dbname):
        ...
        try:
            ...
            con.text_factory = bytes
            cu = con.cursor()
            cu.execute("SELECT id, name, pagetype, width, hight, unitproperty02 FROM list_page WHERE unitproperty01 = '1' ORDER BY showOrder")
            data = []
