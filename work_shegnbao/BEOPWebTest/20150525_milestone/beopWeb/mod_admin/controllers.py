...
def load_project_permission():
    ...
    for i in permission:
        if temp_project_id != i.get('projectId'):
            temp_project_id = i.get('projectId')
            existed_project = {}
            for project_item in project_list:
                if temp_project_id == project_item.get('id'):
                    existed_project = project_item
            if existed_project:
                project = existed_project
            else:
                project = {'id': i.get('projectId'), 'name': i.get('projectName'), 'roles': []}
                project_list.append(project)

        if temp_role_id != i.get('roleId'):
            temp_role_id = i.get('roleId')
            role = {'id': i.get('roleId'), 'name': i.get('roleName'), 'users': []}
            project['roles'].append(role)
        else:
            for existed_role in project['roles']:
                if existed_role.get('id') == temp_role_id:
                    role = existed_role
