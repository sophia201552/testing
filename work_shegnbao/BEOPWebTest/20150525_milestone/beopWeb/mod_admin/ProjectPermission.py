...
class ProjectPermission:
    ...
    def get_permission(user_id, *obj):
        ...
        if user_id and user_id != 1:
            sql = '''select p.id AS projectId, p.name_cn AS projectName,
            ...
            sql += ' where ur.userid in (' + ','.join(user_id_list) + ')'
    ...
    @staticmethod
    def insert_role_user(user_id, role_id):
        sql = 'insert into `user_role` (userId,roleId) values(%s,%s)'
        return ProjectPermission.db.op_db_update(ProjectPermission.db_name, sql, (user_id, role_id))
