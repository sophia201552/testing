...
class TagDictV2:
    ...
    def get_equipment_tag_list(self):
        rt = []
        self.cursor = self.mdbBb.find({'type': 'Equipment'})
        for item in self.cursor:
            rt.append(item.get('name'))
        return rt

    def get_tags_by_attribute(self):
        rt = {}
        self.cursor = self.mdbBb.find({"attrInputs": {'$nin': [[], None]}})
        for item in self.cursor:
            rt.update({item.get('name'): {'zh': [], 'en': []}})
            attrInputs = item.get('attrInputs')
            for i in attrInputs:
                rt.get(item.get('name')).get('zh').append(i.get('name').get('zh'))
                rt.get(item.get('name')).get('en').append(i.get('name').get('en'))
        return rt
