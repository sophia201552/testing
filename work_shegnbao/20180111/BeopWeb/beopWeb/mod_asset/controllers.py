    ...
    try:
        for file in files:
            ...
            if rv.status == 200:
                ...
                if projId:
                    new_data = MongoConnManager.getConfigConn().getThingDetail(tid)
                    new_data.update({'urlImg': path + file.filename})
                    # 由于新上传的图片的路径+filename和原来一样, 所以oldData和newData是一样的
                    MongoConnManager.getConfigConn().insert_asset_operation_log({'projId': int(projId), 'userId': int(userId)}, new_data, new_data, 'modifyImg', 'Asset_Things')
...
@app.route('/asset/maintainRecords/edit', methods=['POST'])
def edit_maintainRecords():
    error = None
    post_data = request.get_json()
    try:
        if post_data:
            if ObjectId.is_valid(post_data.get('_id')):
                post_data.update({'_id': ObjectId(post_data.get('_id'))})
                rt = MongoConnManager.getConfigConn().edit_maintainRecords(post_data)
            else:
                error = 'Invalid parameter'
        else:
            error = 'Invalid parameter'
    except Exception as e:
        print('edit_maintainRecords error:' + e.__str__())
        logging.error(e.__str__())
        error = e.__str__()
    if error:
...
@app.route('/asset/getAssetThings/<projId>', methods=['GET'])
def getAssetThings(projId):
    try:
        rt, thingList = Asset.getGroupsThings(projId)
        data = Asset.checkThings(rt, thingList)
    except Exception as e:
        print('getAssetThings error:' + e.__str__())
        logging.error(e.__str__())
        return jsonify(dict(msg=e.__str__(), error=1))
    return jsonify(data)


@app.route('/asset/getThingsMaintainRecords/<projId>', methods=['GET'])
def getThingsMaintainRecords(projId):
    try:
        rt, thingList = Asset.getGroupsThings(projId)
        rv = Asset.checkMainTain(rt, thingList)
    except Exception as e:
        print('getAssetThings error:' + e.__str__())
        logging.error(e.__str__())
        return jsonify(dict(msg=e.__str__(), error=1))
