            ...
            for item in dbrv:
                ...
                if buildingId in zone_dict.keys():
                    if subBuildingId in zone_dict.get(buildingId).get('Detail').keys():
                        zone_dict.get(buildingId).get('Detail').get(subBuildingId).get('Detail').append({
                            'EquipmentId': item[0], 'EquipmentName': item[1], 'State': 0 if item[6] else 1})
                    else:
                        zone_dict.get(buildingId).get('Detail').update({
                            subBuildingId: {
        ...
        try:
            ...
            if dbname:
                for userId in userId_list:
                    ...
                    def set_num():
                        if timeList[n] <= item[4] < timeList[n + 1]:
                            total[n] += 1
                            if item[2]:
...
def get_equipmentIntactRate_pandect(projId, lang='zh-CN'):
    ...
    """
    rt = {'status': 0, 'message': None, 'data': [], 'allDtuOffline': False}
    try:
        dbAccess = BEOPDataAccess.getInstance()
        dbname = dbAccess.getProjMysqldb(projId)
        ...
        if dbname:
            ...
            try:
                ...
                pass
            # 3038 该项目中起止时间dtu是否全部掉线
            isAllDtuOffline = requests.get(app.config.get(
                'BEOP_SERVICE_ADDRESS') + '/project/dtu/getAllDtuIsOffline/' + str(
                projId) + '/' + startTime + '/' + endTime,
                                      headers={"Content-Type": "application/json"})
            if isAllDtuOffline.status_code == 200:
                isAllOffline = json.loads(isAllDtuOffline.text)
                rt["allDtuOffline"] = isAllOffline.get('data')
