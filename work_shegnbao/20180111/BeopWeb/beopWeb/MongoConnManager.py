...
class MongoConnManager:
    ...
    def _getConnByAddr(addr, **kwargs):
        ...
        if conn is None:
            logging.info("Initializing connection to %s with %s...", addr, kwargs)
            ...
            conn = BEOPMongoDataAccess(addr, is_ssl=is_ssl, replica_set=replica_set, read_preference=read_preference)
            MongoConnManager._connDict[addr] = conn
        return conn

    @staticmethod
    def getHisConnWrite(projId):
        return MongoConnManager.getHisConn(projId)
    ...
    def getHisConn(projId):
        ...
        try:
            ...
            logging.error("Failed to get Mongo hisdata connection!", exc_info=True, stack_info=True)
        return rt
    ...
    def getConfigConn():
        ...
        for i in range(0, 10):
            try:
                ...
                rt = MongoConnManager._getConnByAddr(configIP, replica_set=replica_set, read_preference=read_preference)
                if rt is not None:
                    return rt
                else:
                    raise Exception('Got a None connection object!')
            except:
                logging.error("Failed to get Mongo config connection! Retry (%s)!", i, exc_info=True, stack_info=True)
        return rt
    ...
    def UpdateProjectInfo():
        ...
        try:
            ...
            logging.error("Failed to update project info!", exc_info=True, stack_info=True)
        return rt
    ...
    def UpdateProjectLocateMap():
        try:
            ...
            logging.error("Failed to update project locate map!", exc_info=True, stack_info=True)
        ...
        for key in mi:
            ...
            if inscont and inscont.get('is_default'):
                if inscont.get('clusterName') == clusterName:
                    ...
                    return MongoConnManager._getConnByAddr(addr, is_ssl=inscont.get('is_ssl'))
                default_inscont = inscont
        # If no default mongo instance found in the current cluster, return the first default instance found
        if default_inscont is not None:
            addr = default_inscont.get('internet_addr')
            return MongoConnManager._getConnByAddr(addr, is_ssl=inscont.get('is_ssl'))
        # Not found anyway
        return None
