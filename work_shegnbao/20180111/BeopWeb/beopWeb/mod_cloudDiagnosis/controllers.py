...
from beopWeb.mod_cxTool.PointTable import PointTable as pt
...
def save_customDiagnosis():
    ...
    if rt:
        if pt.is_conflict(rt.get('moduleName').strip()):
            return Utils.beop_response_error(msg='moduleName conflict with apiName')
...
def remove_customDiagnosis():
    ...
    try:
        if IdList:
            if isinstance(IdList, str):
                IdList = [IdList]
            nList = CloudDiagnosis.get_cloudDiagnosis_customDiagnosisId_by_parentId(IdList)
            IdList.extend(nList)
    ...
    if rt:
        ...
        return Utils.beop_response_error(msg='remove diagnosis failed')
