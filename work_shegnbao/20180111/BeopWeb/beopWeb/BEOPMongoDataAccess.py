...
g_managementReportConfig = 'managementReportConfig'
...
class BEOPMongoDataAccess:
    ...
    def __init__(self, addr, st=None, et=None, is_ssl=False, dbname='beopdata', replica_set=None, read_preference=None):
        connection_info = \
            "addr = %s, st = %s, et = %s, is_ssl = %s, dbname = %s, replicaset = %s, readPreference = %s" \
            % (addr, st, et, is_ssl, dbname, replica_set, read_preference)
        logging.info("Connecting to MongoDB %s", connection_info)
        bOk = False
        try:
            self._st = st
            self._et = et
            ...
            logging.error('Failed to connect to MongoDB %s', connection_info, exc_info=True, stack_info=True)

        if not bOk:
            logging.error('Failed to connect to MongoDB %s', connection_info)
        else:
            logging.info('Connected to MongoDB %s', addr)
        ...
        if isinstance(pointList, list):
            ...
            if length > 0:
                ...
                try:
                    ...
                    logging.error('Failed to query mongo for history data! Parameters: %s',
        ...
        try:
            ...
            if rt is None:
                ret = self.mdbBb[collectionName].find_one(
                    {'pointname': _point_name, 'time': {'$lte': timeEnd+timedelta(minutes=tDistanceMinutesLimit), '$gte': timeEnd},
                     "value": {'$nin': [None, 'Null', 'None']}}, sort=[('time', pymongo.ASCENDING)])
                rt = ret.get('value') if ret else None
        except Exception as e:
    ...
    def getCustomNavAllByRoleAndProject(self, project_id, role_ids, i18n):
        top_nav = []
        func_nav = []
        benchmarks = []
        try:
            ...
            if not role_nav:
                return [], [], []
            top_nav = self._getNavAllByType(role_nav, role_ids, 'nav', i18n)
            func_nav = self._getNavAllByType(role_nav, role_ids, 'funcNav', i18n)
            benchmarks = []
    ...
    def _getNavAllByType(self, role_nav, role_ids, type, i18n):
        ret = []
        try:
            nav_id_list = []
            ...
            for ret_item in nav_item_list:
                ...
                if ret_item.get('parent'):
                    ret_item['parent'] = str(ret_item.get('parent'))
                ret_i18n = ret_item.get('i18n')
                if i18n == 'default' or i18n is None or ret_i18n == i18n or ret_i18n == 'default' or ret_i18n is None:
                    ret.append(ret_item)
            # 还原顺序
            indexed_ids_list = []
            for nav_id in nav_id_list:
                    ...
                    for container in container_list:
                        ...
                        for item in container:
                            ...
                            if modal != None:
                                ...
                                desc = modal.get('desc')
                                variables = modal.get('variables') # factory 新增字段
                                if points != None:
                                    if len(points) > 0:
                                ...
                                modal_item = {
                                    'interval':interval,
                                    'type':type,
                                    'dsChartCog':dsChartCog,
                                    'points':points,
                                    'title':title,
                                    'desc':desc,
                                    'option':option,
                                    'modalText':modalText,
                                    'modalTextUrl':modalTextUrl,
                                    'link':link,
                                    'wikiId':wikiId,
                                    'popId':popId
                                }
                                if not variables is None:
                                    modal_item['variables'] = variables
                                layout_item.append({
                                    'id':container_id,
                                    'spanR':spanR,
                                    'spanC':spanC,
    ...
    def get_daily_ave_temp_hum(self, stationId, startTime, endTime, timezone=None):
        cursor = None
        rv = None
        utcStartTime = None
        utcEndTime = None
        try:
            if timezone:
                utcStartTime = startTime + timedelta(hours=-int(timezone.split(':')[0]))
                utcEndTime = endTime + timedelta(hours=-int(timezone.split(':')[0]))
            else:
                utcStartTime = datetime.utcfromtimestamp(time.mktime(startTime.timetuple()))
                utcEndTime = datetime.utcfromtimestamp(time.mktime(endTime.timetuple()))
            startTimeDayStartObj = utcStartTime.replace(hour=0, minute=0, second=0, microsecond=0)
            endTimeDayStartObj = utcEndTime.replace(hour=0, minute=0, second=0, microsecond=0)
            cursor = self.mdbBb[stationId].find({'utc_time': {'$gte': startTimeDayStartObj, '$lte': endTimeDayStartObj}})
            if startTime > utcStartTime:
                seconds = (startTime - utcStartTime).seconds
            else:
                seconds = -(utcStartTime - startTime).seconds
            # 获取所有温度
            tempDict = {}
            for item in cursor:
                startHour = 0
                endHour = 24
                if item.get('utc_time') == startTimeDayStartObj:
                    startHour = utcStartTime.hour
                if item.get('utc_time') == endTimeDayStartObj:
                    endHour = utcEndTime.hour
                item_value = item.get('value')
                for i in range(startHour, endHour):
                    localdate = item.get('utc_time').replace(hour=i) + timedelta(seconds=seconds)
                    localdateStr = datetime.strftime(localdate, '%Y-%m-%d')
                    weather = item_value.get(str(i))
                    if weather:
                        weather = weather.get('0')
                        tmp = None
                        hum = None
                        if weather and weather.get('heFeng') and weather.get('heFeng').get('tmp') and weather.get(
                                'heFeng').get('hum'):
                            tmp = weather.get('heFeng').get('tmp')
                            hum = weather.get('heFeng').get('hum')
                        if not tmp and weather and weather.get('open') and weather.get('open').get('main'):
                            main = weather.get('open').get('main')
                            if main.get('temp') and main.get('humidity'):
                                tmp = main.get('temp') - 273
                                hum = main.get('humidity')
                        if tmp and hum:
                            if tempDict.get(localdateStr):
                                tempDict[localdateStr]["temp"].append(int(tmp))
                                tempDict[localdateStr]["hum"].append(int(hum))
                            else:
                                tempDict[localdateStr] = {"temp":[int(tmp)],"hum": [int(hum)]}
            # 计算平均温度
            rv = {}
            for day, tempHum in tempDict.items():
                tempList = tempHum.get('temp')
                humList = tempHum.get('hum')
                if tempList and humList:
                    max_temp = max(tempList)
                    min_temp = min(tempList)
                    ave_temp = round(sum(tempList) / len(tempList), 2)
                    ave_hum = round(sum(humList) / len(humList), 2)
                    rv[day] = {"max_temp": max_temp, "min_temp": min_temp, "ave_temp": ave_temp, "ave_hum": ave_hum}
            return rv
        except Exception as e:
            print(e.__str__())
            logging.error(e.__str__())
    ...
    def updateMappingPointRemark(self, projectId, data):
        dataList = []
        try:
            for dataItem in data:
                dataList.append(data[dataItem])
            maxCountOnce = 500  # 一次最大的mongo操作数量
            dataLen = len(dataList)
            loopCount = dataLen // maxCountOnce
            for i in range(loopCount + 1):
                updateList = []
                for index in range(loopCount * maxCountOnce, (loopCount + 1) * maxCountOnce):
                    if index >= dataLen:
                        break
                    value = dataList[index].get('pointName')
                    remark = dataList[index].get('remark')
                    updateList.append(UpdateOne({"projId": int(projectId), "type": 4, "params.flag": 0, "params.mapping.point": value},
                                  {"$set": {"alias": remark}}))
                if updateList:
                    self.mdbBb[g_tableDataSource].bulk_write(updateList)
        except Exception as e:
            print('updateMappingPointRemark failed')
            print(e.__str__())
            logging.error(e.__str__())
            return False
        return True
        ...
        if isinstance(data, dict) and len(list(data.keys())) > 0:
            try:
                for item in data['itemList']:
                    projId = int(item.get('projId', -1))
                    value = str(item.get('value', ''))
                    type = int(item.get('type', -1))
                    alias = str(item.get('alias', ''))
                    note = str(item.get('note', ''))
                    groupId = str(item.get('groupId', ''))
                    geneId = ObjectId() if 'id' not in item.keys() else ObjectId(item['id'])
                    insertDict = {'_id': geneId, 'projId': projId, 'type': type,
 'alias': alias, 'value': value,
                                  'note': note, 'groupId': groupId}
                    itemIdList.append(
                        {'alias': alias, 'id': geneId.__str__(), 'value': value, 'note': note,
                         'groupId': groupId, 'projId': projId, 'type': type})

                    self.mdbBb[g_tableDataSource].update({'_id': geneId}, {'$set': insertDict},
                                                         True)
                    if ObjectId.is_valid(groupId):
                        self.mdbBb[g_tableDataSourceGroupItem].update({'_id': ObjectId(groupId)},
                                                                      {'$addToSet': {'dsList': geneId.__str__()}})
            except Exception as e:
    ...
    def getDataSourceGroupInfoRoot(self, userId):
        ...
        try:
            ...
            if retGroup:
                ...
                defaultId = retGroup.get('default', '')
                if defaultId == '' or defaultId not in groupList:
                    newId = ObjectId().__str__()
                    self.saveDataSourceGroup(userId, 'unassigned', newId, '')
                    self.mdbBb[g_tableDataSourceGroupList].update({'userId': userId}, {'$set': {'default': newId}},
                                                                  True)
                    defaultId = newId
    ...
    def getDataSourceItemInfoByIds(self, itemIds):
        rt = []
        itemObjectIds = [ObjectId(itemId[len('tempvar'):]) if itemId.startswith('tempvar') else ObjectId(itemId) for itemId in itemIds]
        cursor = None
        try:
            cursor = self.mdbBb[g_tableDataSource].find({'_id':{'$in':itemObjectIds}})
            rt = list(cursor)
        except Exception as e:
            print('getListContentByIdAndProjId failed')
            print(e.__str__())
    ...
    def saveHistoryData(self, projId, pointName, pointValue, pointTime, dbNameMongo):
        result = False
        data, v2Data = self.filterData(projId, [(pointTime.strftime("%Y-%m-%d %H:%M:%S") if isinstance(pointTime, datetime) else pointTime, pointName, pointValue)])
        if v2Data:
        ...
        try:
            ...
            if cursor.count() > 0:
                for item in cursor:
                    ...
                    shareLogList.append({'shareLogId': id, 'userId': userId, 'menuItemId': menuItemId, 'shareDate': date.strftime(gmt_format), 'shareDesc': description})
            else:
                ...
                for item in cursor:
                    ...
                    if menuItemId.find('SHARE') != -1:
                        id = ObjectId()
                        shareLogList.append({'shareLogId': id.__str__(), 'userId': userId, 'menuItemId': menuItemId, 'shareDate': datetime.now().strftime(gmt_format), 'shareDesc': ''})
...
#### 温控 app 接口 ####
    ...
    def factoryGetPageIdList(self, projId, isFactory):
        rt = []
        res = None
        try:
            if isFactory:
    ...
    def get_report_count(self, projIdList):
        #获取当前报表（factory报表老报表）的数量
        cursor = None
        cursor1 = None
        cursor2 = None
        try:
            cursor = self.mdbBb[g_tableCustomNav].find(
                {"projectId": {"$in": projIdList}})
            navList = []
            for NavDbRv in cursor:
                navList.extend(NavDbRv.get('list'))
            if not navList:
                return 0
            #老报表数量
            old_count = self.mdbBb[g_tableCustomNavItem].count({"type": 'ReportScreen', "_id": {
                "$in": [ObjectId(nav) for nav in navList]}})
            #新报表数量
            cursor1 = self.mdbBb[g_tableCustomNavItem].find({"type": 'FacReportWrapScreen', "_id": {
                "$in": [ObjectId(nav) for nav in navList]}})
            ids = [str(item.get('_id')) for item in cursor1]
            new_count = 0
            if ids:
                cursor2 = self.mdbBb[g_table_ob_report_wrap].find({"pageId": {"$in": ids}})
                new_count = sum((len(reportGroup.get('list')) for reportGroup in cursor2 if reportGroup.get('list')))
            return old_count + new_count
        except Exception as e:
            print('get_report_count failed:%s' % (e.__str__(),))
            logging.error('get_report_count failed:%s' % (e.__str__(),))
        finally:
            if cursor:
                cursor.close()
            if cursor1:
    ...
    def get_report_by_projIds(self, projInfos):
        # 获取当前报表（factory报表老报表）页
        cursor = None
        cursor1 = None
        cursor2 = None
        rv = []
        try:
            for projInfo in projInfos:
                projId = projInfo.get('id')
                cursor = self.mdbBb[g_tableCustomNav].find({"projectId": projId})
                navList = []
                for NavDbRv in cursor:
                    navList.extend(NavDbRv.get('list'))
                if not navList:
                    continue
                pageList = []
                cursor1 = self.mdbBb[g_tableCustomNavItem].find({"type": {'$in':['ReportScreen','FacReportWrapScreen']}, "_id": {
                    "$in": [ObjectId(nav) for nav in navList]}})
                ids = []
                for item in cursor1:
                    if item.get('type') == 'ReportScreen':
                        pageList.append({"name": item.get('text'),
                                   "id": str(item.get('_id')),
                                   "type": 'ReportScreen'})
                    else:
                ...
                if ids:
                    cursor2 = self.mdbBb[g_table_ob_report_wrap].find({"pageId": {"$in": ids}})
                    for reportGroup in cursor2:
                        facReportList = reportGroup.get('list')
                        if facReportList:
                            for report in facReportList:
                                pageList.append({"name": report.get('reportName'), "id": str(report.get('reportId')),
                                           "type": 'FacReportWrapScreen'})
                if pageList:
                    rv.append({"projectId": projId, "projectNameEn": projInfo.get('name_en'), "pageList": pageList,
                               "projectNameCn": projInfo.get('name_cn')})
        except Exception as e:
            print('get_report_count failed:%s' % (e.__str__(),))
            logging.error('get_report_count failed:%s' % (e.__str__(),))
            return False
    ...
    def RemoveProjectPublish(self, projectId):
        ...
        try:
            ...
            if ret:
                ...
                self.FacRemovePages(projectId, list(cursor), 0)
        ...
        try:
            if ObjectId.is_valid(projectId):
                ...
                if ret:
                    ...
                    self.FacRemovePages(projectId, list(cursor))
    ...
    def factoryMaterialUpdateMany(self, query, update):
        rt = True
        try:
            self.mdbBb[g_table_fac_material].update_many(query, update)
        except Exception as e:
            rt = False
            print('factoryMaterialUpdateMany failed:%s'%(e.__str__(),))
            logging.error('factoryMaterialUpdateMany failed:%s'%(e.__str__(),))
        return rt
    ...
    def factoryMaterialGetByInfo(self, info, userList=[], projection=None):
        rt = []
        creatorlist = []
        cursor = None
        try:
            ...
            for item in cursor:
                ...
                if len(userList) == 0 or creator in userList:
                    if str(creator).isdigit():
                        creatorlist.append(creator)
                    rt.append(item)
            user = User()
            user_list = user.get_user_list_info(creatorlist)
            usermap = {}
            for item in user_list:
                usermap[item.get('id')] = item.get('userfullname')
            for item in rt:
                if item.get('creator', '') in usermap:
                    item['creator'] = usermap.get(item['creator'], 'Get Failed')
        except Exception as e:
    ...
    def factoryMaterialGetByImageId(self, id, userList=[]):
        rt = {}
        cond = {}
        listArr = []
        try:
            cursor = self.mdbBb[g_table_fac_material].find_one({'_id':id})
            curGroupId = cursor.get("group")
            cond['type'] = {'$regex': '^image.*$'}
            cond['group'] = curGroupId
            data = self.factoryMaterialGetByInfo(cond, userList)
            if curGroupId:
                listArr.append(curGroupId)
                groupList = self.factoryMaterialGetGroupList(listArr)
            else:
                groupList = listArr    
            rt["data"] = data
            rt["list"] = groupList
        except Exception as e:
            print('factoryMaterialGetByImageId failed:%s'%(e.__str__(),))
            logging.error('factoryMaterialGetByImageId failed:%s'%(e.__str__(),))
        return rt    

    def factoryMaterialGetGroupList(self, groupList):
        try:
            _id = groupList[len(groupList)-1]
            cursor = self.mdbBb[g_table_fac_material].find_one({'_id':ObjectId(_id)})
            curGroupId = cursor.get("group")
            if curGroupId:
                groupList.append(curGroupId)
                return self.factoryMaterialGetGroupList(groupList)
            else:
                return groupList
        except Exception as e:
            print('factoryMaterialGetGroupList failed:%s'%(e.__str__(),))
            logging.error('factoryMaterialGetGroupList failed:%s'%(e.__str__(),))     

    def factoryMaterialGetByCopyInfo(self, info, type, group, creator, projection=None):
        rt = []
        cursorData = []
        try:
            query = info if info else dict()
            ...
            for item in list(cursor):
                ...
                self.mdbBb[g_table_fac_material].save(item) 
                cursorData.append(item)
            user = User()
            user_list = user.get_user_list_info([creator])
            usermap = {}
            for item in user_list:
                usermap[item.get('id')] = item.get('userfullname')    
            for row in cursorData:
                row.update({'_id':row.get('_id').__str__()})
                if type != "image":
                    row.pop('content') 
                if row.get('creator', '') in usermap: 
                    row['creator'] = usermap.get(row['creator'], 'Get Failed')
                rt.append(row)  
        except Exception as e:    
            print('factoryMaterialGetByCopyInfo failed:%s'%(e.__str__(),))
            logging.error('factoryMaterialGetByCopyInfo failed:%s'%(e.__str__(),))
        finally:
            if cursor:
                cursor.close()    
        return rt    
    ...
    def FacIsProjectExistByName(self, proName):
        try:
            rt = self.mdbBb[g_table_fac_project].find_one({'$or':[{'name_en':proName}]})
            if rt:
                # 修改 项目名称重复，直接返回提示
                return rt.get('_id').__str__()
        except Exception as e:
    ...
    def RemoveProject(self, userId, projId):
        ...
        try:
            ...
            print('RemoveProject failed:%s'%(e.__str__(),))
            logging.error('RemoveProject failed:%s'%(e.__str__(),))
            rt=False
        return rt
    ...
    def getDsItemsById2(self, dsList):
        ''' 优化接口 by Peter '''
        data = []
        try:
            cursor = None
            ...
            for ds in dsList:
                if ObjectId.is_valid(ds):
                    id_list.append(ds)
                elif isinstance(ds, str) and ds[0] == '@':
                    ds = ds[1:].rsplit('|', 1)
                    proj_id = ds[0]
                    if not(proj_id in value_map):
                        value_map[proj_id] = []
                    value_map[proj_id].append(ds[1])
            
            result_map = {}
            cursor = self.mdbBb[g_tableDataSource].find({'_id': {'$in': [ObjectId(x) for x in id_list]}})
            for item in cursor:
                item['projId'] = int(item.get('projId'))
                result_map[str(item.get('_id'))] = item
            cursor.close()
            
            for k, v in value_map.items():
                cursor = self.mdbBb[g_tableDataSource].find({'value': {'$in': v}, 'type':4, 'projId':int(k)})
                for item in cursor:
                    item['projId'] = int(item.get('projId'))
                    result_map['@%s|%s'%(item['projId'], item['value'])] = item
                cursor.close()
            ...
            print('getDsItemsById2 failed:%s'%(e.__str__(),))
            logging.error(e.__str__())
            logging.error('getDsItemsById2 error:' + ds.__str__())
        return data
    ...
    def get_DataSource_MappingPoint_by_value(self, DataSource_Value):
        ...
        try:
            ...
            logging.error("Failed to get data source mapping point by %s", DataSource_Value, exc_info=True, stack_info=True)
        return rt

    def get_DataSource_MappingPoint_by_values(self, DataSource_Values):
        rt = []
        try:
            projMapping = {}
            for item in DataSource_Values:
                if item[0] == '@':
                    projId = item[1:].split('|')[0]
                    value = item[1:].split('|')[1]
                    if projId not in projMapping:
                        projMapping[projId] = []
                    projMapping[projId].append(value)
            for key, value in projMapping.items():
                cursor = None
                try:
                    cursor = self.mdbBb[g_tableDataSource].find({'value': {'$in':value}, 'type': 4,'$or': [{'projId': int(key)},{'projId': str(key)}]})
                    cursor_list = list(cursor)
                    for name in value:
                        for item in cursor_list:
                            if item.get('value') == name:
                                ...
                                if params:
                                    ...
                                    item.update({'type':0})
                                item.update({'_id':"@"+str(key)+"|"+name})
                                rt.append(item)
                                break
                        else:
                            rt.append({'type':0, 'value':name, 'projId':key, '_id':"@" + str(key) + "|" + name})
                except:
                    pass
                finally:
                    if cursor:
    ...
    def getManageReportConfig(self, managementId):
        return self.mdbBb[g_managementReportConfig].find_one({'managementId': managementId})

    def saveManageReportConfig(self, save_data):
        self.mdbBb[g_managementReportConfig].update({'managementId': save_data.get('managementId')},
                                                    {'$set': save_data}, True)
        self.mdbBb[g_managementReportConfig].create_index([('managementId', pymongo.DESCENDING)])
        return self.mdbBb[g_managementReportConfig].find_one({'managementId': save_data.get('managementId')},
                                                             ["_id"]).get('_id')
    ...
    def factoryReplaceId(self, data, replace_id_map, deep=False):
        if data is None or len(replace_id_map.keys()) == 0:
            return
        if isinstance(data, list):
            for key, value in enumerate(data):
                if ObjectId.is_valid(value) and str(value) in replace_id_map:
                    data[key] = replace_id_map[str(value)] if isinstance(value, str) else ObjectId(replace_id_map[str(value)])
                elif deep and isinstance(value, str):
                    for old_id, new_id in replace_id_map.items():
                        data[key] = data[key].replace(old_id, new_id)
                else:
                    self.factoryReplaceId(value, replace_id_map, deep)
        elif isinstance(data, dict):
            for key, value in data.items():
                if ObjectId.is_valid(key) and str(key) in replace_id_map:
                    del data[key]
                    data[replace_id_map[key] if isinstance(key, str) else ObjectId(replace_id_map[str(key)])] = value
                if ObjectId.is_valid(value) and str(value) in replace_id_map:
                    data[key] = replace_id_map[value] if isinstance(value, str) else ObjectId(replace_id_map[str(value)])
                elif deep and isinstance(value, str):
                    for old_id, new_id in replace_id_map.items():
                        data[key] = data[key].replace(old_id, new_id)
                else:
    ...
    def factoryCopyPages(self, page_list, new_proj_id):
        flag = True
        replace_id_map = {}
        new_page_list = []
        try:
            pages = self.mdbBb[g_table_fac_page].find({'_id': {'$in': [ObjectId(x) for x in page_list]}})
            pages = list(pages)
            for page in pages:
                new_id = str(ObjectId())
                replace_id_map[str(page.get('_id'))] = new_id
                new_page_list.append(new_id)
                page['isCopy'] = 1
                page['projId'] = new_proj_id
                page['i18n'] = 'en'
        except Exception as err:
            print('factoryCopyPages find pages error:' + err.__str__())
            logging.error('factoryCopyPages find pages error:' + err.__str__())
            flag = False

        for i, page in enumerate(pages):
            print('%s - 正在处理页面： %s，页面类型：%s' % (i, page.get('text', ''), page.get('type')))
            logging.info('%s - 正在处理页面： %s，页面类型：%s' % (i, page.get('text', ''), page.get('type')))
            try:
                replace_temp_id_map = {}
                page_type = page.get('type')
                page_id = str(page.get('_id'))
                if page_type == 'PageScreen':
                    layers = self.mdbBb[g_table_fac_layer].find({'pageId': page_id})
                    layers = list(layers)
                    widgets = self.mdbBb[g_table_fac_widget].find({'pageId': page_id})
                    widgets = list(widgets)
                    for widget in widgets:
                        replace_temp_id_map[str(widget.get('_id'))] = str(ObjectId())
                    for layer in layers:
                        replace_temp_id_map[str(layer.get('_id'))] = str(ObjectId())
                    replace_temp_id_map.update(replace_id_map)
                    for item in layers:
                        self.factoryReplaceId(item, replace_temp_id_map)
                    for item in widgets:
                        self.factoryReplaceId(item, replace_temp_id_map, True)
                    if len(layers) > 0:
                        self.mdbBb[g_table_fac_layer].insert_many(layers)
                    if len(widgets) > 0:
                        self.mdbBb[g_table_fac_widget].insert_many(widgets)
                elif page_type in ['EnergyScreen_M', 'EnergyScreen', 'FacReportScreen']:
                    layout = self.mdbBb[g_table_fac_spring_layout].find_one({'menuItemId': page_id})
                    replace_temp_id_map[str(layout.get('_id'))] = str(ObjectId())
                    replace_temp_id_map.update(replace_id_map)
                    self.factoryReplaceId(layout, replace_temp_id_map, True)
                    self.mdbBb[g_table_fac_spring_layout].insert_one(layout)
                elif page_type == 'FacReportWrapScreen':
                    report_wrap = self.mdbBb[g_table_fac_report_wrap].find_one({'pageId': page_id})
                    replace_temp_id_map[str(report_wrap.get('_id'))] = str(ObjectId())
                    report_old_ids = []
                    for report in report_wrap['list']:
                        report_old_id = report['reportId']
                        replace_temp_id_map[report_old_id] = str(ObjectId())
                        report_old_ids.append(report_old_id)
                    layouts = self.mdbBb[g_table_fac_spring_layout].find({'menuItemId': {'$in': report_old_ids}})
                    layouts = list(layouts)
                    replace_temp_id_map.update(replace_id_map)
                    for layout in layouts:
                        self.factoryReplaceId(layout, replace_temp_id_map, True)
                    self.mdbBb[g_table_fac_spring_layout].insert_many(layouts)
                    self.factoryReplaceId(report_wrap, replace_temp_id_map)
                    self.mdbBb[g_table_fac_report_wrap].insert_one(report_wrap)
                self.factoryReplaceId(page, replace_id_map if not replace_temp_id_map else replace_temp_id_map)
                self.mdbBb[g_table_fac_page].insert_one(page)
            except Exception as err:
                print('factoryCopyPages copy page error:' + err.__str__())
                print('pageId:' + str(page.get('_id')))
                logging.error('factoryCopyPages copy page error:' + err.__str__())
                logging.error('pageId:' + str(page.get('_id')))
                flag = False

        try:
            self.mdbBb[g_table_fac_navigation].update_one({'_id': ObjectId(new_proj_id)}, {'$addToSet': {'list': {'$each': new_page_list}}}, True)
        except Exception as err:
            print('factoryCopyPages insert into navigation error:' + err.__str__())
            logging.error('factoryCopyPages insert into navigation error:' + err.__str__())
            flag = False
        print('处理完毕')
        logging.info('处理完毕')
        return flag
    ...
    def FacRemoveReportData(self, name, date):
        rs = False
        try:
            rs = self.mdbBb[g_table_fac_report_data].delete_one({'name': name, 'date': date})
            rs = True
        except Exception as e:
            print('FacRemoveReportData error:'+e.__str__())
            logging.error('FacRemoveReportData error:'+e.__str__())
        return rs
    ...
    def edit_maintainRecords(self, data):
        return self.mdbBb['Asset_MaintainRecords'].save(data)
    ...
    def alarm_enable(self, _id, status):
        beUsed = 1 if status else 0
        rt = False
        try:
            result = self.mdbBb['Alarm_moudle'].update({'_id': _id}, {"$set": {'beused': beUsed}})
            if result['ok']:
                rt = True
        except Exception as e:
    ...
    def getHistoryDataByFormat_v2(self, dbname, pointList, timeStart, timeEnd, timeFormat, bSearchNearest = False):
        ...
        fixSpan = timedelta(days=15) if timeFormat == 'M1' else timedelta(days=2)
    ...
    def getHisDataFromDB(self, collectionName, timeFormat, dayStartQueryObject, dayEndQueryObject, startObject, endObject, point_list, data, bSearchNearest=False):
        ...
        try:
            ...
            if timeFormat != 'M1':
                query = {'time': {'$gte': dayStartQueryObject, '$lte': dayEndQueryObject}, 'pointname': {'$in': point_list}}
            else:
                query = {'time': {'$in': tl}, 'pointname': {'$in': point_list}}
            ...
            if bSearchNearest:
                if timeFormat == 'M1':
                    cursor = self.mdbBb[collectionName].find({'time': {'$gte': dayStartQueryObject, '$lte': startObject}, 'pointname': {'$in': point_list}},
                        ...
                        for h, h_item in item_value.items():
                            for m, findValue in h_item.items():
                                ...
                                if t >= dayStartQueryObject and t <= startObject:
                                    fixTime.append({'n': findName, 't': t, 'v': findValue})
                    fixSpan = timedelta(days=15) if timeFormat == 'M1' else timedelta(days=2)
                    cursor = self.mdbBb[collectionName].find({'time': {'$gte': endObject, '$lte': endObject+fixSpan}, 'pointname': {'$in': point_list}},
                                                             {'time': 1, 'pointname': 1, 'value': 1, '_id': 0}).sort(
                        [('pointname', pymongo.ASCENDING), ('time', pymongo.ASCENDING)])
                    for item in cursor.batch_size(100):
                        item_time = item.get('time')
                        ...
                        for h, h_item in item_value.items():
                            for m, findValue in h_item.items():
                                ...
                                t = item_time.replace(hour=h, minute=m)
                                if t >= dayStartQueryObject and t <= startObject:
                                    fixTime_2.append({'n': findName, 't': t, 'v': findValue})
                if fixTime:
                    fixTime.sort(key=lambda x: x.get('t'), reverse=True)
                self.searchNearest(tl, point_list, tndict, fixTime, fixTime_2, data)

            # Sort result to match the point list order
            data.sort(key=lambda item: point_list.index(item.get('pointname')))
        except Exception:
            logging.error('Unhandled exception! collectionName=%s, timeFormat=%s, dayStartQueryObject=%s, '
                          'dayEndQueryObject=%s, startObject=%s, endObject=%s, point_list=%s, bSearchNearest=%s',
                          collectionName, timeFormat, dayStartQueryObject, dayEndQueryObject, startObject,
                          endObject, point_list, bSearchNearest, exc_info=True, stack_info=True)
        finally:
            if cursor:
                cursor.close()

    def searchNearest(self, tl, point_list, tndict, fixTime, fixTime_2, data):
        for pt in point_list:
            for nIndex, f_item in enumerate(data):
                ...
                if f_name == pt:
                    ...
                    if len(f_record) != len(tl):
                        for f_t_m in tl:
                            ...
                            v = r if r is not None else self.get_nearest_data_from_prefetch(f_t_m, pt, fixTime, fixTime_2)
                            if v is not None:
                ...
                tDataRecord = []
                for f_t_m in tl:
                    v = self.get_nearest_data_from_prefetch(f_t_m, pt, fixTime, fixTime_2)
                    if v is not None:
                        tDataRecord.append({'value': v, 'time': f_t_m.strftime('%Y-%m-%d %H:%M:%S')})
                data.append(dict(pointname=pt, record=tDataRecord))

    def get_nearest_data_from_prefetch(slef, f_t_m, pt, fixTime, fixTime_2):
        rt = None
        for item in fixTime:
            if item.get('n') == pt:
                if item.get('t') < f_t_m:
    ...
    def filterData(self, projId, dataList):
        ...
        if v2_time:
            ...
            return dataList, []
        for d in dataList:
            if datetime.strptime(d[0], "%Y-%m-%d %H:%M:%S") >= v2_time:
                v2Data.append(d)
            else:
                oldData.append(d)
    ...
    def getUserConfig(self, userId):
        rt = self.mdbBb['User_config'].find_one({'userId':int(userId)})
        if rt:
            rt = rt.get('option')
        else:
            rt = {}
    ...
    def setUserConfig(self, userId, option = {}):
        rt = self.mdbBb['User_config'].find_one({'userId':int(userId)})
        id = None
        oldOption = {}
        newOption = {}
        ...
        if rt:
            ...
            id = ObjectId()
            updateInfo = {'_id':id,'userId':userId,'option':{}}
            oldOption = updateInfo.get('option')
        try:
            state = True
            newOption = dict(oldOption,**option)
            if updateInfo.get('option') is None:
                updateInfo['option'] = {}
            updateInfo.get('option').update(newOption)
            if id:
                self.mdbBb['User_config'].insert(updateInfo)
            else:
                self.mdbBb['User_config'].save(updateInfo)
            # self.mdbBb['User_config'].update({'_id':id},updateInfo)
        except Exception as e:
            state = False
            print('update User_config failed')
            print(e.__str__())
            logging.error(e.__str__())
            logging.exception(e)
        return {'state':state}
    ...
    def setFeedback(self, feedback):
        ...
        id = None
        if feedback.get('_id'):
            rt = self.mdbBb['Diagnosis_FeedBack'].find_one({'_id':feedback.get('_id')})
        if rt:
            updateInfo = dict(rt, **feedback)
        else:
            id = ObjectId()
            updateInfo = dict({'_id': id}, **feedback)
        try:
            state = True
            if id: 
                self.mdbBb['Diagnosis_FeedBack'].insert(updateInfo)
            else:
                self.mdbBb['Diagnosis_FeedBack'].save(updateInfo)
        except Exception as e:
            state = False
    ...
    def getFeedback(self, condition):
        startTime = condition.get('startTime')
        endTime = condition.get('endTime')
        if startTime and endTime:
            condition.pop('startTime')
            condition.pop('endTime')
            condition['time'] = {"$gte":startTime,"$lte":endTime}
        data = []
        try:   
            rt = self.mdbBb['Diagnosis_FeedBack'].find(condition)
            for item in rt:
                item.update({'_id':str(item.get('_id'))})
                data.append(item)
        except Exception as e:
            state = False
            print('find feedback failed')
            print(e.__str__())
            logging.error(e.__str__())
            logging.exception(e)
        return data

    def InsertCloudPointOperationLog(self, projId, userId, points, type, is_all):
        try:
            self.mdbBb['PointOperationLog'].insert_one({'projId':int(projId),'userId':int(userId), 'points':points, 'type:':type, 'is_all':is_all, 'time':datetime.now()})
            return True
        except:
            return False
        return False
    ...
    def insertTerminalOpLog(self,dbname,dtuId,opType,userId,utcTime,log):
        try:
            self.mdbBb['TerminalOpLog'].insert_one(
                {'dbname':dbname,'dtuId': dtuId, 'userId': int(userId), 'opType': opType, 'utcTime': utcTime, 'log': log})
            return True
        except:
            return False
        return False

    def searchTerminalOpLogByDtuId(self,pageIndex,pageSize,sortOrder,dtuId,startTime,endTime):
        searchResult={"totalCount":0,"searchList":[]}
        sort=sort=[("utcTime",-1)]         #默认降序  时间从近到远
        queryLimit={}
        try:
            if dtuId==None and startTime==None and endTime==None:
                queryLimit={}
            elif dtuId==None and startTime!=None and endTime!=None:
                if startTime<=endTime:
                    queryLimit={"utcTime":{'$lte' :endTime,'$gte':startTime}}
                else:
                    msg="ERROR:StartTime should lower than EndTime"
                    raise Exception(msg)
            elif dtuId!=None and (startTime==None or endTime==None):
                queryLimit={"dtuId":int(dtuId)}
            else:
                queryLimit={"dtuId":int(dtuId),"utcTime":{'$lte' :endTime,'$gte':startTime}}
            if sortOrder=="asc":    #升序  时间从远到近
                sort=[("utcTime",1)]
            skipCount=int(pageSize) * (int(pageIndex) - 1)
            limitCount=int(pageSize)
            totalCount=self.mdbBb["TerminalOpLog"].count()
            cursor=self.mdbBb["TerminalOpLog"].find(queryLimit,sort=sort).skip(skipCount).limit(limitCount)
            searchList=list(cursor)
