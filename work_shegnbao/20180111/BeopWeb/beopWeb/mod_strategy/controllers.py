...
@bp_strategy.route('/template/groupsearch/<key>', methods=['GET'])
def get_template_by_key(key=''):
    ''' 根据组 关键字 获取模板列表 '''
    result = StrategyModel.get_templates_by_key(key)
...
# 根据模板ID，删除模板
@bp_strategy.route('/template/removeByids', methods=['POST'])
def removeStrategyTemplateByIds():
    post_data = request.get_json()
    ids = post_data.get('ids',[])
    # 1、更新到数据库
    result = StrategyModel.remove_template_by_ids(ids)
    # 2、写入LOG

    if None in result:
        return format_result(False, result[1])
    return format_result(True, 'Success', result)

# 根据模板ID，重命名模板
@bp_strategy.route('/template/renameByid', methods=['POST'])
def renameStrategyTemplateById():
    post_data = request.get_json()
    _id = post_data.get('id',[])
    name = post_data.get('name')
    # 1、更新到数据库
    result = StrategyModel.rename_template_by_id(_id, name)
    # 2、写入LOG
...
# 改变模板group
@bp_strategy.route('/template/changeGroup', methods=['POST'])
def changeStrategyTemplateGroup():
    post_data = request.get_json()
    group = post_data.get('group')
    ids = post_data.get('ids', [])
    # 1、更新到数据库
    result = StrategyModel.change_template_group(group, ids)
    # 2、写入LOG
...
# 添加模板group
@bp_strategy.route('/template/addGroup', methods=['POST'])
def addStrategyTemplateGroup():
    post_data = request.get_json()
    name = post_data.get('name')
    group = post_data.get('group')
    userId = post_data.get('userId')
    # 1、更新到数据库
    result = StrategyModel.add_template_group(name, group, userId)
    # 2、写入LOG
...
'''
@bp_strategy.route('/exportStrategyTemplate', methods=['POST'])
def export_strategy_template():
    ''' 导出模板 '''
    data = request.get_json()
    result = StrategyModel.export_strategy_template(data)
    return format_result(result, '')
