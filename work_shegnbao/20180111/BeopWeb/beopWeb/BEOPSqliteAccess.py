...
class BEOPSqliteAccess:
        ...
        if s3dbname != None:
            ...
            try:
                ...
                if not os.path.exists(dbfileDec):
                    logging.debug(dbfileDec + ' file not existing!')
                    return None
                ...
                logging.error("Failed to get point list from s3db %s", s3dbname, exc_info=True, stack_info=True)
            finally:
                if cur is not None:
                    cur.close()
                if con is not None:
        ...
        if s3dbname != None:
            ...
            try:
                ...
                if num == -1:
                    ...
                    sql = 'select name, ch_description, Param15, Param16, Param17, Param18 from list_point limit %d, %d;' % (
                    offset, num)
                cur.execute(sql)
                data = cur.fetchall()
                flag = '\'" "\''
                for item in data:
                    ...
                    if flag == type or '' == type:
                        type = '0'
                    rt.append({'name': item[0].decode('gbk'), 'description': item[1].decode('gbk'), 'custom': custom,
                               'system': int(system), 'device': int(device), 'type': int(type)})
                    ptList.append(item[0].decode('gbk'))
            except Exception as e:
                print(e.__str__())
                logging.error(e.__str__())
                logging.error("s3dbname:%s" % s3dbname.__str__())
            finally:
                if cur is not None:
                    cur.close()
                if con is not None:
    ...
    def getPlantPageDetails(self, s3dbname, isOnlyStickPage=True):
        ...
        try:
            ...
            if isOnlyStickPage:
                cu.execute(
                    "SELECT id, name, pagetype, width, hight, unitproperty02, groupid FROM list_page WHERE pagetype = 'fullscreen' AND unitproperty01 = '1' ORDER BY showOrder")
            else:
                cu.execute(
                    "SELECT id, name, pagetype, width, hight, unitproperty02, groupid FROM list_page ORDER BY showOrder")
            for x in cu.fetchall():
                url = ''
                if x[5] is not None: url = x[5].decode('gbk')
                data.append(
                    {'id': x[0], 'name': x[1].decode('gbk'), 'type': x[2].decode('gbk'), 'width': x[3], 'height': x[4],
                     'url': url, 'groupid': x[6]})
        except Exception:
            logging.error("Failed to get plant page details with s3dbname = %s, isOnlyStickPage = %s", s3dbname,
                          isOnlyStickPage, exc_info=True, stack_info=True)
        finally:
            if cu is not None:
                cu.close()
            if con is not None:
    ...
    def getPlantGroupDetails(self, s3dbname):
        cu = None
        con = None
        data = []
        try:
            ...
            for x in cu.fetchall():
                u1 = ''
                if x[2] is not None: u1 = x[2].decode('gbk')
                data.append({'id': x[0], 'name': x[1].decode('gbk'), 'u1': u1})
        finally:
            if cu is not None:
                cu.close()
            if con is not None:
    ...
    def getPlant(self, s3dbname, pageid):
        ...
        if con != None and cu != None:
            try:
                ...
                # Pipeline
                cu.execute(
                    "SELECT id, PointList, pipeWidth, style, idlist, layer, unitproperty01, innerColor FROM list_line WHERE pageid = %s" % (
                    pageId))
                for item in cu.fetchall():
                    ...
                    dataPipeLines.append({'id': item[0] if item[0] != None else 0,
                                          ...
                                          'color': {'r': color // 256 // 256 % 256, "g": color // 256 % 256,
                                                    'b': color % 256}})
                ...
                FROM page_contain_elements LEFT JOIN event ON event.id = page_contain_elements.elementid WHERE page_contain_elements.pageid = %s" % (
                pageId))
                ...
                for item in cu.fetchall():
                    ...
                    if (str_item != ''):
                        ...
                        for strStatus in temp:
                            ...
                            if len(tempStatus) == 4:
                                animationStatus.update({tempStatus[0]: {'animationId': tempStatus[1],
                ...
                cu.execute(
                    "SELECT id, animationlist FROM lib_animation WHERE id IN (%s)" % (','.join(listAnimationIds)))
                ...
                for item in cu.fetchall():
                    ...
                    dataButtons.append({'id': item[0] if item[0] != None else 0,
                                        ...
                                        'fontColor': {'b': textColor // 256 // 256 % 256, "g": textColor // 256 % 256,
                                                      'r': textColor % 256}})
                ...
                # Gage
                cu.execute(
                    "SELECT ld.id, ld.x, ld.y, ld.width, ld.height, ld.pointname, ld.max, ld.min, lp.pagetype, lp.xposition, lp.yposition FROM list_dashboard ld left join list_page lp on ld.pageid = lp.id WHERE ld.pageid = %s" % (
                    pageId))
                for item in cu.fetchall():
                ...
                cu.execute(
                    "select x, y, width, height, layer, unitproperty04 from list_unit19 where pageid = '%s'" % pageId)
    ...
    def checkResoueceFroms3db(self, s3dbName):
        ...
        try:
            ...
            rv = cu.fetchall()
            for item in rv:
                animationIdList.append(
                    {'id': item[0], 'animationlist': [int(x) for x in item[1].decode('gbk').split(',')]})
            # local event
            strSQL = "select pageid,id,bind_content from event where LENGTH(bind_content) > 0"
            cu.execute(strSQL)
            rv = cu.fetchall()
            for item in rv:
                eventList.append({'pageid': item[0], 'elementid': item[1],
                                  'bind_content': [[int(z) for z in y.split(',')] for y in
                                                   [x for x in item[2].decode('gbk').split('|')]]})
            # check list_button
    ...
    def getPageIdByName(self, s3dbname, pagename):
        if s3dbname == None or pagename == None:
            return None
        cu = None
        con = None
        data = []
        try:
            ...
            con.text_factory = bytes
            cu = con.cursor()
            cu.execute('SELECT id,name FROM list_page')
            for x in cu.fetchall():
                if x[1].decode('gbk') == pagename:
                    data.append({'id': x[0]})
        except Exception as e:
            print(e)
        finally:
