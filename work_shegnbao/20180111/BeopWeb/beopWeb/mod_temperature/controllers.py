...
@bp_temp.route('/room/getDetailByList',methods=['POST'])
def getAllRoomDetail():
    data = request.get_json()
    rt = []
    try:
        for item in data:
            rs = MongoConnManager.getConfigConn().app_temp_get_room_detail(item)
            rt.append(rs)
    except Exception as e:
        print('getAllRoomDetail error:' + e.__str__())
        logging.error('getAllRoomDetail error:' + e.__str__())
    return jsonify(data = rt)
...
@bp_temp.route('/checkTimeOut', methods=['get'])
def check_timeout():
    return json.dumps(True)
