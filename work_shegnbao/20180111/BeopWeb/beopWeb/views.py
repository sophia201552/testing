...
@app.route('/platform/privateGarden')
@app.route('/platform/privateGarden/<target>')
def privateGardenNew(target=None):
    rt = AuthManager.check_token(AuthManager.secret_key)
    ...
    if int(sourceId) == 1 and rt.get('status') == AuthManager.token_valid:
        if target is None:
            ...
            if(BEOPDataAccess.getInstance().validate_user_project_for_privateGarden(sourceId, target)):
                ...
                resp = make_response(render_template('indexPlatform.html', title='BeOP', AppConfig=rv))
                return resp
    return 'False'
    ...
    if rv and rv.get('id') is not None:
        ...
        if request.access_route[0] != '127.0.0.1' and int(rv.get('id')) not in [404, 2265, 77]:
            ...
            if not rv.get('noRecord'):
                if rq_data.get('agent') is None:
                    strInfo = json.dumps(rq_data)
                    logging.error('Login lack of agent info and request data is %s', strInfo, stack_info=True)
                Records.record_login(rv.get('id'), request.access_route[0], rq_data.get('agent'))
            else:
                strInfo = json.dumps(rv)
                logging.error('Login have noRecord, info is : %s', strInfo)
...
def project_create():
    ...
    try:
        ...
        logging.error('update project redis of all cluster failed:' + e.__str__())
    return json.dumps(rv, ensure_ascii=False)
        ...
        if latLon is None:
            ...
            if req.status_code == 200:
                ...
                if status == 'ok':
                    basic = cityDict.get('basic')
                    if basic and 'id' in basic:
                        # 更新project表的stationId
                        cityId = basic.get('id')
                        dbname = app.config.get('DATABASE')
                        sql = 'update project set weather_station_id="%s" where id=%s' % (cityId, projId)
                        projectUpdate = dbaccess._mysqlDBContainer.op_db_update(dbname, sql)
                        mongodbname = 'Weather_CityMetaInfo'
                        cursor = MongoConnManager.getConfigConn().mdbBb[mongodbname].find({}, {"_id": 1})
                        isExist = False
                        for item in cursor:
                            if item.get('_id') == cityId:
                                isExist = True
                                break
                        if not isExist:
                            dbaccess.updateWeatherStation(cityId)
                        if not projectUpdate:
                            logging.error('project update failed' + str(projId))
                            raise Exception('project update failed' + str(projId))
                else:
                    print(status)
...
def sendEmail():
    ...
    rt = Utils.EmailTool.send_email(subject, recipients, body, trackingId=trackingId)
    if rt == True:
...
def insert_into_contactus():
    ...
    if data:
        ...
        Utils.EmailTool.send_email('新的网站留言', recipients, email_html, trackingId='ContactUs')
    return jsonify(data=rt)
...
def clear_sitedata():
    try:
        ...
        if startTimeObj > now_time or endTimeObj > now_time:
            raise Exception("startTime(%s) or endTime(%s) greater than nowTime(%s)" % (startTime, endTime, now_time))
        if not db_name or not startTime or not endTime:
            raise Exception("Failed to get correct dbname(%s),startTime(%s) or endTime(%s)" % (db_name, startTime, endTime))
        ...
        if not delete_site and not delete_buffer:
            raise Exception("delete_site(%s) and delete_buffer(%s) is false" % (delete_site, delete_buffer))
        saveOpLogToMongo(data)
        return Utils.beop_response_success(True)
    except Exception:
        logging.error("Unhandled exception! Locals: %s", locals(), exc_info=True, stack_info=True)
        return Utils.beop_response_error(False)
...
def dataManagement():
    ...
    if token:
        return render_template('index/data_manage.html', token=token, projId=project_id, title='BeOP',AppConfig=AppConfig)
    else:
        return render_template('index/data_manage.html', projId=project_id, title='BeOP')
...
@app.route('/getActiveProjectIds', methods=['GET'])
def getActiveProjectIds():
    rv = Project().get_active_project_ids()
    return json.dumps(rv, ensure_ascii=False)


# 数据报警邮件模版
@app.route('/getAlarmEmailHTML', methods=['POST'])
def getAlarmEmailHTML():
    rv = {"HTML": None, "title": None}
    try:
        alarmmsg = request.get_json()
        projId = alarmmsg.get('projId')
        project_detail = Project().query_project_language_countryCode(projId)
        lang = project_detail.get('defaultLanguage')
        i18n.set_lang(lang)
        emailTitle = i18n.trans('alarm') + ':' + alarmmsg.get('pointName')
        resultHTML = render_template('email/dataAlarmEmail.html', alarm=alarmmsg)
        rv = {"HTML": resultHTML, "title": emailTitle}
    except Exception:
        logging.error('Unhandled exception! Locals: %s', locals(), exc_info=True, stack_info=True)
    return json.dumps(rv)


# 根据user表的country字段获取对应的默认语言存入userConfig中
@app.route('/updateUserConfigLanguage', methods=['get'])
def updateUserConfigLanguage():
    conn = MongoConnManager.getConfigConn()
    userCountry = User().query_all_users('country', 'id')
    for item in userCountry:
        try:
            countryCode = item.get('country')
            language = app.config.get('COUNTRY_DEFAULT_LANGUAGE').get(countryCode)
            if not conn.setUserConfig(item.get('id'), {"language": language}):
                logging.error('serUserConfig failed')
        except Exception as e:
            logging.error('ser UserConfig error:' + e.__str__())
    return json.dumps({})


# 测试模版
@app.route('/testInvite', methods=['get'])
def testInvite():
    Utils.EmailTool.send_invitation_email_with_pwd(
        'linlin1126', 'may', 'maysererererer',
        '1006082777@qq.com', 'may.chen@rnbtech.com.hk', 0,
        'en', 'WW')
