;(function (exports, SuperClass, ChartThemeConfig) {
    ...
    +function () {
        ...
        this.__renderChart = function (extendOptions) {
            ...
            this.chart = echarts.init(this.container,AppConfig.isMobile?AppConfig.chartTheme:{});
        ...
        this.__getChartOptions = function () {
            ...
            if (moduleOptions.chartType === 'pie') {
                ...
                if (AppConfig.isMobile&&_this.variables.a) {//手机APP上的对横坐标的修改
                    // (options.xAxis instanceof Array) && options.xAxis.forEach(function (row) {
                    ...
                    // });
                    // (options.yAxis instanceof Array) && options.yAxis.forEach(function (row) {
                ...
                (options.xAxis instanceof Array) && options.xAxis.forEach(function (row) {
                    Object.keys(row).forEach(v => { 
                        if (typeof row[v] === 'function') { 
                            try {
                                row[v] = row[v].call(null, data, _this.variables);
                            } catch (e) {
                                if ( v === 'data' ) { 
                                    row[v] = []; 
                                } else if( v === 'name' ){
                ...
                // 处理双坐标
                if(this.entity.modal.option.pointsSet && this.entity.modal.option.pointsSet.indexOf(1)>-1 && options.yAxis.length<2){
                    options.yAxis[1] = $.extend(true, {}, options.yAxis[0]);
                }
            }
            ...
            //处理title
            Object.keys(options.title).forEach(function (row) {
                if (row === 'text' && typeof options.title['text'] === 'function') {
                    try {
                        options.title['text'] =  options.title['text'].call(null, data, _this.variables);
                    } catch(e) {
                        options.title['text'] = [];                        
                    }
                }
            ...
            if (moduleOptions.chartType !== 'pie') {
                ...
                if (options.series.length === 0) {
                    series = [];
                    Object.keys(_this.variables).forEach(function(key){
                        if(key.indexOf('__p') === 0){
                            var index = key.slice(3,-2);
                            var id = _this.entity.modal.points[index];
                            var row = _this.variables[key];
                            series.push({
            ...
            if (AppConfig.isMobile) {//手机APP上的对横坐标的修改
                // (options.xAxis instanceof Array) && options.xAxis.forEach(function (row) {
                //     row.axisLabel = row.axisLabel || {};
                //     row.axisLabel.rotate = 270;
                //     delete row.name;
                ...
                //     row.data.textStyle = row.data.textStyle || {};
                //     row.data.textStyle.color = '#d8d6df';
                //     row.axisLabel.interval = 3;
                //     row.formatter = function (value, index) {
                //         // 格式化成月/日，只在第一个刻度显示年份
                //         var date = new Date(value);
                //         if(date == 'Invalid Date')return date;
                //         var texts =date.format('HH:mm');
                //         if (index === 0) {
                ...
                //     }
                // });
                // (options.yAxis instanceof Array) && options.yAxis.forEach(function (row,index) {
                //     row.nameTextStyle = row.nameTextStyle || {};
                //     row.nameTextStyle.color = '#d8d6df';
                //     row.axisLabel = row.axisLabel || {};
                //     row.axisLabel.textStyle = row.axisLabel.textStyle || {};
                ...
                //     row.axisLabel.textStyle.color = '#d8d6df';
                //     row.nameGap=45;
                //     row.axisLine = row.axisLine || {};
                //     row.axisLine.show = false;
                //     row.axisTick = row.axisTick || {};
                //     row.axisTick.show = false;
                //     if(index > 0){
                //         row.splitLine = row.splitLine || {};
                //         row.splitLine.show = false;
                //     }
                // });
                // options.legend.textStyle = options.legend.textStyle || {};
                // options.title.textStyle.color = '#d8d6df';
                if(options.yAxis && options.yAxis.length > 1){
                    options.grid.right = 40
                    options.yAxis.slice(1).forEach(function(item){
        ...
        this.render = function (isProcessTask) {
            ...
            promise.done(function () {
                ...
                var pointsSet = this.entity.modal.pointsSet || this.entity.modal.option.pointsSet || [];
                this.pointsSet = {};
                points&&points.forEach(function(point, index){ //兼容没有pointsSet的数据
