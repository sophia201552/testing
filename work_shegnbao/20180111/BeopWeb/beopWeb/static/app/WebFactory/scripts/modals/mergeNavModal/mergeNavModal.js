                    ...
                    onCheck: function (event, treeId, treeNode) {
                        ...
                        if (treeNode.checked) {
                            if (treeNode.pId === null) { 
                                ...
                                var parentNode = _this.treeResult.getNodesByParam('id', treeNode.pId, null);
                                var sameParentNodes = _this.treeResult.getNodesByParam('pId', treeNode.pId, null);
                                if (parentNode.length !== 0){//结果中已经有父元素了
                                    _this.treeResult.addNodes(_this.treeResult.getNodeByParam('id', treeNode.pId, null), -1, treeNode);
                                } else {
                                    //将文件夹直接add进去 
                                    for (var o=0,ol=outChildren.length;o<ol;o++){
                                        if (outChildren[o].id === treeNode.pId) {
                                            _this.treeResult.addNodes(_this.treeResult.getNodeByParam('id', outChildren[o].pId, null), -1, outChildren[o]);
                                            break;
                                        }
                                    }
                                    // 拿到结果中所有的子文件
                                    var allSameParentArr = _this.treeResult.getNodesByParam('pId', treeNode.pId, null);
                                    //遍历所有的子文件  如果不是当前选中的 遍历之前结果树种有的节点 
                                    for (var a = 0, al = allSameParentArr.length; a < al; a++){
                                        if (allSameParentArr[a].id !== treeNode.id) {
                                            if (sameParentNodes.length === 0){
                                                _this.treeResult.removeNode(allSameParentArr[a], false);
                                            } else {
                                                var filterarray = $.grep(sameParentNodes,function(value){
                                                    return allSameParentArr[a].id !== value.id;//筛选出不在结果树中的元素
                                                });
                                                for (var s = 0, sl = filterarray.length; s < sl;s++){
                                                    _this.treeResult.removeNode(filterarray[s], false);
                                                }
                                            }
                                        }
                                    } 
                            ...
                            if (sameParentNodes.length === 1){
                                _this.treeResult.removeNode(parentNode[0], false);
                            }
