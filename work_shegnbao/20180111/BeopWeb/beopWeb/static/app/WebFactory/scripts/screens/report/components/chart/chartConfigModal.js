...
(function(exports, SuperClass) {
    ...
    function() {
        ...
        this.init = function() {
            ...
            this.$exportSourceData = $('.exportSourceData', this.$wrap);
                    ...
                    // recover
                        ...
                        if (/^[a-z0-9]{24}$/.test(value)) {
                            $ele.attr({
                                'data-value': value,
                                'title': name
                            }).html('<input value="' + (name || value) + '" type="text">');
                        } else {
                            ...
                            $ele.attr({
                                'data-value': value,
                                'title': name
                            }).html('<input value="' + str + '" type="text">');
                        }
        ...
        this.recoverForm = function(options) {
            ...
            if (!form) {
                ...
                this._setField('checkBox', this.$exportSourceData, form.isExportData);
            }

            if (options.points) {
                options.points.forEach(function(point, i) {
                    _this.droparea('droparea', $('.drop-area', _this.$wrap).eq(i), point);
                    if ($('.drop-area', _this.$wrap).closest('.col-md-9').children('.noData').length === 0) {
                        $('.drop-area', _this.$wrap).closest('.col-md-9').append('<div class="col-md-12 drop-area-wrap noData" style="position: relative;padding:0 15px 10px 0;"><div class="drop-area">' +
                            '<span class="glyphicon glyphicon-plus"></span><input value="" type="text" style="display:none;"></div>' +
                            '<div class="drop-text" style="display: none;"><input type="text" i18n="placeholder=report.chartConfig.INTER_WORD"></div><div class="drop-set" style="display: none;"><input type="text" i18n="placeholder=report.chartConfig.INTER_SET" /></div><span class="glyphicon glyphicon-remove spanDataDel"></span></div>'
                        )
                    }
            ...
            var $haveData = $('.drop-area-wrap:not(".noData")', this.$wrap);
            if (form.legend) {
                form.legend.forEach(function(lengend, i) {
                    $('.drop-text', _this.$wrap).eq(i).show();
                    $('.drop-text', _this.$wrap).eq(i).children('input').val(lengend);
                })
            } else if ($haveData.length > 0) {
                $haveData.find('.drop-text').show();
            }
            if (form.pointsSet) {
                form.pointsSet.forEach(function(set, i) {
                    $('.drop-set', _this.$wrap).eq(i).show();
                    $('.drop-set', _this.$wrap).eq(i).children('input').val(set);
                })
            } else if ($haveData.length > 0) {
                $haveData.find('.drop-set').show();
            }
        };
        ...
        this.reset = function() {
            ...
            this._setField('checkBox',this.$exportSourceData);
        ...
        this.dataDeal = function() {
            ...
            $divDataSource.children('.noData').on('click', '.drop-area', function() {
                ...
                if ($this.val() === '' && $this.closest('.drop-area-wrap').hasClass('noData')) {
                    ...
                    $this.closest('.drop-area').siblings('.drop-set').show();
                    if ($this.val().indexOf('@') === 0) {
                        ...
                        if ($this.val().indexOf('|') < 0) {
                            ...
                            $this.closest('.drop-area').siblings('.drop-set').hide();
                            _this.dataDeal();
                            return;
                        }
        ...
        this.attachEvents = function() {
            ...
            this.$btnSubmit.off().click(function(e) {
                ...
                form.isExportData = this.$exportSourceData[0].checked ? true : false;
                modal.points = [];
                modal.option.legend = [];
                modal.option.pointsSet = [];
                $('.drop-area-wrap:not(".noData")', this.$wrap).each(function() {
                    ...
                    if (target) {
                        ...
                        var text = $(this).find('.drop-text').children('input').val(),
                            set = Number($(this).find('.drop-set').children('input').val());
                        ...
                        if (set) {
                            modal.option.pointsSet.push(set);
                        } else {
                            modal.option.pointsSet.push(0);
                        }
                    }
