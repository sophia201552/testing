(function (WidgetProp) {
    ...
    CanvasImageProp.prototype.attachEvent = function () {
        ...
        $ulEnumerate.off('change','input').on('change','input',function(){
            ...
            if(Object.keys(tempOpt.trigger)[index] === 'default'){
                index += 1;
            }
        ...
        $('#enumerateWrap').off('click').on('click', '.btnRemove',function(){
            ...
            var value = $.trim($(this).closest('.enumerateItem').find('.enumerateVal').val());
            var _btn = this;
            if(!key){
                //todo
                alert(I18n.resource.mainPanel.attrPanel.attrImg.ENUMER_INFO);
            }else{
                MaterialModal.show([{'title':'Template',data:['Image'],id:value}], function (data) {
