    ...
    +function () {
        ...
        this.tabOptions = {
            ...
            toolsTpl: '<div class="paneTempButton">\
                    ...
                    <div class="uploadImg" title="Uploading" style="display:none;"><span class="glyphicon glyphicon-upload"></span></div>\
                    ...
                    <input type="file" value="Select image" class="fileInput" multiple="multiple" style="display:none;">\
                </div>',
        ...
        this.attachToolEvents = function () {
            ...
            $tabName.find('.addTempate').off('click').click(function () {
                ...
                $('#nameSub').on('click',function () {
                    ...
                    if (widgetName === '') {
                        ...
                        var $item = $(_this.tabOptions.itemTpl.formatEL(data))
                        ...
                        //创建者为id
                        data.creator = AppConfig.userProfile.id;
            ...
            $tabName.find('.divBatchApply').off('click').on('click',function(){
                ...
                var allPages = _this.store;
                for(var i=0,length=allPages.length;i<length;i++){
                    if(id === allPages[i]._id){
                        if(allPages[i].type === 'page.PageScreen'  || allPages[i].type === 'page'){
        ...
        this.downFolder = function(result,upGroupData){
            ...
            $panelBody.find('.upGroup').off('dblclick').on('dblclick',function(){
                ...
                if(!_this.list[_this.list.length-2]){
                    ...
                    WebAPI.post('/factory/material/group/page',{userList:AppConfig.templateUserList}).done(function(result){
        ...
        this.focusOnSelf = function () {
            ...
            var keyy;//是否在pagescreen页面
            if ($materialModal.length) { keyy = 1; }
            if (keyy) {
                for (var k = 0; k <= this.store.length - 1; k++) {
                    if (this.store[k].creator != name) {
                        if (this.store[k].creator != email) {
                            //用属性查询时间消耗有点多；
                            $materialModal.find('#' + this.store[k]._id).hide();
                        }
                    }
                }
            } else {
        ...
        this.showAll = function () {
            ...
            if (keyy) {
                for (var k = 0; k <= this.store.length - 1; k++) {
                    $materialModal.find('#' + this.store[k]._id).show()
                }
            } else {
                for (var k = 0; k <= this.store.length - 1; k++) {
                    $('#' + this.store[k]._id).show();
                }
            }
