    ...
    PageScreen.prototype = {
        ...
        initNav: function () {
            ...
            $('#variable', this.$pageTopTools).off('click').click(function () {
                ...
                DeclareVariablesModal.show({
                    ...
                    //    rs = code.js;
                    //}
                    _this.page.option.variables = code[0].val;
                    var editData = {
                        id:_this.page.id,
        ...
        updateModelSet: function (data) {
            ...
            if (sortedData && sortedData.length) {
                ...
                //图层面板排序
                this.painter.screen.layerPanel.buildTreeByOrder();
            ...
            if (data.shots) {
                ...
                this.store.snapshotModelSet.append( new Model({
                    _id: ObjectId(),
                    name: I18n.resource.mainPanel.historyPanel.THE_DEFAULT_SNAPSHOTS,
                    content: $.deepClone({
                        list: this.page.layerList,
        ...
        updateModelSetByDiffs: function (nextData) {
            var data = {
                widgets: this.getWidgetsData(),
                layers: this.getLayersData(),
                list: this.getLayerList()
            };
            var activeLayerIds, activeWidgetIds;
            var nextSortedList = this.getSortedList(nextData);
            var set = [], lastInsertType;

            var layerDiffs = this.screen.syncWorker.getLayerDiffs(data.layers.toMap('_id'), Array.toMap(nextData.layers, '_id'));
            var widgetDiffs = this.screen.syncWorker.getWidgetDiffs(data.widgets.toMap('_id'), Array.toMap(nextData.widgets, '_id'));
            var listDiffs = diff(data.list, nextData.list);

            var widgetsToBeAdded = [];
            var widgetsToBeRemoved = [];
            var widgetsToBeModified = [];
            var layersToBeAdded = [];
            var layersToBeRemoved = [];
            var layersToBeModified = [];
            ...
            if (layerDiffs) {
                ...
                layerDiffs.forEach(function (row) {
                    if (row.k === 'D') {
                        layersToBeRemoved.push(row._id);
                    } else if (row.k === 'N') {
                        layersToBeAdded.push(row.v);
                    } else if (row.k === 'E') {
                        layersToBeModified.push(row);
                    }
                });
            }
            ...
            if (widgetDiffs) {
                ...
                widgetDiffs.forEach(function (row) {
                    if (row.k === 'D') {
                        widgetsToBeRemoved.push(row._id);
                    } else if (row.k === 'N') {
                        widgetsToBeAdded.push(row.v);
                    } else if (row.k === 'E') {
                        widgetsToBeModified.push(row);
                    }
                });
            }
            ...
            // 删除，先删 widget，再删 layer
            widgetsToBeRemoved.forEach(function (row) {
                var model = this.getWidgetsData().findByProperty('_id', row);
                this.getWidgetsData().remove(model);
            }, this);
            layersToBeRemoved.forEach(function (row) {
                var model = this.getLayersData().findByProperty('_id', row);
                this.getLayersData().remove(model);
            }, this);

            // 增加，先添加 layer，再添加 widget
            // layers 的添加需要按照顺序，这里做一次排序
            layersToBeAdded
                .concat(widgetsToBeAdded)
                .sort(function (lhs, rhs) {
                    return nextSortedList.indexOf(lhs._id) > nextSortedList.indexOf(rhs._id);
                })
            ...
            // 修改
            widgetsToBeModified.forEach(function (row) {
                var model = this.getWidgetsData().findByProperty('_id', row._id);
                model.update(row.v);
            }, this);
            layersToBeModified.forEach(function (row) {
                var model = this.getLayersData().findByProperty('_id', row._id);
                model.update(row.v);
            }, this);

            if (listDiffs) {
                this.painter.store.pageModel.list(nextData.list);
            }
            ...
            // 恢复选中的图层
            if (activeLayerIds && activeLayerIds.length) {
                this.painter.setActiveLayers(activeLayerIds);
            }
            // 恢复选中的控件
            if (activeWidgetIds && activeWidgetIds.length) {
                this.painter.setActiveWidgets(activeWidgetIds);
            }
        ...
        autoSaveSync: function(isImmediatelyRun, callback) {
            ...
            timer = window.setTimeout(function() {
                ...
                if(isAutoSave == 'true'){
                    ...
                    _this.autoSaveSync(false);
                }
            }, AppConfig.syncInterval);

            if (isImmediatelyRun) {
                this.screen.syncWorker.sync(callback);
            }
        },
        /**
         * 拿到指定类型的控件的 model集合
        ...
        getSortedData: function (data) {
            var layerMap = Array.toMap(data.layers, '_id');
            var widgetMap = Array.toMap(data.widgets, '_id');
            var sortedList = this.getSortedList(data);

            return sortedList.map(function (row) {
                return layerMap[row] || widgetMap[row];
            });
        },
