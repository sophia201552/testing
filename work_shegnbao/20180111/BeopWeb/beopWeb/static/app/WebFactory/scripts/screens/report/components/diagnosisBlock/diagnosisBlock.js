(function (root, factory) {
    ...
    +function () {
        ...
        this.showNoData = function () {
            var str = '<div style="margin: 0 auto;width: 600px;height: 160px;margin-top:200px;">\
        ...
        this.transformChapterToChapterContainer = function (chapter) {
            ...
            if (!Array.isArray(data) && chapter.type !== "summary"){
                data.modal.option['isPageBreak'] = this.entity.modal.option.isPageBreak;
            }
            var layouts;
            chapter.units.forEach(function (unit) {
                ...
                if (unit.unitName) {
                    ...
                    if (unit.unitName.indexOf('.1 ') !== -1) {
                        container.modal.option['isPageBreak'] = false;
                    } else {
                        container.modal.option['isPageBreak'] = this.entity.modal.option.isPageBreak;
                    }
                } else {
        ...
        this.transformToHtml = function (subUnit) {
            var defaultChartHeight = '550px';
            if(subUnit.chartItems && subUnit.chartItems.x.length == 0 && subUnit.chartItems.y.length == 0){
                defaultChartHeight = '0px';
            }
        ...
        this.getTableOptions = function (subUnit) {
            ...
            items.y.forEach(function (row) {
                ...
                for(var k = 0;k<row.value.length;k++){
                    //为特殊数字设置样式根据undeline属性
                    var nowValue = row.value[k];
                    if(row.underline&&row.underline.length){
                        if(row.underline.length>k&&row.underline[k]){
                            arrHtml.push('<td style="color:red">'+nowValue+'</td>');
                        }else{
                            arrHtml.push('<td>'+nowValue+'</td>');
                        }
                    }else{
                        arrHtml.push('<td>'+nowValue+'</td>');
                    }
        ...
        this.getChartOptsFromParam = function (chartParam, type) {
            ...
            if (type === this.SUPPORT_SUBUNIT_TYPE.PIE) {
                ...
                for (var m = 0, noMaxNoMinchartSeriesData = [], length = y.length; m < length; m++) {
                    ...
                    //标志特殊数字通过柱图单个柱子的颜色变化
                    var barIndex;
                    if(yItem.underline&&yItem.underline.length>0){
                        for(var barI = 0;barI<yItem.underline.length;barI++){
                            if(yItem.underline[barI]){
                                barIndex = barI;
                                break;
                            }
                        }
                        if((barIndex==0||barIndex)&&seriesItem.data.length>0){
                            for(var barColorI = 0;barColorI<seriesItem.data.length;barColorI++){
                                if(barIndex==barColorI){
                                    var oldata = seriesItem.data[barColorI];
                                    seriesItem.data[barColorI] = {
                                        value:oldata,
                                        itemStyle:{
                                            normal:{
                                                color:'#0077ee'
                    ...
                    //mark标记
                    if(yItem.markPoint){
                        //兼容echart-3.x版本  4.x后可删
                        yItem.markPoint.label = yItem.markPoint.label||{};
                        yItem.markPoint.label.normal = yItem.markPoint.label.normal||{};
                        yItem.markPoint.label.normal.show=true;
                        yItem.markPoint.label.normal.formatter = '{c}';
                        seriesItem.markPoint = yItem.markPoint;
                    }
                    series.push(seriesItem);
