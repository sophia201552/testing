; (function (exports, SuperClass) {
    ...
    +function () {
        ...
        this.tabOptions = {
            ...
            itemTpl: '<div class="{classAll}" id="{id}" data-id="{_id}" draggable="true" data-type="image" data-groupid ="{group}" data-creator="{creator}">\
                    <div class="progress"></div><span class="animFlag"></span>\
                    ...
                    <div class="caption"><h3 class="imgName">{title}</h3><h3 class="imgSize">{w}*{h}</h3></div>\
                    </div></div>',
            toolsTpl: '<div class="paneTempButton">\
                    ...
                    <div class="focusOnSelf" title="Look at yourself"><span class="glyphicon glyphicon-eye-open"></span></div>\
                    <div class="showAll" title="showAll" style="display:none;"><span class="glyphicon glyphicon-eye-close"></span></div>\
                    <div class="importLink" title="Import link"><span class="glyphicon glyphicon-arrow-down"></span></div>\
                    <div class="shareLink" title="Share link" style="display:none;"><span class="glyphicon glyphicon-pushpin"></span></div>\
        ...
        this.attachEvents = function () {
                ...
                $tabContent.off('dblclick', '.photoItem').on('dblclick', '.photoItem', function (e) {
                    ...
                    WebAPI.post('/factory/material/getByIds', data).done(function (result) {
                        if (result[0].content.w < 0 || result[0].content.h < 0) {
                            ...
                            tmpImg.src = result[0].content.url;

                            result[0].content.w = tmpImg.naturalWidth;
                            result[0].content.h = tmpImg.naturalHeight;
                        }
            ...
            //显示自己
            $tabName.find('.focusOnSelf').off('click').click(function () {
                _this.focusOnSelf();
            });
            //显示所有
            $tabName.find('.showAll').off('click').click(function () {
                _this.showAll();
            });
                            ...
                            if (result.status == "OK") {
                                ...
                                for (var i = 0, len = $active.length; i < len; i++) {
                                    ...
                                    if (imgContentArr[i].content.interval === 0) {
                                        $active.eq(i).removeClass('animationLabel').find(".animFlag").css('background', '#078A98').html('image');
                                        imgUrlDom.removeClass('center');
                                    } else {
                                        $active.eq(i).addClass('animationLabel').find(".animFlag").css('background', '#6D3F1F').html('animation');
                                    }
                    ...
                    function submit() {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                    ...
                                    if (count < imgStandardArr.length) {
                                        for (var j = 0, len = imgContentArr.length; j < len; j++) {
                                            if (imgContentArr[j]._id === standId) {
                                                ...
                                                if (imgContentArr[j]) {
                                                    imgContentArr[j].content.url = 'https://beopweb.oss-cn-hangzhou.aliyuncs.com/' + data[i];
                                                }
                                ...
                                // 让新建单位处于选中状态
                                $tabContent.find('.tpl-item').removeClass('active');
                                domId.forEach(function(rowId){
        ...
        this.renderItems = function (result) {
            ...
            //}
            if(this.list.length > 0 && this.options.showEnumerationImgId){
                var upGroupData = {
                    _id: '...',
                    group: this.list[this.list.length - 1],
                    parentGroupId: this.list.length > 1 ? this.list[this.list.length - 2] : '',
                    type: 'image'
                };
                var activeId = this.options.showEnumerationImgId;
                this.options.showEnumerationImgId = "";
                this.downFolder({'data':result}, upGroupData);
                $("#" + activeId).addClass('active');
                document.getElementById(activeId).scrollIntoView(false);
            }else{
                ...
                if(this.options.showEnumerationImgId){
                    var activeId = this.options.showEnumerationImgId;
                    this.options.showEnumerationImgId = "";
                    $("#" + activeId).addClass('active');
                    document.getElementById(activeId).scrollIntoView(false);
        ...
        this.photoCell = function (result, classAll) {
            ...
            if (parseInt(imgInterval) === 0) {
                ...
                $tabContent.find('.photoItem:last').addClass('animationLabel').find('.animFlag:last').css('background', '#6D3F1F').html('animation');
            }
        };
        ...
        this.downFolder = function (result, upGroupData) {
            ...
            $tabName.find('.showAll').hide();
            $tabName.find('.shareLink').hide();
            $tabName.find('.focusOnSelf').show();
            ...
            $panelBody.find('.upGroup').off('dblclick').on('dblclick', function () {
                if (!$.isEmptyObject(_this.search)) {
                    ...
                    if (_this.search.searchList.length > 0) {
                        WebAPI.post('/factory/material/group/' + $(this)[0].dataset.type + '/' + _this.search.searchList[_this.search.searchList.length - 1],{userList:AppConfig.templateUserList}).done(function (result) {
                            $panelBody.empty();
                ...
                if (!_this.list[_this.list.length - 2]) {
                    ...
                    WebAPI.post('/factory/material/group/image',{userList:AppConfig.templateUserList}).done(function (result) {
                        $panelBody.empty();
                        _this.renderItems(result.data);
                    });
                } else {
        ...
        this.focusOnSelf = function () {
            var name = AppConfig.userProfile.fullname;
            var email = AppConfig.userProfile.name;
            var idd = AppConfig.userProfile.id;
            var $tabContent = $('#tabContent');
            var $showAll = $('.showAll');
            var $focusOnSelf = $('.focusOnSelf');
            var _this=this;
            for (var k = 0; k <= this.store.length - 1; k++) {
                if (this.store[k].creator != name) {
                    if (this.store[k].creator != email) {
                        if (this.store[k].creator != idd) {                   
                            $('[data-id=' + this.store[k]._id + ']').hide()
                        }
                    }
                }
            }
