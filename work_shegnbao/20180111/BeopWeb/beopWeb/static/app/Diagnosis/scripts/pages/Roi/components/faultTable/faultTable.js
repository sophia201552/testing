    ...
    class FaultTable {
        ...
        getTableData(structuresIdArr, faultsIdArr, time, categoriesArr, searchKey) {
            ...
            var opt = {
                ...
                tbodyCol:[
                    {index: 'time', converter: function(value){return value.time ? timeFormat(moment(value.time).format('YYYY-MM-DD HH:mm:ss').toDate(), timeFormatChange('mm-dd hh:ii')) : '--'},title:true},
        ...
        initTable(dataList) {
            ...
            for (let i = 0, ilen = dataList.length; i < ilen; i++){
                ...
                trDom += (tpl.formatEL({
                    ...
                    factor: item.factor,
                    energy: item.energy,
                    time: item.time ? timeFormat(moment(item.time).format('YYYY-MM-DD HH:mm:ss').toDate(), timeFormatChange('mm-dd hh:ii')) : '--',
                    group: item.group,
        ...
        calculateROI(saveMoney, laborCost, hrPrice, hr) {
            ...
            if (laborCost && laborCost > 0) {
                ...
                if (saveMoney == 0) {
                    ...
                    ROI = (saveMoney / laborCost).toFixed(2);
                }
            }
            return ROI;
        }
        attachEvents() {
            ...
            $thisContainer.off('click.save').on('click.save', '.saveRoiBtnCtn', function () {
                ...
                WebAPI.post('/diagnosis_v2/saveRoiInfo', {
                    projectId: AppConfig.projectId,
        ...
        updateEditData($this) {
            ...
            saving = (Number(saving) && saving != 0) ? saving : '--';
