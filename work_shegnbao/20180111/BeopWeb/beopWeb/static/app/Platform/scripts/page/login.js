var PageLogin = (function () {
    ...
    PageLogin.onBeforeLogin = function () {
        return true;
    };
    PageLogin.prototype = {
        show: function () {
            // WebAPI.get('/static/app/Platform/views/page/login.html').done(()=>{
        ...
        login: function () {
            ...
            WebAPI.post("/login", {
                    ...
                    var after_login = function () {
                        if (login_result.status != undefined && login_result.status == true) {
                            ...
                            if (!AppConfig.userConfig){
                                AppConfig.userConfig = login_result.userConfig||{};
                                AppConfig.userConfig.skin = AppConfig.userConfig.skin || localStorage.getItem('systemSkin_' + login_result.id) || 'dark';
                                ...
                                AppConfig.userConfig.zone = AppConfig.userConfig.zone||localStorage.getItem('timeZone')||'localTimeZone';
                            }
                            //添加版本历史
                ...
                //login_result.ip = '114.93.192.71';
                //login_result.ip = '140.207.49.230';
                //login_result.ip = "140.207.49.230, 10.171.218.113";
                //login_result.ip = '155.143.201.70'; //澳洲IP
                //AppConfig.userId = 2672;

                //经过代理后会返回ip列表.临时处理这类情况
                if (login_result.ip && login_result.ip.indexOf(',') !== -1) {
                    login_result.ip = login_result.ip.split(',')[0].trim();
                }
                if ($.inArray(login_result.id, [2397, 2398, 2672, 2399]) > -1) {
                    localStorage.setItem('isGoogleAvailable', true);
                    after_login();
                } else {
                    localStorage.setItem('isGoogleAvailable', I18n.type == 'en');
