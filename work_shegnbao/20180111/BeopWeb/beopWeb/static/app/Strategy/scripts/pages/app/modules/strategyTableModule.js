...
(function(root, factory) {
    ...
    var STRATEGYTABLE_TAGTRABSFER = 'STRATEGYTABLE_TAGTRABSFER';
    // ------------------------------------
    // Actions
    // ------------------------------------
    ...
    var doTagTransfer = function(){
        
        return function(dispatch){
            dispatch({
                type: STRATEGYTABLE_TAGTRABSFER,
                isShowTagTrabsfer: true
            });
            let async = $.Deferred();
            WebAPI.get('/strategy/getProjConfig/'+AppConfig.projectId).done((rs)=>{
                if (rs.status === 'OK') {
                    let config = rs.data && rs.data.config;
                    WebAPI.post('/tag/dataTransfer',{
                        projId: AppConfig.projectId,
            ...
            return async.done(()=>{
                message.success(I18n.resource.message.TAG_TRANSFER_SUCCESS);
            }).fail(()=>{
                message.error(I18n.resource.message.TAG_TRANSFER_FAIL);
            }).always(()=>{
                dispatch({
                    type: STRATEGYTABLE_TAGTRABSFER,
                    isShowTagTrabsfer: false
                });
            });
        }
    }
    ...
    const ACTION_HANDLERS = {
        ...
        [STRATEGYTABLE_TAGTRABSFER]:(state, action) => {
            return Object.assign({},state, {
                isShowTagTrabsfer: action.isShowTagTrabsfer
            });
        }
    };
