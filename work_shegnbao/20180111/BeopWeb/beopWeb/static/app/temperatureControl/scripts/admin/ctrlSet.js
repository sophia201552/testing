        ...
        init: function(isNotRequestData) {
            ...
            if (this.type != 'sensor' && (ctrAll || ctrAll.length > 0)) {
                ...
                ctrAll.forEach(function(ctr) {
                    new CtrlItem(_this,ctr, _this.container, _this.type);
                ...
                sensorAll.forEach(function(sensor) {
                    new CtrlItem(_this,sensor, _this.container, _this.type);
...
var CtrlItem = (function() {
    ...
    function CtrlItem(parent,ctrl, ctn, type) {
        _this = this;
        this.parent = parent
        this.ctn = ctn;
        this.type = type;
    ...
    CtrlItem.prototype = {
        tplDevice: '<li>\
            <div class="deviceScreen ctrl zepto-ev" id="ds_{id}">\
                <div class="head">\
                    <span class="headerAttr"><span class="iconfont icon-medium icon-xinhao3 signal" id="{signalId}"></span></span>\
                    <span class="headerAttr"><span class="iconfont icon-medium icon-dianliangtiao3 electricity" id="{electricityId}"></span></span>\
                </div>\
                <div class="checkBox">\
                ...
                <div class="attrContent" data-attr="speed">\
                    <i class="iconfont icon-fengsushezhi"></i><span id="{windSpeedId}" class="value">{windSpeed}</span>\
                </div>\
                <div class="attrContent" data-attr="valve"><i class="iconfont icon-zhenkongfamen"></i><span id="{valveId}" class="value">{valve}</span></div>\
                <div class="btn-group">\
        ...
        tplSensor: '<li>\
            <div class="deviceScreen sensor zepto-ev" id="ds_{id}">\
                <div class="head">\
                    <span class="headerAttr" data-attr="signal"><span class="iconfont icon-medium icon-xinhao3 signal" id="{SensorSignalStrengthId}"></span></span>\
                    <span class="headerAttr" data-attr="power"><span class="iconfont icon-medium icon-dianliangtiao3 electricity" id="{powerId}"></span><span class="value">{power}</span></span>\
                </div>\
                <div class="headMsg">\
                    <div>{name}</div>\
                </div>\
                <div class="attrContent" data-attr="temperature">\
                    <i class="iconfont" style="margin-right:5px;">&#xe660;</i><span id="{SensorTId}" class="value tempVal">{SensorT}</span>\
                </div>\
                <div class="attrContent" data-attr="humidity">\
                    <span class="iconfont icon-shuidi" style="margin-right:5px;font-size: 1.7rem;left: -.1rem;"></span><span id="{HumidityId}" class="value" >{Humidity}</span>\
                </div>\
                <div class="btn-group">\
                    <button class="btn btnMatch"><span class="glyphicon glyphicon-link"></span></button>\
                    <button class="btn btnHist"><span class="glyphicon glyphicon-time"></span></button>\
        ...
        init: function(device) {
            if (this.type == 'device') {
                $(this.ctn).append(this.tplDevice.formatEL({
                    name: device.name.split('_').join(' '),
                    signalId: device.arrP.FCUSignalStrength,
                    electricityId: device.arrP.FCUBattery,
                    id: device._id,
                    windSpeed: '--',
                    windSpeedId: device.arrP.FCUSpeed,
                    valve:'--',
                    valveId:device.arrP.ValveOnOff
                }));
                this.attachEventsDevice($(this.ctn).find('#ds_' + device._id));
            } else if (this.type == 'mode') {
                $(this.ctn).append(this.tplMode.formatEL({
                    FCUId: device._id,
                    name: device.name.split('_').join(' '),
                    temp: '--',
                ...
                this.attachEventsMode($(this.ctn).find('#ds_' + device._id));
            } else if (this.type == 'sensor') {
                $(this.ctn).append(this.tplSensor.formatEL({
                    powerId:device.arrP.Power,
                    power:'',
                    HumidityId:device.arrP.ValveOnOff,
                    Humidity:'--',
                ...
                this.attachEventsSensor($(this.ctn).find('#ds_' + device._id));
            }
        ...
        attachEventsDevice: function($deviceScreen) {
            ...
            $deviceScreen.find('.btnHist').off('touchend').on('touchend', function(e) {
                ...
                $('#divChartWrap').show();
                $(_this.ctn).hide().next('.ulPage').hide();
            });
            //设置风速
            $deviceScreen.find('.speed').off('touchend').on('touchend', function(e) {
                ...
                var $speedVal = $(this).find('[data-attr="speed"] .value');
