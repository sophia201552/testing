...
var BaseObserverScreen = function() {
    ...
    function BaseObserverScreen(container, option) {
        ...
        if (option && option.roomInfo) {
            ...
            AppConfig.roomId = _this.roomInfo._id;
        } else {
    ...
    BaseObserverScreen.prototype = {
        ...
        show: function() {
            try {
                localStorage.setItem('lastOpenRoom', JSON.stringify(this.roomInfo));
            } catch (e) {
        ...
        getRoomListFromNet: function(flag) {
            ...
            WebAPI.get('/appTemperature/room/getlist/' + AppConfig.userId).done(function(resultData) {
                ...
                _this.refreshRoomListCache(resultData.roomList);
                flag.resolve();
            }).fail(function() {
                AppConfig.isLocalMode = true;
        ...
        refreshRoomListCache: function(newRoomList) {
            var roomList = [];
        ...
        getRoomDetailFromCache: function(flag) {
            ...
            var fileName = 'RoomDetail.' + this.roomInfo._id;
            FileAPI.read(fileName).done(function(result) {
                if (result && result[fileName].status == 'success' && result[fileName].result) {
                    sensorAll = result[fileName].result.sensor;
                    ctrAll = result[fileName].result.controller;
                    spaceAll = result[fileName].result.space;
                } else {
                    window.plugins && window.plugins.toast.show(I18n.resource.admin.indexMap.OPEN_CLOSE_INFO, 'short', 'center');
                    if (AppConfig.landscape) {
                        new localListScreen(document.getElementById('containerObserver')).show();
                    } else {
                        router.to({ typeClass: new localListScreen() });
                    }
                }
                flag.resolve();
            }).fail(function() {
        ...
        initNav: function() {
            ...
            var $main = $('#warpObserverScreen');
            var $side = $('#wrapAddition');
            var $btnConfig = $('#btnConfig');
            var $btnRoomList = $('#btnRoomList');
            // 配置按钮
            $btnConfig.off('touchstart').on('touchstart', function(e) {
                if (!AppConfig.landscape) {
                    ...
                    if ($side[0].dataset.content == 'config' || !$side[0].dataset.content) {
                        $main.toggleClass('fullPage');
                        $side.toggleClass('hide');
                    } else {
                        $main.removeClass('fullPage');
                        $side.removeClass('hide')
                    }
                    $side[0].dataset.content = 'config';
                    _this.screenResize();
                    new AdminConfigure(document.getElementById('wrapAddition')).show()
                }
                e.preventDefault();
            });
            $main.off('transitionend').on('transitionend', function() {
                this.screenResize();
            })
            $main.off('webkitTransitionEnd').on('webkitTransitionEnd', function() {
                this.screenResize();
            })
            $btnRoomList.off('touchstart').on('touchstart', function(e) {
                if (!AppConfig.landscape) {
                    ...
                    if ($side[0].dataset.content == 'roomList' || !$side[0].dataset.content) {
                        $main.toggleClass('fullPage');
                        $side.toggleClass('hide');
                    } else {
                        $main.removeClass('fullPage');
                    ...
                    $side[0].dataset.content = 'roomList';
                    _this.screenResize();
        ...
        initSpace: function() {
            ...
            for (var i = 0; i < spaceAll.length; i++) {
                ...
                space.setAttribute('path', spaceAll[i].params.path)
        ...
        showSetTempHistory: function(list, gps) {
            ...
            list.forEach(function(record) {
                ...
                strHtml += (tpl.formatEL({
                    name: record.name,
                    urlImg: 'https://beopweb.oss-cn-hangzhou.aliyuncs.com/' + record.urlImg,
