...
$(document).ready(function() {
    ...
    InitI18nResource(navigator.language.split('-')[0], null, '/static/app/temperatureControl/views/i18n/').always(function(rs) {
    ...
    IndexScreen.prototype = {
        ...
        login: function() {
            ...
            if (!(userName && password)) {
                loginFail(I18n.resource.admin.index.NAME_ERROR);
                return;
            }
                    ...
                    var after_login = function() {
                        if (login_result.status != undefined && login_result.status == true) {
                            if (login_result.projects && login_result.projects.length > 0) {
                                ...
                                //    roomAll = rs.roomList;
                                //    roomAll.forEach(function(val){val.baseType = 'groups'});
                                //    ScreenManager.show(ConfigScreen);
                                //}).fail(function(){
                                //
                                //});
                            } else {
                                loginFail(I18n.resource.admin.index.LOGIN_ERROR_INFO);
                            }
                        } else {
                            loginFail(I18n.resource.admin.index.NAME_ERROR);
                        }
