(function (beop) {
    var configMap = {
        ...
        refreshOperateBtn, showTaskEditSection, hideTaskEditSection;
    //事件列表
    ...
    setJqueryMap = function () {
        ...
        jqueryMap = $.extend(jqueryMap ? jqueryMap : {}, {
            ...
            $faultCurveBox: $container.find('#fault-curve-box'),
            $taskOperation: $container.find('#operation'),
            $taskZoneLocation: $container.find('#taskZoneLocation')
        });
    };
    ...
    initCreateTask = function () {
        ...
        return configMap.groupModel.getTaskGroup(beop.model.stateMap.currentGroupId).done(function (result) {
            ...
            return WebAPI.get(configMap.htmlURL).done(function (resultHtml) {
                ...
                stateMap.$container.find('#workflow-task').removeClass(beop.constants.allViewType.join(' ')).addClass(stateMap.viewType);
    ...
    attachEvents = function () {
        jqueryMap.$box.on('change', '#taskGroupSelector', onChangeTaskGroup);
        jqueryMap.$box.on('change', '#taskProcessSelector', changeProcess);
        jqueryMap.$box.on('click.taskSave', '#taskSave', taskSave);
        jqueryMap.$box.on('click.taskComplete', '#taskComplete', taskComplete);
        jqueryMap.$box.on('click.taskNoPass', '#taskNoPass', taskNoPass);
        jqueryMap.$box.on('click.taskPass', '#taskPass', taskPass);
        jqueryMap.$box.off('.tagDelete').on('click.tagDelete', '.wf-detail-tag-delete', tagDelete);
        jqueryMap.$box.off('.tagSelect').on('change.tagSelect', '#wf-labelNames', tagSelect);
        jqueryMap.$box.off('.togglePeople').on('click.togglePeople', '.process-people-show', togglePeople);
        jqueryMap.$box.off('.wfWatchersAdd').on('click.wfWatchersAdd', '#wfWatchersAdd', watchersAddWin);
        jqueryMap.$outline.off('.taskEdit').on('click.taskEdit', '#taskEdit', taskEdit);
        jqueryMap.$outline.off('.taskModify').on('click.taskModify', '#taskModify', taskModify);
        ...
        jqueryMap.$outline.off('.taskDelete').on('click.taskDelete', '#taskDelete', taskDelete);
        jqueryMap.$outline.off('.taskReSave').on('click.taskReSave', '#taskReSave', taskReSave);
    };

//region---------DOM操作------
    var renderGroupProcess, initDatePicker, renderTemplate, renderTaskDetail,
        renderProcessFlow, renderProcessMember, renderTaskZone;
    ...
    renderTaskZone = function (zone, equipment) {
        var $diagnosisZone = jqueryMap.$taskZoneLocation.find('#taskDiagnosisZone'),
            $diagnosisEquipment = jqueryMap.$taskZoneLocation.find('#taskDiagnosisEquipment');
        $diagnosisZone.text(zone);
        $diagnosisEquipment.text(equipment);
    };
    ...
    togglePeople = function () {
        ...
        var members = [];
        if (index == stateMap.current_node_index) {
            members = stateMap.currentProcess.nodes[index].candidate;
        } else {
            members = stateMap.currentProcess.nodes[index].members;
        }
        beop.view.memberSelected.init($('body'), {
            configModel: {
                cb_dialog_hide: function (selectedUser) {
                    if (stateMap.processUnsaved) {
                        stateMap.processUnsaved[index] = {
                            member: selectedUser.map(function (item) {
                                return item.id
                            })
                        };
                    }
                ...
                userMemberMap: uniqueMembers(members)
            }
        });
    };
    ...
    renderProcessFlow = function (process_nodes, node_index) {
        ...
        if (stateMap.viewType == beop.constants.viewType.SHOW || stateMap.viewType == beop.constants.viewType.EDIT) {
    ...
    renderTaskDetail = function () {
        ...
        if (stateMap.viewModel.fields && stateMap.viewModel.fields.charts) {
            ...
            beop.view.faultCurve.init(jqueryMap.$faultCurveBox.find('.fault-curve'), {
        ...
        $("#wfWatchersAdd").hide();
        renderTaskZone(stateMap.viewModel.fields.diagnosisZone, stateMap.viewModel.fields.diagnosisEquipmentName);
        jqueryMap.$taskGroupProcess.empty().append(beopTmpl('tpl_wf_task_group_process', vm));
        initFileUpload();
        //reply list
    ...
    showTaskEditSection = function () {
        jqueryMap.$taskContent.find('.taskEditSection').show();
        if (stateMap.viewType == beop.constants.viewType.CREATE) {
            jqueryMap.$faultCurveBox.hide();
            jqueryMap.$taskZoneLocation.hide();
            return false;
        }
        if (!stateMap.viewModel.fields || !stateMap.viewModel.fields.charts) {
            jqueryMap.$faultCurveBox.hide();
        }
        if (!stateMap.viewModel.fields || !stateMap.viewModel.fields.diagnosisZone || !stateMap.viewModel.fields.diagnosisEquipmentName) {
            jqueryMap.$taskZoneLocation.hide();
        }
    };
    hideTaskEditSection = function () {
...
//region---------事件---------
    ...
    renderTaskTags = function () {
        ...
        if (stateMap.viewModel.taskGroup[0].tags) {
    ...
    taskModify = function () { // 工单编辑
        ...
        stateMap.addedWatchersList = stateMap.viewModel.watchersInfo;
        stateMap.viewType = beop.constants.viewType.EDIT;
        if (stateMap.currentProcess.nodes) {
            stateMap.processUnsaved = {};
            for (var i = 0; i < stateMap.currentProcess.nodes.length; i++) {
                var ids = [];
                for (var j = 0, jLen = stateMap.currentProcess.nodes[i].members.length; j < jLen; j++) {
                    ids.push(stateMap.currentProcess.nodes[i].members[j].id);
                }
                stateMap.processUnsaved[i] = {member: ids};
            }
        }
        $("#wfWatchersAdd").show();
        $("#taskModify").hide();
    ...
    taskDelete = function () {
        confirm.danger(I18n.resource.admin.versionHistory.SURE_DELETE_THIS_TASK, function () {
            Spinner.spin(ElScreenContainer);
    ...
    taskSave = function (ev) {
        trackEvent('工单新建点击', 'Workflow.Create.Click');
        ...
        fields.workorderCreateTime = new Date().format(DateUtil.DATA_FORMAT.FULL_DATETIME_CHANGE);
        var submitModel = {
            fields: fields,
    ...
    taskComplete = function () {
        ...
        var nextNode = getNextNode();
        if ((nextNode && nextNode.members.length !== 1 && !stateMap.processUnsaved) || (stateMap.processUnsaved && stateMap.processUnsaved[stateMap.current_node_index + 1].member.length !== 1)) {
            var behaviour = I18n.resource.workflow.team['ADD_PROCESS_BEHAVIOR_TYPE_' + nextNode.behaviour];
        ...
        } else if (stateMap.processUnsaved && stateMap.processUnsaved[stateMap.current_node_index + 1].member.length === 1) {
            taskAction(configMap.taskModel.completeTask, [stateMap.taskId, stateMap.processUnsaved[stateMap.current_node_index + 1].member[0]]);
        }
    };
