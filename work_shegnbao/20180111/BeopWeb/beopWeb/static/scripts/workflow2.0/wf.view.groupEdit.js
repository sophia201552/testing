(function (beop) {
    ...
    var getGroupData, setEditJqueryMap, renderAddedUsers, resetData, renderTeamAddUsers, onAddUserDialogOpen, getCheckedProcess,
        ...
        setGroupMenuStyle, findArchByArchId,uniqueMembers,showSelsectPeopple,changeSelsectPeopple,overdueAttachEvents,
        showOverdueProcess;
    ...
    commonInit = function (result) {
        ...
        // 逾期流程:
        jqueryMap.$addPushPeople.click(function(){
            var  $this = $(this);
            var  type = $this.attr('data-type');
            
            beop.view.memberSelected.configModel({
                cb_dialog_hide: function (addedUserList) {
                    showSelsectPeopple(type,addedUserList);
                },
                maxSelected: 1,
                userMemberMap: uniqueMembers(stateMap.allMembers),
    ...
    uniqueMembers = function(members){
        var tempMap = {};
    ...
    showSelsectPeopple = function(type,addedUserList){
        var autoEscalationConfig = [];
        var newAddedUserList = [];
        var AddedUserObj = {};
        for(key in addedUserList[0]){
            if(key != 'escalateAfter'){
                if(!AddedUserObj[key]){
                    AddedUserObj[key] = addedUserList[0][key];
                }
            }
        }
        newAddedUserList.push(AddedUserObj);
        stateMap.showPushMembers[type].push(newAddedUserList);
        $(".push-"+ type + "-people").empty().append(beopTmpl('wf_push_people_list',{type:type,data:stateMap.showPushMembers}));
        overdueAttachEvents();
        I18n.fillArea($('#overdue-process-box'));
    };


    overdueAttachEvents = function(){
        jqueryMap.$pushPeopleList.find('.process-people-show').click(function(){
            var $this = $(this);
            var index = $this.attr('data-index'),
                type= $this.attr('data-type');
            beop.view.memberSelected.configModel({
                cb_dialog_hide: function (addedUserList) {
                    changeSelsectPeopple(type,index,addedUserList);
                },
                maxSelected: 1,
                userMemberMap: uniqueMembers(stateMap.allMembers),
                userHasSelected: stateMap.showPushMembers[type][index]
            });
            watchersPicker = beop.view.memberSelected.init(stateMap.$container);
        });


        jqueryMap.$pushPeopleList.off("click.deletePeople").on('click.deletePeople','.deletePeople',function(){
            var $this = $(this);
            var index = $this.attr('data-index'),
                type= $this.attr('data-type');
            if(stateMap.showPushMembers[type].length){
                stateMap.showPushMembers[type].splice(index,1);
                $(".push-"+ type + "-people").empty().append(beopTmpl('wf_push_people_list',{type:type,data:stateMap.showPushMembers}));
            }
            overdueAttachEvents();
        });

        jqueryMap.$pushPeopleList.find('.push-time-select').change(function(){ 
            var $this = $(this),
                val = $this.val();

            var $pushTimeInput =  $this.closest('.push-time-select-box').siblings('.push-time-input-box').find('.push-time-input');
            var  $pushTimecontent = $this.closest('.push-time-content');
            var type = $pushTimecontent.attr('data-type'),
                index = $pushTimecontent.attr('data-index');
        
            addTimeParameter($pushTimeInput,$this,type,index);
            
        });

        jqueryMap.$pushPeopleList.off('blur.pushTimeInput').on('blur.pushTimeInput','.push-time-input',function(){ 
            var $this = $(this),
                value = $this.val();
            var $pushTimeSelect = $this.closest('.push-time-input-box').siblings('.push-time-select-box').find('.push-time-select');
            var $pushTimecontent = $this.closest('.push-time-content');
            var type = $pushTimecontent.attr('data-type'),
                index = $pushTimecontent.attr('data-index');
            var reg = /^[1-9]\d*$/;
            if(value && !reg.test(value)){
                alert(I18n.resource.appDashboard.workflow.PLEASE_ENTER_POSITIVE_INTEGER);
            }
            addTimeParameter($this,$pushTimeSelect,type,index);
        });
    };

    addTimeParameter = function($pushTimeInput,$pushTimeSelect,type,index){
        var selectVal = $pushTimeSelect.val(),
            inputVal = $pushTimeInput.val();
        if(selectVal != 'deadline'){
            if(inputVal){
                stateMap.showPushMembers[type][index][0].escalateAfter = inputVal + selectVal;
            }
            $pushTimeInput.prop('disabled',false);
        }else{
            stateMap.showPushMembers[type][index][0].escalateAfter = 'deadline';
            $pushTimeInput.val('').prop('disabled',true);
        }
    };

    //详情,编辑页面逾期流程:
    showOverdueProcess = function(data){
        var autoEscalationConfig = data.autoEscalationConfig;

        stateMap.showPushMembers = {
            normal:[],
            serious:[],
            urgent:[] 
        }
        
        if(autoEscalationConfig && autoEscalationConfig.length){
            autoEscalationConfig.forEach(function(item,index){
                if(item.targets && item.targets.length){
                    if(item.critical == 0){
                        for(var i = 0; i< item.targets.length; i++ ){
                            stateMap.showPushMembers.normal.push([{
                                escalateAfter: item.targets[i].escalateAfter,
                                id:  item.targets[i].userId,
                                useremail: item.targets[i].userInfo.useremail,
                                userfullname:item.targets[i].userInfo.userfullname,
                                userpic:item.targets[i].userInfo.userpic
                            }])
                        }   
                    }else if(item.critical == 1){
                        for(var i = 0; i< item.targets.length; i++ ){
                            stateMap.showPushMembers.serious.push([{
                                escalateAfter: item.targets[i].escalateAfter,
                                id:  item.targets[i].userId,
                                useremail: item.targets[i].userInfo.useremail,
                                userfullname:item.targets[i].userInfo.userfullname,
                    ...
                    }else if(item.critical == 2){
                        for(var i = 0; i< item.targets.length; i++ ){
                            stateMap.showPushMembers.urgent.push([{
                                escalateAfter: item.targets[i].escalateAfter,
                                id:  item.targets[i].userId,
                                useremail: item.targets[i].userInfo.useremail,
                                userfullname:item.targets[i].userInfo.userfullname,
        ...
        for( key in stateMap.showPushMembers){
            $(".push-"+ key + "-people").empty().append(beopTmpl('wf_push_people_list',{type:key,data:stateMap.showPushMembers})); 
        }
        setEditJqueryMap();
        overdueAttachEvents();
        I18n.fillArea($('#overdue-process-box'));
    ...
    changeSelsectPeopple = function(type,index,addedUserList){
        var newAddedUserList = [];
        var AddedUserObj = {};
        for(key in addedUserList[0]){
            if(key != 'escalateAfter'){
        ...
        newAddedUserList.push(AddedUserObj);
        if(stateMap.showPushMembers[type][index][0].escalateAfter){
            newAddedUserList[0].escalateAfter = stateMap.showPushMembers[type][index][0].escalateAfter;
        }
        stateMap.showPushMembers[type][index] = newAddedUserList;
        $(".push-"+ type + "-people").empty().append(beopTmpl('wf_push_people_list',{type:type,data:stateMap.showPushMembers}));
        overdueAttachEvents();
        I18n.fillArea($('#overdue-process-box'));
    };
    ...
    init = function ($container) {
        ...
        WebAPI.get(configMap.htmlURL).done(function (resultHtml) {
            ...
            WebAPI.get('/workflow/teamArch/').done(function (result) {
                if (result.success) {
                    stateMap.allMembers = result.data;
                    //如果为添加项目
                    ...
                    getGroupData(true).done(function (result) {
                        if (result.success) {
                            ...
                            showOverdueProcess(result.data);
                            commonInit(result);
                        }
                    }).fail(function () {
    ...
    setEditJqueryMap = function () {
        ...
        jqueryMap.$addPushPeople = jqueryMap.$container.find('.addPushPeople');
        jqueryMap.$pushPeopleList = jqueryMap.$container.find('.push-people-list');
    ...
    onAddTeamUserDialogOpen = function (ev) {
        ...
        if (!userHasSelected || !userMemberMap) {
            alert.danger('can\'t load data');
            console.error("加载任务选择框失败,userMemberMap或者userHasSelected不存在");
    ...
    onSubmit = function () {
        ...
        var $overdueProcessBox= $('#overdue-process-box');
        var $pushTimeInput = $overdueProcessBox.find('.push-time-input:not(:disabled)');
       

        for(var i= 0 ; i< $pushTimeInput.length; i++){
            var val = $pushTimeInput.eq(i).val();
            if(!val){
                alert(I18n.resource.appDashboard.workflow.DURATION_REQUIRED);
                return;
                break;
            }
        }
        ...
        //逾期参数
        var autoEscalationConfig = [];
        var autoEscalationObj ={
            critical: "",
            targets:[]
        };
        var showPushMembers = $.extend(true, {}, stateMap.showPushMembers);
        for(key in showPushMembers){
            if(showPushMembers[key].length){
                autoEscalationObj = {
                    critical: "",
                    targets:[]
                };
                if(key == overdueState.normal){
                    autoEscalationObj.critical = 0;
                }else if(key == overdueState.serious){
                    autoEscalationObj.critical = 1;
                }else if(key == overdueState.urgent){
                    autoEscalationObj.critical = 2;
                }
                showPushMembers[key].forEach(function(item){
                    autoEscalationObj.targets.push({
                            userId: item[0].id,
                            escalateAfter: item[0].escalateAfter
                    });
                });
                autoEscalationConfig.push(autoEscalationObj);
            }  
        }
        ...
        var model = {
            ...
            arch: undefined,
            process: undefined,
            autoEscalationConfig : autoEscalationConfig
        };
        ...
        if (stateMap.addGroup) {
            ...
            data.autoEscalationConfig = autoEscalationConfig;
