...
(function($, window, document, undefined) {
    ...
    function Plugin(element, options) {
        ...
        var searchText, pageNum = 1,
            pageSize, itemList, totalItems = [],
            searchOrder;
    ...
    $.extend(Plugin.prototype, {
        ...
        _requestAsyncData: function() {
            ...
            try {
                new RegExp("(" + self.search.searchText + ")(?![^<]*>|[^<>]*</)", "gi");
            } catch (e) {
                var searchTextTip = I18n.type === 'zh' ? '系统不支持 ' + self.search.searchText + ' 的搜索' : 'The system does not support the search for ' + self.search.searchText;
                alert.danger(searchTextTip);
                return false;
            }
        ...
        _createTable: function() {
            ...
            if (this.settings.isSearch) {
                if (this.settings.searchInput) {
                    ...
                    this.$searchIco = this.$searchInput.siblings('.icon.iconfont.searchIcon');
                } else {
        ...
        _initRow: function(rowData, lineNum) {
            ...
            if (this.settings.rowIdKey) {
                if (this.settings.rowPrtIdKey) {
                    html = '<tr prt="' + rowData[this.settings.rowPrtIdKey] + '" id="' + rowData[this.settings.rowIdKey] + '" ';
                } else {
                    html = '<tr id="' + rowData[this.settings.rowIdKey] + '" ';
                }
                if (this.settings.pointType) {
                    if (rowData[this.settings.pointType]) {
            ...
            for (var i = 0; i < colModel.length; i++) {
                ...
                if (col.isCopyValue && value) {
                    var title = I18n.type == "zh" ? "复制点名" : "Copy point name";
                    html += '<span class="copyPointName cp" title="' + title + '"><span class="icon iconfont"></span></span>';
                }
        ...
        _replaceHighlightText: function(value, key) {
            ...
            if (ret.indexOf('<') !== -1 || ret.indexOf('>') !== -1) {
                ret = ret.replace(new RegExp("<", "gi"), '&lt&');
                ret = ret.replace(new RegExp(">", "gi"), '@gt@');
            }
            var  index = keywordList.indexOf('\\D');

            if(index && index !==-1){
                keywordList.splice(index,1);
                keywordList.unshift('\\D');
            }
            ...
            if (ret.indexOf('&lt&') !== -1 || ret.indexOf('@gt@') !== -1) {
                if (keywordList.indexOf('<') !== -1) {
                    ret = ret.replace(new RegExp("&lt&", "gi"), '<span class="keywords-text"><</span>');
                } else {
                    ret = ret.replace(new RegExp("&lt&", "gi"), '<span><</span>');
                }
                if (keywordList.indexOf('>') !== -1) {
                    ret = ret.replace(new RegExp("@gt@", "gi"), '<span class="keywords-text">></span>');
                } else {
                    ret = ret.replace(new RegExp("@gt@", "gi"), '<span>></span>');
                }
            }
            return ret;
        },
        ...
        _attachEvent: function() {
            ...
            self.$el.on('click', '.table-body tr', function(e) { //单击table row
                ...
                if (e.ctrlKey) {
                    ...
                    if (!allSelectedRow.length) {
                        ...
                        var minIndex = Math.min($this.index(), selectedMinIndex),
                            maxIndex = Math.max($this.index(), selectedMaxIndex);
            ...
            self.$el.on('click', '.copyPointName', function(e) {
                if (self.settings.onCopyPointName && typeof self.settings.onCopyPointName === 'function') {
                    self.settings.onCopyPointName(this, $(this).closest('tr').data('value'), e);
                }
            });
            ...
            self.$el.on('click', '.checkbox-single', function(e) { // 第一列静态checkbox
                ...
                if ($(this).is(':checked')) {
                    var isALlSelectFlag = true,
                        $checkbox = self.$el.find('.checkbox-single');
        ...
        _makeHeaderFix: function() {
            ...
            function setPixelWidths(i, pixelWidth) {
                ...
                $headers.eq(i).attr('data-width', pixelWidth);
