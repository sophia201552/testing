    ...
    class ProjectPanel extends exports.BaseView {
                    ...
                    if (result.success) {
                        ...
                        for (var i = 0; i < result.data.dtulist.length; i++) {
                            ...
                            subList.push({
                                ...
                                'type': node.type,
                                'prefix': node.prefix,
                                'obixTag': node.obixTag || '',
                                'obixUrl': node.obixUrl || '',
                                'obixUser': node.obixUser || '',
                                'obixPsw': node.obixPsw || ''
                            });
                        }
                        _this.treeData = [{
                        ...
                        _this.transLateMsgCode(result.code);
                    }
                }).always(function () {
                    Spinner.stop();
                });
        ...
        _attachEvent() {
            ...
            $("#modbusContainer").off('click.exportLocalFile').on('click.exportLocalFile', '#exportLocalFile', function () {
                ...
                postData['projId'] = AppConfig.projectId;
                $.ajax({
                    url: postUrl,
                    type: 'post',
                    data: postData,
                    ...
                    if (result.success) {
                        ...
                        alert.danger(_this.transLateMsgCode(result.code));
                    }
                }).fail(function () {
                ...
                if ($target.closest('.removeNode').length) {
                    confirm(_this.i18n.CONFIRM_DELETING_DEVICE, function () {
                        WebAPI.post('/modbus/dtu/del', {
                            'dtuId': treeNode.id, projId: AppConfig.projectId
                        }).done(function (result) {
                            if (result.success) {
                                _this.zTreeInstance.removeNode(treeNode, true);
                                $box.empty();
                            } else {
                                alert(_this.transLateMsgCode(result.code));
                                Spinner.stop();
                            }
                        }).always(function () {
                            Spinner.stop();
                ...
                if ($target.closest('.edit').length) {
                    let $updateWin = $("#editTerminalPrefixWin"),
                        $editObixInfoBox = $("#editObixInfoBox");
                    $updateWin.find('.dtuName').val(treeNode.originalName);
                    $updateWin.find('.dtuPrefix').val(treeNode.prefix);
                    if (treeNode.type == 'obix') {
                        $updateWin.find('.url').val(treeNode.obixUrl);
                        $updateWin.find('.userName').val(treeNode.obixUser);
                        $updateWin.find('.pwd').val(treeNode.obixPsw);
                        $editObixInfoBox.show();
                    } else if (treeNode.type == 'modbus') {
                        $editObixInfoBox.hide();
                    }
        ...
        stopDTU(treeNode) {
            ...
            confirm(_this.i18n.IS_CONFIRM_STOP_DEVICE, function () {
                Spinner.spin(ElScreenContainer);
                WebAPI.post('/modbus/dtu/stop', {dtuId: treeNode.id, projId: AppConfig.projectId}).done(function (result) {
                    if (result.success) {
        ...
        runDTU(treeNode) {
            ...
            confirm(_this.i18n.IS_CONFIRM_ENABLE_DEVICE, function () {
                Spinner.spin(ElScreenContainer);
                WebAPI.post('/modbus/dtu/run', {dtuId: treeNode.id, projId: AppConfig.projectId}).done(function (result) {
                    if (result.success) {
                        alert.success(_this.i18n.ALREADY_ENABLE);
                        $("#dtuOpBtnGroup").find('.startUp').hide().end().find('.endDown').show();
                        treeNode.isRun = 1;
                    } else {
                        alert.danger(_this.transLateMsgCode(result.code) || _this.i18n.ALREADY_ENABLE);
                    }
                }).fail(function (msg) {
                    alert.danger(I18n.resource.common.SERVER_REQUEST_FAILED);
                }).always(function () {
