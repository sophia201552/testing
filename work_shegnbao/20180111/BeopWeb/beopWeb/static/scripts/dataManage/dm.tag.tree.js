(function(beop) {
    var initMap = {
        ...
        dealKeywordArr, showTagsStyle, requestDict, getEquipmentTags, getSameNames,FormatBrushZtreeOnClick;
    ...
    loadTree = function() {
        ...
        var zTreeSetting = {
            ...
            callback: {
                ...
                beforeAsync: function(treeId, treeNode) {
                    if (!treeNode) {
                        stateMap.isShowChildrenList = false;
                        configMap.isOnlyGroup = true;
                        Spinner.spin($('#tagTreeInfo')[0]);
                    }else{
                        if(treeNode.isParent){
                            if(configMap.isAlarm || configMap.calcPoint || configMap.diagnosis){
                                stateMap.isShowChildrenList = true;
                                configMap.isOnlyGroup = false;
                            }
                        }
                    }
                },
                onDrop: function(event, treeId, treeNodes, targetNode, moveType) {
                    ...
                    if (treeNodes && treeNodes.length && targetNode && targetNode.children) {
            ...
            async: {
                ...
                dataType: 'json',
                otherParam: {
                    'isOnlyGroup':function(a,b){
                        if(stateMap.isShowChildrenList){
                            return false;
                        }else{
                            return true;
                        }
        ...
        if(stateMap.treeType == treeType.tagTree){
            ...
            if (configMap.otherNodes && configMap.otherNodes && configMap.otherNodes.length) {
                ...
                if(configMap.calcPoint ){
                    setTimeout(function() {
                        configMap.newSpinner.stop();
                    }, 60);
                }
            }
        }
        ...
        $.ajax({
            ...
            if (result.success) {
                if (result.data && result.data.length) {
                    stateMap.stateMapEquipmentTags = result.data;
    ...
    showTreeItem = function() {
        ...
        var $tagTreeInfoHeader = $('#tagTreeInfoHeader');
        if (!configMap.calcPoint && !configMap.diagnosis && !configMap.isAlarm) {
            if (configMap.showALLNodes) {
                $tagTreeInfoHeader.css('background', '#29bb50');
            } else {
                $tagTreeInfoHeader.css('background', '#00a7f9');
            }
        }
    ...
    tagNodesAdd = function(nodes, reAsyncPrt, isRoot, addSource) {
        if (reAsyncPrt && !isRoot) {
            ...
            //新增根目录或者根目录显示的位置
            if (isRoot) {
                if (addSource && addSource == 'keywords') {} else {
    ...
    FormatBrushZtreeOnClick = function(event, treeId, treeNode){
    ...
    attachEvents = function() {
        var isFirstCopy = true,
            isFirstCopyTreeLoad = true,
            isFirstPasteTreeLoad = true;
        ...
        stateMap.$container.off('click.allNodesFolder').on('click.allNodesFolder', '#allNodesFolder', function() {
            ...
            var selectedNode = getSelectTreeNode()[0];
            if (selectedNode) {
                $.fn.zTree.getZTreeObj("tagTreeUl").cancelSelectedNode();
            }
        ...
        stateMap.$container.off('click.dragDirectory').on('click.dragDirectory', '#dragDirectory', function(e) {
            var $modalWin = $("#tagFormatBrushWin"),
                $tagCopyAndPasteBox = $("#tagCopyAndPasteBox");
            if (!isFirstCopy) {
                $modalWin.modal();
                return;
            }
            ...
            var copyTreeSetting = {
                ...
                callback: {
                    ...
                    onClick: FormatBrushZtreeOnClick
                },
                async: {
                    enable: true,
                    autoParam: ['_id=Prt', '_id'],
                    dataType: 'json',
                    otherParam: ['projId', _this.projectId, 'isOnlyGroup', !!configMap.isOnlyGroup],
                    url: '/tag/getThingTree',
                    dataFilter: function(treeId, parentNode, responseData) {
                        if (!responseData || !responseData.success || !responseData.data) {
                            Spinner.stop();
        ...
        stateMap.$container.off('click.autoMatch').on('click.autoMatch', '#autoMatch', function(e) {
            configMap.cb_on_click('autoMatch');
        });
        ...
        $("#tagFormatBrushConfirm").off().on('click', function(e) {
            ...
            var pasteTagIds = [],copyTagIds =[],
                copyTagNodes = stateMap.zTreeCopyInstance.getSelectedNodes(),
                pasteTagNodes = stateMap.zTreePasteInstance.getSelectedNodes();
            copyTagNodes.forEach(function(item){
                copyTagIds.push(item._id);
            })
            for (var i = 0; i < pasteTagNodes.length; i++) {
                pasteTagIds.push(pasteTagNodes[i]._id);
            }
            WebAPI.post('/tag/formatPainterV2', {
                'projId': _this.projectId,
                'sample': copyTagIds,
                'target': pasteTagIds
            }).done(function(result) {
        ...
        configMap.itemShowList.map(function(item) {
            if (item === 'search') {
                var returnData = [];
                _this.tagsArr = [];
                stateMap.stateMapEquipmentTags.map(function(item, index) {
                    var children = [];
