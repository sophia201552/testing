(function(beop) {
    var configMap = {
        ...
        enlargeFullScreen, batchAllPointsChange, tableCopyBoxStyle, getApiFolderNameList, positiveAppraise, negativeAppraise, dmComment, getRowDataType, saveEditAfterPoint;
    ...
    setPointErrorNum = function() {
        return WebAPI.post('/api/errorlog/countoflog', {
            ...
            if (result.success) {
                ...
                var $sheetBody = stateMap.$datatable.find('tbody'),
                    $sheetBodyClone = $sheetBody.clone(true);
    ...
    loadPointTable = function(page) { // 加载点表
        var $dmCommentModal = $('#dmCommentModal'),
            $referenceRecordModal = $('#referenceRecordModal');
        setJqueryMap();
        ...
        jqueryMap.$container.off('mousedown.dragChangeWidth').on('mousedown.dragChangeWidth', '#dragIconBox', function(e) {
            e.stopPropagation();
            var $cloudEdit, $treeBox, cloudEditWidth, treeBoxWidth, dragStartClientX, dragStartClientY, movingDistance;
            var dragging = true;
            $cloudEdit = jqueryMap.$cloudPointWrapper.find('.cloudEdit');
            $treeBox = jqueryMap.$cloudPointWrapper.find('.treeBox');
            dragStartClientX = e.clientX;
            dragStartClientY = e.clientY;
            cloudEditWidth = $cloudEdit.outerWidth();
            treeBoxWidth = $treeBox.outerWidth();
            $(window).off('mousemove.dragChangeWidth').on('mousemove.dragChangeWidth', function(e) {
                if (dragging) {
                    movingDistanceX = e.clientX - dragStartClientX;

                    if (treeBoxWidth - movingDistanceX <= 230) {
                        $cloudEdit.css('width', cloudEditWidth + treeBoxWidth - 230);
                        $treeBox.css('width', 230);
                    } else {
                        $cloudEdit.css('width', cloudEditWidth + movingDistanceX);
                        $treeBox.css('width', treeBoxWidth - movingDistanceX);
                    }
                }
            }).one('mouseup.dragChangeWidth', function() {
                dragging = false;
            });
        });
    ...
    batch_history_win = function() {
        ...
        if (localStorage.getItem('batchHistoryGenerateType') == 'point') {
            ...
            jqueryMap.$batchHistoryTimeStart.val(localStorage.getItem('batchHistoryStartTimePoint') ? localStorage.getItem('batchHistoryStartTime') : new Date().format('yyyy-MM-dd HH:mm'));
            ...
            jqueryMap.$batchHistoryTimeStart.val(localStorage.getItem('batchHistoryStartTime') ? localStorage.getItem('batchHistoryStartTime') : new Date().format('yyyy-MM-dd HH:mm'));
            jqueryMap.$batchHistoryTimeEnd.val(localStorage.getItem('batchHistoryEndTime') ? localStorage.getItem('batchHistoryEndTime') : new Date().format('yyyy-MM-dd HH:mm'));
    ...
    online_test_request = function(content, pointName, moduleName) { // 在线测试请求
        ...
        var url = beop.dm.constants.EXPERT_SERVICE_URL,
            sendData = {
    ...
    loadPointDetailPage = function() {
        ...
        if (stateMap.pointType == configMap.cloudPointType.MAPPING_POINT) {
            ...
            var $pointMapping = $("#point_mapping"),
            ...
            stateMap.newSpinner = new LoadingSpinner({ color: '#00FFFF' });
            stateMap.newSpinner.spin($('#cloudPointWrapper').get(0));
            loadTree();
    ...
    onEditPoint = function() {
        ...
        stateMap.editPointBeforData = data;
    ...
    confirmCalcPointRequest = function(url, pointModel) {
        ...
        points.forEach(function(point) {
            ...
            var one_point_request;
            var opt,
                historyTestTime = $("#historyTestTimePicker").val() || '';
            if (!stateMap.editPointBeforData || point.value != stateMap.editPointBeforData.value || point.logic != stateMap.editPointBeforData.params.logic) {
                one_point_request = online_test_request(point['logic'], point['value'], point['moduleName']).then(function(resultDict) {
                    opt = {};
                    opt.historyTestTime = historyTestTime;
                    opt.newDate = new Date().format('yyyy-MM-dd HH:mm:ss');
                    if (parseInt(resultDict.error) == 0) {
                        saveEditAfterPoint(opt, url, point);
                    } else {
                        if (stateMap.isMultiple) {
                ...
                opt.historyTestTime = historyTestTime;
                opt.newDate = new Date().format('yyyy-MM-dd HH:mm:ss');
                one_point_request = saveEditAfterPoint(opt, url, point);
            }
        ...
        $.when.apply(null, requestPromises).done(function() {
            stateMap.editPointBeforData = null;
            Alert.success(document.body, I18n.resource.debugTools.info.MODIFY_SUCCESS).showAtTop(2000);
            if (stateMap.operateType == configMap.operateType.SKIP) {
                window.location.href = '#page=PointManagerCloudPoint&pointType=' + stateMap.pointType + '&projectId=' + AppConfig.projectId;
            } else {
    ...
    saveEditAfterPoint = function(opt, url, point) {
        return $.post(url, point).then(function(result) {
            if (result.success) {
        ...
        if (stateMap.pointType == configMap.cloudPointType.MAPPING_POINT) {
            dataTableOptions = $.extend(dataTableOptions, {
                ...
                colModel: [
                    ...
                    { index: 'params.mapping.point', width: '20%', highlight: true },
    ...
    importSheet = function(event) {
        ...
        $.ajax({
            ...
            if (result.success) {
                ...
                if (result.code == 2) {
                    alert.danger(i18n_resource.dataManage.IMPORT_NAME_REPEAT_INFO1 + '\n' +
                        i18n_resource.dataManage.IMPORT_NAME_REPEAT_INFO2 +
                        '<span style="color: #d9534f;">' + result.data.duplicated.toString() + '</span>');
                } else {
                    alert.danger(result.msg || i18n_resource.common.REQUEST_ERROR);
                }
            }
    ...
    loadTree = function() {
        stateMap.tagTreePromise.done(function() {
            ...
            beop.tag.tree.configModel({
                ...
                isLoadCache: true,
                isAlarm: false,
                newSpinner: stateMap.newSpinner,
