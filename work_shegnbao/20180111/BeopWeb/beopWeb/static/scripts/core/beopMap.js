...
(function (beop) {
    ...
    BeopMap.prototype.openProjectsCard = function () {
        ...
        $projectContainer.append('<div id="AddProjectInHomePage" class="media effect">+</div>');
        configMap.isFirstComeIn = false;
        this.$projectsCard.show();
        if (AppConfig.beopCreateNewProject) {
            $projectContainer.closest('.projectsContainer').scrollTop(10000);
    ...
    BeopMap.prototype.attachPanelEvent = function () {
        ...
        var searchMain = function () {
            ...
            _this.addMarkers(AppConfig.projectList);
        };
        ...
        $('.map-card, .map-project-selector').on('click', '.media', function () {
            ...
            if (AppConfig.projectCurrent && AppConfig.projectCurrent.i18n == true){
                I18n.getProjectI18n(projectId,AppConfig.language).done(()=>{
            ...
            var pId = $(this).closest('.effect').attr('project-id');
            _this.openInfoWindow(_this.infoWindows[pId], _this.markers[pId]);
            e.stopPropagation();
        });
    };
    ...
    BeopMap.prototype.getInfoWindowContent = function (project) {
        var info = [];
        info.push("<div class='projectNameWindow' project-id='" + project.id + "'><b class='media' project-id='" + project.id + "'>" + StringUtil.getI18nProjectName(project) + "</b>");
        if (project.online) {
    ...
    BeopMap.prototype.addMarkers = function (markerCollection, isForce) {
        ...
        for (var m = 0; m < markerCollection.length; m++) {
            ...
            marker = this.addMarker(item.id, item.latlng, item.online);
            if (!marker) {
                continue;
            }
            infoWindow = this.addInfoWindow(item.id, this.getInfoWindowContent(item));
            let $projectInfoWindow = $('#map-canvas').find('.projectNameWindow:visible');
            if ($projectInfoWindow.length) {
                let selectedId = $projectInfoWindow.attr('project-id');
                if (selectedId == item.id) {
                    $projectInfoWindow.find('.projectNameWindowOnlineStatus').text(I18n.resource.admin.projectSelector.PROJECT_STATUS + " : " + item.online);
                    if ($projectInfoWindow.find('.projectNameWindowLastReceivedTime').length) {
                        $projectInfoWindow.find('.projectNameWindowLastReceivedTime').text(I18n.resource.admin.projectSelector.PROJECT_LAST_RECEIVED_TIME + " : " + item.lastReceivedTime);
                    }
