...
var EnergyScreen = (function() {
    function EnergyScreen(id, store, shareSaveChange, dsInfoList, isForMobile) {
        ...
        this.isForMobile = this.options.isForMobile ? this.options.isForMobile || AppConfig.isMobile : AppConfig.isMobile;
    }
    ...
    EnergyScreen.prototype = {
        ...
        close: function() {
            ...
            document.getElementsByTagName('html')[0].classList.remove('mobilePage')
        ...
        initToolBar: function() {
            ...
            if (this.isForReport) {
                ...
                if (AppConfig.permission && AppConfig.permission.DBWrite) {
                    var $liConfig = $('<li class="iconWrap" id="btnConfigDashboard" permission="DBWrite"><a><span class="glyphicon glyphicon-cog"></span><span class="dropdownNav">' + I18n.resource.observer.widgets.CONFIG_MODE + '</span> </a></li>');
                    var btnMode = $('<span>')
                        .addClass('badge grow')
        ...
        initLayout: function() {
            if (this.options.isForMobile) {
                document.getElementsByTagName('html')[0].classList.add('mobilePage')
                this.$obCanvasContainer.addClass('forMobile');
            }
            this.listEntity = {};
            this.arrEntityOrder = [];
                    ...
                    if (entity.entity.modal && entity.entity.modal.points) {
                        ...
                        if (entity.entity.modal.type == "ModalAppGauge") {
                            arrUpdataData.push({
                                ...
                                'guageMax': entity.entity.modal.option.guageMax,
        ...
        saveLayout: function() {
            ...
            for (var i = 0; i < this.arrEntityOrder.length; i++) {
                ...
                } else if (entity.modal.type == 'ModalAppButton') {
                    if (entity.modal.option && entity.modal.option.appButton && entity.modal.option.appButton.length != 0) {
                        arrEntity.push(entity);
                    }
            ...
            WebAPI.post('/spring/saveLayout', data).done(function(result) {
                if (_this.isForReport) {
                    ...
                    if (_this.options.isForMobile) {
                        ScreenManager.applyScreen(EnergyScreen, null, null, null, null, true);
                    } else {
                        ScreenManager.applyScreen(EnergyScreen);
                    }
                }
        ...
        saveLayoutOnly: function() {
            ...
            Spinner.spin(ElScreenContainer);
            WebAPI.post('/spring/saveLayout', data).done(function(result) {}).always(function(result) {
                Spinner.stop();
            });
        },
        ...
        replaceEntity: function(sourceId, targetId, parentId) {
            ...
            if (source.entity.modal.type == 'ModalMix') {
                source.entity.modal.option && source.entity.modal.option.subChartIds && source.entity.modal.option.subChartIds.forEach(function(i) {
                    var entity = _this.listEntity[i.id].entity,
                        modelClass, item;
        ...
        showConfigMode: function() {
            ...
            if (!this.modalConfigPane) {
                this.modalConfigPane = new modalConfigurePane(document.getElementById('energyModal'), this, 'dashboard');
        ...
        initToolBox: function() {
            ...
            var $toolBoxBody = $toolBox.find('.panel-container');
            var $modals = $('<div id="modalCt" class="panel-body">').appendTo($toolBoxBody);
        ...
        initReplay: function() {
            ...
            btnTimeShaft.eventOn('click', function(e) {
                if (_this.isPlayback) {
                    trackEvent('顶部导航数据回放-DashBoard收缩','TopNav.BackTrace.DashBoard.Close') 
                    ...
                    trackEvent('顶部导航数据回放-DashBoard展开','TopNav.BackTrace.DashBoard.Open')  
        ...
        renderData: function(dataList, curTm, startTm, endTm) {
            var data = {};
            data.dsItemList = dataList;
