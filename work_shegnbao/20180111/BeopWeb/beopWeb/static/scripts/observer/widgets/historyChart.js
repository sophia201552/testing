...
var HistoryChart = (function () {
    function HistoryChart(obj) {
        ...
        this.dtuId = obj.dtuId || '';
        this.dtuName = obj.dtuName || '';
    ...
    HistoryChart.prototype = {
        ...
        loadTree: function () {
            ...
            var nodes = this.zTreeInstance.getNodes()[0].children;
            this.showTagsStyle(nodes);
        },
        ...
        showTagsStyle: function (allNodes) {
            ...
            for (var i = 0; i < allNodes.length; i++) {
                ...
                if (allNodes[i].pId == 1 && allNodes[i].tagBox) {
        ...
        changeToZTreeNodes: function () {
            ...
            if (this.tagList && this.tagList.length) {
                for (var i = 0; i < this.tagList.length; i++) {
                    var tagArr = this.tagList[i];
                    for (var k = 0; k < this.treeList.length; k++) {
                        if (tagArr.pointName == this.treeList[k].name && this.treeList[k].pId == 1) {
                            this.treeList[k].tag = tagArr.tagList;
                            if (tagArr.tagList && tagArr.tagList.length) {
                                this.treeList[k].tagBox = tagBoxHtml(tagArr.tagList)[0].outerHTML;                                
                            }
                        }
                    }
                }
            ...
            function tagBoxHtml(tagList) {
                var $htmlBox = $('<span class="tagBox"></span>');
                var html = '';
                for (var i = 0; i < tagList.length; i++) {
                    html += '<span class="treeTagItem active" data-tag="' + tagList[i].toLocaleLowerCase() + '" ' +
                    'data-toggle="tooltip" data-placement="right">' + tagList[i] + '</span>';
                }
                $htmlBox.html(html);
                return $htmlBox;
            }
        ...
        fetchData: function () {
            ...
            if (_this.dtuId) { // 终端调试
                ...
                postMap.dtuId = _this.dtuId;
                postMap.dtuName = _this.dtuName;
            } else { // 其它 (数据管理...)
                url = '/get_history_data_padded';
                postMap.projectId = _this.projectId;
            }
            WebAPI.post(url, postMap).done(function (data) {
        ...
        refreshChart: function ($container) {
            ...
            for (var pointName in this.historyData.data) {
                if (this.historyData.data.hasOwnProperty(pointName)) {
                    ...
                    var showSeriesData = seriesData.map(function (item) {
                    ...
                    var iLen = showSeriesData.length - 1;
                    var isShowHistory = false;
                    var beforeValue, afterValue;
                    var indexArr = [], newPointData = [];
                    for (var i = 0; i < showSeriesData.length; i++) {
                        if (i !== 0) {
                            beforeValue = showSeriesData[i - 1];
                        }
                        if (i !== iLen) {
                            afterValue = showSeriesData[i + 1];
                        }
                        if (showSeriesData[i]) {
                            if (i == 0) {
                                if (!afterValue) {
                    ...
                    showSeriesData.map(function (item, index) {
                    ...
                    if (isShowHistory && _this.hidePaddingData) {
                        series.push({
                            name: pointName,
                            type: 'line',
                            showAllSymbol: true,
            ...
            var option = {
                tooltip: {
                    ...
                    formatter: function (value) {
                        var valueObj = {};
                        var dom = ``;
                        value.length && value.forEach(function (item) {
                            if (!valueObj[item.seriesName]) {
                                valueObj[item.seriesName] = item.seriesName;
                                //var tagArr = seriseTag[item.seriesName] 
                                dom += `<div style='padding-left:10px;padding-right:10px;padding-top:8px;'>
                                <span style='border-radius:9px;display:inline-block;width:8px;height:8px;background:` + item.color + `;'></span>
                                <span style="display:inline-block;padding-left:12px;">`  + item.seriesName + `</span>:` + ' ' + 
                                `<span style="display:inline-block;padding-left:1px;">` + item.value + `</span>`;
                                if (tagArr && tagArr.length) {
                                    for (var i = 0; i < tagArr.length; i++) {
                                        if (tagArr[i].pointName == item.seriesName) {
                                            var tag = tagArr[i].tagList;
                                            if (tag && tag.length) {
                                                var div = `<div style="margin: 3px 0 0 12px;">`;
                                                for (var k = 0; k < tag.length; k++) {
                                                    div += `<span style="display:inline-block !important; padding: 0 5px; border-radius: 6px; background: #29bb4f; color: #000; margin-right: 8px;">`+ tag[k] +`</span>`;
                                                }
                                                div += `</div>`;
                                                dom += div;
                                            }
                    ...
                    feature: {
                        ...
                        dataView: {
                            ...
                            optionToContent: function (opt) {
                                $('.historyDataTable').closest('div').css('overflow','auto');
                                //图例切换为数据视图
                                var axisData = opt.xAxis && opt.xAxis[0].data,
                                    seriesObj = {};
                                var series =[];
                                opt.series.forEach(function(item){
                                    if(!seriesObj[item.name]){
                                        seriesObj[item.name] = item.name;
                                        series.push(item);
                                    }
                                });
                                //series = opt.series;
                                //var title = '<p style=" text-align: center; font-size: 15px;font-weight: bold;">' + opt.title.text + '</p>';
                                var html = '<div class="historyDataTable gray-scrollbar"><table  class="table table-bordered table-hover table-striped" style="-webkit-user-select:text; user-select:text; width: 80%;margin: 0 auto;"><tbody>';
            ...
            $container.css('width', ($container.parent().width() * 0.83).toFixed(1) - 2 + 'px');
