var AlarmLogging = (function () {
    ...
    AlarmLogging.prototype = {
        show: function () {
            ...
            WebAPI.get("/static/views/observer/widgets/alarmLogging.html").done(function (resultHtml) {
        ...
        init: function () {
            ...
            // Deprecate
            this.$iptEndTime.val( new Date().format('yyyy-MM-dd HH:mm:ss') ).datetimepicker('update');
            // start auto load
            this.startAutoload();
        ...
        attachEvents: function () {
            ...
            this.$btnCog.click(function (e) {
                ...
                WebAPI.getJSON('/warning/getConfig/'+AppConfig.projectId, function (rs) {
            ...
            this.$tbAlarm.on('click', '.dropdown-menu>li', function (e) {
                ...
                var deadline = new Date(new Date().valueOf()+minutes*60*1000).format('yyyy-MM-dd HH:mm:ss');
            ...
            this.$btnRuleSubmit.click(function () {
                ...
                if(p.warningtype !== alarmMode.LIMIT_VALUE) {
                    ...
                    // valid it
                    valid = _this.validator.not('limitValueInfo').valid();
                } else {
                    ...
                    p.boolwarninginfo = '';
                    // valid it
                    valid = _this.validator.not('alarmInfo').valid();
                }
                ...
                valid.done(function () {
                    ...
                    $.post(url+AppConfig.projectId, p)
                        .done(function (rs) {
                            if(rs.status !== 0) {
                                alert('Sorry, operation faild, we will fix this as soon as possible.');
                                return;
                            }
                            // reload table
                            _this.$tbRule.table('loading');
        ...
        setDelayTime: function (row, delayTime) {
            ...
            var datetimeNow = new Date().format('yyyy-MM-dd HH:mm:ss');
        ...
        setModal: function (data) {
            // reset validtor status
            this.validator.resetStatus();
            // reset value
            ...
            // 对 checkbox 的状态进行刷新
            util.checkbox.keepOneCheckbox( this.$divRuleEnable.find(':checkbox') );
