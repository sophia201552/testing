var modalConfigurePane = (function () {
    ...
    modalConfigurePane.prototype = {
        ...
        initOption: function () {
            ...
            var $panelUnit = $('#panelUnit');
            var $divGaugeUnit = $panelUnit.parent();
            ...
            var totalModeType = {
                    ...
                    gauge: [$divGauge, $divGaugeMode, $divGaugeUnit],
                    weather: [],
            ...
            function initMode(mode) {
                ...
                var $addTimeDot = $('#addTimeDot');
                 $('.inputCompareBox').empty().remove();
                if (_this.option.modeUsable && _this.option.modeUsable[0] && (_this.option.templateType === 'AnlzHistoryCompare'||_this.option.templateType === 'AnlzHistoryCompare_Line')) {
                    $addTimeDot.show();
                    var timeList,defaultTimeArr;
                    if($inputMode.val()=='easyCompareAnalyz'){
                        timeList = _this.screen.curModal.easyTimeObj?_this.screen.curModal.easyTimeObj.timeList:undefined;
                        defaultTimeArr = _this.screen.curModal.easyTimeObj?_this.screen.curModal.easyTimeObj.defaultTimeArr:undefined;
                    }else if($inputMode.val()=='compareSensor'){
                        timeList = _this.screen.curModal.sensorTimeObj?_this.screen.curModal.sensorTimeObj.timeList:undefined;
                        defaultTimeArr = _this.screen.curModal.sensorTimeObj?_this.screen.curModal.sensorTimeObj.defaultTimeArr:undefined;
                    }else{
                        timeList = _this.screen.curModal.meterTimeObj?_this.screen.curModal.meterTimeObj.timeList:undefined;
                        defaultTimeArr = _this.screen.curModal.meterTimeObj?_this.screen.curModal.meterTimeObj.defaultTimeArr:undefined;
                    }
                    if(!_this.optionType){
                        if(defaultTimeArr&&defaultTimeArr.length==2){
                            $('#inputCompareDate1').val(new Date(defaultTimeArr[0]).format('yyyy-MM-dd 00:00'));
                            $('#inputCompareDate2').val(new Date(defaultTimeArr[1]).format('yyyy-MM-dd 00:00'));
                        }
                        $('#inputComparePeriod').val(_this.screen.curModal.comparePeriod);
                    }
                    if(_this.option.optionPara.dataItem.length!==0&&timeList&&timeList.length>0){
                        for(var i = 0;i<timeList.length;i++){
                            var item = timeList[i];
                            if(item!=new Date(defaultTimeArr[0]).format('yyyy-MM-dd 00:00:00')&&item!=new Date(defaultTimeArr[1]).format('yyyy-MM-dd 00:00:00')){
                                var $divCompareDatePa = $('#divCompareDate');
                                var count = $divCompareDatePa.find('.form-group').length+1;
                                var timeDom = '<div class="form-group col-xs-6 inputCompareBox">\
                                            <label for="inputCompareDate'+count+'"><span i18n="modalConfig.option.LABEL_COMPARE_DATE"></span>'+count+'</label>\
                                            <input type="text" class="form-control form_datetime compare inputCompareDateList" id="inputCompareDate'+count+'" placeholder="" data-format="yyyy-mm-dd" datetimepicker>\
                                            <span class="glyphicon glyphicon-remove timeRemove"></span>\
                                        </div>';
                                $(timeDom).insertBefore($('#addTimeDot'));
                                I18n.fillArea($divCompareDatePa);
                                var $inputCompareDateList = $('#inputCompareDate'+count);
                                $inputCompareDateList.datetime();
                                var now = new Date();
                                $inputCompareDateList.val(new Date(item).format('yyyy-MM-dd HH:mm'));
                            }
                        }
                    }
                }else{
            ...
            if (_this.option.optionPara.scaleList) {
                //设高温异常还是低温异常
            if (_this.option.optionPara.scaleList[3] > _this.option.optionPara.scaleList[0]) {
                $('#gaugeMode option').eq(0).attr("selected", "selected").siblings().remove("selected");
                sessionStorage.setItem('gaugeMode', $gaugeMode.val())
            } else {
                $('#gaugeMode option').eq(1).attr("selected", "selected").siblings().remove("selected");
                sessionStorage.setItem('gaugeMode', $gaugeMode.val())
            }  var newScaleList=[]
           //解决点击设置后不保存 scalelist会被改变
               newScaleList=_this.option.optionPara.scaleList.slice();
                newScaleList.sort(function (a, b) {
                    return a > b ? 1 : -1
                });
            }
            ...
            if (_this.optionType && sessionStorage.getItem('gaugeLowerLimit') != null) {
                ...
                $gaugeLowerLimit.val(newScaleList[0])
            }
            ...
            if (_this.optionType && sessionStorage.getItem('gaugeUpperLimit') != null) {
                ...
                $gaugeUpperLimit.val(newScaleList[3])
            }
            ...
            if (_this.optionType && sessionStorage.getItem('normalLowerLimit') != null) {
                ...
                $normalLowerLimit.val(newScaleList[1])
            }
            ...
            if (_this.optionType && sessionStorage.getItem('normalUpperLimit') != null) {
                ...
                $normalUpperLimit.val(newScaleList[2])
            }
            $normalUpperLimit.change(function () {
                sessionStorage.setItem('normalUpperLimit', $normalUpperLimit.val())
            });
            if (_this.optionType && sessionStorage.getItem('panelUnit') != null) {
                $panelUnit.val(sessionStorage.getItem('panelUnit'));
            } else if (!_this.optionType && _this.option.optionPara.panelUnit) {
                $panelUnit.val(_this.option.optionPara.panelUnit);
            }
            $panelUnit.change(function () {
                sessionStorage.setItem('panelUnit', $panelUnit.val())
            });
            ...
            //时间节点增加按钮
            $('#addTimeDot').off('click').click(function(){
                var $divCompareDatePa = $(this).parent();
                if($(this).siblings().length==20){
                    alert(I18n.resource.modalConfig.option.TIME_LIMIT);
                    return;
                }
                var count = $divCompareDatePa.find('.form-group').length+1;
                var timeDom = '<div class="form-group col-xs-6 inputCompareBox">\
                ...
                $inputCompareDateList.datetime();
                var now = new Date();
                $inputCompareDateList.val(new Date(now.getFullYear(), now.getMonth() - count).format('yyyy-MM-dd HH:mm'));
                _this.timeRemove();
            })
            _this.timeRemove();
        },
        timeRemove:function(){
            // 时间节点删除按钮
            $('.timeRemove').off('click').on('click',function(){
                var $this = $(this);
                var $timeParent = $this.parent();
                var timeNeedRemove = $this.prev('.inputCompareDateList').val();
                var $inputModal = $('#inputMode');
                var timeListArr;
                if($inputModal.val()=='easyCompareAnalyz'){
                    timeListArr = _this.screen.curModal.easyTimeObj.timeList;
                }else if($inputModal.val()=='compareSensor'){
                    timeListArr = _this.screen.curModal.sensorTimeObj.timeList;
                }else{
                    timeListArr = _this.screen.curModal.meterTimeObj.timeList;
                }
                for(var i = 0;i<timeListArr.length;i++){
                    if(timeListArr[i].indexOf(timeNeedRemove)>=0){
                        timeListArr.splice(i,1)
                    }
                }
                $timeParent.empty().remove();
            })
        },
        initConfigData: function () {
            ...
            for (var i = 0; i < _this.option.rowDataType.length; ++i) {
                strConfigModalBody += '<div class="divConfigData" dataType="' + _this.option.rowDataType[i] + '">\
                    ...
                    <div class="row rowDataValue"><div class="dataDragTip col-lg-3 col-xs-4"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></div></div>\</div>';
            }
            ...
            if (_this.option.modeUsable && _this.option.modeUsable[0] && _this.option.templateType === 'ModalAppGauge') {
                ...
                $('#appGuageMax').val(modalOption ? modalOption.guageMax : '');
            ...
            for (i = 0; i < _this.option.optionPara.dataItem.length; ++i) {
                ...
                for (var j = 0; j < _this.option.optionPara.dataItem[i].dsId.length; ++j) {
                    ...
                    var originName = EventAdapter.getData() ? EventAdapter.getData().originName : "";
                    strDSConfig = '<div class="col-lg-3 col-xs-4 divDSConfigure grow ' + loseJudge + '" dsid="' + _this.option.optionPara.dataItem[i].dsId[j] + '" foldername="' +originName+ '"><span class="contentDS" title="' + _this.option.optionPara.dataItem[i].dsName[j] + '">' +
            ...
            if (_this.dataLose) {
                new Alert($("#configAlert"), "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE11 + "</strong>").show(1000);
            }
        ...
        initChartPtCog: function (index) {
            ...
            $('#btnPtCogSure').off('click').click(function () {
                ...
                if (isNaN(Number($inputUpper.val())) || isNaN(Number($inputLower.val())) || isNaN(Number($inputAccuracy.val())) || isNaN(Number($inputLineVal1.val())) || isNaN(Number($inputLineVal2.val())) || isNaN(Number($inputLineVal3.val())) || isNaN(Number($inputLineVal4.val()))) {
                    new Alert($("#configAlert"), "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE10 + "</strong>").show(1000);
                    return;
                }
                if ($inputUpper.val() != '' && $inputLower.val() != '' && Number($inputUpper.val()) < Number($inputLower.val())) {
                    new Alert($("#configAlert"), "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE12 + "</strong>").show(1000);
                    return;
                }
        ...
        initDrag: function () {
            ...
            $divConfigData.on('drop', function (e, arg) {
                ...
                if (_this.option.templateType === "ModalAPPMonthHistory" || _this.option.templateType === "ModalAppDiagRanking" || _this.option.templateType === "ModalDataMonitorList") {
                    ...
                    if (dsItemLength === 2) {
                        new Alert($("#configAlert"), "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE13 + "</strong>").show(1000);
                        return;
                    }
                }
                ...
                if (Object.prototype.toString.call(targetId) === '[object Array]') {
                    ...
                    for (var i = 0; i < len; i++) {
                        if ($rowTempData.find('.divDSConfigure[dsid="' + targetId[i] + '"]').length > 0) {
                            ...
                            targetContent = AppConfig.datasource.getDSItemById(targetId[i]).value;
                    ...
                    if (targetId.length < 1) {
                        new Alert($("#configAlert"), "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE2 + "</strong>").show(1000);
                    } else {
                        $('#startConfig').removeClass('disabled');
                    }
                } else {
                    var info = AppConfig.datasource.getDSItemById(targetId);
                    //targetContent = info.alias === "" ? info.value : info.alias || EventAdapter.getData().originName;
                    targetContent =  info.value || EventAdapter.getData().originName;
                    if ($rowTempData.find('.divDSConfigure[dsid="' + targetId + '"]').length > 0) {
                        new Alert($("#configAlert"), "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE2 + "</strong>").show(1000);
                        return;
                    }
                    var strDSConfig = '<div class="col-lg-3 col-xs-4 divDSConfigure grow" dsid="' + targetId + '" foldername="' + EventAdapter.getData().originName + '"><span class="contentDS" title="' + targetContent + '">' +
                        targetContent +
                        '</span><span class="glyphicon glyphicon-remove btnRemoveDS" aria-hidden="true" ></span></div>';
                    $dataDragTip.before(strDSConfig);
                    $(strDSConfig).tooltip({
        ...
        initConfigStart: function () {
            ...
            document.getElementById('startConfig').onclick = function () {
                ...
                if(_this.modalType){trackEvent('数据分析新建-'+_this.modalType,' DataAnalysis.'+_this.modalType+'.Create');}
                var tempStartTime, tempEndTime, tempPeriodTime,timeListArr = [],defaultTimeArr = [];
                var timeRecent;
                var tempInterval = 0;
                var $inputPeriodValue = $('#inputPeriodValue');
                ...
                var getTagPoint = function (id) {
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        data: {
                            projId: beop.tag.externalTree.getProjectId(),
                            isOnlyGroup: false,
                            Prt: id
                        }
                ...
                for (var i = 0; i < $rowDataValue.length; ++i) {
                    ...
                    for (var j = 0; j < $rowDataValue[i].children.length - 1; ++j) {
                        var folderName = $rowDataValue[i].children[j].getAttribute('folderName');
                        if (folderName && folderName!== 'undefined') {
                            getTagPoint($rowDataValue[i].children[j].getAttribute('dsid'));
                            tempDSId.push.apply(tempDSId, ids);
                            ids = [];
                        } else {
                ...
                switch ($inputModal.val()) {
                    case totalModeType['fixed']:
                        ...
                        if (tempStartTime.toDate() >= tempEndTime.toDate()) {
                            new Alert($("#configAlert"), "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE3 + "</strong>").show(1000);
                            return;
                        }
                        if (tempEndTime.toDate() >= new Date()) {
                            new Alert($("#configAlert"), "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE4 + "</strong>").show(1000);
                            return;
                        }
                        break;
                    ...
                    case totalModeType['gauge']:
                        ...
                        if (tempArr.toString() != scaleList.toString()) {
                            new Alert($("#configAlert"), "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE5 + "</strong>").show(1000);
                            return;
                        }
                        break;
                ...
                if(_this.option.templateType=='AnlzHistoryCompare'||_this.option.templateType=='AnlzHistoryCompare_Line'){
                    defaultTimeArr.push($inputCompareDate1.val());
                    defaultTimeArr.push($inputCompareDate2.val());
                    var $divCompareDate = $('#divCompareDate');
                    var $CompareDateForm = $divCompareDate.find('.form-group');
                    if($CompareDateForm.length>2){
                        for(var i = 0;i<$CompareDateForm.length;i++){
                            if(tempPeriodTime=='week'){
                                timeListArr.push(new Date($CompareDateForm.eq(i).find('.compare ').val()).format('yyyy-MM-dd 00:00:00'));
                            }else{
                                timeListArr.push(initCompareTime(tempPeriodTime, $CompareDateForm.eq(i).find('.compare ').val()));
                            }
                        }
                        timeListArr.sort(function(a,b){ return new Date(b).valueOf()-new Date(a).valueOf()})
                        tempStartTime = timeListArr[timeListArr.length-1];
                        tempEndTime = timeListArr[0];
                    }
                }
                //根据对比周期获取对比开始时间
                ...
                if (_this.modalType == "dataAnalysis") {
                    ...
                    if(_this.option.templateType=='AnlzHistoryCompare'||_this.option.templateType=='AnlzHistoryCompare_Line'){
                        if($inputModal.val()=='easyCompareAnalyz'){
                            _this.screen.curModal['easyTimeObj'] = {
                                timeList:timeListArr,
                                defaultTimeArr:defaultTimeArr
                            }
                        }else if($inputModal.val()=='compareSensor'){
                            _this.screen.curModal['sensorTimeObj'] = {
                                timeList:timeListArr,
                                defaultTimeArr:defaultTimeArr
                            }
                        }else{
                            _this.screen.curModal['meterTimeObj'] = {
                                timeList:timeListArr,
                                defaultTimeArr:defaultTimeArr
                            }
                        }
                    ...
                    if (_this.option.modeUsable && _this.option.modeUsable[0] && _this.option.templateType === 'ModalAppGauge') {
                        ...
                        var guageMax = $('#appGuageMax').val().trim();
                    ...
                    panelUnit && (option.panelUnit = panelUnit);
                    ...
                    if (_this.option.templateType === 'ModalAppGauge') {
                        ...
                        option['guageMax'] = guageMax !== '' ? guageMax : null;
