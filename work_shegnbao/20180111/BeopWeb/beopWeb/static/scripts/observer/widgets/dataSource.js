    ...
    DataSource.prototype = {
        ...
        initTreeNavStyle: function () {
            WebAPI.get('/static/views/observer/dataSource.html').done(function (resultHtml) {
                ...
                var $PageTag = _this.$dsNavContain.find('#pageTag');
                //主页和factory不同处理
                AppConfig.isFactory && _this.$dsNavContain.find('#liMine').css('display', 'none').attr('class', '');
                AppConfig.isStratefy && _this.$dsNavContain.find('#liMine').css('display', 'none').attr('class', '');
                //开放点击 我的 的功能
                _this.$dsNavContain.find('#liMine').click(function (e) {
                    $('#liMine').addClass('active').siblings().removeClass('active');
                    $pageMine.show();
                    $pageCloud.hide();
                    $pageIot.hide();
                ...
                _this.$dsNavContain.find('#liCloud').click(function (e) {
                    ...
                    $('#liCloud').addClass('active').siblings().removeClass('active');
                    $pageMine.hide();
                    $pageIot.hide();
                    $PageTag.hide();
                    $pageCloud.show();
                ...
                //         $('#liIot').addClass('active').siblings().removeClass('active');
                //         $pageMine.hide();
                //         $pageIot.html('').show();
                //         $PageTag.hide();
                //         $pageCloud.hide();
                ...
                //     _this.initIotFilter();
                // }
                _this.$dsNavContain.off('click.liTag').on('click.liTag', '#liTag', function () {
                    $('#liTag').addClass('active').siblings().removeClass('active');
                    $pageMine.hide();
                    $pageIot.hide();
                    $PageTag.html('').show();
                    $pageCloud.hide();
                    _this.currentObj = 'tag';
                    _this.initTagTree();
                });
                //开放点击 我的 的功能
                _this.initDsMine();
                ...
                AppConfig.isStratefy && $('#liTag').click();
                //开放 我的 有关功能
                var addGroup = $('<div id="dataSrcAddGroup"><input type="text" id="inputAddGroup" placeholder="+ Add group" ></div>');
        ...
        tagDragGroupStart: function (e, treeNode, tar) {
            var projId = beop.tag.externalTree.getProjectId();
            var getParentName = function (treeNode, nameArr) {
                var parent = treeNode.getParentNode();
                if (parent) {
                    nameArr.unshift(parent.originName);
                    return getParentName(parent, nameArr);
                } else {
                    return nameArr.join('_');
                }
            };
            var parentName = getParentName(treeNode, []);
            var getGroup = function (treeNode, arr, parentName) {
                if (treeNode.entityType != undefined && treeNode.entityType == 1) {//说明是设备 无需查找子文件夹
                    if (treeNode.children) {
                        var tempArr = treeNode.children.map(function (node) {
                            if (node.type == 'thing') {
                                return {
                                    _id: node._id,
                                    tag: node.tag,
                                    ...
                                    parentName: parentName + '_' + treeNode.originName
                                }
                            }
                        });
                        arr.push({
                            ...
                            prt: treeNode.prt,
                            children: tempArr,
                            originName: treeNode.originName,
                ...
                } else if (treeNode.entityType != undefined && treeNode.entityType == 0) {//普通文件夹
                    arr.push({
                        _id: treeNode._id,
                        tag: treeNode.tag,
                        name: treeNode.name,
            ...
            var jsonData = [];
            getGroup(treeNode, jsonData, parentName);

            e.originalEvent.dataTransfer.setData('tagGroupId', JSON.stringify({
                data: jsonData
            }));
            node = beop.tag.externalTree.getNodeById(tar.attr('nodeId'));
            EventAdapter.setData({
                'dsItemId': tar.attr('nodeId'),
                'name': beop.tag.externalTree.getNodeById(tar.attr('nodeId')).name,
                'originName': beop.tag.externalTree.getNodeById(tar.attr('nodeId')).originName
            });
        },
         ...
         * 数据源tag拖拽开始(点)
         */
        tagDragPointStart: function (e, name, id) {
            var projId = beop.tag.externalTree.getProjectId();
            var node = beop.tag.externalTree.getNodeById(id);
            var data = {
                id:'@' + projId + '|' + name,
                tag: node.tag
            }
            e.originalEvent.dataTransfer.setData('tagItemId', JSON.stringify(data));
            EventAdapter.setData({
                'dsItemId': id
            });
        },
         ...
         * @param $element
         */
        tagDragOver: function ($element) {
            EventAdapter.on($element, 'dragover', function (e) {
                e.preventDefault();
            });
        },
         ...
         * @param $element
         */
        tagDragEnd: function ($element) {
            EventAdapter.on($element, 'dragend', function (e) {
        ...
        initTagTree: function () {
            beop.tag.externalTree.configModel({
                itemShowList: ['search', 'selectPrj'],
                cb_on_list_add_dom: function ($li, listName) {
                    EventAdapter.on($li, 'dragstart', function (e) {
                        $('.templatePara').css('display', 'block');
                        _this.tagDragPointStart(e, listName, $(e.target).attr('id'));
                    });
                    _this.tagDragOver($li);
                    _this.tagDragEnd($li);
                },
                cb_on_add_dom: function (treeId, treeNode) {
                    var $itemLeaf = $('#' + treeNode.tId).children('a');
                    var ids = [];
                    $itemLeaf.attr('nodeId', treeNode._id);
                    $itemLeaf.attr('isParent', treeNode.isParent);
                    $itemLeaf.attr('draggable', true);
                    if (!treeNode.isParent) {
                        $itemLeaf.find("#" + treeNode.tId + "_span").addClass("spanName").attr('data-value', treeNode.originName);
                    }
                    EventAdapter.on($itemLeaf, 'dragstart', function (e) {
                        ...
                        var tar = $(e.target);
                        var dragSrcId, node;
                        if (tar.attr('isParent') == 'true') {
                            _this.tagDragGroupStart(e, treeNode, tar);
                        } else {
                            _this.tagDragPointStart(e, treeNode.originName, tar.attr('nodeId'));
                        }
                    });
                    _this.tagDragOver($itemLeaf);
                    _this.tagDragEnd($itemLeaf);
                }
            });
            beop.tag.externalTree.init(_this.$dsNavContain.find('#pageTag'));
        },
        //注释 点击 设备的 功能
                ...
                if (13 == e.keyCode && searchVal) {
                    WebAPI.get('/analysis/datasource/searchDsItemInfo/' + AppConfig.userId + '/' + searchVal).done(function (result) {
                        ...
                        function addHoverDom(treeId, treeNode) {
                            if (!treeNode.isParent) {
                                if (treeNode.name == 'unassigned') {
                                    var $treeNodeA = $('#' + treeNode.tId + '_a');
                                    $treeNodeA.find('.remove').remove();
                                }
        ...
        initDsMineTree: function () {
            ...
            function addDiyDom(treeId, treeNode) {
                if (treeNode.isParent) {
                    ...
                    var $badge;
                    if (treeNode.num) {
                        $badge = $('<span class="badge treeBadge">' + treeNode.num + '</span>');
                    } else {
                        $badge = $('<span class="badge treeBadge"></span>');
                    }
            ...
            function addHoverDom(treeId, treeNode) {
                if (treeNode.isParent) { // add btn
                    ...
                    if (0 == $("#addBtnFormula_" + treeNode.tId).length) { // add formula
                        ...
                        var addStr = "<span class='iconfont' id='addBtnFormula_" + treeNode.tId + "' title='add group' onfocus='this.blur();' style='margin-left:4px;margin-right:4px;'>&#xe63e;</span>";
                        sObj.after(addStr);
                                ...
                                if (groupId) {
                                    if (_this.treeObj) {
                                        if (treeNode.zAsync) {
                                            ...
                                            _this.adjustmentNum(treeNode, 'addNode');
                                        } else {
                                            _this.treeObj.expandNode(treeNode, true, false, true);
                                            _this.adjustmentNum(treeNode, 'addNode');
                                        }
        ...
        adjustmentNum: function (treeNode, state) {
            var allParent = treeNode.getPath();
            var $spanBadge, num;
            if (allParent.length > 1) {
                for (var i = allParent.length - 1; i--; i >= 0) {
                    $spanBadge = $("li[ptid='" + allParent[i].id + "']").find('.treeBadge');
                    if ($($spanBadge).text()) {
                        num = Number($($spanBadge).text());
                    } else {
                        num = 0;
                    }
                    if (state == 'addNode') {
                        ...
                        num -= 1;
                        if ($spanBadge && $spanBadge.length > 0) {
                            $spanBadge.text(num == 0 ? '' : num);
                        }
                    }
                }
            } else {
                if (state == 'addNode') {
                    $spanBadge = $("li[ptid='" + treeNode.id + "']").find('.treeBadge');
                    if ($($spanBadge).text()) {
                        num = Number($($spanBadge).text());
                    } else {
                        num = 0;
        ...
        zTreeBeforeRemoveFunc: function (treeNode) {
            if (treeNode.isParent) {
                _this.adjustmentNum(treeNode, 'removeNode');
            ...
            if ($tableBody) {
                ...
                for (var i = 0, len = ptTable.length; i < len; i++) {
                    ...
                    EventAdapter.on($itemTr, 'click', function (e) {
                      ...
                      EventAdapter.on($itemTr, 'dblclick', function (e) {
                        //触发拖拽事件
                        $(this).trigger('dragstart');
                        $('[data-dbclick-trigger-drop-event]').trigger('drop');
                      });
                    ...
                    EventAdapter.on($itemTr, 'dragstart', function (e) {
                        ...
                        EventAdapter.setData({'dsItemId': dragSrcId});
                        if(e.originalEvent){
                            e.originalEvent.dataTransfer.setData('dsItemId', dragSrcId);
                        }
                    });
        ...
        insertIntoMineTable: function (newNode) {
            var nodeParent = _this.treeObj.getNodesByParam('name', 'unassigned');
            if (nodeParent.length > 0) {
                ...
                WebAPI.post('/analysis/datasource/saveMulti', postData).done(function (data) {
                    if (data.itemIdList.length > 0) {
                        ...
                        _this.treeObj.reAsyncChildNodes(nodeParent[0], "refresh", false);
                        _this.adjustmentNum(nodeParent[0], 'addNode');
                        alert(I18n.resource.dataSource.ADD_OK);
                    }
                }).always(function (e) {
                });
            }
        ...
        insertIntoDataList: function (customName, ptName, ptDesc, prjName, iconColor, itemId, baseNode, bIsAppend, bIsSelected) { // useless now
            ...
            var inputCustName = $('<div class="form-group"><label style="color:#eee">Custom Name</label><input type="text" class="form-control" value="" placeholder="Custom Name"></div>');
            ...
            var inputDesc = $('<div class="form-group"><label style="color:#eee">Description</label><input type="text" class="form-control" value="" placeholder="Description"></div>');
        ...
        getDSItemById: function (datasourceItemId) {
            ...
            var dsRet = $.ajax({
                type: 'POST',
                url: (typeof cordova == "undefined" ? '' : AppConfig.host) + '/analysis/datasource/getDsItemsById',
        ...
        getDSItemDataMulti: function (target, arrDSItemIds) {
            ...
            function postDateHistory(time) {
                if (_this.curModal.type == 'AnlzHistoryCompare' || _this.curModal.type == 'AnlzHistoryCompare_Line') {
                    var modalVal = _this.curModal.mode;
                    if (modalVal == 'easyCompareAnalyz') {
                        timeList = _this.curModal.easyTimeObj ? _this.curModal.easyTimeObj.timeList : undefined;
                    } else if (modalVal == 'compareSensor') {
                        timeList = _this.curModal.sensorTimeObj ? _this.curModal.sensorTimeObj.timeList : undefined;
                    } else {
                        timeList = _this.curModal.meterTimeObj ? _this.curModal.meterTimeObj.timeList : undefined;
                    }
                    if (timeList && timeList.length > 2) {
                        ...
                        for (var i = timeList.length - 1; i >= 0; i--) {
                            ...
                            if (comparePeriod == 'hour') {
                                _this.curModal.arrComparePeriodLabel.push(item.getFullYear() + '-' + (item.getMonth() + 1) + '-' + item.getDate() + ' ' + item.getHours() + ':00');
                                var periodMin = new Date(item.getTime() + time).format('yyyy-MM-dd HH:mm:ss');
                            } else if (comparePeriod == 'day') {
                                _this.curModal.arrComparePeriodLabel.push(item.getFullYear() + '-' + (item.getMonth() + 1) + '-' + item.getDate());
                                var periodMin = new Date(item.getTime() + time).format('yyyy-MM-dd HH:mm:ss');
                            } else if (comparePeriod == 'week') {
                                var weekDay = item.getDay();
                                var date = item.getDate();
                                item = new Date(item.setDate(date - weekDay + 1)).format('yyyy-MM-dd') + ' 00:00:00';
                                _this.curModal.arrComparePeriodLabel.push(compareDateI18n.YEAR_WEEK.replace('<%year%>', startDate.getFullYear()).replace('<%week%>', getWeekNumber(item)));
                                var periodMin = new Date(new Date(item).getTime() + time).format('yyyy-MM-dd HH:mm:ss');
                            } else if (comparePeriod == 'month') {
                                _this.curModal.arrComparePeriodLabel.push(item.getFullYear() + '-' + (item.getMonth() + 1));
                                var periodMin = getCurrentMonthLastDay(item.format('yyyy-MM-dd HH:mm:ss'));
                            }
                            timeGroupArr.push({
                                timeStart: new Date(item).format('yyyy-MM-dd HH:mm:ss'),
                                timeEnd: periodMin.format('yyyy-MM-dd HH:mm:ss')
                            })
                        }
                    }
                    return timeGroupArr;
            ...
            if (comparePeriod == 'hour') {
                ...
                timeGroupArr = postDateHistory(time);
            }
            ...
            if ((_this.curModal.type == 'AnlzHistoryCompare' || _this.curModal.type == 'AnlzHistoryCompare_Line') && timeList && timeList.length > 2) {
                postData = [];
                for (var i = 0; i < timeGroupArr.length; i++) {
                    postData.push({
                        dsItemIds: arrDSItemIds,
                        timeStart: timeGroupArr[i].timeStart,
                        timeEnd: timeGroupArr[i].timeEnd,
                        timeFormat: tmFmt
                    })
                }
            }
