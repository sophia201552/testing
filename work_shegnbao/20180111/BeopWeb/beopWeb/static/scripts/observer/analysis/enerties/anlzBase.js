...
var DrawGraph = (function () {
    ...
    DrawGraph.prototype = {
        ...
        createBoard: function () {
            ...
            if (_this.screenPre.curModal.type === 'AnlzTendency') {
                $boxWOrH = $('#anlsPaneContain .panel-body .containerChi');
            } else {
                $boxWOrH = $('#anlsPaneContain .panel-body');
            }
            this.svgW = $boxWOrH.width();
            this.svgH = $boxWOrH.height();
        ...
        //拖动图形
        graphDrag: function (graphDraw) {
            var $target = $('#' + graphDraw.id);
            if ($target.length === 0) {
                return;
            }
            ...
            svgGraphCopy.onmousedown = function (e) {
                ...
                this.onmouseup = function (e) {
                    ...
                    var graph = {
                        id: graphDraw.id,
                        downX: svgCopyBox.offsetLeft / _this.$svgBox.width(),
                        downY: svgCopyBox.offsetTop / _this.$svgBox.height(),
                        upX: svgCopyBox.offsetLeft / _this.$svgBox.width() + graphDraw.upX - graphDraw.downX,
                        upY: svgCopyBox.offsetTop / _this.$svgBox.height() + graphDraw.upY - graphDraw.downY
                    }
        ...
        saveGraph: function (graph) {
            ...
            if (index == undefined) {
                ...
                if (graph.downX && graph.downX != graph_old.downX) {
                    isSave = true
                }
                if (!isSave && graph.downY && graph.downY != graph_old.downY) {
                    isSave = true
                }
                if (!isSave && graph.upX && graph.upX != graph_old.upX) {
                    isSave = true
                }
                if (!isSave && graph.upY && graph.upY != graph_old.upY) {
                    isSave = true
                }
                if (isSave) {
                    _this.screenPre.curModal.graphList[index].downX = graph.downX;
                    _this.screenPre.curModal.graphList[index].downY = graph.downY;
                    _this.screenPre.curModal.graphList[index].upX = graph.upX;
                    _this.screenPre.curModal.graphList[index].upY = graph.upY;
                    doSave();
                }
            }
            function doSave() {
...
var DrawArrow = (function () {
    ...
    function DrawArrow(screenPre, graph, container, color) {
        ...
        this.$svgBox = $(this.container).find('.svgBox');
        this.$parentContainer = $(this.container);
        if (!this.$svgBox || $('.svgBox').length === 0 || $(this.container).find('.svgBox').length === 0) {
            this.$svgBox = null;
    ...
    DrawArrow.prototype.show = function () {
        ...
        _this.graphDrag(this.graph);
    };
    //渲染箭头
    DrawArrow.prototype.showArrow = function (graphDraw) {
        var $svgGraphCopy = $('.svgGraphCopy');
        var $svgCopyBox = $('.svgCopyBox');
        var svgBoxW = _this.$svgBox.width();
        var svgBoxH = _this.$svgBox.height();
        var downX = graphDraw.downX * svgBoxW;
        var downY = graphDraw.downY * svgBoxH;
        var upX = graphDraw.upX * svgBoxW;
        var upY = graphDraw.upY * svgBoxH;
        var xDiffer = Math.abs(upX - downX);
        var yDiffer = Math.abs(upY - downY);
        if (xDiffer < 20) {
            xDiffer = 20
        }
        if (yDiffer < 20) {
        ...
        if (upX >= downX && upY < downY) {
            $svgCopyBox = $('<div class="svgCopyBox" style="width:' + (xDiffer + 8) + 'px;height:' + (yDiffer + 8) + 'px;padding:2px;position:absolute;left:' + (downX * 100 / svgBoxW) + '%;top:' + (upY * 100 / svgBoxH - 0.6) + '%;z-index:149;">' +
                '<span class="glyphicon glyphicon-remove deleteGraph deleteGraphTem"></span></div>');
            $svgGraphCopy = $('<svg class="svgGraphCopy"  id="' + graphDraw.id + '" viewBox="0 0 ' + xDiffer + ' ' + yDiffer + '" width="' + xDiffer + '" height="' + yDiffer + '" type="arrow">' +
                ...
                '<path class="arrow-line2" marker-end="url(#marker_' + graphDraw.id + ')" stroke-width="4" stroke="' + graphDraw.color + '" d="M0 , ' + yDiffer + 'L' + Math.abs(upX - downX - 8) + ',8"/>' +
                '</svg>');
        }
            // 4
        else if (upX > downX && upY >= downY) {
            $svgCopyBox = $('<div class="svgCopyBox" style="width:' + (xDiffer + 8) + 'px;height:' + (yDiffer + 8) + 'px;padding:2px;position:absolute;left:' + (downX * 100 / svgBoxW) + '%;top:' + (downY * 100 / svgBoxH) + '%;z-index:149;">' +
               '<span class="glyphicon glyphicon-remove deleteGraph deleteGraphTem"></span></div>');
            $svgGraphCopy = $('<svg class="svgGraphCopy" id="' + graphDraw.id + '" viewBox="0 0 ' + xDiffer + ' ' + yDiffer + '" width="' + xDiffer + '" height="' + yDiffer + '" type="arrow">' +
                '<defs><marker id="marker_' + graphDraw.id + '" orient="auto" markerWidth="2" markerHeight="4" refX="0.1" refY="2"><path d="M0,0 V4 L2,2 Z" fill="' + graphDraw.color + '"/></marker></defs>' +
                '<path class="arrow-line1" marker-end="url(#marker_' + graphDraw.id + ')" stroke-width="4" stroke="' + graphDraw.color + '" d="M0 ,0 ' + 'L' + Math.abs(upX - downX - 8) + ',' + Math.abs(upY - downY - 8) + '"/>' +
                '</svg>');
        }
            // 2
        else if (upX <= downX && upY <= downY) {
            $svgCopyBox = $('<div class="svgCopyBox" style="width:' + (xDiffer + 8) + 'px;height:' + (yDiffer + 8) + 'px;padding:2px;position:absolute;left:' + (upX * 100 / svgBoxW) + '%;top:' + ((upY) * 100 / svgBoxH) + '%;z-index:149;">' +
               '<span class="glyphicon glyphicon-remove deleteGraph deleteGraphTem"></span></div>');
            $svgGraphCopy = $('<svg class="svgGraphCopy" id="' + graphDraw.id + '" id="new Date().getTime()" viewBox="0 0 ' + xDiffer + ' ' + yDiffer + '" width="' + xDiffer + '" height="' + yDiffer + '" type="arrow">' +
                    '<defs><marker id="marker_' + graphDraw.id + '" orient="auto" markerWidth="2" markerHeight="4" refX="0.1" refY="2"><path d="M0,0 V4 L2,2 Z" fill="' + graphDraw.color + '"/></marker></defs>' +
                    '<path class="arrow-line1" marker-end="url(#marker_' + graphDraw.id + ')" stroke-width="4" stroke="' + graphDraw.color + '" d="M ' + xDiffer + ',' + yDiffer + 'L8,8"/>' +
                    '</svg>');
        }
            // 3
        else if (upX < downX && upY >= downY) {
            $svgCopyBox = $('<div class="svgCopyBox" style="width:' + (xDiffer + 8) + 'px;height:' + (yDiffer + 8) + 'px;padding:2px;position:absolute;left:' + (upX * 100 / svgBoxW) + '%;top:' + (downY * 100 / svgBoxH) + '%;z-index:149;">' +
               '<span class="glyphicon glyphicon-remove deleteGraph deleteGraphTem"></span></div>');
            $svgGraphCopy = $('<svg class="svgGraphCopy" id="' + graphDraw.id + '" viewBox="0 0 ' + xDiffer + ' ' + yDiffer + '" width="' + xDiffer + '" height="' + yDiffer + '" type="arrow" >\
                            <defs><marker id="marker_' + graphDraw.id + '" orient="auto" markerWidth="2" markerHeight="4" refX="0.1" refY="2"><path d="M0,0 V4 L2,2 Z" fill="' + graphDraw.color + '"/></marker></defs>\
    ...
    DrawArrow.prototype.render = function (graph) {
        var $box_arrow = _this.showArrow(graph);
        ...
        $svgGraph[0].onmousedown = function (e) {
            ...
            $svgGraph[0].onmouseup = function (e) {
                ...
                var graphDraw = {
                    ...
                    color: _this.color,
                    id: newGraphId,
                    downX: downX / _this.$svgBox.width(),
                    downY: downY / _this.$svgBox.height(),
                    upX: e.offsetX / _this.$svgBox.width(),
                    upY: e.offsetY / _this.$svgBox.height()
                };
                $svgCopyBox = _this.showArrow(graphDraw);
                _this.$svgBox.append($svgCopyBox);
                _this.graphHover($('#' + newGraphId).parent('.svgCopyBox'));
                _this.graphDrag(graphDraw);
                $('.svgGraph').remove();
                _this.saveGraph(graphDraw);
...
var DrawCircle = (function () {
    ...
    //渲染圆形
    DrawCircle.prototype.showCircle = function (graphDraw) {
        var $svgGraphCopy = $('.svgGraphCopy');
        var $svgCopyBox = $('.svgCopyBox');
        var svgBoxW = _this.$svgBox.width();
        var svgBoxH = _this.$svgBox.height();
        ...
        var upX = graphDraw.upX * svgBoxW;
        var upY = graphDraw.upY * svgBoxH;
        $svgCopyBox = $('<div class="svgCopyBox" style="width:' + (Math.abs(upX - downX) + 8) + 'px;height:' + (Math.abs(upY - downY) + 8) + 'px;position:absolute;left:' + (downX * 100 / svgBoxW) + '%;top:' + (downY * 100 / svgBoxH) + '%;z-index:149;padding:2px;">' +
                       '<span class="glyphicon glyphicon-remove deleteGraph deleteGraphTem"></span></div>');
        $svgGraphCopy = $('<svg class="svgGraphCopy" id="' + graphDraw.id + '" viewBox="0 0 ' + Math.abs(upX - downX) + ' ' + Math.abs(upY - downY) + '" width="' + Math.abs(upX - downX) + '" height="' + Math.abs(upY - downY) + '" type="circle">' +
                '<ellipse style="fill:rgba(0,0,0,0);stroke-width:3px;stroke:' + graphDraw.color + '" orient="auto" cx="' + Math.abs(upX - downX) / 2 + '" cy="' + Math.abs(upY - downY) / 2 + '" rx="' + Math.abs(upX - downX - 4) / 2 + '" ry="' + Math.abs(upY - downY - 4) / 2 + '" x="0" y="0"/>' +
                '</svg>');
        $svgCopyBox.append($svgGraphCopy);
        return $svgCopyBox;
    };
    ...
    DrawCircle.prototype.render = function (graph) {
        var $box_cicle = _this.showCircle(graph);
    ...
    DrawCircle.prototype.drawing = function () {
        ...
        $svgGraph[0].onmousedown = function (e) {
            ...
            $svgGraph[0].onmouseup = function (e) {
                ...
                $svgCopyBox = _this.showCircle(graphDraw);
                _this.$svgBox.append($svgCopyBox);
                _this.graphHover($('#'+newGraphId).parent('.svgCopyBox'));
                _this.graphDrag(graphDraw);
                _this.saveGraph(graphDraw);
...
var DrawRect = (function () {
    ...
    //渲染方形
    DrawRect.prototype.showRect = function (graphDraw) {
        var $svgGraphCopy = $('.svgGraphCopy');
        var $svgCopyBox = $('.svgCopyBox');
        var svgBoxW = _this.$svgBox.width();
        var svgBoxH = _this.$svgBox.height();
        ...
        var upX = graphDraw.upX * svgBoxW;
        var upY = graphDraw.upY * svgBoxH;
        $svgCopyBox = $('<div class="svgCopyBox" style="width:' + (Math.abs(upX - downX) + 8) + 'px;height:' + (Math.abs(upY - downY) + 8) + 'px;padding:2px;position:absolute;left:' + (downX * 100 / svgBoxW) + '%;top:' + (downY * 100 / svgBoxH) + '%;z-index:149;">' +
                       '<span class="glyphicon glyphicon-remove deleteGraph deleteGraphTem"></span></div>');
        $svgGraphCopy = $('<svg class="svgGraphCopy" id="' + graphDraw.id + '" viewBox="0 0 ' + Math.abs(upX - downX) + ' ' + Math.abs(upY - downY) + '" width="' + Math.abs(upX - downX) + '" height="' + Math.abs(upY - downY) + '" type="rect">' +
                '<rect style="fill:rgba(0,0,0,0);stroke-width:4px;stroke:' + graphDraw.color + '" orient="auto" width="' + Math.abs(upX - downX) + '" height="' + Math.abs(upY - downY) + '"/>' +
                '</svg>');
    ...
    DrawRect.prototype.render = function (graph) {
        var $box_rect = _this.showRect(graph);
    ...
    DrawRect.prototype.drawing = function () {
        ...
        $svgGraph[0].onmousedown = function (e) {
            ...
            $svgGraph[0].onmouseup = function (e) {
                ...
                $svgCopyBox = _this.showRect(graphDraw);
                _this.$svgBox.append($svgCopyBox);
                _this.graphHover($('#' + newGraphId).parent('.svgCopyBox'));
                _this.graphDrag(graphDraw);
                _this.saveGraph(graphDraw);
    ...
    AnlzBase.prototype = {
        show: function () {
            ...
            if (!this.screen.curModal.graphList) {
                ...
                var graph = undefined;
                var containerChilBox;
                if (_this.screen.curModal.type === 'AnlzTendency') {
                    containerChilBox = $(_this.container).find('.containerChi')[0];
                } else {
                    containerChilBox = _this.container;
                }
                for (var i = 0; i < graphList.length; i++) {
                    if (graphList[i].type == 'arrow') {
                        graph = new DrawArrow(_this.screen, graphList[i], containerChilBox, '');
                    } else if (graphList[i].type == 'rect') {
                        graph = new DrawRect(_this.screen, graphList[i], containerChilBox, '');
                    } else if (graphList[i].type == 'circle') {
                        graph = new DrawCircle(_this.screen, graphList[i], containerChilBox, '');
                    }
                    graph.show();
                }
            }
        ...
        errAlert: function(err){
            ...
            switch(err){
                case 'no data history':
                    new Alert($dataAlert, "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE6 + "</strong>").show(1000);
                    break;
                case 'time':
                    new Alert($dataAlert, "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE7 + "</strong>").show(1000);
                    break;
                case 'invalid time string':
                    new Alert($dataAlert, "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE8 + "</strong>").show(1000);
                    break;
                default :
                    new Alert($dataAlert, "danger", "<strong>" + I18n.resource.modalConfig.err.TYPE9 + "</strong>").show(1000);
                    break;
            }
            //setTimeout(function () {
        ...
        initTools: function () {
            ...
            $('.itemTools .glyphicon-cog').eventOff('click').eventOn('click', function () {
                trackEvent('数据分析配置点击','DataAnalysis.Config.Click');
                ...
                var option = {
                    ...
                    optionPara: {
                        ...
                        timeRecent:_this.screen.curModal.timeRecent,
                        dataItem: data
                    }
                };
            ...
            $('.itemTools .glyphicon-edit').eventOff('click').eventOn('click',function () {
                _this.createNote();
                $noteHideShow.removeClass('glyphicon-eye-close').addClass('glyphicon-eye-open');
                trackEvent('数据分析便签插入','DataAnalysis.Note.Insert');
                $('.divNote').show();
            },'数据分析插入便签');
            //导出excel按钮
            $('#btnDownLoadExcel_a').eventOff('click').eventOn('click', function (e) {
                e.preventDefault();
                trackEvent('数据分析数据下载','DataAnalysis.DataExcel.Download');
                ...
                if (dictDataList) {
                    ...
                    str = i18n_resource.analysis.workspace.NO_DATA;
                }
                ...
                jQuery.getScript("/static/scripts/lib/html-docx/FileSaver.js").done(function () {
                    saveAs(blob, document.getElementById("btnDownLoadExcel_a").getAttribute('download'));
                });
            },'数据分析下载数据');
            //眼睛按钮
            $noteHideShow.removeClass('glyphicon-eye-close').addClass('glyphicon-eye-open');
            ...
            function drawGraph(graphType,color){
                ...
                if (graphType == 'arrow') {
                    drawGraph = new DrawArrow(_this.screen, {}, containerChilBox, color);
                } else if (graphType == 'rect') {
                    drawGraph = new DrawRect(_this.screen, {}, containerChilBox, color);
                } else if (graphType == 'circle') {
                    drawGraph = new DrawCircle(_this.screen, {}, containerChilBox, color);
                }
                drawGraph.drawing();
            }
        },
        createNote: function (note) {
            ...
            var $divDrag = $('<div class="divDrag">').appendTo($divNote);

            var $divTitle = $('<div class="title">').appendTo($divDrag);

            /*$divTitle[0].draggable = true;
            setDrag($divTitle[0]);*/

            $divDrag[0].draggable = true;
            setDrag($divDrag[0]);
            ...
            var $divPin = $('<div class="glyphicon pinIcon grow">&times;</div>').appendTo($divTitle);
            $divPin.mousedown(function(e){
                _this.removeNote(this);
                e.stopPropagation();
            });

            var $divText = $('<div class="divText gray-scrollbar">').appendTo($divDrag);

            if(_this.isShareMode !== 1){
                +function () {
                    var timer = null, mousePos = null;
                    $divText.off('mousedown').mousedown(function (e) {
                        mousePos = {x: e.clientX, y: e.clientY};
                    });
                    $divText.off('mouseup').mouseup(function (e) {
                        if(timer !== null) {
                            return;
                        }
                        ...
                        if(Math.abs(mousePos.x - e.clientX) < 10 && Math.abs(mousePos.y - e.clientY) < 10){
                            showEditor();
                        }
                        mousePos = null;
                    });
