...
var AnlzHistoryCompare_Line = (function () {
    ...
    AnlzHistoryCompare_Line.prototype.initChart = function (data) {
        ...
        var dataList;
        for(var i = 0;i<data.length;i++){
            if(data[i].timeShaft.length==0&&i!=data.length-1){
                dataList = data[i+1];
            }else{
                dataList = data[i];
                break;
        ...
        for (var i = 0; i< dataList.timeShaft.length ; ++i){
            xAxis.push('Day' + new Date(dataList.timeShaft[i]).getDate() +' ' + new Date(dataList.timeShaft[i]).format("HH:mm:ss"));
        }

        var arrItem = AppConfig.datasource.getDSItemById(_this.options.itemDS[0].arrId)
        var i18nEcharts = I18n.resource.echarts;
        var option = {
            title: {
                text: ''
            ...
            tooltip: {
                ...
                formatter: function (params) {
                    var strToolTip = '',date;
                    // var arrItem = [];
                    var modalVal = _this.screen.curModal.mode;
                    var listNum;
                    if(modalVal =='easyCompareAnalyz'){
                        listNum = _this.screen.curModal.easyTimeObj?_this.screen.curModal.easyTimeObj.timeList.length:2;
                    }else if(modalVal =='compareSensor'){
                        listNum = _this.screen.curModal.sensorTimeObj?_this.screen.curModal.sensorTimeObj.timeList.length:2;
                    }else{
                        listNum = _this.screen.curModal.meterTimeObj?_this.screen.curModal.meterTimeObj.timeList.length:2;
                    }
                    listNum = listNum?listNum:2;
                    // arrItem = AppConfig.datasource.getDSItemById(_this.options.itemDS[0].arrId);
                    for (var i = 0;i < params.length;i++){
                        if(i % listNum == 0){
                            for (var m = 0; m < arrItem.length; m++) {
                                if (_this.options.itemDS[0].arrId[i/listNum] == arrItem[m].id) {
                                    strToolTip += arrItem[m].alias + '<br/>';
                                    strToolTip += (data[0].timeShaft?data[0].timeShaft[params[i].dataIndex]:'--') + '<br/>Value: ' + (params[i].value || params[i].value === 0?params[i].value:'--') + '<br/>';
                                    break;
                                }
                            }
                            ...
                            strToolTip +=(data[i % listNum].timeShaft?data[i % listNum].timeShaft[params[i].dataIndex]:'--') + '<br/>Value: ' +(params[i].value || params[i].value === 0?params[i].value:'--') + '<br/>';
                        }
                    }
                    return strToolTip;
                }
        ...
        if( mode == 'easyCompareAnalyz' || mode == 'compareSensor'){
            for (var i = 0; i < data.length; i++) {
                ...
                for(var j = 0; j < item.list.length; j++){//第i个绑点
                    ...
                    if(i == 0){
                        arrId.push(point.dsItemId);
                        for(var m = 0;m<this.options.arrComparePeriodLabel.length;m++){
                            var arrLengendName = this.options.arrComparePeriodLabel[m];
                            arrLegendData.push(arrLengendName + ':' + pointName);
                        }
                    }
                    for(var k = 0; k < point.data.length; k++){
