...
var ReportScreen = (function () {
    ...
    function ReportScreen(reportId, reportType, reportFolder, reportDate, reportText) {
        var _this = this;
        _this.closeDateReport = $.Deferred().resolve();
        ...
        var reportFolderName = _this.getReportFolder();
        _this.closeDateReport = $.ajax("/report/getReportList/" + AppConfig.projectName + "/" + reportFolderName + '/', {'async': false}).done(function (result) {
            if (result.success) {
        ...
        if (reportDate) {
            ...
            if (this.reportType == this.reportTypeMap.monthly) {
                if (new Date().getMonth() == new Date(reportDate + '-15').getMonth()) {
                    var date = new Date(reportDate + '-15');
            ...
            _this.closeDateReport.done(function (result) {
                if (result.success) {
                    _this.reportDate = _this.getLastReportDate(_this.dateList);
                }
            });
        }
    ...
    ReportScreen.prototype = {
        ...
        init: function () {
            var _this = this;
            _this.closeDateReport.done(function () {
                setTimeout(function () {
                    if (_this.reportDate) {
                ...
                _this.chartList = {};
                _this.registerExportPDFEvent();
                _this.registerExportWordEvent();
            });
        ...
        optionToContent: function (opt) {
            ...
            var axisData = opt.xAxis && opt.xAxis[0].data,
                series = opt.series;
            var title = '<p style=" text-align: center; font-size: 15px;font-weight: bold;">' + opt.title[0].text + '</p>';
        ...
        requestWordDownload: function () {
            ...
            $.when.apply(null, uploadPromises).then(function () {
                var dateWord = $("#beopReport").find(' .input-append input').val();
                WebAPI.post('/report/exportWord/', {
                    ...
                    if (result.success) {
                        _this.downloadFile(result.data, _this.reportText + '_' + (_this.getReportType() == _this.reportTypeMap.weekly ? _this.getWeekDate(_this.reportDate) : timeFormat(dateWord, timeFormatChange('yyyy-mm-dd'))) + '.docx');
                    } else {
        ...
        getWeekDate: function (date) {
            ...
            if (date.indexOf('w') !== -1) {
                return dateArr[0] + '-week-' + dateArr[1];
            }
            var week = _this.iso8601Week(date);
            return dateArr[0] + '-week-' + week;
        },
        ...
        getWeekVersion: function (version) {
            if (version.indexOf('w') === -1) {
                return version
            }
            var list = version.split('-');
            return list[0] + ' week ' + list[1];
        },
        registerExportPDFEvent: function () {
            ...
            $(ElScreenContainer).eventOff('click.report.exportPDF', '#exportPDF').eventOn('click.report.exportPDF', '#exportPDF', function () {
                var chart = null,
                    ...
                    pdfName = AppConfig.projectShowName + '.' + _this.getReportName() + '.' + (_this.reportTypeMap.weekly ? _this.getWeekVersion(version) : timeFormat(version, timeFormatChange('yyyy-mm-dd'))) + '.pdf',
                    uploadPromises = [];
                        ...
                        $.when(pdfTemplatePromise, coverPagePromise).done(function (template, result) {
                            WebAPI.get("/project/getinfo/" + AppConfig.projectId).done(function (rs) {
                                ...
                                result = result[0].formatEL({ // 根据返回的封面页组装数据
                                    ...
                                    date: _this.getReportType() == _this.reportTypeMap.weekly ? _this.getWeekVersion(version) : timeFormat($("#beopReport").find(' .input-append input').val(), timeFormatChange('yyyy-mm-dd')),
                                ...
                                template = template[0].formatEL({
                                    ...
                                    date: (_this.reportTypeMap.weekly ? _this.getWeekVersion(version) : timeFormat(version, timeFormatChange('yyyy-mm-dd'))),
                                    logo: BEOPUtil.getProjectImgByType(AppConfig.projectId, BEOPUtil.projectImgType.logo),
        ...
        //如果当前天没有报表,则初始化显示最近那天报表日期;
        getLastReportDate: function () {
            if (!this.dateList || !this.dateList.length) {
                if (this.getReportType() == this.reportTypeMap.weekly) {
                    return new Date().format('yyyy-MM-dd');
                } else if (this.getReportType() == this.reportTypeMap.daily) {
                    return new Date().format('yyyy-MM-dd');
            ...
            var maxFunc = function (array) {
                var sorted = array.sort();
                return array[sorted.length - 1];
            };
            if (this.getReportType() == this.reportTypeMap.weekly) {
                //取得最近周报时间max;
                var arr = [];   //保存最近周报的日期格式为  年-周
                for (var i = 0; i < this.dateList.length; i++) {
                    var a = this.dateList[i].split('-');
                    //如果周数小于10改为'0' + 周数;为了比较大小;
                    if (parseInt(a[1]) < 10) {
                        a[1] = "0" + a[1];
                    }
                    var b = [a[0], a[1]].join('-');
                ...
                var max = maxFunc(arr).split('-');  //最近周;
                var weekLastDate = max.map(function (item) {
                        return parseInt(item);
                    }).join('-') + '-w';
                return this.isoWeek.apply(this, weekLastDate.split('-')).format('yyyy-MM-dd');
            } else if (this.getReportType() == this.reportTypeMap.daily) {
                return new Date(maxFunc(this.dateList)).format('yyyy-MM-dd');
            } else if (this.getReportType() == this.reportTypeMap.monthly) {
                return new Date(maxFunc(this.dateList)).format('yyyy-MM');
            }
        },
        ...
        initReportList: function () {
            ...
            if (AppConfig.isPlatform){
                var navLeftPosi = function () {
                    $reportNavList.css({
                        left: parseInt($stepContainer[0].offsetLeft) - parseInt($reportNavList.width()) + 'px'
                    });
                    $(".reportNavigation").css({
                        left: parseInt($stepContainer[0].offsetLeft) + parseInt($stepContainer.width()) + 40 + 'px',
                        top: "92px"
                    });
                };
                $stepContainer.css("margin-left", "10%");
            }else{
                var navLeftPosi = function () {
                    $reportNavList.css({
        ...
        getReport: function (year, month, day) {
            ...
            return WebAPI.get("/report/getReport/" + AppConfig.projectName + "/" + reportFolderName + '/' + version).done(function (resultHtml) {
                _this.reportMsgDestroy().done(function () {
                    ...
                    if (Permission.hasPermission('DReport') || Permission.hasPermission('Dreport')) {
                        var $pdfContent = '<a id="exportPDF" class="report_file_download exportPDF pa" permission="DReport;Dreport"><span class="add-on"><img class="report_file_img" src="/static/images/pdf_download.png"　alt="PDF Dowload" /></span><span class="report_file_text" i18n="observer.reportScreen.PDF_DOWNLOAD"></span></a>';
                        ...
                        var $wordContent = '<a id="exportWord" class="report_file_download exportWord pa" permission="DReport;Dreport"><span class="add-on"><img class="report_file_img" src="/static/images/word_download.png"　alt="Word Dowload" /></span><span class="report_file_text" i18n="observer.reportScreen.WORD_DOWNLOAD"></span></a>';
        ...
        isoWeek: function (year, week, paramW) {
            //周报时间转换为"yyyy-MM-dd";
            var nowYearTime, nowWeek;
            //当前年的时间戳;
            nowYearTime = new Date(year + '-01' + '-01').getTime();

            //当前周的时间戳;
            nowWeek = parseInt(week) * 7 * 24 * 60 * 60 * 1000;
            return new Date(nowWeek + nowYearTime - 3 * 24 * 60 * 60 * 1000);
        },

        iso8601Week: function (date) {
            //现在我们的项目把周日当做第一天,但是iso标准将周一当做一周中的第一天
            var time,
                checkDate = new Date(new Date(date).getTime() + 24 * 60 * 60 * 1000);
        ...
        initChartLine: function (chartDom, chartParam) {
            ...
            if (jscd.mobile) {
                ...
                if (option.xAxis && option.xAxis.data) {
                    for (var i = 0, xLabel; i < option.xAxis.data.length; i++) {
                        if(!(option.xAxis.data[i] && option.xAxis.data[i].split))continue;
            ...
            var instance = echarts.init(chartDom,AppConfig.isMobile?AppConfig.chartTheme:{});
        ...
        initChartBar: function (chartDom, chartParam) {
            var _this = this;
            //正常情况一行7个,行间距24px;canvas高度550px;
            // 则legendHei为legend在画布中多占的高度比例,近似于图例相对于顶部的top的比例值;
            var legendHei, gridTop, curScreenW;
            curScreenW = parseInt($(window).width());
            //兼容小屏幕echarts视图配置
            if (curScreenW <= 1366) {
                legendHei = Math.ceil(chartParam.chartItems.yOrder.length / 6) * 24 / 550;
                gridTop = Math.ceil(legendHei * 100) + 10 + '%';
            } else {
                legendHei = Math.ceil(chartParam.chartItems.yOrder.length / 8) * 24 / 550;
                gridTop = Math.ceil(legendHei * 100) + 9 + '%';
            }

            //原视图legend配置
            /*legendHei = Math.ceil(chartParam.chartItems.yOrder.length / 7) * 24 / 550;
            gridTop = Math.ceil(legendHei * 100) + 10 + '%';*/
        ...
        initChartTable: function (chartDom, chartParam) {
            if (chartParam && chartParam.chartItems && chartDom) {
                ...
                if ($tableContainer.find('thead th').length > 5) {
                    $tableContainer.filter('table').attr('style', 'width:90%').removeClass('min-table');
                } else {
                    $tableContainer.filter('table').addClass('min-table');
                }
