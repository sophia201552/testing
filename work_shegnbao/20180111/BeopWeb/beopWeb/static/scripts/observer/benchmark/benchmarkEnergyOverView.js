class BenchmarkEnergyOverView{
        ...
        $('.calcButton').off('click').click(function () {
           ...
           _this.selectNode =  _this.screen.iotFilter.tree.getSelectedNodes()[0];
            ...
            if(this.id==='btnDay'){
                ...
                var currentMonth = new Date().getMonth()==0?'12':new Date().getMonth().toString();
                var year;
                ...
                if(new Date().getDate()===1){
                    ...
                    postDataPieChart = {
                        ...
                        timeEnd: new Date().format('yyyy-MM-dd HH:mm:ss'),
                        timeFormat: 'd1',
                        timeStart: year + '-' + currentMonth +'-'+ new Date(year,currentMonth,0).getDate() + ' 23:59:59'
                    }
                }
            }
    ...
    countCost(corresTimeH){
        ...
        for (var j = 0; j < _this.costFinally.length; j++) {
            var costTime = parseInt(_this.costFinally[j].time.split(':')[0]);//当前循环到的时间值
            if(costTime!==0&&!costTime) continue;
            if(j === 0 ){
                if(corresTimeH===costTime){// 传过来的时间值  和  当前的cost是否是一样的  
                    nowCost = parseFloat(_this.costFinally[j].cost);
                    return nowCost;
                }
            }else{
                var costLastTime = parseInt(_this.costFinally[j-1].time.split(':')[0]);
                if(corresTimeH === costTime){
                    nowCost = parseFloat(_this.costFinally[j].cost);
                    return nowCost;
                }else if(costLastTime < corresTimeH && costTime > corresTimeH ){
                    nowCost = parseFloat(_this.costFinally[j-1].cost);
                    return nowCost;
                }
                if (j === _this.costFinally.length - 1) {//最后一个
                    if (corresTimeH > costTime) {
                        nowCost = parseFloat(_this.costFinally[0].cost);
                        return nowCost;
                    }
                }
            }
    ...
    TopOneDataShow(){
        ...
        var dataList = this.screen.opt.point?this.screen.opt.point[selectNodeId]:null;
        ...
        if(dataList){
            ...
            var oneHourEnergy;
            $('.curSymbol').html(AppConfig.unit_currency);
            ...
            WebAPI.post('/analysis/startWorkspaceDataGenHistogram',{
                    ...
                    for(var i = 0;i<resultList.length;i++){
                        if(resultList[i].dsItemId===dataList.energy){//能耗或电量
                            ...
                            if(resultList[i].data.length>0){
                                ...
                                oneHourEnergy = resultListData[resultListData.length - 1] - resultListData[resultListData.length - 2];
                                //费用计算
                                if(energyDataFin<1){
                ...
                if(dataList.power){
                    WebAPI.post('/analysis/startWorkspaceDataGenPieChart', { dsItemIds: dsItemIds1 }).done(function (resultPower) {
                        ...
                        if (!resultPower || !resultPower.dsItemList || !(resultPower.dsItemList instanceof Array)) {
                ...
                } else if (dataList.energy && oneHourEnergy !== undefined) {
                    var floatDate = parseFloat(oneHourEnergy);
                    if (floatDate < 0) {
                        oneHourEnergy = 0;
                    }else if(floatDate < 1){
                        oneHourEnergy = kIntSeparate(floatDate,1);
                    }else{
                        oneHourEnergy = kIntSeparate(parseInt(oneHourEnergy));
                    }
                    $threeItem3Span.html(oneHourEnergy);
    ...
    TopTwoDataShow(id){
        ...
        if(dataList){
            var dsItemIds = [];
            dsItemIds.push(dataList.energy);
            var timeYesday = new Date(new Date().valueOf() - 86400000).format('yyyy-MM-dd HH:00:00');//mm:ss2017/6/9日需求把时间改成整点同line388
            ...
            WebAPI.post('/analysis/startWorkspaceDataGenHistogram',{
                dsItemIds:dsItemIds,
                timeEnd:new Date().format('yyyy-MM-dd HH:00:00'),//new Date().format('yyyy-MM-dd HH:mm:ss'),
                timeFormat:'h1',
                timeStart:timeYesday.split(' ')[0]+' '+((parseInt(timeYesday.split(' ')[1].split(':')[0])-1)>=10?(parseInt(timeYesday.split(' ')[1].split(':')[0])-1):('0'+(parseInt(timeYesday.split(' ')[1].split(':')[0])-1)).toString())+':'+timeYesday.split(' ')[1].split(':')[1]+':'+timeYesday.split(' ')[1].split(':')[2]
            }).done(result=>{
                var resultList = result.list;
                ...
                if(resultList&&resultList[0].data.length>0){
                   ...
                   // resultList[0].name = '逐时能耗';
                    ...
                    for(var i = 1;i<powerList.data.length;i++){
                        powerList.data[i-1] = (powerList.data[i] - powerList.data[i-1])<0?0:(powerList.data[i] - powerList.data[i-1]).toFixed(2)//kIntSeparate(powerList.data[i] - powerList.data[i-1]);//.toFixed(2);
                    }
                    if(!id||id==='energyDay'){
                        ...
                        for(var i = 1;i<resultList[0].data.length;i++){
                            ...
                            var powerListData = ((resultList[0].data[i]-resultList[0].data[i-1])*nowCost)<0?0:((resultList[0].data[i]-resultList[0].data[i-1])*nowCost).toFixed(2);
                            powerList.data.push(powerListData);
                        }
    ...
    TopThreeFirstDataShow(period){
        //下面左半部分
        var _this = this;
        if(_this.selectNode.isParent&&!_this.selectNode.children) return;
        var pointsAll = _this.screen.opt.point;
        var legendColor=['#fed001', 'rgb(248,245,124)', 'rgb(182,209,78)', 'rgb(146,192,129)', 'rgb(105,170,187)', 'rgb(73,152,234)'];
        var pointsIdList = [];
        var pointsNameList = [];
        ...
        var childrenPoints =(!_this.selectNode.isParent&&!_this.selectNode.children)? _this.selectNode.getParentNode().children: _this.selectNode.children;
        var isExidEnergy = false;
        for(var i = 0;i<childrenPoints.length;i++){
            if(pointsAll[childrenPoints[i]._id]){
                isExidEnergy = true;
    ...
    TopThreeFirstEcharts(resultData){
        ...
        for(var i= 0;i<resultData.data.length;i++){
            ...
            j++;
            option.series[0].data.push({value:parseFloat(resultData.data[i]).toFixed(0),name:resultData.nameList[i]});
        }
    ...
    TopThreeSecondDataShow(){
        ...
        if(dataList) {
            ...
            WebAPI.post('/analysis/startWorkspaceDataGenHistogram', {
                ...
                $('#costValueSymbol').html(AppConfig.unit_currency);
    ...
    echartsBar(resultData){
        ...
        var option = {
         ...
         resultData.domBox && (chart = echarts.init(resultData.domBox, AppConfig.chartTheme));
