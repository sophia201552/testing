...
(function() {
    ...
    var ModalHtmlConfig = (function() {
        ...
        ModalHtmlConfig.prototype.attachEvents = function () {
            ...
            this.$textarea.on("dragover", function(event) {
                event.preventDefault();
            });
            this.$textarea.on("drop", function(event) {
                event.preventDefault();
                console.log('出发了')
                var itemId = EventAdapter.getData().dsItemId;
                if (Object.prototype.toString.call(itemId) === '[object Array]') {
    ...
    this.ModalHtml = (function(ToolTipMixin) {
        ...
        ModalHtml.prototype.initCustomVaribles = function () {
                    ...
                    case 'EnergyScreen':
                        if(!ctn) {
                            ...
                            if(AppConfig.isFactory){
                                var energyScreen = observer.screens.EnergyScreen;
                                screen = new energyScreen({id: menuId}, ctn);
                                screen.show();
                            }else{
            ...
            this.customVaribles.exportExcel=function(tableId,exportName){
                var headData = [], ContentData = [];
                if (typeof (tableId) == 'object') {
                    headData=tableId.head;
                    ContentData=tableId.data;
                } else {
                    var table = $('#' + tableId);
                    var $th = $('th', table);
                    //th数据
                    $.each($th, function (i, th) {
                        headData.push($(th).text())
                    })
                ...
                WebAPI.post('/logistics/export/excel/2', postData)
                    .done(function (result) {
                        var aTag = document.createElement('a');
                        aTag.download = exportName?exportName:'表格下载' + '.xls'
                        aTag.href = result;
                        aTag.setAttribute('nothash','');
        ...
        ModalHtml.prototype.initCustomExportExcel= function(){
            var _this = this;
            var $container = $(this.container);
            $container.on('click', '[data-export]', function (e) {
                var exportId=this.dataset['export'];
                var exportName=this.dataset['excelname'];
                _this.customVaribles.exportExcel(exportId,exportName)
                e.preventDefault();
            });
        }
        ...
        ModalHtml.prototype.setToolTip = function (node, params){
            ...
            typeof this.initTooltip === 'function' && this.initTooltip({
                shape: domWrap,
                ds: params.dsId,
                    ...
                    Object.defineProperty(ds, name, {
                        ...
                        set: function (value) {
                            ...
                            textNodeMap[name].forEach(function (row) {
                                ...
                                if( (!isNaN(value) || value === 'Null') && !AppConfig.isMobile) {
        ...
        ModalHtml.prototype.renderModal = function() {
            ...
            info = this.getFormatHtml(I18n.trans(options.html) );
