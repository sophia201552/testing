(function($) {
    ...
    ModalDBHistory.prototype.initCustomShow = function() {
        ...
        if (this.modal.type === "ModalMultiple") {
            ...
            // 向前推100个小时
            showOptions.startTimeStr = timeFormat(new Date(now.valueOf() - 360000000 /* 100*60*60*1000 */ ), timeFormatChange('yyyy-mm-dd hh:00'));
            showOptions.endTimeStr = timeFormat(now, timeFormatChange('yyyy-mm-dd hh:00'));
    ...
    ModalDBHistory.prototype.attachEvents = function() {
        ...
        this.$modal.off('hidden.bs.modal').on('hidden.bs.modal', function(e) {
            ...
            _this.reset();
            //_this.$wrap.detach();
            $('#modalDBHistoryWrap.modal-db-history-wrap').detach();
            e.preventDefault();
            e.stopPropagation();
        });
    ...
    ModalDBHistory.prototype.updateChart = function(start, end, timeFmt) {
        ...
        if (!points || !points.length) {
            //处理之前已经拖过的累积量控件 因为点存储在option下面 没有在points下面 这里重新赋值
            if (this.modal.option && this.modal.option.dsItemIds.length !== 0) {
                points = this.modal.option.dsItemIds;
            } else {
    ...
    ModalBase.prototype = {
        ...
        initContainer: function(replacedElementId) {
            ...
            if (!(this instanceof ModalAnalysis)) {
                ...
                // jump button
                if (this.entity.modal.points && this.entity.modal.points.length > 0 || ['ModalCumulantChart'].indexOf(this.entity.modal.type) > -1) {
                    var lkJump, arrPoints;
                    ...
                    switch (this.entity.modal.type) {
                        case 'ModalCumulantChart':
                            try {
                                arrPoints = this.entity.modal.option.dsItemIds;
                            } catch (e) {
                                arrPoints = [];
                            }
                            break;
                        default:
                            arrPoints = this.entity.modal.points;
                            break;
                    }

                    lkJump.onclick = function() {
                        new ModalAppendPointToDs(true, arrPoints, null).show();
                    }
                }

                // '历史数据' 按钮逻辑开始
                if (['ModalHtml', 'ModalMix', 'ModalObserver', 'ModalRank', 'ModalRankNormal', 'ModalNone', 'ModalInteract'].indexOf(this.entity.modal.type) == -1 &&
                    this.entity.modal.points && this.entity.modal.points.length || ['ModalCumulantChart'].indexOf(this.entity.modal.type) > -1) {
        ...
        configure: function() {
            ...
            if (type === 'ModalColdHotAreaSummary' || type === 'ModalEquipmentPerfectRate' || type === 'ModalWorkOrderStatistics' || type === 'ModalPriorityHandlingFaultList' || type === 'ModalEnergyTrendAnalysis' || type === 'ModalEquipmentRateAndHistoryData' || type === 'ModalInteractiveTrendChart' || type === 'ModalPriorityHandlingFaults') {
                Spinner.stop();
                return;
            }
        ...
        modalInit: function() {
            ...
            if (_this.screen.entity && _this.screen.entity.type === 'DashboardWidget') {
        ...
        showConfigModal: function() {
            ...
            var ctn;
            try {
                ctn = this.screen.container.querySelector('.cfgModal');
            } catch (e) {
                ctn = this.screen.shape.parentNode.parentNode.querySelector('.cfgModal')
            }
            if (!ctn) {
        ...
        attachLkHistoryEvents: function(domItem) {
            ...
            domItem.onclick = function(e) {
                ...
                trackEvent('Dashboard历史曲线点击', 'Dashboard.HistoryCurve.Click');
                e.preventDefault();
                e.stopPropagation();
            };
        },
