...
@bp_report.route('/management/getReportPageList/<managementId>', methods=["get"])
def getReportPageList(managementId):
    '''

    :param managementId: 集团id
    :return: {
        "data": [
            {
                "projectNameCn": "Hospital Demo",
                "pageList": [
                    {
                        "type": "ReportScreen",
                        "id": "148523629576507747a5a175",
                        "name": "Data Quality Report"
                    },
                    {
                        "type": "ReportScreen",
                        "id": "1485236342195077fb35f22a",
                        "name": "Monthly Diagnosis Report"
                    }
                ],
                "projectNameEn": "jasperhospita",
                "projectId": 446
            }
        ],
        "msg": "",
    ...
    try:
        ...
        projInfos = management.get_management_project_list([managementId])
        projInfoList = []
        for key, value in projInfos.items():
            if value:
                projInfoList += value
        rv = MongoConnManager.getConfigConn().get_report_by_projIds(projInfoList)
        return Utils.beop_response_success(data=rv)
    except Exception as e:
        print('getReportPageList error:' + e.__str__())
        logging.error(e.__str__())
        return Utils.beop_response_error(msg=e.__str__())

@bp_report.route('/management/save/configPushReport', methods=['POST'])
def saveConfigPushReport():
    try:
        data = request.get_json()
        if not data:
            raise Exception('post data is necessary')
        rv = MongoConnManager.getConfigConn().saveManageReportConfig(data)
        return Utils.beop_response_success(data=rv)
    except Exception as e:
        print('saveConfigPushReport error' + e.__str__())
        logging.error(e.__str__())
        return Utils.beop_response_error(msg=e.__str__())

@bp_report.route('/management/get/configPushReport/<managementId>', methods=['GET'])
def getConfigPushReport(managementId):
    try:
        if not managementId:
            raise Exception('managementId is necessary')
        rv = MongoConnManager.getConfigConn().getManageReportConfig(managementId)
        return Utils.beop_response_success(data=rv)
    except Exception as e:
        print('getConfigPushReport error' + e.__str__())
        logging.error(e.__str__())
        return Utils.beop_response_error(msg=e.__str__())