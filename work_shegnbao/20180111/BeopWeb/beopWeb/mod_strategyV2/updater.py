...
CALC_REMOTE_URL = app.config.get('ALGOSVC3_ADDRESS')  #'http://192.168.1.32:5123'
...
class Updater(object):
    ...
    def update_dqd(self, data):
        ''' 通知算法服务器，更新数据质量检测配置 '''
        update_result = None
        headers = {'content-type': 'application/json'}
        try:
            update_res = requests.post(CALC_REMOTE_URL + '/diagnosis/update/dqd',
                                       json.dumps(data), headers=headers, timeout=300)
            if update_res.status_code == 200:
                update_result = update_res.text
        except Exception as expt:
            print('Error in method "update_dqd": ' + expt.__str__())
        return update_result
