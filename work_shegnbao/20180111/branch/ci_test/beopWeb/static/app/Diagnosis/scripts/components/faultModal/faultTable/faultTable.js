...
;(function (exports, diagnosisEnum){
    class FaultPanelTable {
        ...
        show(){
            var _this = this;
            var postData = {"projectId": AppConfig.projectId};
            var opt = {
                ...
                tbodyCol:[
                    ...
                    {index: 'grade',converter: function (value) {
                        return ('<a style="color:'+ (value["grade"] === 1?'#facc04':'#ea595c') +'";>'+ diagnosisEnum.faultGradeName[value["grade"]] + '</a>')
                    },className: "grade"},
                ...
                onDoneRender: function(value){
                    this.updateState(this.store.faults)
                }.bind(this),
        ...
        updateState(rs) {
            if(rs.status === "OK"){
                ...
                var data = rs.data;
                data.forEach(function(row){
                    $(`tr[data-entityid="${row.entityId}"][data-faultid="${row.faultId}"] td.happen`).removeClass('green').addClass('red').html(I18n.resource.faultModal.ISHAPPEN);
                })
            } 
        }
        attachEvents(){
            ...
            $container.off("click","tbody tr").on("click","tbody tr",function(){
            ...
            if($trActive.length == 0){
                ...
                WebAPI.post("/diagnosis/faults/relatedUsers", {
                    ...
                    if (result.data) {
                        ...
                        this._openUserMapModal(userHasSelected);
                    }
                }).fail(function () {
