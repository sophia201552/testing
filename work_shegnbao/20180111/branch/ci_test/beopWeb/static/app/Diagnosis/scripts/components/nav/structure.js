...
;(function (exports) {
    class Structure {
        ...
        init() {
            ...
            const headHtml = `<div class="itemHead"><span class="headIcon"><span class="out"></span><span class="in"></span></span><span class="headText">${I18n.resource.nav.STRUCTURE}</span></div><ul id="${+new Date()}" class="itemList ztree"></ul>`;
        ...
        getIntactRate() {
            ...
            return $.get('/diagnosis_v2/getEntitiesWrongQuantity',{"projectId": AppConfig.projectId,"startTime":startTime,"endTime":endTime}).done((rs)=>{
                if(rs&&rs.status=='OK'){
        ...
        getData() {
            return $.get('/diagnosis_v2/getEntities',{"projectId": AppConfig.projectId}).done((rs)=>{
                if(rs&&rs.status=='OK'){
