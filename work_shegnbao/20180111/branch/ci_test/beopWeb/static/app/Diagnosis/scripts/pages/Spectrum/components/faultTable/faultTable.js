    ...
    class FaultTable extends Capture(Object) {
        ...
        getAllEnergy(data){
            ...
            for (let j = 0, jlen = arrNoticeData.length; j < jlen; j++) {
                ...
                let endTime = arrNoticeData[j].endTime ? new Date(arrNoticeData[j].endTime).getTime() : new Date().getTime();
                let startTime = new Date(arrNoticeData[j].time).getTime();
        ...
        getEchartsData(arrNoticeData) {
            ...
            for (let j = 0; j < arrNoticeData.length; j++){
                let time = new Date(arrNoticeData[j].time).format('yyyy-MM-dd HH:mm:00');
                let endTime = arrNoticeData[j].endTime;
                let startIndex = this.timeArr.indexOf(time);
                let endIndex;
                if (endTime !== null) {
                    endTime = new Date(arrNoticeData[j].endTime).format('yyyy-MM-dd HH:mm:00');
        ...
        getTimeArr(timeArr) {
            let min = 60000,
                day = 86400000;
            let startTime = +new Date(timeArr.startTime),
                endTime = +new Date(timeArr.endTime) + day;
            this.timeArr = [];
            for (;startTime < endTime; startTime+=min){
                this.timeArr.push(new Date(startTime).timeFormat());
            }
        }
        ...
        attachEvents() {
            ...
            $thisContainer.off('click.tr').on('click.tr', 'tbody tr', function (e) {
                ...
                _this.faultDetailPanel.show(selectData($(this)));
            });
