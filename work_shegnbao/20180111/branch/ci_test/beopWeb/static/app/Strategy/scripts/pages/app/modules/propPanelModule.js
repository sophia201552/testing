;(function(root, factory) {
            ...
            if (items.length > 0) {
                data = (function () {
                    ...
                    if (items.length === 1) {
                        ...
                        [
                            'name', 'status', 'type', 'desc', 'trigger', 'option',
    ...
    var doUpdateFaultTable = function(){
        return function (dispatch, getState) {
            var state = getState();
            var strategyIdList = state.strategyTable.selectedIds;
            dispatch({
                type: PROPpANEL_BTN_LOADING,
                visible: true
            });
            WebAPI.post('/strategy/updateFaultTable', {
                strategyIdList: strategyIdList,
                projectId : AppConfig.projectId
            }).done(function(result){
                if(result.status === 'OK'){
                    message.success(I18n.resource.message.UPDATE_SUCCESS);
                    let actions = namespace('beop.strategy.modules.StrategyTable.actions');
                    let data = deepClone({},state.propPanel.data);
                    data.option['needSyncFaultTable'] = 0;
                    dispatch(changeProp('option',data.option));
                    dispatch(actions.doUpdate(strategyIdList, data));
                } else {
