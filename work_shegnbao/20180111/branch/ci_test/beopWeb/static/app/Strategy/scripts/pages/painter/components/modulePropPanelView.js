...
(function(root, factory) {
    ...
    var theme = {
        propView: function(props) {
            const {data, changeProp, tagList} = props;
            if (data && !$.isEmptyObject(data)) {
                ...
                switch (type) {
                    case enumerators.moduleTypes.PYTHON:
                        theme.configTag(data,changeProp, tagList);
        ...
        configTag: function(data, changeProp, tagList){
            if(data.option){
                let tags = [];
                tagList.forEach(group => {
                    tags = tags.concat(group.tags);
                });
                let options = tags.map(tag=>(h(Option,{
                        value:tag.name
                    },[tag.name])));
                children.push(
                    theme.createRow(I18n.resource.moduleProp.LABEL,h(Select,{
                        placeholder: I18n.resource.placeholder.SELECT_FILL_LABEL,
                        mode:"tags",
                        style:{ width: '100%' },
                        value: data.option.tags,
