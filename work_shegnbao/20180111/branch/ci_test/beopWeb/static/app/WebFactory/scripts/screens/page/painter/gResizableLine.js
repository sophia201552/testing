(function(Group) {
    ...
    function() {
        ...
        this.onResizingShapeActionPerformed = function(e) {
            ...
            //管道吸附
            hitShape = GUtil.getIntersectionByPoint(evt.layerX, evt.layerY, this.painter.getCanvasLayer(), this.painter.getRootLayer(), ['Pipe','CustomLine']);
            if (hitShape) {
                targetPipe = GUtil.getIntersectionByPoint(evt.layerX, evt.layerY, this.painter.getCanvasLayer(), this.painter.getRootLayer());
                if(targetPipe.store.model.type()===_this.shape.store.model.type()){
                    if (targetPipe.store.model._id() != _this.shape.store.model._id()) {
                        //连接点在线上还是头上区别处理
                        if (hitShape.hasName('pipe-line')||hitShape.hasName('customLine-line')) {
                            radius = Math.max(hitShape.strokeWidth() / 2, 6);
                            points = hitShape.points();
                            p = GUtil.getPointProjectionOnLine(
                                // p1
                                [points[0], points[1]],
                                // p2
                                [points[2], points[3]], [transformedX, transformedY]
                            );
                        }
                        if(hitShape.hasName('customLine-line')){
                            var hitShapePoints = hitShape.points();
                            radius = Math.max(hitShape.strokeWidth() / 2, 6);
                            var isNodeRs = isNode(hitShapePoints,evt.transformedX,evt.transformedY,radius,true);
        ...
        this.onResizeShapeActionPerformed = function(e) {
            ...
            var hitShape, targetPipe, points;
            var radius, p;
            var opt, optPoints, pointsArr, pointsArr2, entity;
            if (_this.circle) {
                _this.circle.destroy();
                _this.layer.draw();
            }
                        ...
                        if (hitShape.hasName('pipe-line')) {
                            ...
                            entity = (function(points,opt) {
                                ...
                                entity.option = {
                                    ...
                                    width: opt.width === undefined ? 8 : opt.width,
                                    direction: opt.direction === undefined ? 0 : opt.direction,
                                    preview: [],
                                    logic: opt.logic === undefined ? 0 : opt.logic,
                                    pipeAnimation: opt.pipeAnimation === undefined ? 0 : opt.pipeAnimation
                                };
                                return entity;
                            })(pointsArr2,targetPipe.store.model.option());
    ...
    function isNode(points,mDownX,mDownY,RANGE,isOther){
        if(points.length >= 4){
