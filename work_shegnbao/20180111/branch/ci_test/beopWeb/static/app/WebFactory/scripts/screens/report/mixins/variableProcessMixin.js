    ...
    +function () {
        ...
        this.processTask = (function () {
            ...
            function getDelayTime(row, time, map) {
                ...
                if(p.from){                   
                    timeStart = moment(moment(timeStart) + moment.duration(p.from)).format('YYYY-MM-DD HH:mm:ss');
                }
                if(p.to){
                    timeEnd = moment(moment(timeEnd) + moment.duration(p.to)).format('YYYY-MM-DD HH:mm:ss');
                }
                if(p.etaas){
                    timeEnd = timeFormat(Date.parse(timeEnd.replace(/-/g, '/')) + 1000); 
                }
                return {
                    timeStart: timeStart,
                    timeEnd: timeEnd
