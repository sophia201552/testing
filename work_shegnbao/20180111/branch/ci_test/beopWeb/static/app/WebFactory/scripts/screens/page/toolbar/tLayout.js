(function(){
    ...
    <button class="dropdown-toggle" data-toggle="dropdown" data-type="layoutCtrl" title="对齐" role="button" aria-expanded="false"><span class = "iconfont icon-duiqi"></span></button>\
    <ul class="dropdown-menu dropdown-menu-right" style="left:32px;top:-50px;">\
        <li><a href="javascript:;" data-value="top"><span class="iconfont icon-shangduiqi" aria-hidden="true"></span><span class="topAlign">上端对齐</span></a></li>\
        <li><a href="javascript:;" data-value="right"><span class="iconfont icon-youduiqi" aria-hidden="true"></span><span class="rightAlign">右端对齐</span></a></li>\
        <li><a href="javascript:;" data-value="down"><span class="iconfont icon-xiaduiqi" aria-hidden="true"></span><span class="bottomAlign">下端对齐</span></a></li>\
        <li><a href="javascript:;" data-value="left"><span class="iconfont icon-zuoduiqi" aria-hidden="true"></span><span class="leftAlign">左端对齐</span></a></li>\
        <li><a href="javascript:;" data-value="level"><span class="iconfont icon-shuipingdengjianju" aria-hidden="true"></span><span class="centerAlign">水平等间距</span></a></li>\
        <li><a href="javascript:;" data-value="vertical"><span class="iconfont icon-chuizhidengjianju" aria-hidden="true"></span><span class="moddleAlign">垂直等间距</span></a></li>\
    </ul>\
    </div>';
    ...
    TLayout.prototype.init = function () {
        ...
        $(this.container).find('button[data-type="layoutCtrl"]').attr('title', I18n.resource.mainPanel.toolbar.LAYOUT_TOOL);
        $(this.container).find('.topAlign').text(I18n.resource.mainPanel.tLayout.TOPALIGN);
        $(this.container).find('.rightAlign').text(I18n.resource.mainPanel.tLayout.RIGHTALIGN);
        $(this.container).find('.bottomAlign').text(I18n.resource.mainPanel.tLayout.BOTTOMALIGN);
        $(this.container).find('.leftAlign').text(I18n.resource.mainPanel.tLayout.LEFTALIGN);
        $(this.container).find('.centerAlign').text(I18n.resource.mainPanel.tLayout.CENTERALIGN);
        $(this.container).find('.moddleAlign').text(I18n.resource.mainPanel.tLayout.MODDLEALIGN);
    };
    ...
    TLayout.prototype.attachEvents = function () {
        ...
        $(this.element).children('.dropdown-menu').on('click', 'a', function () {
            ...
            if(len > 1){
                for(var i=0;i<len;i++){
                    if (activeWidgets[i].store.model.type() == 'CanvasPipe' || activeWidgets[i].store.model.type() == 'CanvasHeat' || activeWidgets[i].store.model.type() == 'CanvasPolygon' || activeWidgets[i].store.model.type() == 'CanvasCustomLine') {
                        return;
                    }
                }
                for(var i=0;i<len;i++){
