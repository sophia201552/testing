...
var BasicInfo = (function(){
    ...
    BasicInfo.prototype.show = function(data,opt){
        ...
        this.$assetPhoto = $tabCtn.find('#divAssetImg').empty();//资产照片
        ...
        this.$btnDelImg = $tabCtn.find('#btnDelImg');
        //输入框置为空
    ...
    BasicInfo.prototype.attachEvent = function(){
        ...
        this.$uploadPhoto.off('change').on('change', function(e){
            ...
            formData.append('_id', _this.id)// 附件用到thing id, 每次上传图片都插入一条修改操作记录
            $.ajax({
                ...
                success: function(result){
                    ...
                    result.data.forEach(function(i){
                        _this.$assetPhoto.html('<div class="imgItem"><img src="https://beopweb.oss-cn-hangzhou.aliyuncs.com/' + i + '?_=' + timestamp + '" class="imgAsset" id="imgAsset"/></div>');
                        _this.toggleImgWrap(true);
                    });
                }
            })
        ...
        //删除图片
        this.$btnDelImg.off('click').on('click', (e) => {
            //todo 没有接口
            this.$assetPhoto.find('img').attr('src', '');
            this.$uploadPhoto.val('');//input的值置为空
            this.toggleImgWrap(false);
        });
    };

    BasicInfo.prototype.save = function(){
        var urlImg = this.$assetPhoto.find('img').attr('src');
        if(urlImg && urlImg.indexOf('?')){
        ...
        WebAPI.post('/asset/saveThing',postData).done(function(result){
            ...
            //table选中状态保持
            if(_this.screen.$tbAsset.find('tr.selected').length === 0){
                _this.screen.$tbAsset.find("tr[data-id="+ _this.id +"]").addClass('selected');
            }
        });
    ...
    BasicInfo.prototype.render = function(data){
        ...
        WebAPI.get('/asset/getThingDetail/' + this.id).done(function(result){
            ...
            _this.$purchaseTime.val(result.buyingTime ? formatDate(result.buyingTime) : '');//购置时间
            _this.$overtime.val(result.guaranteeTime ? formatDate(result.guaranteeTime): '');//过保时间
            _this.$putIntoUse.val(result.activeTime ? formatDate(result.activeTime) : '');//投入使用
            _this.$endTime.val(result.endTime ? formatDate(result.endTime) : '');//到期时间
            _this.$productArea.val(result.productArea ? result.productArea : '');//产地
            _this.$productTime.val(result.productTime ? formatDate(result.productTime) : '');//出厂日期
            _this.$serviceLife.val(result.serviceLife ? result.serviceLife: '');//使用寿命

            _this.$remark.val(result.other ? result.other : '');//备注

            if(result.urlImg){
                _this.$assetPhoto.append('<div class="imgItem"><img src="' + result.urlImg + '?_=' + Date.now().valueOf() + '" class="imgAsset" id="imgAsset"/></div>');
            }
    ...
    BasicInfo.prototype.toggleImgWrap = function(isShow){
        if(isShow){
            if(this.$assetPhoto.find('img').attr('src')){//有图片
                if(!this.$btnSaveBaseInfo.is(':hidden')){//编辑模式
                    this.$btnDelImg.show();
                }else{
                    this.$btnDelImg.hide();
                }
                this.$assetPhoto.show();
            }else{
                this.$btnDelImg.hide();
                this.$assetPhoto.hide();
            }
        }else{
            this.$btnDelImg.hide();
            this.$assetPhoto.hide();
