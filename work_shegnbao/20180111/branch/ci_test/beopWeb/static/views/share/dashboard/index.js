...
(function ($, window, undefined) {
    ...
    var Init = function () {
        ...
        ScreenCurrent = new EnergyScreen(hidData.menuId);
        ScreenCurrent.show();
    }
            ...
            for (var k in entities) {
                ...
                if(!entity.entityAnalysis.chart) {
                    ...
                    var isDataEmpty = (function (s) {
                        ...
                        return true;
                    } (entity.entityAnalysis.chart.getOption().series));
                ...
                if(isChartEmpty) {
                    ...
                    posParams.imgSrc = entity.entityAnalysis.chart.getDataURL();
                ...
                if(!graphList || graphList.length == 0){
                    ...
                    //svg模板
                    var temp;
                    var tpl_arrow = '<div class="svgCopyBox" style="width:{boxWidth}px;height:{boxHeight}px;padding:2px;position:absolute;left:{left}%;top:{top}%;z-index:201;">\
                    ...
                    var tpl_circle = '<div class="svgCopyBox" style="width:{boxWidth}px;height:{boxHeight}px;padding:2px;position:absolute;left:{left}%;top:{top}%;z-index:201;">\
                    ...
                    var tpl_rect = '<div class="svgCopyBox" style="width:{boxWidth}px;height:{boxHeight}px;padding:2px;position:absolute;left:{left}%;top:{top}%;z-index:201;">\
                                    <svg class="svgGraphCopy" id="{id}" viewBox="0 0 {width} {height}" width="{width}" height="{height}" >\
                    ...
                    graphList.forEach(function (row) {
                        var boxWidth = parseInt(params.width.split('px')[0]);
                        var boxHeight = parseInt(params.height.split('px')[0]);
                        var downX = row.downX * boxWidth;
                        var downY = row.downY * boxHeight;
                        var upX = row.upX * boxWidth;
                        var upY = row.upY * boxHeight;
                        ...
                        if (row.type == 'arrow') {
                            if (upX >= downX && upY < downY) {
                                temp = tpl_arrow.formatEL({
                                    id: row.id,
                                    width: xDiffer,
                                    height: yDiffer,
                                    marker_id: 'marker_' + row.id,
                                    color: row.color,
                                    path_d: 'M0,' + yDiffer + ' L' + Math.abs(upX - downX - 8) + ',8',
                                    boxWidth: xDiffer + 8,
                                    boxHeight: yDiffer + 8,
                                    left: downX * 100 / boxWidth - 1,
                                    top: upY * 100 / boxHeight + 4
                                });// + '</div>'
                                ...
                                temp = tpl_arrow.formatEL({
                                    ...
                                    marker_id: 'marker_' + row.id,
                                    color: row.color,
                                    path_d: 'M0,0 L' + Math.abs(upX - downX - 8) + ',' + Math.abs(upY - downY - 8),
                                    boxWidth: xDiffer + 8,
                                    boxHeight: yDiffer + 8,
                                    left: downX * 100 / boxWidth - 1,
                                    top: downY * 100 / boxHeight + 4
                                });
                            } else if (upX <= downX && upY <= downY) {
                                temp = tpl_arrow.formatEL({
                                    id: row.id,
                                    ...
                                    marker_id: 'marker_' + row.id,
                                    color: row.color,
                                    path_d: 'M' + xDiffer + ',' + yDiffer + ' L8,8',
                                    boxWidth: xDiffer + 8,
                                    boxHeight: yDiffer + 8,
                                    left: upX * 100 / boxWidth - 1,
                                    top: upY * 100 / boxHeight + 4
                                }) ;
                            } else if (upX < downX && upY >= downY) {
                                temp = tpl_arrow.formatEL({
                                    ...
                                    marker_id: 'marker_' +row.id,
                                    color: row.color,
                                    path_d: 'M' + xDiffer + ',0 L8 ,' + Math.abs(upY - downY - 8),
                                    boxWidth: xDiffer + 8,
                                    boxHeight: yDiffer + 8,
                                    left: upX * 100 / boxWidth - 1,
                                    top: downY * 100 / boxHeight + 4
                            ...
                            temp = tpl_circle.formatEL({
                                ...
                                cx: xDiffer / 2,
                                cy: yDiffer / 2,
                                rx: Math.abs(upX - downX -4) / 2,
                                ry: Math.abs(upY - downY - 4) / 2,
                                boxWidth: xDiffer + 8,
                                boxHeight: yDiffer + 8,
                                left: downX * 100 / boxWidth - 1,
                                top: downY * 100 / boxHeight + 4
    ...
    var initEmailBox = function () {
        ...
        function addEmail(index,val) {
            var row = store[index];
            if($mailTo.find('.selectedMailList').find('li').length === 0){
                var inputBox = '<li class="inputSearchBox">\
                                    <input type="text">\
                                </li>';
                $mailTo.find('.selectedMailList').html(inputBox);
            }
            if(index === undefined){
                if($mailTo.find('[data-mail="'+val+'"]').length > 0) return;
                index = 'selfAdd';
                var html = '<li  class="inner-mail-wrap" data-mail="{1}" data-index="{0}">\
                                <div>\
                                    <span class="mail-addr">&lt;{1}&gt;</span>\
                                </div>\
                                <span class="iconfont icon-cuowu removeEmail"></span>\
                            </li>'
                            .format(index, val)
            }else{
                // 如果指定的地址已经存在，则不添加
                if($mailTo.find('[data-mail="'+row.useremail+'"]').length > 0) return;
                var html = '<li  class="inner-mail-wrap" data-mail="{2}" data-index="{0}">\
                                <div>\
            ...
            $mailTo.find('.selectedMailList .inputSearchBox').before(html);
            selectedIndex.push(Number(index));
        }

        function removeEmail(index) {
            var i = selectedIndex.indexOf(Number(index));
        ...
        function loadMailList() {
            // 如果已经加载过一次，则无需再次加载
            if($mailList.find('.item-mail').length > 0) {
                Spinner.stop();
                return;
            }
        ...
        function showDropdownList() {
            if( $modalSendEmail.find('.dropDwonList').length === 0 ){
                $mailTo.after('<ul class="dropDwonList"></ul>');
                var lis = [];
                ...
                $modalSendEmail.find('.dropDwonList').html(lis.join(''));
            }else{
                $modalSendEmail.find('.dropDwonList').show();
            }
        }

        function showSearchResult(rs){
            var arrHtml = [];
            rs.forEach(function (row) {
                if(row.selected){
                 ...
                 arrHtml.push(
                    '<li data-index="{0}" class="{3}">\
                        <span class="user-name">{1}</span>\
                        <span class="mail-addr">&lt;{2}&gt;</span>\
                        <span class="glyphicon glyphicon-ok"></span>\
                    </li>'
                    .format(row.index, row.con.userfullname, row.con.useremail, className)
                );
            });
            $modalSendEmail.find('.dropDwonList').html(arrHtml.join(''));
        }

        function searchMail(searchVal){
            var result = [];
            for(var i=0,length=store.length;i<length;i++){
                if($.trim(store[i].userfullname).indexOf(searchVal) !== -1 || $.trim(store[i].useremail).indexOf(searchVal) !== -1){
        ...
        function renderContactsList(){
            ...
            store.forEach(function (row, i) {
                ...
                arrHtml.push(
                    '<li class="{2}" data-index="{3}">\
                        <span class="user-name">{0}</span>\
                        <span class="mail-addr">&lt;{1}&gt;</span>\
                        <span class="glyphicon glyphicon-ok"></span>\
                    </li>'
                    .format(row.userfullname, row.useremail, selectedClass, i)
                );
                contactDom.push(
                    '<div class="list-group-item item-mail {3}" data-index="{0}">\
                    <span class="user-name">{1}</span>\
                    <span class="mail-addr">&lt;{2}&gt;</span>\
                    <span class="item-status glyphicon glyphicon-ok-circle"></span>\
                    </div>'
                    .format(i, row.userfullname, row.useremail, selectedClass)
                );
            });
            $modalSendEmail.find('.dropDwonList').html(arrHtml.join(''));
            $mailList.html(contactDom);
        }
        ...
        $modalSendEmail.off('hidden').on('hidden.bs.modal', function () {
            // 清空邮件输入栏
            $mailTo.find('.selectedMailList').html('');
            // 清空下拉列表
            $modalSendEmail.find('.dropDwonList').remove();
            // 取消联系人列表的选中状态
            $mailList.find('.on').removeClass('on');
            selectedIndex = [];
        });

        $modalSendEmail.off('click.failed').on('click.failed', '.item-failed>a', function (e) {
            loadMailList();
            e.stopPropagation();
        });
        //点击收件人框 出现光标
        $mailTo.off('click').on('click',function(){
            if(store===null)return;
            if($(this).find('.inputSearchBox').length === 0 ){
                var inputBox = '<li class="inputSearchBox">\
                                    <input type="text">\
                                </li>';
                $mailTo.find('.selectedMailList').html(inputBox);
            }
            $(this).find('.inputSearchBox input').focus();
            showDropdownList();
        });
        //点击收件人列表
        $modalSendEmail.off('click.mail').on('click.mail', '.item-mail', function () {
            var $this = $(this);
            var index = $this.data('index');
            var isRemove = $this.hasClass('on');
            if(isRemove) {
                $this.removeClass('on');
                $modalSendEmail.find('.dropDwonList li[data-index="'+index+'"]').removeClass('on');
                removeEmail($this.attr('data-index'));
            }else{
                $this.addClass('on');
                $modalSendEmail.find('.dropDwonList li[data-index="'+index+'"]').addClass('on');
                addEmail($this.attr('data-index'));
            }
        });
        //按键抬起
        $modalSendEmail.off('keyup').on('keyup','.inputSearchBox input',function(e){
            var val = $(this).val();
            var textWidth = function(text){ 
                var sensor = $('<pre style="font-size:15px;">'+ text +'</pre>').css({display: 'none'}); 
                $('body').append(sensor); 
                var width = sensor.width()+25;
                sensor.remove(); 
                return width;
            };
            $(this).width(textWidth(val));
            searchMail(val);
            if(val === ''){
                renderContactsList();
            }
            if (13 == e.keyCode) {
                if( !(val === '' || $('.dropDwonList').find('li').length === 0)){
                    var index = $('.dropDwonList').find('li:first-child').data('index');
                    addEmail(index);
                    $(this).val('').focus();
                    renderContactsList();
                }else if($('.dropDwonList').find('li').length === 0){
                    ...
                    if (filter.test(val)){
                        ...
                        $('.dropDwonList').html('<li style="text-align:center;"><span>'+I18n.resource.shareBoard.VALID_EMAIL+'</span></li>')
                    }
                }
            }
        });
        $modalSendEmail.off('keydown').on('keydown','.inputSearchBox input',function(e){
            if( $(this).val() ==='' && (8 == e.keyCode || 46 == e.keyCode)){
                var removeIndex = $(this).closest('li').prev('li').data('index');
                if(removeIndex === 'selfAdd'){
                    $(this).closest('li').prev('li').remove();
                }else{
                    removeEmail(removeIndex);
                    renderContactsList();
                }
            }
        });
        $modalSendEmail.off('click.modal-dialog').on('click.modal-dialog','.modal-dialog',function(){
            $('.dropDwonList').hide();
        });
        $modalSendEmail.off('click.form-group').on('click.form-group','.form-group:first-child',function(event){
            event.stopPropagation(); 
        });
        
        $modalSendEmail.off('click.dropdown').on('click.dropdown','.dropDwonList li',function(){
            var index = $(this).data('index');
            $('.item-mail[data-index='+index+']').trigger('click');
        });
        //删除选中的收件人
        $modalSendEmail.off('click.remove').on('click.remove','.removeEmail',function(){
            var removeIndex = $(this).closest('.inner-mail-wrap').data('index');
        ...
        $btnSend.click(function () {
            var list = $mailTo.find('.inner-mail-wrap').map(function () {return $(this).attr('data-mail')});
    ...
    $(function () {
        //whether is running with core device.
