    ...
    PaneProjectSelector.prototype = {
        ...
        showPanelSelector: function() {
            ...
            return WebAPI.get(configMap.paneTemplate).done(function(resultHtml) {
                ...
                imgListCon.append('<div id="AddProjectInHomePage" class="media effect">+</div>');
                //添加版本历史支持
        ...
        init: function() {
            ...
            $labelChangeSkin.eventOff('click').eventOn('click', function(e) {
                ...
                var skinModal = '<div class="modal fade" id="skinManege" tabindex="-1" role="dialog" style="margin-top:60px;">\
                  <div class="modal-dialog" role="document">\
                    <div class="modal-content">\
                      <div class="modal-header">\
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>\
                        <h4 class="modal-title" i18n="admin.ddlUser.MENU_CHANGE_SKIN_TITLE">皮肤切换</h4>\
                      </div>\
                ...
                if($skinManege.length!=0) {
                    $skinManege.empty().remove();
                }
                $('#indexMain').append(skinModal);
                var $skinManege = $('#skinManege');
                I18n.fillArea($skinManege);
                $skinManege.modal('show');
                new SkinSelector().show($skinManege.find('.modal-body'));
            }, 'navTool-skinLabel');
        ...
        getProjectLineState: function(projectId) {
            return $.when(WebAPI.get('/project/status/' + projectId), WebAPI.post('/project/status/history/', {
                'projectId': AppConfig.projectId,
                'dtu': '',
                'startTime': '',
                'endTime': ''
        ...
        initNavigationPane: function(navItems) {
            ...
            function replaceClass(navItemPic,liTag){
                if(navItemPic && navItemPic.indexOf('iconfont')>=0){
                    liTag = liTag.replace('glyphicon glyphicon-<%iconMenuPicDefault%>', navItemPic);
                }else{
