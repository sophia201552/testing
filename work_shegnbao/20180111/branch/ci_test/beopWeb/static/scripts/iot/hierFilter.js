...
var HierFilter = (function() {
    ...
    HierFilter.prototype = {
                ...
                $.when(
                    ...
                    WebAPI.post('/iot/search', postData).done(function(resultDate) {
                        ...
                        if (resultDate.projects.length == 0) {
                            $.when(_this.setIotProject()).done(function(result) {
        ...
        setIotProject: function() {
            var projectList = [];
            if (!(AppConfig && (AppConfig.projectList instanceof Array))) {
                if (!(window.parent && window.parent.AppConfig.projectList)) return;
                projectList = window.parent.AppConfig.projectList;
            } else {
                projectList = AppConfig.projectList;
            }
            if (!this.projectId) return;
            var info;
            for (var i = 0; i < projectList.length; i++) {
                if (projectList[i].id == this.projectId) {
                    info = projectList[i];
                    break;
                }
            }
                ...
                if (13 == e.keyCode) {
                    ...
                    if ($iptSearch.val() == '') {
                        ...
                        postData = {
                            ...
                            projId: projId ? projId : _this.projectList
                        };
                        if (!_this.opt.base.divideByProject) delete postData.projId;
            ...
            if (_this.opt.tree && _this.opt.tree.show) {
                ...
                _this.$paneTree.on('click', '.btnDelete', function(e) {
                    ...
                    function okCallback() {
                        ...
                        WebAPI.post('/iot/delIotInfo', { itemList: postData, projId: treeNode.projId }).done(function() {
                ...
                _this.$paneTree.on('click', '.projects>.button.switch', function(e) {
                    ...
                    var treeId = _this.tree.setting.treeId;
                    //node.open = !node.open;
                    var func = function() {
                        if (_this.opt.tree.event.click) {
                            if (!_this.opt.tree.event.click instanceof Array) {
                                _this.opt.tree.event.click.call(_this, e, treeId, treeNode);
                            } else {
                                for (var i = 0; i < _this.opt.tree.event.click.length; i++) {
                                    if (_this.opt.tree.event.click[i].tar.toString().indexOf(node.baseType) > -1 || _this.opt.tree.event.click[i].tar == 'all') {
                                        _this.opt.tree.event.click[i].act.call(_this, e, treeId, node)
                                    }
                                }
                            }
                        }
                    }
                    _this.childAppend(node, func);
                    return false;
                });
                _this.$paneTree.on('click', '.groups>.button.switch', function(e) {
                    ...
                    var func = function() {
                        if (_this.opt.tree.event.click) {
                            if (!_this.opt.tree.event.click instanceof Array) {
                                _this.opt.tree.event.click.call(that, e, treeId, treeNode);
                            } else {
                                for (var i = 0; i < _this.opt.tree.event.click.length; i++) {
                                    if (_this.opt.tree.event.click[i].tar.toString().indexOf(node.baseType) > -1 || _this.opt.tree.event.click[i].tar == 'all') {
                                        _this.opt.tree.event.click[i].act.call(_this, e, treeId, node)
