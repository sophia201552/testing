    ...
    BenchmarkScreen.prototype = {
        ...
        initBtnList: function() {
            ...
            Object.keys(this.moduleList).forEach(val => {
                //如果is_diag字段为false, 不显示诊断模块
                if (val === 'diagnosis') {
                    for (var i of AppConfig.projectList) {
                        if (i.id == AppConfig.projectId) {
                            if (!i.is_diag) {
                                return;
                            }
                        }
        ...
        setNodeConfigStatus: function() {
            ...
            for (var i = 0; i < arrNode.length; i++) {
                ...
                if (ptConfig) {
                    ...
                    if (ptConfig.model) {
                        if (ptConfig.model instanceof Array && ptConfig.model.length > 0) {
                            $target.addClass('hasModel');
                        } else if (!jQuery.isEmptyObject(ptConfig.model)) {
                            $target.addClass('hasModel');
                        }
                    }
                    num++
