    ...
    PointManager.prototype = {
        ...
        highlightMenu: function () {
            this.jqueryMap.$menuContainer.find('.page-link').removeClass('active').each(function (index, item) {
                ...
                //引用记录跳转高亮问题
                if (location.hash.indexOf('&pointID') !== -1) {
                    hash = location.hash.substr(0, location.hash.indexOf('&pointID'));
                } else if (location.hash.indexOf('&pointName') !== -1) {
                    hash = location.hash.substr(0, location.hash.indexOf('&pointName'));
                } else if (location.hash.indexOf('&router') !== -1) {
                    hash = location.hash.substr(0, location.hash.indexOf('&router'));
                }
