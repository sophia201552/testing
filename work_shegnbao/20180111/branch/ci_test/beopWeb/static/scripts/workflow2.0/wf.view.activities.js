(function (beop) {
    ...
    renderActivities = function () {
        ...
        if (beop.model.stateMap.activities.totalCount > 0) {
            ...
            stateMap.curWorker = stateMap.curWorker.concat(beop.model.stateMap.activities.msg.reverse());
...
//加载更多
    onGetMoreActivitiesClick = function () {
        ...
        configMap.activities_model.getActivities(beop.model.stateMap.activities.activity_type, stateMap.page).done(function (result) {
            if (result.success) {
                beop.model.stateMap.activities.msg = stateMap.curWorker.concat(result.data.msg);
...
//---------事件---------
    ...
    onReplySubmit = function () {
        var $form = $(this);
        var paramReply = $form.serializeObject();
        var $formLastReply = $form.parent().prev();
        configMap.reply_mode.insertReplyToComment(paramReply).done(function (result) {
            if (result.success) {
                var momentTime, titleTime;
                titleTime = timeFormat(new Date(), timeFormatChange(DateUtil.DATA_FORMAT.FULL_DATETIME));
                moment.locale(localStorage.getItem('language'));
                momentTime = moment(new Date());
                var activities = {
                    content: paramReply.content,
                    picture: AppConfig.userProfile.picture,
                    userName: AppConfig.userProfile.fullname,
                    replyTime: momentTime.fromNow(),
                    userId: AppConfig.userProfile.id,
                    titleTime: titleTime
                };
                var html = beopTmpl('workflow_done_reply', {
                    activities: activities
                });

                $formLastReply.after(html);

                $form.find('textarea.reply-text-area').val('');
                Alert.success(ElScreenContainer, I18n.resource.workflow.activities.REPLY_SUCCESS).showAtTop(2000);
            }
        });
    };
