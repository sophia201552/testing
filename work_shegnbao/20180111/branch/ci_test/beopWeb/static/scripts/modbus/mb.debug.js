...
(function (exports) {
    class Debug extends exports.BaseView {
        ...
        requestCommand(postData) {
            WebAPI.post('/modbus/dtu/debug/command', postData).done(function (result) {
                if (result.success) {
                    var logHtml = '', dataShowHtml = '', data = result.data;
                    ...
                    alert(I18n.resource.common.SERVER_REQUEST_FAILED);
                }
            }).fail(function (e) {
        ...
        _attachEvent() {
            ...
            $("#modbusContainer").off('click.equipDebugSend').on('click.equipDebugSend', '#equipDebugSend', function () {
                ...
                WebAPI.post('modbus/dtu/debug/equipment', postMap).done(function (result) {
                    if (result.success) {
                        var logHtml = '', dataShowHtml = '', data = result.data;
                        for (var i = 0; i < data.log.length; i++) {
                            logHtml += '<li>' + data.log[i].log + '</li>';
                        ...
                        alert(result.data.msg || I18n.resource.common.REQUEST_ERROR);
                    }
                }).fail(function (e) {
                    alert.danger(I18n.resource.common.SERVER_REQUEST_FAILED);
                }).always(function () {
