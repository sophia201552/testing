    ...
    try:
        if post_data:
            ...
            if post_data.get('tags'):
                ...
                res = pointTag.get_thingTree_detail(projId, prt, searchText, hasTag, limit, skip, isAll)
        else:
            error = 'Invalid parameter'
    except Exception as e:
...
<<<<<<< .mine
@bp_tag.route('/tagTransfer/<int:projId>', methods=['GET'])
def tag_transfer():
=======
@bp_tag.route('/dataTransfer/<int:projId>', methods=['POST'])
def tag_transfer(projId):
>>>>>>> .r42566
    equipment_list = pointTag.get_equipment()
    tag_tree = pointTag.import_tag(equipment_list, projId)
    pointTag.export_tag(tag_tree)
    return Utils.beop_response_success()
<<<<<<< .mine
=======
...
def energy_config_get(projId):
    rt = pointTag.energy_config_get(projId)
    return Utils.beop_response_success(rt)
...
def point_auto_match_v2():
    ...
    for node, tag_list in zip(data, c_list):
        pointTag.add_tag_list(node['id'], tag_list)

    return Utils.beop_response_success()

@bp_tag.route('/testci', methods=['GET'])
def test_ci():
    return Utils.beop_response_success('pupupupupupupupupupu')
