            ...
            if(chapter['store'][i]['template'] == 'table'):
                if (chapter['store'][i]['role'] == 'title'):
                    ...
                    for j in range(0,len(chapter['store'][i]['data'])):
                        ...
                        if temp_row == 1:
                            if chapter['store'][i]['role'] == 'head' :
                                sheet.write(row,temp_col1_offset+j,data,set_style('table','head',excel))
                            elif (i - headCol)% 2 == 1:
                                sheet.write(row,temp_col1_offset+j,data,set_style('table','odd',excel))
                            else:
                                sheet.write(row,temp_col1_offset+j,data,set_style('table','even',excel))
                        else:
                            if chapter['store'][i]['role'] == 'head' :
                                headCol = i
                                sheet.merge_range(row,temp_col1_offset+j,row+temp_row-1,temp_col1_offset+j,data,set_style('table','head',excel))
                            elif (i - headCol)% 2 == 1:
                                sheet.merge_range(row,temp_col1_offset+j,row+temp_row-1,temp_col1_offset+j,data,set_style('table','span',excel))
                            else:
                                sheet.merge_range(row,temp_col1_offset+j,row+temp_row-1,temp_col1_offset+j,data,set_style('table','span',excel))
                    row = row + 1
