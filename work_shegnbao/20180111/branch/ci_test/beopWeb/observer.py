        ...
        if preFilterInfo is not None and len(preFilterInfo) >= 2:  # all new cloud points and only one project
            ...
            if 'error' not in data.keys():
                ...
                for ptName, ptValueList in ptDataList.items():
                    ...
                    tempPVDict['@%s|%s' % (projId, ptName)] = ptValueList
                try:
                    if len(ptAttrList.get(ptName)) == len(ptDataList.get(ptName)):
            ...
            for itemvar in itemVarIdList:
                value_array = tempPVDict.get(itemvar, [])
                '''if len(value_array) > 0:  # 直接将获得的点值进行两位小数保留
                    value_array = [effect_num(x, 2) if x is not None else None for x in value_array]'''
                        ...
                        try:
                            ...
                            if (not e_1[index]) and (not e_1[index - 1]):#判断原始数据是否为真实值，false为真实值
                                if differ < 0:
                                    ...
                                    differ = effect_num(differ, 2) if differ is not None else None
                                    flag.append(0)
                                    data_new.append(differ)
                            else:
                                differ = effect_num(differ, 2) if differ is not None else None
                                flag.append(1)
                                data_new.append(differ)
                ...
                if d_l and e_1:
                    ...
                    for index in range(1, len(d_l), 1):
                        try:
                            ...
                            data_new.append(0)
                            flag.append(0)
                    if datetime.strptime(timeEnd, '%Y-%m-%d %H:%M:%S') > datetime.now():
                        if len(data_new) > 0:
                            data_new = data_new[:-1]
                            flag = flag[:-1]
                            ...
                            if rt:
                                data_new.append(float(rt[0].get('value'))-d_l[len(d_l)-1])
                                if status:#如果倒数第二个数是假的，则相减后的数值为假的
                                    flag.append(1)
                                else:
                                    flag.append(0)
                    item.update({'data': data_new, 'errorFlag': flag})
        ...
        for rd in request_data:
            ...
            for tm in tms:
                ...
                for key in group_info:
                    ...
                    rt_his = BEOPDataAccess.getInstance().get_history_data_padded(key, pointNameList, timeStart,
                                                                                  timeEnd, format)
...
def startWorkspaceDataGenHistogram_fuc(timeStart, timeEnd, timeFormat, dataSouceId=[], itemVarIdList=[],
