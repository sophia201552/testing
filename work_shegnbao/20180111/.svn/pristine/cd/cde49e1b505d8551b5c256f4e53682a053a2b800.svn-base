...
const TABLE_NAME_EN = 'diagnosis_fault_2';
const TABLE_NAME_ZH = 'diagnosis_fault_2_zh';
...
const generateFault = function (data, lang='en') {
    ...
    return {
        ...
        "description": isEn ? 
            `${data.Detail}. ${data.Suggestion || ''}.` :
            `${data.DetailCh}。${data.SuggestionCh || ''}。`,
        "grade": data.Grade,
...
'${obj.name.replace(/'/g, '\\\'')}', '${obj.description.replace(/'/g, '\\\'')}', '${obj.grade}', '${obj.faultType}', \
'${obj.faultGroup}', '${obj.runMode}', '${obj.consequence}', '${obj.runDay}', \
'${obj.runWeek}', '${obj.runMonth}', '${obj.runYear}', '${obj.chartTitle}', \
'${obj.lastModifyUser}', '${obj.lastModifyTime}', '${obj.className}', '${obj.isPublic}', '${obj.imported}');`
}

const generateDeleteSql = (lang='en') => {
    let isEn = lang === 'en';
    return `DELETE FROM ${isEn ? TABLE_NAME_EN : TABLE_NAME_ZH};`;
}
        ...
        (row, i) => {
            ...
            if (!row['Name']) {
                ...
                if (!chartInfo) {
                    ...
                    let unique = row['Name'] + row['FullName'] + row['Detail'] + row['Suggestion'];
