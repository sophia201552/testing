    ...
    var DmTagTreeEditFunc = {
                                ...
                                WebAPI.post('/tag/moveThings', {
                                    ...
                                    if (result.success) {
                                        if (treeNode._id !== _this.nodeId) {
                                            stateMap.allDragPoints = _this.points + _this.folderHavePoints;
                            ...
                            if (clickType) {//点击图标
                                if (treeNode.isParent) {
                                    if (clickType === 'addNode') {
                                        ...
                                        confirm(I18n.resource.tag.edit.THE_AUTO_MATCH_DEVICE_TYPE + '<select class="form-control" id="autoMatchType"><option value="AHU">AHU</option><option value="CH">CH</option></select>', function () {
                                            $("#ruleForm").remove();
                                            $catalogName.show();
            ...
            for (var i = 0; i < parentNodes.length; i++) {
                if (parentNodes == _this.treeParentsNodes) {
                    if (stateMap.drag == dragType.keyword) {
                        ...
                        parentNodes[i].pointCount -= stateMap.allDragPoints;
                    }
                    if (parentNodes['' + ilen + '']._id == node._id && !node.prt) {
                        parentNodes[i].noGroupPointCount -= stateMap.allDragPoints;
                    }
                } else {
                    if (stateMap.drag == dragType.keyword) {
                        parentNodes[i].pointCount += _this.dragPointsCount;
        ...
        loadAutoMatchResult: function (matchResult) {
            ...
            var dataTableOptions = {
                ...
                dataFilter: function (result) {
                    if (result.success) {
                        ...
                        for (var i = 0; i < _this.pointList.length; i++) {
                            ...
                            if (matchResult && matchResult[point.name] && matchResult[point.name] === 100) {
                ...
                totalNumIndex: 'data.count',
                colNames: [
                    I18n.resource.tag.edit.NAME,
                    I18n.resource.tag.edit.REMARK,
                    I18n.resource.tag.edit.VALUE,
                    I18n.resource.tag.edit.RECOMMENDED_TAG,
                    I18n.resource.tag.edit.SUITABILITY,
                    I18n.resource.tag.edit.REFERENCE
                ],
                colModel: [
                    {
                        index: 'name', html: true, width: '296px', highlight: true,
