...
class BaseConfigModule {
    ...
    initOption() {
        if (this.defaultOpt) {
            ...
            if (this.area.custom) {
                this.initModuleWidget(this.area.custom)
            }
        }
    }
    initModuleWidget(dictWidget) {
        Object.keys(dictWidget).forEach(selector => {
            if (selector[0] == "#") {
                this.area.widget.forEach(w => {
                    if (w.id == selector.slice(1)) {
                        this.area.widget[w] = $.extend(true, {}, this.area.widget[w], dictWidget[selector])
                    }
                })
            } else {
                this.area.widget.forEach((w, index) => {
                    if (w.type == selector) {
                        this.area.widget[index] = $.extend(true, {}, w, dictWidget[selector])
                    }
                })
            }
        })
...
class TimeConfigModule extends BaseConfigModule {
                                ...
                                if (type == 'mode') {
                                    switch (store.val) {
                                        case '0':
                                            ...
                                            break;
                                        default:
                                            if($(this.objModal.modal).find('.divOption').attr('data-type')&&$(this.objModal.modal).find('.divOption').attr('data-type')=='ModalHistoryChartYearOnYearLine') break;
                    ...
                    subWidget: [{
                        ...
                        callback: function(type, store) {
                            if (type == 'recentTimeCustom') {
                                ...
                                this.widget.data = { val: firstSelect };
                                var $widgetDivOption = $(this.widget.dom).parent('.divOption');
                                if($widgetDivOption.attr('data-type')&&$widgetDivOption.attr('data-type')=='ModalHistoryChartYearOnYearLine'){
                                    $widgetDivOption.next('.divOption').find('select').val(firstSelect);
                                }
                                this.widget.instantiation.initData();
                            }
                        }
                    }]
                },
                {
                    ...
                    subWidget: [{
                        ...
                        callback: function(type, store) {
                            ...
                            this.widget.dom.querySelector('select').value = interval;
                            this.widget.data = { val: interval };
                            this.widget.instantiation.initData();
                        }
                    }]
                },
    ...
    initData() {
        ...
        for (let i = 0; i < this.area.widget.length; i++) {
            data = undefined;
            switch (this.area.widget[i].type) {
                case 'mode':
                    if (typeof this.area.data.mode != 'undefined') data = { val: this.area.data.mode };
                    break;
                case 'interval':
                    if (typeof this.area.data.interval != 'undefined') data = { val: this.area.data.interval };
                    break;
                case 'recentTime':
                    if (typeof this.area.data.timeRecent == 'string') data = this.area.data.timeRecent;
                    break;
                case 'recentTimeCustom':
                    if (['undefined', 'string'].indexOf(typeof this.area.data.timeRecent) == -1) data = this.area.data.timeRecent;
                    break;
                case 'date':
                    if (this.area.data.timeStart || this.area.data.timeEnd) {
                        data = {};
                        this.area.data.timeStart && (data.timeStart = this.area.data.timeStart);
                        this.area.data.timeEnd && (data.timeEnd = this.area.data.timeEnd);
                    }
                    break;
            }
            if (typeof data != 'undefined') this.area.widget[i].data = data;
        }
    }
    getData() {
        for (let i = 0; i < this.area.widget.length; i++) {
        ...
        if (this.store.mode.val == '0') {
            result = {
                timeRecent: this.store.recentTime.recentTime
            }
        } else if (this.store.mode.val == '1') {
            result = {
                timeStart: this.store.date.timeStart,
                timeEnd: this.store.date.timeEnd
            }
        ...
        result.interval = this.store.interval.val;
        return result;
    }
}
    ...
    constructor(objModal, area) {
        ...
        this.defaultOpt = {
            ...
            widget: [{
                    ...
                    opt: {
                        option: [
                            { val: 'high', name: I18n.resource.modalConfig.option.GAUGE_MODE_HIGH },
                            { val: 'low', name: I18n.resource.modalConfig.option.GAUGE_MODE_LOW }
                        ]
                    },
                    subWidget: [{
                        type: 'range',
                ...
                },
                {
                    ...
                    opt: {
                        input: [
                            { val: '', suffix: { text: I18n.resource.modalConfig.option.GAUGE_GREEN, style: { background: 'lightseagreen' }, cls: 'gaugeGreen' } },
                            { val: '', suffix: { text: I18n.resource.modalConfig.option.GAUGE_NORMAL, style: { background: '#428bca' }, cls: 'gaugeBlue' } },
                            { val: '', suffix: { text: I18n.resource.modalConfig.option.GAUGE_RED, style: { background: '#ff6c60' }, cls: 'gaugeRed' } },
                            { val: '' }
                        ]
                    }
                }
