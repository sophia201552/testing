                        ...
                        promise.done(function(html) {
                            ...
                            xhr.onload = function() {
                                ...
                                if (this.status === 200) {
                                    ...
                                    lkPdfFile.href = url;
                                    lkPdfFile.download = reportTitle + ' ' + _this.getReportShownDate({
                                        format: 'week'
                                    }) + '.pdf';
            ...
            $('.excelDownCtn').eventOff('click').eventOn('click', function() {
                ...
                xhr.onload = function() {
                    ...
                    if (this.status === 200 && this.response) {
                        ...
                        // 是因为 window.open 不能自定义下载文件的名称
                        // 而 a 标签可以通过设置 download 属性来自定义下载文件的名称
                        lkExcelFile = document.createElement('a');
                        lkExcelFile.style = "display: none";
                        lkExcelFile.id = "lkExcelFile";
                        lkExcelFile.href = url;
                        lkExcelFile.setAttribute('nothash', true)
                        lkExcelFile.download = curReport.reportName + ' ' + _this.getReportShownDate({
                            format: curReport.period
                        }) + '.xlsx';

                        document.body.appendChild(lkExcelFile);
                        lkExcelFile.click();
                        window.URL.revokeObjectURL(url);
                        window.setTimeout(function() {
                            lkExcelFile.parentNode.removeChild(lkExcelFile);
                        }, 0);
                    } else {
                        alert('Generate report failed, please try it again soon!');
                    }
        ...
        this.setExcelChapterStore = function($ctn) {
            ...
            if ($ctn[0].querySelector('table')) {
                $ctn.find('table').each(function(index, tb) {
                    ...
                    $tb.find('thead th').each(function(i, th) {
                        if(th.getAttribute('rowspan')){
                            arrHead.push({row:parseInt(th.getAttribute('rowspan')),content:th.innerHTML})
                        }else {
                            arrHead.push(th.innerHTML)
                        }
                    })
                    $tb.find('thead td').each(function(i, th) {
                        if(th.getAttribute('rowspan')){
                            arrHead.push({row:parseInt(th.getAttribute('rowspan')),content:th.innerHTML})
                        }else {
                            arrHead.push(th.innerHTML)
                    ...
                    $tb.find('tbody tr').each(function(i, tr) {
                        ...
                        $(tr).find('td').each(function(j, td) {
                            if(td.getAttribute('rowspan')){
                                arrData.push({row:parseInt(td.getAttribute('rowspan')),content:td.innerHTML})
                            }else {
