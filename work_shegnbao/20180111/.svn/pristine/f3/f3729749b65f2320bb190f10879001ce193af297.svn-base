...
var UserSelScreen = (function(){
    ...
    UserSelScreen.navOptions = {
        ...
        <span id="btnRecord" class="topNavRight zepto-ev glyphicon glyphicon-th-list"></span>\
    ...
    UserSelScreen.prototype = {
        ...
        initNav:function(){
            $('#btnUpdate').off('tap').on('tap',function(){
                $(document.getElementById('ctnUpdateRecord')).addClass('hide')
                dataManager.update(_this.init);
                //dataManager.Deferred.done(function(){
                //    _this.init();
                //});
            });
            $('#btnRecord').off('tap').on('tap',function(){
                _this.toggleUpdateRecord();
            });
            ...
            document.getElementById('ctnUpdateRecord').onclick = function(){
                $(this).addClass('hide')
            }
        },
        toggleUpdateRecord:function(){
            var container = document.getElementById('ctnUpdateRecord');
            if ($(container).hasClass('hide')){
                container.innerHTML = ''
                AppConfig.updateRecord.forEach(function(item){
                    var dom = document.createElement('div');
                    dom.className = 'divRecord';
                    dom.innerHTML = '<span class="time">'+ new Date(item.time).format('yyyy-MM-dd HH:mm:ss') +'</span><span class="text">'+ item.text +'</span>';
