        ...
        this.attachEvents = function () {
            ...
            if (event.type === 'hover') {
                this.shape.on('mouseenter', function (e) {
                    if (e.evt && e.evt.type !== 'mousemove') {
                        return false;
                    }
                    if (e.evt && (!e.evt.target || !$(e.evt.target).hasClass('html-layer'))) {
                        return false;
                    }
                ...
                this.shape.on('mouseleave', function (e) {
                    if (e.evt && e.evt.type !== 'mousemove' && e.evt.type !== 'mouseout') {
                        return false;
                    }
                    if (e.evt && (!e.evt.target || !$(e.evt.target).hasClass('html-layer'))) {
                        return false;
                    ...
                    console.log('mouseleave', relatedTarget);
                    if (relatedTarget instanceof HTMLElement && e.evt.type === 'mouseout') {
                        var $wrap = $(relatedTarget).parentsUntil('.html-layer').filter('.html-container');
                        if ($wrap.length) {
                            $($wrap).off('mouseleave').one('mouseleave', function (event) {
                                console.log('mouseleave2');

                                if (_this.shape !== _this.painter.getStage().getIntersection({
                                    x: event.pageX,
                                    y: event.pageY - 53
                                })) {
                                    maskShape.hide();
                            ...
                            $($wrap).off('mouseout').on('mouseout', function (e) {
                                e.preventDefault();
                                e.stopPropagation();
                            });
                        }
