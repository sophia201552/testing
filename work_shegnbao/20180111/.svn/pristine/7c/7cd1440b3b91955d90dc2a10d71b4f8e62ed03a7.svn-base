;(function(root, factory) {
    ...
    let formatter = {};
    let fuzzyRuleParser = exports.fuzzyRuleParser = {};

    const fromModule = formatter.fromModule = module => {
        let arr = [];
        let option = module.option;
        ...
        // Output
        option.output.forEach(
            output => {
                arr.push(fromOutput(output));
            }
        );
        arr.push(fromRuleBlock(option.content.ruleBlock));
    ...
    const fromTerm = formatter.fromTerm = term => {
        ...
        switch(term.type) {
            ...
            case enums.fuzzyRuleShapeTypes.TRAPEZOID:
                arr.push('Trapezoid');
                break;
            case enums.fuzzyRuleShapeTypes.GAUSSIAN:
                arr.push('Gaussian');
                break;
            case enums.fuzzyRuleShapeTypes.ZSHAPE:
                arr.push('ZShape');
                break;
            case enums.fuzzyRuleShapeTypes.SSHAPE:
                arr.push('SShape');
                break;
            default:
                arr.push('Unknow');
        ...
        // 点
        term.points.forEach( p => arr.push(p.toFixed(3)) );
    ...
    const fromInput = formatter.fromInput = input => {
        ...
        obj[PARSER_STRUCT_BODY] = [{
            ...
            k: 'range',
            v: [option.min.toFixed(3), option.max.toFixed(3)].join(' ')
        }, ...option.terms.map(
            term => {
                return {
                    k: 'term',
    ...
    const fromRule = formatter.fromRule = rule => {
        ...
        let arr = items.concat(results).map(
            row => [
                row.continuity,
                row.name,
                row.judge,
    ...
    const parseTerm = fuzzyRuleParser.parseTerm = str => {
        ...
        let arr = str.split(' ');
        let term = {};
        let type = enums.fuzzyRuleShapeTypes[arr[1].toUpperCase()];
     ...
     */
    fuzzyRuleParser.stringify = module => {
        let structList = fromModule(module);
        let arr = [];
    ...
    ['Input', 'Output', 'RuleBlock', 'Rule', 'Term'].forEach(
        name => {
            fuzzyRuleParser[`stringify${name}`] = struct => stringify$1(formatter[`from${name}`](struct));
        }
    );
}));