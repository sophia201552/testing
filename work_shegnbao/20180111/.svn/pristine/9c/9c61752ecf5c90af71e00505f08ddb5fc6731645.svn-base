...
! function($) {
    ...
    var Datetimepicker = function(element, options) {
        ...
        if (this.showWeek) {
            ...
            var nowWeek = this.iso8601Week(oldDate);
            this.element.val(oldDate.getFullYear() + '-' + nowWeek + '-week');
        }
    ...
    Datetimepicker.prototype = {
        ...
        update: function() {
            ...
            // if (typeof date == 'string') {
            //     var zone =new Date(date).getTimezoneOffset() / 60
            //     var symbol = zone > 0 ?'-':'+'
            //     var value = Math.abs(zone)
            //     var intVal = Math.floor(value)
            //     var floatVal = (value - intVal) * 60
            //     if(value >= 10){
            //         zone = (intVal*100 + floatVal).toString()
            //     }else{
            //         zone = '0' + (intVal*100 + floatVal)
            //     }
            //     date = date + ' UTC'+symbol+ zone
            // }
            if (typeof date == 'string' && date.search(/[a-zA-Z]/g) < 0 ) {
                var strDateAndTime = date.replace(/^\s+|\s+$/g, '').replace(/\s+/g,' ').replace(/-/g,'/')
                ...
                date = [strYear,strMonth,strDay].join('/') + ' ' + [strHour,strMin,strSecond].join(':')
            }
            if (!date) {
                date = new Date();
                fromArgs = false;
            }
            this.date = DPGlobal.parseDate(new Date(date), this.format, this.language, this.formatType);
                ...
                switch (target[0].nodeName.toLowerCase()) {
                        ...
                        switch (target[0].className) {
                            ...
                            case 'today':
                                ...
                                if (this.showWeek) {
                                    ...
                                    this.weeklyString = date.getFullYear() + '-' + week + '-' + 'week';
                                }
                    ...
                    case 'td':
                        if (target.is('.day') && !target.is('.disabled')) {
                            ...
                            if (this.showWeek) {
                                this.weeklyString = new Date(this.viewDate).getFullYear() + '-' + $(e.target).parent().find('td:first').text() + '-week';
                                this.element.trigger({
                                    type: 'changeDay',
                                    date: this.viewDate,
