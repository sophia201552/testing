;(function(root, factory) {
    ...
    class StrategyConfigModal extends React.Component{
        constructor(props){
            ...
            this.isGroup = false;
            this.defaultJsonObj = {
                timeBetweenFDDCycles : 300, //诊断的运行周期
                timeBetweenDataCycles : 300, //数据的获取周期
                EnableSensorCheck : 1, //是否进行传感器检查
                RunCommand : 1, //项目的启用禁用
                Language : 'zh', //项目的语言
                WarningRatio : 0.6, //报警概率设定
                RelativeId : 0, //demo项目的id
                SensorSequenceNum : 12, //传感器检查的数据个数
                DeadSequenceNum : 60, //数据未变动的数据个数
                server_tz : 'Asia/Shanghai', //服务器的时区
                proj_tz : 'Asia/Shanghai', //项目的时区	
                FaultCheckNum: 6,
            };
            ...
            if(props.strategyId!==undefined){
                ...
                this.async = WebAPI.get('/strategy/getProjConfig/'+props.groupId).done((rs)=>{
                    if (rs.status === 'OK') {
                        ...
                        if(!rs.data._id){
                            Object.assign(config,this.defaultJsonObj);
                        }
                        show(config);
                    }else{
                        this.setState({
