    ...
    AdminConfigure.prototype = {
        ...
        initNav: function () {
            ...
            if(AppConfig.userProfile && AppConfig.userProfile.picture){
                $('.imgUser', $linkToInfoCenter).attr('src', 'https://beopweb.oss-cn-hangzhou.aliyuncs.com' + AppConfig.userProfile.picture);
            }
        ...
        initList: function () {
            ...
            if (curRoom && curRoom.grade != '0' && AppConfig.isOnline && !AppConfig.isLocalMode && !AppConfig.fixLocalMode) {
                $setPwd.show();
            } else {
                $setPwd.hide();
            }
        ...
        initVersion:function(){
            ...
            $('#mVersion').hammer().off('tap').on('tap',function(){
        ...
        initConfig: function(){
            ...
            $('#mDownloadRoom', this.$container).hammer().off('tap').on('tap', function (e) {
                ...
                SpinnerControl.show();
                var promiseRoomList = $.Deferred();
                var promiseRoomDetail = $.Deferred();
                var promiseMapImg = $.Deferred();
                WebAPI.get('/appTemperature/room/getlist/' + AppConfig.userId).done(function(result){
                    var newRoomList = result.roomList;
                    var roomList = [];
                    FileAPI.download(newRoomList.map(function(item){return {url:item.params.map.img,name:item._id,path:'mapImg'}}))
                    .done(function(result){
                            promiseMapImg.resolve();
                            console.log('地图下载成功');
                        }).fail(function(){
                            promiseMapImg.reject();
                            console.log('地图下载失败');
                        });
                    FileAPI.read('RoomList').done(function(result){
                        if(result && result.RoomList.status == 'success' && result.RoomList.result){
                            ...
                            FileAPI.write([{type:'RoomList',data:roomList}]).done(function(){
                                promiseRoomList.resolve();
                            }).fail(function(){
                                promiseRoomList.reject();
                            })
                        }else{
                        ...
                        FileAPI.write([{type:'RoomList',data:newRoomList}]).done(function(){
                            promiseRoomList.resolve();
                        }).fail(function(){
                            promiseRoomList.reject();
                        })
                ...
                if(roomAll instanceof Array && roomAll.length > 0) {
                    WebAPI.post('/appTemperature/room/getDetailByList', roomAll.map(function(item){return item._id})).done(function (result) {
                        var arrRoomDetailList = [];
                        result.data && result.data.forEach && result.data.forEach(function(room){
                            arrRoomDetailList.push({
                                type:'RoomDetail.'+ room._id,
                                data:room
                            });
                        });
                        FileAPI.write(arrRoomDetailList).done(function(){
                            promiseRoomDetail.resolve();
                        }).fail(function(){
                            promiseRoomDetail.reject();
                        })
                    }).fail(function(){
                        promiseRoomDetail.reject();
                    })
                }
                $.when(promiseRoomDetail,promiseRoomList,promiseMapImg).always(function(list,detail,map){
                    SpinnerControl.hide();
                }).done(function(list,detail,map){
                    window.plugins && window.plugins.toast.show(I18n.resource.admin.setUp.DOWNLOAD_SUCCESS, 'short', 'center');
                    console.log(I18n.resource.admin.setUp.DOWNLOAD_SUCCESS);
                }).fail(function(list,detail,map){
                    window.plugins && window.plugins.toast.show(I18n.resource.admin.setUp.DOWNLOAD_FAIL, 'short', 'center');
                    console.log(I18n.resource.admin.setUp.DOWNLOAD_FAIL);
                });
                e.preventDefault();

                function updateMapImg(url){
