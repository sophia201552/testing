        ...
        this.ajaxRequestTree.done(function (result) {
            ...
            if (result && result.data) {
                ...
                function zTreeOnNodeCreated(event, treeId, treeNode){
                    ...
                    if(treeNode.type != 'DropDownList' && treeNode.iconSkin && treeNode.iconSkin !='') {
                        if(treeNode.iconSkin.indexOf('iconfont')>=0){
                            $("#" + treeNode.tId + "_switch",_this.$treeWrap).removeClass("button").addClass(treeNode.iconSkin);
                        }else{
                ...
                function zTreeBeforeDrop(treeId, treeNodes, targetNode, moveType) {
                    $(_this.container).find("#treeControl li").removeClass("curSelectedNode");
                ...
                function  zTreeOnDrop(event, treeId, treeNodes, targetNode, moveType) {
                    treeNodes.forEach(function(row){
                        ...
                        if(row.type !='DropDownList' && row.iconSkin && row.iconSkin !=''){
                            var currentClass = row.iconSkin.indexOf('iconfont')>=0?row.iconSkin:"glyphicon glyphicon-"+row.iconSkin;
                            $("#" + row.tId + "_switch",_this.$treeWrap).removeClass("button").addClass(currentClass);
                        }
                    });
    ...
    TreeView.prototype.generateTreeEx = function (arr) {
        ...
        for (var i = 0, len = arr.length; i < len; i++) {
            ...
            if ('DropDownList' == item.type) {
                params = {projId:item.projId, id:item._id, pId:item.parent, name:item.text, type: item.type};
    ...
    TreeView.prototype.funcEditPage = function () {
        ...
        if (selectNodes && selectNodes.length === 1) {
            if(selectNodes[0].type === 'DropDownList'){
                $(".modal-title").attr("i18n","mainPanel.pageEditModal.EDIT_PAGE_DIRECTORY");
            }else{
        ...
        if (selectNodes && selectNodes.length > 0) {
            ...
            if(selectNodes[0].type != 'DropDownList' && selectNodes[0].iconSkin && selectNodes[0].iconSkin !=''){
                if(selectNodes[0].iconSkin.indexOf('iconfont')>=0){
                    $('#'+selectNodes[0].tId+'_switch').removeClass("button").addClass(selectNodes[0].iconSkin);
                }else{
