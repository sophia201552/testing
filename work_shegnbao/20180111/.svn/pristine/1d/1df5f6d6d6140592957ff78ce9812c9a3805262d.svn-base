class BenchmarkEnergyForecast{
    ...
    attEvents(){
        ...
        //下载模板
        $('.excelTemplate').off('click').click(function(){
            var timeSelsect = $('.timeSelsect').val();
            var itemIdss = [];
            if(_this.paramsRelate.length>0){
                for(var i = 0;i<_this.paramsRelate.length;i++){
                    itemIdss.push( _this.paramsRelate[i].name);
                }
            }
            var startTime = $('#modalConfigStartTime').val();
            var endTime = $('#modalConfigEndTime').val();
            ...
            var postData = {
                'timeStart': startTime,
                'timeEnd': endTime,
                'timeFormat': timeSelsect,
                'pointList': itemIdss
            };
            var formData = new FormData();
            formData.append('timeStart', startTime);
            formData.append('timeEnd', endTime);
            formData.append('timeFormat', timeSelsect);
            formData.append('pointList', [itemIdss]);
            var xhr = new XMLHttpRequest();
            xhr.open('POST', '/benchmark/export/emptyFile');
            xhr.responseType = 'arraybuffer';
            xhr.onload = function () {
                var blob, url, lkPdfFile;
                if (this.status === 200) {
                    blob = new Blob([xhr.response], {type: "application/xlsx"});
                    ...
                    lkPdfFile.id = "lkXlsxFile";
                    lkPdfFile.href = url;
                    lkPdfFile.download = 'template.xlsx';
        ...
        $('#importDataIpt').change(function(e){
            ...
            function timeFormatFun(timeArrF,formatT){
                var timeArr = [];
                for(var i = 0;i<timeArrF.length;i++){
                    var currentTime = timeArrF[i];
                    if(formatT=='d1'){
                        timeArr.push(currentTime.split(' ')[0]);
                    }else if(formatT=='M1'){
                        timeArr.push(currentTime.split('-')[0]+'-'+currentTime.split('-')[1]);
                    }else{
                        timeArr.push(currentTime.split(':')[0]+':'+currentTime.split(':')[1]);
                    }
                }
                ...
                $.ajax({
                    ...
                    success: function(result){
                        ...
                        if(!result.data||result.time.length===0){
                            ...
                            var period = $('.timeSelsect').val();
                            var timeFormArr = [];
                            timeFormArr = timeFormatFun(result.time,period);
                            $('#modalConfigStartTime').val(timeFormArr[0]);
                            $('#modalConfigEndTime').val(timeFormArr[timeFormArr.length-1]);
                            var cellOther = '';
                            var timeListBox = '<div class="tableHeadBox clearfix"><div class="tabalCellH" i18n="benchmark.energyForecast.TIME">时间</div>';
                            ...
                            for(var item in resultData){
                                ...
                                if(xDataList.length!==0){
                                    for(var i=0;i<xDataList.length;i++){
                                        if(itemCount===0){
                                            var timeListBox1 = '<div class="tableContentBox clearfix"><div class="tabalCellT">'+timeFormArr[i]+'</div>';
                        ...
                        //    _this.$divAssetImg.html('<div class="imgItem"><img src="https://beopweb.oss-cn-hangzhou.aliyuncs.com/' + i + '?_=' + Date.now().valueOf() + '" class="imgAsset" id="imgAsset"/></div>');//
                        //});
                    }
                })
            }
    ...
    echartsInit(result){
        ...
        var option = {
           ...
           xAxis: [
            ...
            series: [
                {
                    name:I18n.resource.benchmark.energyForecast.PREDICTED_VALUE,//'预测值'
                    type: 'line',
                    smooth: true,
        ...
        if(result.length>1){
            option.series[0].name=I18n.resource.benchmark.energyForecast.HISTORICAL_VALUE;//'历史值'
