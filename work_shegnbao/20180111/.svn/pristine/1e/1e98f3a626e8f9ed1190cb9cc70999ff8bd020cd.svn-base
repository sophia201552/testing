    ...
    FactoryIoC.prototype = {
        ...
        getModel: function(strModelName) {
            for (var i = 0, len = this.listClass.length; i < len; i++) {
                if (strModelName.toLowerCase() == this.listClass[i].toString().match(/function\s*(\w+)/)[1].toLowerCase()) {
        ...
        initDashboardModule: function() {
            ...
            this.add(ModalEquipmentRateAndHistoryData); //设备完好率
            this.add(ModalInteractiveTrendChart); //交互趋势图
            this.add(ModalPriorityHandlingFaults); //华为优先处理故障列表 new
            this.add(modalRealTimeAndHistoryChartCoolSkin);
            this.add(modalAppHistoryDiagnosis);//历史诊断
        },
        ...
        initReportModule: function() {
            ...
            this.add(ns.DataBlock);
            this.add(ns.Table);
            this.add(ns.DynamicTable);
