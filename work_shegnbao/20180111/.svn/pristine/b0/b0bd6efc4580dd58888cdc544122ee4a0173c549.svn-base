	...
	ReportConfigPanel.prototype = {
			...
			switch (period){
				...
				case 'week':
					if(AppConfig.language === "zh"){
						...
						["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"].forEach(function(row,i){
							if(startTime === i){
								periodOptions += '<option value="'+ i +'" selected>'+ row +'</option>'
							}else{
								periodOptions += '<option value="'+ i +'">'+ row +'</option>'
							}
						})
				...
				case 'month':
					if(AppConfig.language === "zh"){
						...
						for(var j=0;j<28;j++){
							if(startTime === j){
								periodOptions+='<option value="'+ j +'" selected>'+ (j+1) +'</option>'
							}else{
								periodOptions+='<option value="'+ j +'">'+ (j+1) +'</option>'
							}
						}
					}
					break;
				case 'year':
					if(AppConfig.language === "zh"){
						...
						["January","February","March","April","May","June","July","August","September","October","November","December"].forEach(function(row,i){
							if(startTime === i){
								periodOptions += '<option value="'+ i +'" selected>'+ row +'</option>'
							}else{
								periodOptions += '<option value="'+ i +'">'+ row +'</option>'
		...
		attachEvents: function () {
			...
			$(this.wrapContainer).on('click','#addReport',function(){
				...
				var tpl = '<li id="'+ newReportId +'" class="liReport" draggable="true"><span class="reportName" style="display:none;">untitled</span><input class="editName" type="text" value="untitled"><select class="startTimeSelect">'+_this.getPeriodStartTime('day')+'</select><select class="periodSelect">'+_this.getPeriodKinds()+'</select></li>';
			...
			$(this.wrapContainer).on('change','.editName',function(){
				...
				if(!newReportName){
					newReportName = $(this).prev('.reportName').text();
				}
				$(this).hide();
