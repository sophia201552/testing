...
(function () {
    ...
    WidgetProp.prototype.attachPubEvent = function (model) {
        var _this = this;
        this.$propertyList.off('click.property-value').on('click.property-value', '.property-value.p_span',function(){
            var $input = $(this).hide().next('.p_input');
            $input.show().focus().val($input.val());
        });
        this.$propertyList.off('focusout.property-value').on('focusout.property-value','.property-value.p_input', function(){
        ...
        this.$propertyList.off('keydown.shape.position').on('keydown.shape.position', '.property-value.p_input',function(event){
            if(event.keyCode == 13){
                $(this).trigger("blur");
            } 
        }); 
...
(function () {
    ...
    function PropModel(models, screen) {
        this.models = models;
        this.screen = screen;
    }

    PropModel.prototype.update = function (k) {
        this.screen.painter.batchDraw(function(){
