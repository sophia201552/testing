...
class Asset:
    ...
    def putin_inventory(cls, post_data, userId):
        ...
        try:
            ...
            cnn.mdbBb[g_table_asset_inventory].insert(post_data)
            rt = post_data.get('_id').__str__()
            for p in parts:
                pId = p.get('part_id')
                qty = p.get('qty')
                cnn.mdbBb[g_table_asset_part].update({'_id': ObjectId(pId)}, {'$inc': {'qty': abs(qty)}})
    ...
    def getSonGroupThings(cls, projId, parent):
        ...
        try:
            ...
            for item in cursor:
                query = {item.get('_id').__str__(): {'name': item.get('name'), 'parent': item.get('_idPrt')}}
                rt.update(query)
            return rt
        except Exception:
            logging.error('Unhandled exception! Locals: %s', locals(), exc_info=True, stack_info=True)
    ...
    def checkMainTain(cls, rt, thingList):
        ...
        try:
            ...
            for item in cursor:
                thing_id = ObjectId(item.get('thing_id')) if isinstance(item.get('thing_id'), str) else item.get(
                    'thing_id')
