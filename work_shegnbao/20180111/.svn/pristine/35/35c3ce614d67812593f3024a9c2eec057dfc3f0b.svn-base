    ...
    var doSave = function(fn) {
        return function(dispatch, getState) {
            ...
            if (diffData) {
                ...
                return {
                    async: $.when(strategyPromise, modulePromise).done(function () {
            ...
            switch (dragType) {
                case 'tagGroupId':
                    ...
                    let loop = (children, prt, parentName)=>{
                        children.forEach((c, i)=>{
                            let isOnlyGroup,
                                doSth;
                            if(c.equipmentId && c.equipmentId != null){//设备文件夹
                                isOnlyGroup = false;
                                doSth = (result)=>{
                                    tempD.push({_id: c._id,tag: c.tag, name: c.name, prt: c.prt, children: result.data.filter(d=>d.type=='thing'), originName: c.name, parentName:parentName, equipmentId:c.equipmentId});
                                };
                            }else{//普通文件夹
                                wrongCount++;
                                isOnlyGroup = true;
                    ...
                    tempD.forEach(t=>{//新版tempD length为1
                        if(!t.children){
                            WebAPI.post('/tag/getThingTreeNew', t.postData).done(function (result) {
                                if (result.data && result.data.length) {
                                    ...
                                    if(t.equipmentId && t.equipmentId != null){//设备文件夹
                                        t.children = result.data.filter(d=>d.type=='thing');
                                        promise.resolve();
                                    }else if(t.equipmentId == null || t.equipmentId == ''){//普通文件夹
                                        wrongCount++;
                    ...
                    promise.fail(()=>{
                        ...
                        tempD.forEach(temp=>{
                            ...
                            for(let moduleId in moduleMap){
                                equipmentId[moduleId] = temp.equipmentId;
                ...
                case 'rule':
                    ...
                    let async = WebAPI.get('/strategy/template/' + info.dataId).done(function(rs) {
                        ...
                        if (rs.status === 'OK') {
                            var data = rs.data;
                            let module = data.data.modules[0];
                            let inputIdMap = {};
