...
(function (window, $, nsBaseMap) {
    ...
    var CONSTS = {
        ...
        GMap: {
            ...
            params: {
                ...
                language: window.AppConfig && window.AppConfig.language ? window.AppConfig.language : 'en'
            }
        }
    };
    ...
    var defaults = {
        ...
        // 地图标记的图片
        markerImgPath: '/static/images/map_marker_online.png'
    };
    ...
    GDMap.load = function (options, callback) {
        ...
        if (!GDMap.isLoaded()) {
            BaseMap.load('AMap', function () {
                var ins = new GDMap(options);
                AMap.event.addListenerOnce(ins.map, 'complete', function () {
                    typeof callback === 'function' && callback(ins);
                });
            });
        } else {
    ...
    GDMap.prototype.init = function () {
        ...
        var lan = window.AppConfig && window.AppConfig.language ? AppConfig.language : 'en';
        this.map.setLang(window.AppConfig.language)
    };
    ...
    GoogleMap.load = function (options, callback) {
        ...
        if (!GoogleMap.isLoaded()) {
            BaseMap.load('GLoader', function () {
                ...
                google.load('maps', 3, {
                    other_params: p.join('&'), callback: function () {
                        var ins = new GoogleMap(options);
                        google.maps.event.addListenerOnce(ins.map, 'tilesloaded', function(){
                            typeof callback === 'function' && callback(ins);
                        });
                    }
                });
    ...
    GoogleMap.prototype.addSearchBox = function (iptSch) {
        ...
        google.maps.event.addListener(searchBox, 'places_changed', function () {
            ...
            _this.trigger('click', {lnglat: places[0].geometry.location});
        });
    ...
    nsBaseMap.loadAccurateMap = function (type, callback) {
        if (type === 'AMap') {
            GDMap.load(callback);
        } else if (type === 'GMap') {
            GoogleMap.load(callback);
        } else {
            throw new SyntaxError('Map type is illegal !')
        }
    };
