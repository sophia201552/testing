    ...
    PageScreen.prototype = {
        show: function() {
            ...
            return promise.done(function(rs) {
                ...
                var widgetsTypes = rs.data.widgets.map(function(v){return v.type;});
                if(widgetsTypes.indexOf('HtmlHeat3D')>-1){//判断页面是否是heat3d
                    window.isHtmlHeat3D = true;
                }else{
                    window.isHtmlHeat3D = false;
                }

                if(widgetsTypes.indexOf('CanvasHeatColor')>-1){//判断页面是否是heatcolor
                    window.canvasHeatColorOption = rs.data.widgets[widgetsTypes.indexOf('CanvasHeatColor')].option;
                    window.isCanvasHeatColor = true;
                }else{
                    window.canvasHeatColorOption = null;
                    window.isCanvasHeatColor = false;
                }
                ...
                this.store.widgetModelSet.forEach(function(model) {
                    ...
                    if (this.preview === '2' && dsId.length !== 0 && AppConfig.isFactory === 1) {
                        var id = model._id();
                        var type = model.type();
                        var x = model.x();
                        var y = model.y();
                        if (type === "CanvasPipe" || type === "CanvasCustomLine") {
                            x = model.option().points[0].x;
                            y = model.option().points[0].y;
                        }
                        var str = '<div style="top:' + y + 'px;left:' + x + 'px;" class="pointNameLabel" data-id="' + id + '" data-type="' + type + '" draggable="true">\
                                        <span class="pointName">' + dsId + '</span>\
                                    </div>';
                        $(this.painter.getHtmlLayer().shape).append($(str));
                        $(this.painterCtn).off('click').on('click.pointNameLabel', '.pointNameLabel', function() {
                            ...
                            //拖拽
                        var _this = this;
                        var orderNum = $(this.painterCtn).find('.html-layer').eq(0).attr('style').indexOf('scale');
                        var str = $(this.painterCtn).find('.html-layer').eq(0).attr('style').substring(orderNum + 6);

                        var selfX, selfY, lastX, lastY;
                        $(this.painterCtn).off('dragstart', '.pointNameLabel').on('dragstart', '.pointNameLabel', function(ev) {
                            ev = ev || window.event;
                            selfX = ev.target.offsetLeft;
                            selfY = ev.target.offsetTop;

                            lastX = ev.originalEvent.clientX;
                            lastY = ev.originalEvent.clientY;
                        });
                        $(this.painterCtn).off('dragend', '.pointNameLabel').on('dragend', '.pointNameLabel', function(ev) {
                            ev.preventDefault();
                            ev = ev || window.event;
                            currentX = ev.originalEvent.clientX;
                            currentY = ev.originalEvent.clientY;

                            $(ev.target).css({ left: (selfX + (currentX - lastX)) + 'px', top: (selfY + (currentY - lastY)) + 'px' });

                            //拿到标签对应的控件的 起始 x y w
                            var thisType = $(ev.target).attr('data-type');
                            var thisId = $(ev.target).attr('data-id');
                            var lineStartX, lineStartY, width, height;
                            $.each(_this.typeGrouping, function(index, row) {
                                if (row.type === thisType) {
                                    for (var i = 0, length = row.models.length; i < length; i++) {
                                        if (row.models[i].store.model._id() === thisId) {
                                            lineStartX = row.models[i].store.model.x();
                                            lineStartY = row.models[i].store.model.y();
                                            width = row.models[i].store.model.w();
                                            height = row.models[i].store.model.h();
                                        }
                                    }
                                }
                            })
                            var labelWidth = ev.target.offsetWidth,
                                labelHeight = ev.target.offsetHeight;
                            //画线
                            var startPointX = lineStartX + width / 2,
                                startPointY = lineStartY + height / 2,
                                endPointX = (selfX + (currentX - lastX)),
                                endPointY = (selfY + (currentY - lastY)) + labelHeight / 2;
                            var isExisted = false;
                            ...
                            var children = _this.painter.canvasStage.shape.children;
                            //拿到shape下面的all line
                            for (var i = 0, length = children.length; i < length; i++) {
                                if (children[i].className === 'Line' && thisId === children[i].attrs.id) { //有这个id  直接更新
                                    children[i].destroy()
                                    _this.painter.canvasStage.shape.add(line);
                                    _this.painter.canvasStage.shape.draw(line);
                                    isExisted = true;
                                    break;
                                }
                            }
        ...
        showColorSetBtn: function(isShow) {
            if (typeof ModelTempDistribution === 'undefined') return;
            if (isShow) {
                ...
                $("#btnTemperatureSetting").show().eventOff('click').eventOn('click', function(e) {
                    new TemperatureSetting(window.isHtmlHeat3D?'3d':'co', true).show();
                });
            } else {
        ...
        startWorker: function() {
            ...
            this.workerUpdate.postMessage({ pointList: dsIds, type: "datasourceRealtime", projectId: AppConfig.projectId });
        },
        ...
        modelInt: function() {
            ...
            if (this.page.type === 'PageScreen') {
                $checkPreviewPattern.css('display', 'flex');
            } else {
                this.preview = '0';
            }
            //选择预览模式
            ...
            $('#pointViewModal').on('hide.bs.modal', function() {
                    $('#pointViewModal table').html('');
                    var color = $('#pointViewModal .lineColor input').val();
        ...
        pageWidgetsType: function(obj) {
            ...
            switch (obj.type) {
                case 'CanvasText':
                    modalType = i18n_resource.pagePreview.widget.CANVAS_TEXT;
                    break;
                case 'HtmlText':
                    modalType = i18n_resource.pagePreview.widget.HTML_TEXT;
                    break;
                case 'HtmlButton':
                    modalType = i18n_resource.pagePreview.widget.HTML_BUTTON;
                    break;
                case 'HtmlContainer':
                    modalType = i18n_resource.pagePreview.widget.HTML_CONTAINER;
                    noSupportPoint = 'disabled'
                    break;
                case 'HtmlScreenContainer':
                    modalType = i18n_resource.pagePreview.widget.HTML_SCREEN_CONTAINER;
                    noSupportPoint = 'disabled'
                    break;
                case 'CanvasImage':
                    modalType = i18n_resource.pagePreview.widget.CANVAS_IMAGE;
                    break;
                case 'CanvasPipe':
                    modalType = i18n_resource.pagePreview.widget.CANVAS_PIPE;
                    break;
                case 'CanvasHeat':
                    modalType = i18n_resource.pagePreview.widget.CANVAS_HEAT;
                    noSupportPoint = 'disabled'
                    break;
                case 'CanvasHeatP':
                    modalType = i18n_resource.pagePreview.widget.CANVAS_HEATP;
                    break;
                case 'HtmlDashboard':
                    modalType = i18n_resource.pagePreview.widget.HTML_DASHBOARD;
                    noSupportPoint = 'disabled'
                    break;
                case 'CanvasPolygon':
                    modalType = i18n_resource.pagePreview.widget.CANVAS_POLYGON;
                    noSupportPoint = 'disabled'
                    break;
                case 'CanvasCustomLine':
                    modalType = i18n_resource.pagePreview.widget.CANVAS_LINE;
                    break;
            }
            ...
            var str = '<tr data-type="' + obj.type + '">\
                        ...
                        <td>\
                            <button type="button" class="btn ' + swClassName + ' showBtn showWidget"  i18n="pagePreview.SHOW_WIDGET"></button>\
                            <button type="button" class="btn ' + spClassName + ' showBtn showPoint" ' + noSupportPoint + ' i18n="pagePreview.SHOW_POINT"></button>\
                            <button type="button" class="btn ' + nsClassName + ' showBtn noShow" i18n="pagePreview.NO_SHOW"></button>\
                        </td>\
                    </tr>';
        ...
        // 初始化数据回放
        initReplay: function() {
            if (typeof TimeShaftPageScreen === 'undefined') return;
            // 新增"数据回放"按钮
            var _this = this;
            ...
            $btnTimeShaft.eventOn('click', function(e) {
                ...
                if (_this.isReplay) {
                    ...
                    $(this).find('.dropdownNav').text(I18n.resource.observer.widgets.BACKTRACE);
                    _this.quitReplayMode();
                   trackEvent('顶部导航数据回放-PageScreen收缩','TopNav.BackTrace.PageScreen.Close') 
                } else {
                    // 调整按钮状态和文字
                    this.classList.add('selected');
                    $(this).find('.dropdownNav').text(I18n.resource.observer.widgets.QUIT_TRACE);
                    _this.enterReplayMode();
                    trackEvent('顶部导航数据回放-PageScreen展开','TopNav.BackTrace.PageScreen.Open') 
                }
