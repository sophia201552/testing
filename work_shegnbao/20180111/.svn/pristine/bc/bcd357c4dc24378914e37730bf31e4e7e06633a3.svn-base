...
var Permission = (function () {
    if (!String.prototype.endsWith) { // ie11 兼容
    ...
    Permission = {
        addPermission: function (permission) {
            if (permission && permission.trim()) {
                window.parent.AppConfig.permission[permission.trim()] = true;
            }
        },
        deletePermission: function (permission) {
            if (permission && permission.trim()) {
                window.parent.AppConfig.permission[permission.trim()] = false;
            }
        },

        setPermission: function (permissionList) {
            window.parent.AppConfig.permission = {};
            if (permissionList && permissionList.length) {
                for (var m = 0, n = permissionList.length; m < n; m++) {
                    if (permissionList[m]) {
                        window.parent.AppConfig.permission[permissionList[m].trim()] = true;
                    }
                }
            }
        },
        ...
        hasPermission: function (permission) {
            if (window.parent.AppConfig.userId == 1 || window.parent.AppConfig.permission['BRoot'] || window.parent.AppConfig.permission['all']) {
                return true;
            }
            if (/\d+/.test(permission)) {//判断是否为用户ID
                return parseInt(permission) === window.parent.AppConfig.userId;
            }
            return window.parent.AppConfig.permission && permission && window.parent.AppConfig.permission[permission.trim()];
        },
        ...
        check: function (area) {
            try {
                if (!window.parent.AppConfig.permission) {
                    return false;
                }
            } catch (e) {
                console.error('window.parent.AppConfig.permission未定义');
            }
            if (!area) {
                return false;
            }
