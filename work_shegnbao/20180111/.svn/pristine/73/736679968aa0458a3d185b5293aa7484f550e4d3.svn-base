...
class TagThing:
    ...
    def get_count_by_groupId(self, groupId_list):
        num = self.mdbBb.count({'projId': self.projId, 'type': self.type, 'prt': {'$in': groupId_list}})
        return num
    ...
    def format_things(self, lst):
        ...
        for item in lst:
            rt.append({'_id': item.get('_id').__str__(), 'name': item.get('name'), 'type': item.get('type'),
                       'tag': item.get('tag'), 'attrP': item.get('attrP'), 'flag': item.get('flag'),
...
class ThingsTree:
    ...
    def get_thingsTreeNew(self, prt, isOnlyGroup, limit, skip, tagThing, group_count_dic):
        ...
        if isOnlyGroup or not ObjectId.is_valid(prt):
            query.update({'type': 'group'})
        self.cursor = self.mdbBb.find(query, sort=SORT_BY_NAME, limit=limit, skip=(skip - 1) * limit,
                                      projection={'_id': 1, 'prt': 1, 'name': 1, 'type': 1, 'rule': 1, 'tag': 1, 'attrP': 1, 'entityId': 1, 'entityType': 1})
        if self.cursor:
            for item in self.cursor:
                rt.append({'_id': item.get('_id').__str__(), 'name': item.get('name'), 'type': item.get('type'),
                           'rule': item.get('rule'),
                ...
                if item.get('type') == 'group':
                    ...
                    rt[-1].update({
                        ...
                        'entityId': item.get('entityId', None),
                        'entityType': item.get('entityType', None)
                    })
        return rt
    ...
    def get_detail_new(self, query, limit, skip, sort=SORT_BY_NAME):
        query.update({'projId': self.projId})
        return self.mdbBb.find(query, limit=limit, skip=(skip-1)*limit, sort=sort)

    def get_detail_new2(self, query, projection, limit=0, skip=0, sort=SORT_BY_NAME):
        query.update({'projId': self.projId})
        return self.mdbBb.find(query, projection, limit=limit, skip=skip, sort=sort)
    ...
    def search(self, query, limit, skip, sort=SORT_BY_NAME):
        rt = []
        rtd_map = {}
        p_list = []
        if query:
            query.update({'projId': self.projId})
            self.cursor = self.mdbBb.find(query, limit=limit, skip=limit * (skip-1), sort=sort)
            for item in self.cursor:
                rt.append({'_id': item.get('_id').__str__(), 'name': item.get('name'), 'type': item.get('type'),
                           'tag': item.get('tag'), 'attrP': item.get('attrP'),
                           'prt': item.get('prt').__str__() if isinstance(item.get('prt'), ObjectId) else None,
    ...
    def set_tag(self, Ids, tag):
        dbrv = self.mdbBb.update({'_id': {'$in': [ObjectId(x) for x in Ids if ObjectId.is_valid(x)]}, 'projId': self.projId},
    ...
    def set_group_tag(self, groupIds, tags):
        dbrv = self.mdbBb.update({'prt': {'$in': groupIds}, 'projId': self.projId}, {'$addToSet': {'tag': {'$each': tags}}}, multi=True)
        if dbrv:
            if dbrv.get('ok'):
                return True
        return False
    ...
    def del_tag(self, Ids, tag):
        dbrv = self.mdbBb.update({'_id': {'$in': [ObjectId(x) for x in Ids if ObjectId.is_valid(x)]}},
                                 {'$pull': {'tag': tag}, '$unset': {'attrP.' + str(tag).lower(): 1}}, multi=True)
        if dbrv:
            if dbrv.get('ok'):
                return True
            else:
    ...
    def del_group_tag(self, Ids, tag):
        ...
        if dbrv and Ids:
            if dbrv.get('ok'):
                self.cursor = self.mdbBb.find({'prt': {'$in': [ObjectId(x) for x in Ids if ObjectId.is_valid(x)]},
                                                'type': 'group'})
    ...
    def del_tag_v2(self, project_id, ids, tag, inheritable):
        self.mdbBb.update_many({'_id': {'$in': ids}},
                               {'$pull': {'tag': tag}, '$unset': {'attrP.' + str(tag).lower(): 1}})
        if inheritable:
            children_ids = list(self.mdbBb.find({'projId': project_id, 'prt': {'$in': ids}}, {'_id': 1}))
            if children_ids:
                self.del_tag_v2(project_id, children_ids, tag, inheritable)
            else:
                return
    ...
    def set_group_attrP(self, groupId_list, update):
        dbrv = self.mdbBb.update({'projId': self.projId, 'prt': {'$in': groupId_list}},
                                 update, multi=True)
        if dbrv:
            if dbrv.get('ok'):
                return True
    ...
    def update_set(self, query, update):
        query.update({'projId': self.projId})
        dbrv = self.mdbBb.update(query, {'$set': update}, multi=True)
        if dbrv:
            if dbrv.get('ok'):
                return True
            else:
...
class TagStatistics():
    ...
    def group_by_tags(self, match):
        self.cursor = self.mdbBb.aggregate([
            {'$match': match},
            {'$unwind': '$tag'},
            {'$group': {'_id': '$tag', 'Ids': {'$addToSet': '$_id'}}}
        ])
        return self.cursor

    def group_by_prt(self, match):
        self.cursor = self.mdbBb.aggregate([
            {'$match': match},
            {'$group': {'_id': '$prt', 'Ids': {'$addToSet': '$_id'}}}
        ])
        return self.cursor
