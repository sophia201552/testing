...
(function(win){
    ...
    AddByTplModal.prototype.init = function (type) {
        ...
        ////// 杭州华为加速脚本，请勿删除，请勿解除注释
        // $('#iptTplDs', this.$addByTplModal).val('I1_1F_V07_01');
        // $('#iptReplace', this.$addByTplModal).val('<#name#>');
        // $('#iptPageNameBatch', this.$addByTplModal).val('<#name#>');
        // this.map = {
        //     name: {
        //         type: 1,
        //         value: window.__temp
        //     }
        // }
        // $('#iptPlaceBatch', this.$addByTplModal).val('{name: '+ JSON.stringify(window.__temp) +'}');
        //////
        ...
        $('#buttonOk', this.$addByTplModal).off('click').on('click',function(){
            ...
            if(_this.step === 1){
                ...
                if($('.single', _this.$addByTplModal).is(':hidden')){
                    ...
                    _this.getNameList(names);
                    names.forEach(function(name){
                        pageInfo.push(_this.newPageByTpl(name))
                    });
                }else{
                    //单个增加
                    if(!pageNameVal) {
                ...
                pageInfo.forEach(function (row, index) {
                    _this.newPageByIds(row, index);
                });
    ...
    AddByTplModal.prototype.getNameList = function(names){
    ...
    AddByTplModal.prototype.newPageByTpl = function(name){
    ...
    AddByTplModal.prototype.newPageByIds = function(page, index) {
         var tplDsVal = $('#iptTplDs', this.$addByTplModal).val().trim();
         var replaceVal = $('#iptReplace', this.$addByTplModal).val().trim();
         var params, match;
        //替换 查找字符不为空
        if(tplDsVal && page.widgets && page.widgets.length){
            if(replaceVal.indexOf('<#') > -1) {
                params = this.__getTplParams();
                replaceVal = replaceVal.replace(/<#(\w+?)#>/g, function ($0, $1) {
                    if (params[$1] && typeof params[$1][index] !== 'undefined') {
            ...
            page.widgets.forEach(function(widget){
                if(widget.type == 'HtmlContainer') {
                    _this.replaceHtml(tplDsVal, replaceVal, widget);
                } else if(widget.type == 'HTMLScreenContainer') {

                } else {
                    _this.replaceId(tplDsVal, replaceVal, widget);
                }
            });
        }
    };
