...
class Management:
    ...
    def get_management_project_list(self,id_list):
        sql = 'SELECT id,name_cn,name_en,pic,management FROM project WHERE management in (%s)'%(','.join(map(str,id_list)))
        rv = Management.db_read.op_db_query(Management.db_name, sql, ())
        rt = {}
        keys = ('id','name_cn','name_en','pic','management')
        for item in rv:
    ...
    def get_management_list(user_id,projects):
        ...
        if len(id_list) == 0:
            return []
        sql = 'SELECT id, name_cn, name_en, code_name FROM %s WHERE id in (%s)'%(Management.table_name,','.join(id_list))
        rv = Management.db_read.op_db_query(Management.db_name, sql, ())
        item_list = []
        for item in rv:
            item_list.append({
    ...
    def create_management(data):
        ...
        创建新集团项目
        '''
        sql = 'INSERT INTO '+ Management.table_name +' (name_cn, name_en, code_name) VALUES (%s,%s,%s)'
        id = Management.db.op_db_update_with_id(Management.db_name, sql,
        (data.get('name_cn',''),data.get('name_en',''), data.get('code_name',''),))
        return id
