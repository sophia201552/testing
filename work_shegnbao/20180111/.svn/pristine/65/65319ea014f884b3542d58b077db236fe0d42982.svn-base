...
import copy
from beopWeb.observer import checkTime, getOneItemData, do_startWorkspaceDataGenPieChart
from beopWeb.models import effect_num
from beopWeb.mod_factory.service import FactoryService
...
def factoryProjectPreview(userId,projectId):
    bindId = MongoConnManager.getConfigConn().factoryGetBindProjId(projectId)
    return render_template('mod_factory/previewProject.html',
                           pageInfo=json.dumps({"userId": userId, "projectId": projectId,'onlineProjId': bindId}))
...
def factoryPageSave(projId):
    ...
    data = request.get_json()
    if data:
        rt = FactoryService.factoryPageSave(projId, data)
    return jsonify(formatResult(rt))
...
def factoryPageDelete(projId):
    ...
    data = request.get_json()
    if data:
        rt = FactoryService.factoryPageDelete(projId, data)
    return jsonify(formatResult(rt))
...
def factoryMaterialSaveForm():
    ...
    for data in dataArr:
        ...
        data["creator"] = int(data.get("creator",1))
        fp = files[index]
        id = data.get('_id')
        name = data.get('name')
        ...
        if uploadtooss.status == 200:
            content.update({'url':'https://beopweb.oss-cn-hangzhou.aliyuncs.com/'+uploadpath})
...
@bp_factory.route('/material/group/<type>', methods=['POST'])
@bp_factory.route('/material/group/<type>/<groupId>', methods=['POST'])
def factoryMaterialGetByGroupId(type=None, groupId=None):
    ...
    if groupId != None:
        ...
        cond['group'] = ''
    if type == 'image':
        data = MongoConnManager.getConfigConn().factoryMaterialGetByInfo(cond, userList)
    else:
        data = MongoConnManager.getConfigConn().factoryMaterialGetByInfo(cond, userList, {'content': 0})
    return jsonify(formatResult(True, '', data))

@bp_factory.route('/material/groupByimage/<id>', methods=['POST'])
def factoryMaterialGetByImageId(id=None):
    data = None
    jsondata = request.get_json()
    ...
    if isinstance(userList, str):
        try:
            ...
            userList = []
    data = MongoConnManager.getConfigConn().factoryMaterialGetByImageId(ObjectId(id), userList)
    return jsonify(formatResult(True, msg='', data = data)) 

@bp_factory.route('/material/copy', methods=['POST'])
def factoryMaterialCopy():
    data = request.get_json()
    type = data.get("type")
    ids = data.get("linkList")
    ...
    idArr = []
    for item in ids:
        idArr.append(ObjectId(item))
    cond["_id"] = {'$in':idArr}
    rt = MongoConnManager.getConfigConn().factoryMaterialGetByCopyInfo(cond,type,group,creator)
...
def factoryMaterialSearch():
    ...
    rs = MongoConnManager.getConfigConn().factoryMaterialGetByInfo(cond, userList)
    return jsonify(formatResult(True, '', rs))

@bp_factory.route('/recycle',methods=['GET'])
@bp_factory.route('/recycle/<status>',methods=['GET'])
@bp_factory.route('/pageRecycle',methods=['GET'])
@bp_factory.route('/pageRecycle/<status>',methods=['GET'])
...
def factoryModifyProject(projId):
    ...
    data = request.get_json()
    if 'pageList' in data:
        rt = FactoryService.ModifyPageList(projId, data.get('pageList'), data.get('pages'), data.get('pageListInfo'))
    return jsonify(formatResult(rt))
...
@bp_factory.route('/reportData/remove', methods=['POST'])
def factoryRemoveReportData():
    rt = None
    data = request.get_json()
    rt = MongoConnManager.getConfigConn().FacRemoveReportData(data.get('name'), data.get('date'))

    if rt:
        return jsonify(formatResult(True, '删除报表数据成功'))
    return jsonify(formatResult(False, ''))
...
@bp_factory.route('/copyPages', methods=['POST'])
def factoryCopyPages():
    data = request.get_json()
    pageList = MongoConnManager.getConfigConn().factoryGetPageIdList('59ca1dd6833c977befc7ad04', 1)
    data = {
        'pageList': pageList,
        'destProjId': '598d0e8e833c970e001c32e8'
    }
    rt = MongoConnManager.getConfigConn().factoryCopyPages(data.get('pageList', []), data.get('destProjId'))
    return jsonify(formatResult(rt))

@bp_factory.route('/emptyProject/<proj_id>', methods=['POST'])
def factoryRemovePages(proj_id):
    # 该接口删除的页面无法恢复，有危险性，暂时屏蔽
    return
    if not ObjectId.is_valid(proj_id):
        return jsonify(formatResult(False, '项目id有误'))
    pages = MongoConnManager.getConfigConn().factoryGetPageList(proj_id, 1)
    rt = MongoConnManager.getConfigConn().FacRemovePages(proj_id, pages)
    return jsonify(formatResult(rt))

@bp_factory.route('/updateUserNameToId', methods=['GET'])
def updateUserNameToId():
    user = User()
    usermap = user.get_all_user_map();
    user_name_id_map = {x['userfullname']: x['id'] for x in usermap.values()}
    user_email_id_map = {x['useremail']: x['id'] for x in usermap.values()}
    materials = MongoConnManager.getConfigConn().factoryMaterialGetByInfo({}, None, {'content': 0})
    query_map = {}
    for item in materials:
        creator = item.get('creator', 'admin')
        if isinstance(creator, str):
            if creator in query_map:
                query_map[creator].append(ObjectId(item.get('_id')))
            else:
                query_map[creator] = [ObjectId(item.get('_id'))]
    
    if len(query_map) > 0:
        for key, value in query_map.items():
            name = 'Unknow'
            if key in user_name_id_map:
                name = user_name_id_map[key]
            elif key in user_email_id_map:
                name = user_email_id_map[key]
            elif key.isdigit():
                name = int(key)
            print('key: %s, value: %s, name: %s\n' % (key, value, name))
            MongoConnManager.getConfigConn().factoryMaterialUpdateMany({'_id': {'$in': value}}, {'$set': {'creator': name}})
...
# 查询历史数据包括不填充的，字符串的
@bp_factory.route('/getHistoryData', methods=['POST'])
def getHistoryData():
    result = []
    data = request.get_json()
    for item in data:
        if item.get('noAutoComplete'):
            result.extend(getHistoryDataBy_dac(item))
        elif item.get('strHistorical'):
            result.extend(getHistoryDataBy_str(item))
        elif item.get('realtime'):
            result.extend(getRealtimeData(item))
        else:
            result.extend(getHistoryDataBy_time(item))
    if result:
...
def getHistoryDataBy_time(sub):
    ...
    rt.append({"list": arrHistory, "timeShaft": timeRvList})
    return rt


def getRealtimeData(item):
    strProjectId = int(strItem.get('projectId'))
    dsItemIds = strItem.get('dsItemIds', [])
    result = json.loads(
        do_startWorkspaceDataGenPieChart({
            'dsItemIds': ['@%s|%s' % (strProjectId, x) for x in dsItemIds]
        })
    )
    return [{
        'list': result.get('dsItemList', []),
        'timeShaft': [timeStart]
    }]


def getHistoryDataBy_str(strItem):
    strRt = []
    strHistory = []
    strTime = []
    strProjectId = int(strItem.get('projectId'))
    timeStart = strItem.get('timeStart')
    timeEnd = strItem.get('timeEnd')
    dsItemIds = strItem.get('dsItemIds', [])

    strData = BEOPDataAccess.getInstance().get_history_data_padded(
        strProjectId, dsItemIds, timeStart, timeEnd,
        strItem.get('timeFormat'), strItem.get('searchNearest', False))
    if not (isinstance(strData, dict) and 'error' in strData.keys()):
        for strItemData in strData:
            strValueData = []
            strHistoryData = strItemData.get('history')
            for strRow in strHistoryData:
                strValueData.append(strRow['value'])
            strHistory.append(dict(dsItemId="@" + str(strProjectId) + "|" + strItemData.get("name"), data=strValueData))
            if len(strTime) == 0:
                for s in strHistoryData:
                    strTime.append(s['time'])
    strRt.append({"list": strHistory, "timeShaft": strTime})  
    return strRt


def getHistoryDataBy_dac(dacItem):
    dacRt = []
    dacProjectId = int(dacItem.get('projectId'))
    dacData = BEOPDataAccess.getInstance().get_history_data_padded(dacProjectId, dacItem.get('dsItemIds'), dacItem.get('timeStart'), dacItem.get('timeEnd'),
                                                                dacItem.get('timeFormat'), dacItem.get('searchNearest', False))
    if not (isinstance(dacData, dict) and 'error' in dacData.keys()):
        for dacItemData in dacData:
            dacHistory = []
            dacTime = []
            dacValueData = []
            dacHistoryData = dacItemData.get('history')
            for dacRow in dacHistoryData: 
                if not dacRow["error"] :
                    dacValueData.append(dacRow["value"])
                else:
                    dacValueData.append(None)
            dacHistory.append(dict(dsItemId= "@" + str(dacProjectId) + "|" + dacItemData.get("name"), data=dacValueData))
            for d in dacHistoryData:
                dacTime.append(d['time'])
        dacRt.append({"list": dacHistory, "timeShaft": dacTime})
    else:
        dacRt.append({"list": [], "timeShaft": []})    
    return dacRt


@bp_factory.route('/recoverPageList/<proj_id>', methods=['GET'])
def recoverPageList(proj_id):
    if proj_id == '':
        return formatResult(False, '未获取到正确的项目id')
    result = FactoryService.recover_page_list(proj_id)
    return jsonify(formatResult(result))
