        ...
        try:
            ...
            for item in cursor:
                try:
                    ...
                    app.logger.error("Failed to get workflow task.", exc_info=True, stack_info=True)
        except Exception as e:
            app.logger.error("Failed to get workflowtask with fieldtype == 4", exc_info=True, stack_info=True)
    ...
    def get_exetime(cls, maintainCycle, effectiveDate, reminderDay):
        ...
        if maintainCycle == 0:
            while exeTime.date() < (nowdate + timedelta(days=int(reminderDay))).date():
                if 1 == exeTime.month and exeTime.day in [29, 30, 31]:
                    exeTime = exeTime.replace(month=exeTime.month + 1, day=28)
                elif exeTime.month in [3,5,8,10] and 31 == exeTime.day:
                    exeTime = exeTime.replace(month=exeTime.month + 1, day=30)
                elif 12 == exeTime.month:
