...
class BEOPMongoDataAccess:
    ...
    def __init__(self, addr, st=None, et=None, is_ssl=False):
        bOk = False
        try:
            self._st = st
            self._et = et
            ...
            if ':' in addr:
                ...
                if len(addrArr) == 2:
                    ...
                    port = int(addrArr[1])
                    self._hostAddr = addr
                    self.mdbConnection = pymongo.MongoClient(host=hostAddr,port=port, ssl=is_ssl, ssl_cert_reqs=ssl.CERT_NONE)
                    self.mdbBb = self.mdbConnection.beopdata
                    if is_ssl:
                        bOk = self.mdbBb.authenticate('rnbtech','bGbRZxeHL9jsEM8xGgL0CrlsTGKOnSpnMlzn8QiGHEYIMxuiT2qgU94OLLGCQEzoNvkzeEsEapsyayJXBZR6QA==')
                    else:
    ...
    def _heartbeat(self):
        ...
        try:
            ...
            logging.debug("%s heartbeated with %d collections" % (self._hostAddr, collection_count))
        except:
            logging.error("Failed to heartbeat %s due to error: %s" % (self._hostAddr, traceback.format_exc()))
...
#### 温控 app 接口 ####
    ...
    def factoryCopyPages(self, page_list, new_proj_id):
        ...
        try:
            ...
            print('factoryShadowCopyPages find pages error:' + err.__str__())
            logging.error('factoryShadowCopyPages find pages error:' + err.__str__())
            flag = False
        ...
        for i, page in enumerate(pages):
            ...
            try:
                ...
                print('factoryShadowCopyPages copy page error:' + err.__str__())
                print('pageId:' + str(page.get('_id')))
                logging.error('factoryShadowCopyPages copy page error:' + err.__str__())
                logging.error('pageId:' + str(page.get('_id')))
                flag = False

        try:
            self.mdbBb[g_table_fac_navigation].update_one({'_id': ObjectId(new_proj_id)}, {'$addToSet': {'list': {'$each': new_page_list}}})
        except Exception as err:
            print('factoryShadowCopyPages insert into navigation error:' + err.__str__())
            logging.error('factoryShadowCopyPages insert into navigation error:' + err.__str__())
            flag = False
        print('处理完毕')
        logging.info('处理完毕')
        return flag
    ...
    def getHisDataFromDB(self, collectionName, timeFormat, dayStartQueryObject, dayEndQueryObject, startObject, endObject, point_list, data, bSearchNearest=False):
        try:
            ...
            if bSearchNearest:
                ...
                self.searchNearest(tl, point_list, tndict, fixTime, data)
        except Exception as e:
            print(e)
        finally:
            if cursor:
