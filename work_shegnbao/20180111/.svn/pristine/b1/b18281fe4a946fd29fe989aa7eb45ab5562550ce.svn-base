    ...
    try:
        ...
        if dbname:
            if time_zone:
                ...
                tNow = datetime.now()
            tStart = tNow - timedelta(minutes=60 * 12)  # only get the notice from latest 12 hours
        ...
        if dbname != None:
            if postData != None:
                ...
                if range == 'fault':
                    ...
                    logging.debug('%s is not valid parameter', range)
                    result = False
    except Exception:
        logging.error('Unhandled exception! Locals: %s', locals(), exc_info=True, stack_info=True)
    return json.dumps(result, ensure_ascii=False)
                ...
                if dbrv:
                    for item in dbrv:
                        ...
                        try:
                            ...
                            logging.error('Failed to update fault dict! Locals: %s',
                                          locals(), exc_info=True, stack_info=True)
...
def get_target_by_equipment(EquipmentName, EquipmentId):
    rt = ''
    headers = {'content-type': 'application/json'}
    url = app.config.get('ALGOSVC2_ADDRESS') + '/equipmentTarget/getTarget'
    try:
...
def updateDiagnosisFaultMulti111(projectId):
    ...
    try:
        ...
        if dbname != None:
            ...
            strSQL = 'SELECT n.Id, max(n.Time) AS FaultNoticeTime, f.Name, GROUP_CONCAT(distinct e.Name) as EquipName, \
                      f.Description, max(n.OrderId), n.Status, n.CheckTime \
                      FROM %s_diagnosis_notices AS n \
                      LEFT JOIN %s_diagnosis_faults AS f ON n.FaultId = f.Id \
                      LEFT JOIN %s_diagnosis_equipments AS e on e.Id = f.EquipmentId \
                      WHERE f.DefaultGrade = 2 and n.Time > "%s" \
                      GROUP BY f.Name \
                      ORDER BY FaultNoticeTime DESC \
...
@app.route('/diagnosis/getEnergyAvgByFaultNames', methods=['POST'])
def get_energyAvg_by_faultName():
    """
    David 20171124
    """
    post_data = request.get_json()
    ...
    if post_data:
        try:
            projId = int(post_data.get('projId'))
            faultNameList = post_data.get('faultNameList')
            startTime = post_data.get('startTime')
            endTime = post_data.get('endTime')
            interval = post_data.get('interval')
            if isinstance(faultNameList, str):
                faultNameList = [faultNameList]
            rt = {}
            dbAccess = BEOPDataAccess.getInstance()
            mysqlname = dbAccess.getProjMysqldb(projId)
            if mysqlname:
                for faultName in faultNameList:
                    rt.update({faultName: None})
                    strSQL = 'SELECT AVG(n.Energy), f.`Name`, u.Factor, f.RunTimeDay, '\
                            ...
                            'AND f.`Name` = "%s" GROUP BY f.EquipmentId' % (mysqlname, mysqlname, endTime, startTime, faultName)
                    dbrv = dbAccess._mysqlDBContainerReadOnly.op_db_query('diagnosis', strSQL, ())
                    if dbrv:
...
@app.route('/diagnosis/statFaultByFaultName', methods=['POST'])
def stat_fault_by_faultName():
    """
    David 20171124
    """
    post_data = request.get_json()
        ...
        try:
            ...
            if mysqlname:
                strSQL = 'SELECT FaultName, SUM(nf.t) as duration FROM ('\
                         ...
                         'GROUP BY FaultName' % (mysqlname, mysqlname, startTime, endTime)
                dbrv = dbAccess._mysqlDBContainerReadOnly.op_db_query('diagnosis', strSQL, ())
                if dbrv:
                    faultname_duration = {}
                    for item in dbrv:
                ...
                strSQL = 'SELECT FaultName, count(*) AS eNum, SUM(selectTable.fNum) AS fNum FROM ('\
                         ...
                         'GROUP BY selectTable.FaultName ORDER BY eNum DESC' % (mysqlname, mysqlname, mysqlname, startTime, endTime)
                dbrv = dbAccess._mysqlDBContainerReadOnly.op_db_query('diagnosis', strSQL, ())
                if dbrv:
                    for item in dbrv:
                        if item[0] in faultname_duration.keys():
                            d = math.ceil(faultname_duration.get(item[0]) / int(item[2]) / (60*60))  #故障总时长除以诊断条数，单位小时
                        else:
                            d = None
