(function (beop) {
    ...
    var configMap = {
        ...
        setJqueryMap,
        configModel,
        init, onTableHeadClick, resetState, renderTaskList, loadTaskList, loadTaskDetail, searchTaskList,
        paginationRefresh, onClickReturnActivities, bindTaskFilterEvent, dealTaskFilter, searchTaskFilter,
        initTaskFilterDateTimePicker, bindTaskFilterCustomDate, resetTaskFilter, getFilterTimeRangeQuery, searchTask;
    ...
    dealTaskFilter = function ($label, type, selectValue) {
        ...
        stateMap.currentPage = beop.model.stateMap.currentPage = 1;
        //选取自定义时间的时候
        if (type === "dueDate") {
            model["dueDate"] = {};
        ...
        var getPageClickCallback = function () {
            if (beop.model.stateMap.isShowTaskFilter && Object.keys(beop.model.stateMap.filter.param).length) {
                return function (event, page) {
                    stateMap.currentPage = beop.model.stateMap.currentPage = page;
                    searchTaskFilter();
                }
            } else {
                return function (event, page) {
        ...
        var switchType = function (type) {
            ...
            switch (type) {
                ...
                case "taskGroup":
                {
                    beop.model.stateMap.filter.taskTypeQuery = {
                        "taskGroupId": param,
                        "$or": [{"creator": AppConfig.userId}, {"executor": AppConfig.userId},
                            {"process.nodes.member": AppConfig.userId}, {"watchers": AppConfig.userId}]
                    };
                    return configMap.transactions_model.taskTypeFilter(currentPage, orderObject).done(function (result) {
                        if (result.success) {
                            renderTaskList();
