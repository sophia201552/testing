...
function requestDiagnosisNotices() {
    var data = {};
    var xhrNotice = new XMLHttpRequest(),
        xhrCount = new XMLHttpRequest();;
        ...
        if (xhrNotice.status == 200) {
            ...
            xhrCount.onload = function() {
                if (xhrCount.status == 200) {
                    ...
                    //TODO: for BOMA_Lab
                    if (projectId == 581) {
                        setTimeout(requestAllDiagnosisNotices, 8000);
                    } else {
                        setTimeout(requestAllDiagnosisNotices, 600000); //600000
                    }
                    // setTimeout(requestAllDiagnosisNotices, 600000); //600000
                } else {
                    postMessage({ error: '1000' });
...
function requestAllDiagnosisNotices() {
    var data = {};
    var xhrNotice = new XMLHttpRequest(),
        xhrCount = new XMLHttpRequest();;
    var urlNotice, urlCount;
    ...
    urlNotice = '/diagnosis/getRealtimeFault/' + projectId;
    xhrNotice.open('GET', urlNotice);
    xhrNotice.setRequestHeader("Content-Type", "application/json");
    xhrNotice.send();
}
...
function requestRoomInfoRealtime() {
    var xhr = new XMLHttpRequest();
    xhr.onload = function() {
        if (xhr.status == 200) {
            postMessage(JSON.parse(xhr.responseText));
            setTimeout(requestRoomInfoRealtime, interval);
        } else {
            postMessage({ error: '1000' });
        }
    };
    xhr.open("POST", "/appTemperature/room/getRealtimeData/" + roomId);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.send(JSON.stringify({ dsItemIds: pointList }));
}
...
function messageHandler(e) {
    ...
    switch (type) {
        ...
        case 'diagnosisScreen':
            ...
            break;
        case 'allDiagnosisScreen':
            projectId = e.data.projectId;
            requestAllDiagnosisNotices();
            break;
        case 'datasourceRealtime':
            pointList = e.data.pointList;
            if (e.data.projectId && e.data.projectId === 581) {
                interval = 8000;
            }
        ...
        case 'clearTimer':
            ...
            break;
        case 'roomInfoRealtime':
            pointList = e.data.pointList;
            roomId = e.data.roomId;
            requestRoomInfoRealtime();
            break;
    }
}
