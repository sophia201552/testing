...
import s from './DataSortingPanel.css';
...
class DataSortingPanel extends React.Component {
  constructor(props) {
    super(props);
  ...
  static resetName(name, names, index = 0) {
    ...
    if (names.indexOf(finName) < 0) {
      ...
      return DataSortingPanel.resetName(name, names, index + 1);
    }
  }
  componentDidMount() {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    let newNamesArr = DataSortingPanel.nameSuffix(items.map(v => v.newName));
    items.forEach((it, i) => {
  ...
  render() {
    ...
    return (
      <div className={css('clear dataSortingPanel')}>
                    ...
                    switch (propName) {
                      ...
                      case 'newName':
                        ...
                        if (items[index][propName] != newV) {
                          newV = DataSortingPanel.resetName(
                            newV,
...
export default DataSortingPanel;
