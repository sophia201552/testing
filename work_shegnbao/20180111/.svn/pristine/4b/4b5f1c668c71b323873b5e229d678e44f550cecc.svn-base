...
class User(DbEntity):
    ...
    @staticmethod
    def query_user_by_email(email, *obj):
        """
        根据用户用户名返回帐号信息, 帐号信息可以自定义字段
        ...
        :return: 帐号信息
        """
        sql = 'select ' + ','.join(obj) + ' from ' + User.table_name + ' where useremail=%s'
        rv = User.db.op_db_query_one(User.db_name, sql, (email,))
        return {key: value for key, value in zip(obj, rv)}

    @staticmethod
    def query_user_by_mobile(usermobile, *obj):
        """
        根据用户用户名返回帐号信息, 帐号信息可以自定义字段
        ...
        :return: 帐号信息
        """
        sql = 'select ' + ','.join(obj) + ' from ' + User.table_name + ' where usermobile=%s'
        rv = User.db.op_db_query_one(User.db_name, sql, (usermobile,))
        return {key: value for key, value in zip(obj, rv)}
                    ...
                    if old_userpic.find('default') == -1:  # 默认头像不删除
                        ...
                        if os.path.exists(old_userpic_path):
                            ...
                            if pos >= 0:
                                ...
                                oldfileName = ''
                            oss.delete_object('beopweb', 'static/images/avatar/user/' + oldfileName)
    ...
    def get_hierarchical_users(root_id=1, *obj):
        ...
        def build_tree(nodes):
            ...
            if not tree:
                ...
                return build_tree_recursive(tree, tree.get('id'), nodes)
