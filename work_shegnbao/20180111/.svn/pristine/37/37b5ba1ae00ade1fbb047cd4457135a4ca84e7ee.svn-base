var UpdateHelper = (function() {
    ...
    UpdateHelper.prototype = {
        ...
        //},
        modalTpl: '\
        <div id="ctnUpdateHelper" style="'+ (window.device && window.device.platform =="iOS"?'display:none;':'') +' position: fixed;z-index: 1004;\
        ...
        <div class="backdrop zepto-ev" style="'+ (window.device && window.device.platform =="iOS"?'display:none;':'') +' position: fixed;top: 0;left: 0;right: 0;bottom: 0;\
        ...
        getLastVersion: function() {
            if(window.device.platform =="iOS")return;
            WebAPI.get('/appCommon/getVersion/' + this.name + '/' + this.curVersion).done(function(result) {
        ...
        showUpdateModel: function(result, dictIgnore) {
            ...
            this.$modal = $(html);
            if (AppConfig && AppConfig.language == 'en' && result.detail_en) {
                this.$modal.find('#divHelperDesc').html(result.detail_en);
            } else {
