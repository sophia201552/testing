var ModelTempDistribution = (function () {
    var DEFAULT_MIN = 20,
        DEFAULT_MAX = 28,
        DEFAULT_GradientColor = '0,#3434ff;0.17,#35ffff;0.33,#36fe94;0.5,#6ff71c;0.67,#9fff39;0.83,#ffa922;1,#ff2323;';
    var defaultMin = 10,
        defaultMax = 40,
        defaultRadius = 50,
    ...
    ModelTempDistribution.prototype.init = function () {
        ...
        $(ElScreenContainer).off('refreshHeatMap').on('refreshHeatMap', function () {
            ...
            _this.loadColorSettingPromise.done(function () {
                Spinner.spin(ElScreenContainer);
                _this.pageScreen.painter.rootLayer.refresh();
                Spinner.stop();
            });
        })
    };
    ...
    ModelTempDistribution.prototype.loadColorSetting = function () {
        ...
        this.loadColorSettingPromise = WebAPI.post('/admin/getColorSetting', {
            ...
            heatType: window.isHtmlHeat3D?'3d':(this.heatType||'co')
        }).done(function (result) {
            if (result.success && result.data) {
                _this.colorSettings = result.data;
                //用户配置
                window.colorGettings = result.data;
              } else if (window.isCanvasHeatColor) {
                //控件配置
                window.colorGettings = window.canvasHeatColorOption;
              } else {
                //默认值
                window.colorGettings = {
                    isGradient: defaultIsGradient,
                    radius: defaultRadius,
                    gradientColor: defaultColor[window.isHtmlHeat3D?'3d':(this.heatType||'co')],
                    min: defaultMin,
                    max: defaultMax
                  };
              }
    ...
    ModelTempDistribution.prototype.resetMaxMinTemp = function () {
        configMap.TEMP_RANGE.max = DEFAULT_MAX;
        configMap.TEMP_RANGE.min = DEFAULT_MIN;
    };
            ...
            for (tempSetType in this.colorSettings) {
                if (this.colorSettings.hasOwnProperty(tempSetType)) {
                    if (tempSetType == 'max') {
                        ...
                        configMap.TEMP_RANGE.max = (typeof setValue == 'undefined' || setValue === '') ? DEFAULT_MAX : Number(setValue);
                    } else if (tempSetType == 'min') {
                        setValue = this.colorSettings[tempSetType].value;
                        configMap.TEMP_RANGE.min = (typeof setValue == 'undefined' || setValue === '') ? DEFAULT_MIN : Number(setValue);
                    }
                }
            }
        } else {
