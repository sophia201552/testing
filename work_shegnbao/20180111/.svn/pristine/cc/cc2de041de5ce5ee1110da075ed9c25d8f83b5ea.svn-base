...
class BenchmarkConfig{
    ...
    initConfigNav(){
        ...
        $(nav).off('click').on('click','.navItem', e=>{
            ...
            var currentDom = $('.ctn' + type[0].toUpperCase() + type.slice(1) + 'Config');
            currentDom.addClass('focus');
            var $divSummaryPt = currentDom.find('.divSummaryPt');
            if((!this.screen.iotFilter.tree.getSelectedNodes()[0].getParentNode())&&$divSummaryPt.length>0){
                $divSummaryPt.show();
            }else{
                $divSummaryPt.hide();
            }
    ...
    initPointConfig(){
        ...
        $(ctn).off('drop').on('drop','.divPtCfg',e=>{
            ...
            if(!this.store.point) this.store.point = {};
    ...
    setPointConfigVal(){
        if((!this.screen.iotFilter.tree.getSelectedNodes()[0].getParentNode())&&$('.navConfig').find('.selected').attr('data-type')==='point'){
                $('.divSummaryPt').show();
            }else{
                $('.divSummaryPt').hide();
            }
        if (!this.selectNode){
            ...
            let nodeConfig = this.store.point?this.store.point[this.selectNode['_id']]:null;
            let power='',energy='',summary = '';
            if (nodeConfig){
                power = nodeConfig.power ? nodeConfig.power:'';
                energy = nodeConfig.energy ? nodeConfig.energy:'';
                summary = nodeConfig.summary?nodeConfig.summary:'';
            }
            var $divPowerPt = $('.divPowerPt');
            var $divConsumePt = $('.divConsumePt');
            var $divSummaryPt = $('.divSummaryPt');
            var point;
            if(power) {
            ...
            if (summary) {
                $divSummaryPt[0].dataset.id= summary;
                point = this.screen.dataSource.getDSItemById(summary);
                $divSummaryPt.addClass('hasDs').find('.spPtVal').text(point.alias?point.alias:point.value)
            }else{
                $divSummaryPt.removeClass('hasDs').find('.spPtVal').text('');
            }
            $('.divPtVal').on('mouseenter',function(e){
                var spanValue=$(e.currentTarget).find('.spPtVal').text();
                
                $(this).parent().append('<span id="showPtVal">'+spanValue+'</span>');
