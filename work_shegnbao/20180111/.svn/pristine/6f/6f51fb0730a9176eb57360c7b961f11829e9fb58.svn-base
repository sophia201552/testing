...
var WorkflowInsert = (function() {
    ...
    WorkflowInsert.prototype = {
        ...
        _bindEventsCommon: function() {
            ...
            //选择设备更改曲线
            this.jqueryMap.$container.on('change.wf-detail-equipment', '.wf-detail-equipment', this._modifyEchartTime.bind(this));
            //选择故障时间更改曲线
            this.jqueryMap.$container.on('change.wf-detail-faultTime', '.wf-detail-faultTime', this._modifyFaultTime.bind(this));            
            //点击确定
            this.jqueryMap.$container.on('click.workflow-insert-submit-btn', ConfigMap.el.confirmBtn, this._onConfirm.bind(this));
        },
        //修改echart曲线显示的设备
        _modifyEchartTime: function(ev){
            var $selectEquipment = $(ev.currentTarget)[0].value;
            var tpl = '<option value="{index}">{Time}</option>', strHtml = '';
            var $selectTime = this.insertData.arrayEquipment[$selectEquipment].arrNoticeTime;
            var $monentTime = $selectTime[0].Time;
            this.insertData.chartStartTime = new Date(new Date($monentTime.replace(/-/g,'/')).getTime() - 3 * 60 * 60 *1000).format('yyyy-MM-dd HH:mm:ss');
            this.insertData.chartEndTime = new Date(new Date($monentTime.replace(/-/g,'/')).getTime() + 3 * 60 * 60 *1000).format('yyyy-MM-dd HH:mm:ss');
            $selectTime.forEach(function (ele, index) {
                strHtml += (tpl.formatEL({
                    index: index,
                    Time: ele.Time
                }));
            });
            this.jqueryMap.$modalBody.find('.wf-detail-faultTime').empty().html(strHtml);            
            this._renderFaultChart().done(function(result) {
                if (result.success) {
                    I18n.fillArea(this.jqueryMap.$container);
                }
        ...
        //修改echart出错误的时间段
        _modifyFaultTime: function(ev){
            var $target = $(ev.currentTarget)[0];
            var $selectTime = $target.options[$target.value].text;
            this.insertData.chartStartTime = new Date(new Date($selectTime.replace(/-/g,'/')).getTime() - 3 * 60 * 60 *1000).format('yyyy-MM-dd HH:mm:ss');
            this.insertData.chartEndTime = new Date(new Date($selectTime.replace(/-/g,'/')).getTime() + 3 * 60 * 60 *1000).format('yyyy-MM-dd HH:mm:ss'); 
            this._renderFaultChart().done(function(result) {
                if (result.success) {
                    I18n.fillArea(this.jqueryMap.$container);
                }
        ...
        _loadDataNormal: function() {
            ...
            var description = this.insertData.description;
            description = description.replace(/\<b\>/gi,'').replace(/\<p\>/gi,'').replace(/\<\/b\>/gi,'').replace(/\<\/p\>/gi,'\n');
            this.insertData.description = description;
        ...
        _addUserSelectDialog: function(ev) {
            var $modalBody = this.jqueryMap.$modalBody,
                $this = $(ev.target);
        ...
        _onConfirm: function() {
            var _this = this;
            if (!_this.stateMap.userSelectedMap.executor.length) {
                alert(I18n.resource.workflow.main.SELECT_EXECUTOR_FIRST);
                return;
            }
        ...
        _pageTaskCheck: function() { //任务页面校验
            var $dialog = $("#workflow-insert-container"),
                i18n = I18n.resource.workflow.common;
        ...
        _getSubmitData: function() {
            ...
            var fields = {
                fields: {
                    diagnosisZone: _this.insertData.zone,
                    diagnosisEquipmentName: _this.insertData.equipmentName,
                    faultId: _this.insertData.faultId,
                    noticeId: _this.insertData.noticeId,
            ...
            var chartIns = echarts.getInstanceById($('#workflow-insert-curve')[0].getAttribute('_echarts_instance_'));
            var image = {
                image: chartIns.getDataURL()
            };
            this.taskModelInfo = $.extend(true, {}, fields, processMember, attachment, image);
