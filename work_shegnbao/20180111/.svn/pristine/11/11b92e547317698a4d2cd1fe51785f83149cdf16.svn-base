...
var ProjectList = (function() {
    ...
    ProjectList.prototype = {
        ...
        init: function() {
            ...
            if (!(_this.priority && _this.priority instanceof Array) ){
                _this.priority = [];
                for (var i = 0; i < ProjectConfig.projectList.length; i++) {
                    _this.priority.push(ProjectConfig.projectList[i].id)
                }
        ...
        initProjectList: function(arg) {
            ...
            for (var i = 0, status = ''; i < arrProject.length; ++i) {
                ...
                if (arrProject[i].online && arrProject[i].lastReceivedTime) {
                    strLiProject.append('   <div class="projectStatus online">' + DateUtil.getRelativeDateInfo(new Date(), new Date(+moment(arrProject[i].lastReceivedTime))) + '</div>');
                } else {
        ...
        initProjectDisplay: function() {
            ...
            $('.liProject').off('tap').on('tap', function(e) {
                ...
                _this.setPriority(ProjectConfig.projectId);
                for (var i = 0; i < ProjectConfig.projectList.length; i++) {
        ...
        setPriority:function(id){
            id = id || ProjectConfig.projectId;
            var list = [id];
            _this.priority.forEach(function(item){
                if(!item)return;
        ...
        sortProjByPriority: function(arrProj) {
            ...
            var arrIndex = [];
            var flag = false;
            for (var i = 0 ; i < _this.priority.length ;i++){
                for (var  j = 0 ; j < arrProj.length ;j++){
                    if(arrProj[j].id == _this.priority[i]){
                        arr.push(arrProj[j])
                        arrIndex.push(j)
                        break;
                    }
            ...
            arr = [].concat(arr,arrProj.filter(function(item,index){
                return (arrIndex.indexOf(index) < 0)
            }))
            return arr;
        },
        initTopNav: function() {
