...
class TaskGroup:
    ...
    def get_task_group_with_autoEscalationConfig(self):
        '''
        获取所有有autoEscalationConfig的group
        :return:
        '''
        query = {"autoEscalationConfig": {"$exists": True}}
    ...
    def get_task_group_by_id(self, id):
        ...
        if result:
            ...
            autoEscalationConfig = result.get('autoEscalationConfig')
            if autoEscalationConfig:
                for config in autoEscalationConfig:
                    targets = config.get('targets')
                    if targets:
                        for target in targets:
                            target['userInfo'] = user.query_user_by_id(target.get('userId'), 'userfullname', 'useremail', 'userpic')
    ...
    def get_team_id_by_group_id(self, group_id):
        team_id = self.db.find_one({'_id': ObjectId(group_id)}, {'_id': 0, 'team_id': 1}).get('team_id')
