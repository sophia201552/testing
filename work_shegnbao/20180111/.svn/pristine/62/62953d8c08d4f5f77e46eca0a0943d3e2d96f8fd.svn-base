        ...
        this.infoMap.attachment && this.infoMap.attachment.forEach(function (item) {
            htmlStr += beopTmpl('attachment_item_templ', {
                file: {
                    ...
                    url: 'https://beopweb.oss-cn-hangzhou.aliyuncs.com/workflow/attachment/' + item.uid + '-' + item.fileName,
                    userId: item.userId,
                ...
                success: function (result) {
                    if (result.data && result.data.length) {
                        $("#divAssetImg").html('<div class="imgItem dn">' +
                            '<img src="https://beopweb.oss-cn-hangzhou.aliyuncs.com/' + result.data[0] + '?_=' + Date.now().valueOf() + '" class="imgAsset" id="imgAsset"/>' +
                            '</div>');
                    }
                }
            });
