...
;(function (exports, diagnosisEnum) {
    class PagingTable {
        constructor(container, options) {
            ...
            var nextPage = I18n.type == 'zh' ? '下一页' : 'Next page',
                prevPage = I18n.type == 'zh' ? '上一页' : 'Prev page';
            ...
            "           <div class='widget-sdt-prev-page'><button type='button' class='btn btn-info btn-sm prev-page'>" + prevPage + "</button></div>"+
            "           <div class='widget-sdt-more'><button type='button' class='btn btn-info btn-sm more style='margin-left:15px'>" + nextPage + "</button></div>"+
            "        </div>" +
            "    </div>" +
            "    </div>" +
            // "    <div class='table-footer widget-sdt-footer'>" +
        ...
        ajax(){
            ...
            if(this.options.method === "get"){
                return WebAPI.get(this.options.url).done(function(result){
                ...
                if ($('.taskTable').length !==0){
                    if (this.options.selectedStatus && this.defaultsOpt.selectedStatus.length!==0) {
                        this.options.postData.filter = this.options.selectedStatus(this.defaultsOpt.selectedStatus);
                    }
                }
                return WebAPI.post(this.options.url,this.options.postData).done(function(result){
                    _this.store = _this.options.dataFilter(result);
                    _this.initTableBody(_this.store);
                    _this.options.onDoneRender();
                    //分页处理
        ...
        initTableBody(data){
            ...
            this.$tBody.empty().append(noFaultHtml);
            }
        ...
        initRowData(rowData){   
            ...
            this.options.tbodyCol.forEach(function(col,i){
                ...
                if(col.name){
                    ...
                    if(col.converter){
                        value = `<span class="ellipsis" style="${col.style||'flex:1;'}">${col.converter(rowData)}</span>`;
        ...
        initTableMore(){
            var $tableMore = this.$dataTable.find(".widget-sdt-more"),
                $tableLastPage = this.$dataTable.find('.widget-sdt-prev-page');
            if(this.options.more){
        ...
        initTotalNum(){
            this.setting.totalNum = this.setting.totalNum ? this.setting.totalNum : 0;
        ...
        attachEvents(){
            ...
            this.$dataTable.off('click', '.widget-sdt-prev-page .prev-page').on('click', '.widget-sdt-prev-page .prev-page', function () {
            ...
            //筛选任务
            $('.taskTable').off('click.btn').on('click.btn', '.statusBtns > div',function(){
                let status = $(this).data('status');
                if ($(this).hasClass('highLight')) {
                    $(this).removeClass('highLight');
                    let index = _this.defaultsOpt.selectedStatus.findIndex((row) => { return row === status });
                    _this.defaultsOpt.selectedStatus.splice(index,1);
                } else {
                    $(this).addClass('highLight');
                    _this.defaultsOpt.selectedStatus.push(status);
                }
                _this.show();
            })
        }
