...
class BEOPDataBufferManager:
    ...
    def getProjectDataBuffer(self, projId):
        ...
        if not self.getProjNeedUpdated(projId):
            ...
            if projName != None:
                ...
                for item in rv:
                    buf._dataRealtime[item[1]] = [item[0], item[2], item[3]]
                self._projectDataList[projId] = buf

        return buf
		
    def getDtuname(self, projId,pointNameList):
        projName = BEOPDataAccess.getInstance().getProjNameById(projId)
        if projName != None:
            pointList = BEOPDataAccess.getInstance().getInputTableIncludeDtuname(projName,pointNameList)
        return pointList
    ...
    def setDataByProj(self, proj, point, value):
        ...
        if dbname is None:
            errInf = 'error: finding project database failed'
            print(errInf)
    ...
    def setMutilData(self, projName,pointList, valueList):
        ...
        if nPointCount <= 0 or nPointCount!=nValueCount:
            errInf = 'failed for count not right'
            print(errInf)
            logging.error(errInf)
            return errInf
    ...
    def setMutilDataToSiteByProjectId(self,projectId, pointNameList, updatedValueList,dtuName=""):
        try:
            nPointCount = len(pointNameList)
            pointDict={}  #{key:[]}
            pointDict["param"] = {}
            bResult=False
            for i in range(nPointCount):
                pointDict["param"][pointNameList[i]]=[pointNameList[i],updatedValueList[i],dtuName]
            pointDtunameList = []
            if dtuName is None or dtuName=="":
                pointDtuList = self.getDtuname(projectId, pointNameList)
                if pointDtuList!=None:
                    for point in pointDtuList:
                       pointDict['param'][point[0]][2]=point[1]
            strErrorPointName=""
            errorPointNameList=[]
            for pointName in pointDict['param']:
                if pointDict["param"][pointName][2]==None or pointDict["param"][pointName][2]=="":
                    strErrorPointName+=pointName+','
                    errorPointNameList.append(pointName)
            for i in range(len(errorPointNameList)):
                pointDict["param"].pop(errorPointNameList[i])
            nRealCount=len(pointDict["param"])
            if nRealCount==0:
                strLog = "error:all point modification failed"
                print(strLog)
                logging.error(strLog)
                # return strLog
            else:
                bResult = BEOPDataAccess.getInstance().appendMutilOutputToSiteTableIncludeDtu( pointDict)
            if bResult == True and nRealCount==nPointCount:
                strLog = "succeeded sending command for updating point value."
                print(strLog)
            elif bResult==True and nRealCount!=nPointCount:
                strLog="error:some point modification failed(%s)" %(strErrorPointName[:-1])
                print(strLog)
                logging.error(strLog)
            else:
                strLog = "error:all point modification failed"
