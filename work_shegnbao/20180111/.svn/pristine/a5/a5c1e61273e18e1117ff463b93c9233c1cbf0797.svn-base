(function (window, $) {
    ...
    var opTypes = {
        ...
        CHANGE_PIPE_WIDGET_ANIMATION: new OpType(807, ''),
        // 创建图片控件
        ...
        // 创建线段控件
        CREATE_LINE_WIDGET: new OpType(1300, ''),
        // 删除线段控件
        DELETE_LINE_WIDGET: new OpType(1301, ''),
        //改变线段控件长度
        CHANGE_LINE_WIDGET_WIDTH: new OpType(1302, ''),
        //改变线段控件颜色
        CHANGE_LINE_WIDGET_COLOR: new OpType(1303, ''),
        //改变线段控件样式
        CHANGE_LINE_WIDGET_STYLE: new OpType(1304, ''),
        // 创建热点控件
        CREATE_HOTSPOT_WIDGET: new OpType(1400, ''),
        // 删除热点控件
        DELETE_HOTSPOT_WIDGET: new OpType(1401, '')
    };
    var OP_TYPE = {};
        ...
        widgetModelSet.addEventListener('insert', function (e, data) {
            ...
            switch(type) {
                ...
                case 'CanvasHeatP':
                    ...
                    break;
                case 'CanvasPolygon':
                     this.addRecord('widget',OP_TYPE.CREATE_HOTSPOT_WIDGET);
                    break;
                case 'CanvasCustomLine':
                    this.addRecord('widget',OP_TYPE.CREATE_LINE_WIDGET);
                break;
            }
        }, this);
        widgetModelSet.addEventListener('remove', function(e,data){
            ...
            switch(type) {
                ...
                case 'CanvasHeatP':
                    ...
                    break;
                case 'CanvasPolygon':
                     this.addRecord('widget',OP_TYPE.DELETE_HOTSPOT_WIDGET);
                    break;
                case 'CanvasCustomLine':
                    this.addRecord('widget',OP_TYPE.DELETE_LINE_WIDGET);
                break;
            }
        }, this);
        // widget models
    ...
    HistoryController.prototype.bindWidgetOpOb = function (model) {
        ...
        var opMap = {
            ...
            'update.option.points' : OP_TYPE.MOVE_WIDGET_OR_RESIZE_WIDGET,
            ...
            'update.option.style': OP_TYPE.CHANGE_LINE_WIDGET_STYLE,
            'update.option.pipeAnimation': OP_TYPE.CHANGE_PIPE_WIDGET_ANIMATION,
        };
        ...
        switch(type) {
            ...
            case 'CanvasHeat':
                ...
                break;
            case 'CanvasCustomLine':
                opMap['update.option.width'] = OP_TYPE.CHANGE_LINE_WIDGET_WIDTH;
                opMap['update.option.color'] = OP_TYPE.CHANGE_LINE_WIDGET_COLOR;
                opMap['update.option.style'] = OP_TYPE.CHANGE_LINE_WIDGET_STYLE;
                break;
            default:
                break;
        }
    ...
    HistoryController.prototype.update = function (data, type) {
        data = $.deepClone(data);
        this.frozen = true;
        this.page.updateModelSetByDiffs(data);
        this.frozen = false;
    };
    ...
    HistoryController.prototype.addRecord = function (type, op) {
        ...
        // 合并期间不添加任何记录
        if(isNeedMergeRecord(type, op)) return;
        this.add(op);
    };

    HistoryController.prototype.add = function(op){
        var state, data;
    ...
    function isNeedMergeRecord(type, op){
        var actives = [],
            r = true;
        switch(type){
            case 'widget':
                actives = _this.page.painter.state.activeWidgets();
                break;
            case 'layer':
                actives = _this.page.painter.state.activeLayers().concat(_this.page.painter.state.activeWidgets());
                break;
        }
        if(actives.length < 1){
            r = false;
            ...
            if(!_this.mergeTime){
                _this.mergeTime = setTimeout(function(){
                    _this.add(op);
                    _this.mergeTime = undefined;
                },100);
            }
        }
        return r;
