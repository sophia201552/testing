...
def analysisExecRelationAnalysis():
    try:
        ...
        data = request.get_json()
        rt = requests.post(app.config.get('ALGOSVC1_ADDRESS') + '/correlationanalysis/analysis/relationalysis', data=json.dumps(data), headers=headers)
    except Exception as e:
...
def analysisRegression():
    try:
        headers = {'content-type': 'application/json'}
        data = request.get_json()
        rt = requests.post(app.config.get('ALGOSVC1_ADDRESS') + '/correlationanalysis/analysis/modelpredictdata', data=json.dumps(data), headers=headers)
    except Exception as e:
...
def analysisGetModel():
    ...
    try:
        headers = {'content-type': 'application/json'}
        data = request.get_json()
        rt = requests.post(app.config.get('ALGOSVC1_ADDRESS') + '/correlationanalysis/analysis/analysisModel', data=json.dumps(data), headers=headers, timeout=60)
        if rt and rt.text:
            rt = rt.text
    except Exception as e:
...
def analysisExecuteModel():
    try:
        headers = {'content-type': 'application/json'}
        data = request.get_json()
        rt = requests.post(app.config.get('ALGOSVC1_ADDRESS') + '/correlationanalysis/analysis/analysispredict', data=json.dumps(data), headers=headers)
    except Exception as e:
    ...
    try:
        ...
        if dbname:
            ...
            if res:
                lis = []
                for i in res:
                    lis.append({'name': i[0], 'coust': int(i[2]) * elec if i[2] else None, 'energy': int(i[2]) if i[2] else None, 'detail': i[1]})
...
def analysisCalcSimilarDays():
    try:
        headers = {'content-type': 'application/json'}
        data = request.get_json()
        rt = requests.post(app.config.get('ALGOSVC1_ADDRESS') + '/correlationanalysis/analysis/similarday', data=json.dumps(data), headers=headers)
    except Exception as e:
