...
;(function (exports) {
    class Category {
        ...
        init() {
            const headHtml = `<div class="itemHead"><span class="headIcon"><span class="out"></span><span class="in"></span></span><span class="headText">${I18n.resource.nav.EQUIPMENTS}</span></div><div class="itemList"></div>`;
        ...
        bindStateOb() {
            this.diagnosis.conditionModel.addEventListener('update.activeCategories',this.updateActiveCategories,this);
        }
        unbindStateOb() {
            this.diagnosis.conditionModel.removeEventListener('update.activeCategories',this.updateActiveCategories,this);
        }
        updateActiveCategories() {
            ...
            activeCategories.forEach(v=>{
                ...
                $(`.item[data-id='${v.className}']`,this.container).addClass('selected');
            });
        }
        getData() {
            let time = this.diagnosis.conditionModel.time();
            return $.get('/diagnosis_v2/getEquipmentAvailability',{projectId: AppConfig.projectId, startTime: time.startTime, endTime: time.endTime}).done((rs)=>{
                if(rs&&rs.status=='OK'){
        ...
        createItem(data = []) {
            ...
            data.forEach((v)=>{
                let errorClassName = '';
                let intactRate = (v.goodNum/v.totalNum*100).toFixed(0);
                if(intactRate<60){
