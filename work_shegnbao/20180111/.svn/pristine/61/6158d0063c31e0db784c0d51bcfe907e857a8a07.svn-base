...
class Utils:
    ...
    class EmailTool:
        ...
        def send_invitation_email_with_pwd(
            ...
            if management_id:
                ...
                if management_info:
                    if management_info.get('phone'):
                        phone = management_info.get('phone')
                    else:
                        phone = app.config.get('BEOP_PHONE')
                    if language == 'zh':
                        name_key = 'name_cn'
        ...
        def send_email(subject, recipients, html, sender=None, attachment_list=None,
                       cc=None, bcc=None, countryCode='WW', strategy='v1', trackingId=None):
            """
            发送邮件
            :param subject: 邮件主题
            ...
            :param trackingId: 用于在日志中跟踪邮件原发地的标识符
            :return True 发送成功; False 发送失败
            """
            ...
            try:
                ...
                va.update({'type': 'email', 'subject': subject, 'recipients': recipients, 'msgId': ObjectId().__str__(),
                           ...
                           'strategy': strategy, 'trackingId': trackingId})
    ...
    @staticmethod
    def file_response_wrapper(function):
        @functools.wraps(function)
        def wrapper(*args, **kwargs):
            try:
                tmp_file = tempfile.NamedTemporaryFile(delete=False)
                function(tmp_file=tmp_file, *args, **kwargs)
    ...
    def post_json(url, data=None, **kwargs):
        ...
        try:
            ...
            if len(post_data) > max_log_size:
                logging.info('Calling %s with %s...', url, post_data[:max_log_size])
            else:
                logging.info('Calling %s with %s...', url, post_data)
            headers = {'token': app.config.get('TOKEN_WHITE_LIST')[0], "content-type": "application/json"}
            response = requests.post(url=url, headers=headers, data=post_data, timeout=timeout)
            if response.status_code == 200:
                logging.info('Result: %s', response.text)
                response_json = json.loads(response.text)
                return response_json
            elif log_error:
                logging.error('Unexpected result: %s %s', response.status_code, response.text)
            else:
                pass
        except Exception as e:
            if log_error:
                logging.error('Unhandled exception! %s', locals(), exc_info=True, stack_info=True)
            if raise_error:
                raise Exception('Failed to post JSON request to beopweb!')
            else:
                return {'success': False, 'msg': e.__str__()}

    @staticmethod
    def parse_ds_item_id(ds_item_id):
        """
        Parse data source item id into project ID and point name
        :param ds_item_id: Data source item id in format @<PROJID>|<POINTNAME>
        :return: (PROJID, POINTNAME)
        """
        if ds_item_id is None:
            raise Exception('ds_item_id cannot be None')
        if not isinstance(ds_item_id, str):
            raise Exception('ds_item_id is not a string, but a %s' % type(ds_item_id))
        if ds_item_id[0] != '@':
            raise Exception('ds_item_id %s does not start with @!' % ds_item_id)
        parts = ds_item_id[1:].rsplit('|', 1)
        if len(parts) < 2:
            raise Exception('Invalid ds_item_id: %s' % ds_item_id)
