        ...
        this.attachEvents = function () {
            ...
            this.$btnSaveAndExit.off('click').on('click', function () {
                if(typeof _this.callback === 'function'){
                    var rs = _this.callback(_this.getCode());
            ...
            //图片管理器按钮事件
            this.btnImg.off('click').on('click', function () {
                MaterialModal.show([{ data: ['Image'], title: 'Template' }], function (data) {
                    var copyBox = '<div class="modal fade" id="copyModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">\
                        <div class="modal-dialog">\
                            <div class="modal-content">\
                                <div class="modal-header" style="height:32px; padding: 5px 5px 0 0;">\
                                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>\
                                </div>\
                                <div class="modal-body">\
                                    <div class="imgBox" style="width:100%;">\
                                        <image style="width:100%;" src="' + data.url + '" />\
                                    </div>\
                                    <input id="copyStr" value ="' + data.url + '" style="width:100%;" readonly="readonly"/>\
                                </div>\
                    ...
                    $('body').append($(copyBox));
                    var $copyBox = $('#copyModal');
                    var $btnCopy = $('.btn-primary', $copyBox);
                    $copyBox.on('show.bs.modal', function () {
                        $btnCopy.off('click').on('click', function () {
                            $('#copyStr', $copyBox)[0].select();
                            var isSuccess = window.document.execCommand('copy');
                            if (isSuccess) {
                                alert(I18n.resource.report.chartConfig.COPY_IMAGE_SUCCESS);
                            } else {
                        ...
                        keyboard: false
                    }).modal('show').css('z-index', 2001);
                });
                $('#materialModal').css('z-index', 2000);
            });
        };
