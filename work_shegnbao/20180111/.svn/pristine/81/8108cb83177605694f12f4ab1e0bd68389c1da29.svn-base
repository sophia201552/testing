...
class Task:
    ...
    def get_task(self):
        ...
        if process and process.get('nodes'):
            ...
            for node_index, node in enumerate(process.get('nodes')):
                try:
                    if task.get('node_index') is not None and node_index >= task.get('node_index') and raw_process:
            ...
            try:
                ...
                if action == TaskAction.NO_PASS:  # 不通过情况
                    ...
                    if next_node:  # 流程没有结束
                        ...
                        email_type = 'PASS'
                        self.make_diagnosis_feedback(task, FeedBackStatus.DONE)
                TransactionEmail.send_task_email(task_id, email_option, task.get('attachment'), email_type=email_type)
            except Exception as e:
    ...
    def add_comment_by_task_id(self, task_id, user_id, content):
        ...
        self.db.find_one_and_update({"_id": task_id}, {
            "$push": {"comment": {"id": ObjectId(), "userId": user_id, "content": content, "time": datetime.now()}}
        }, upsert=True)
        return True
