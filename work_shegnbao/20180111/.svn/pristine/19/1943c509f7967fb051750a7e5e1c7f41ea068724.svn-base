...
class EnergyProjectStandard {
 constructor(opt) {
    ...
    init() {
        var _this = this;
        WebAPI.get('/static/app/EnergyManagement/views/module/energyProjectStandard.html').done(function (result) {
        ...
        var option = {
            tooltip: {
                ...
                formatter: function (params) {
                    ...
                    if (l > 1) {
                        str += I18n.resource.energyManagement.projectBenchmark.TOTAL + ' : ' + total.toFixed(2);
                    }
...
/*        !isDetail && echart.on('click', (params) => {
        ...
        function getNormalSeries(){
            var dataType = {
                'water': [I18n.resource.energyManagement.projectBenchmark.WATER, '#45ABFF'],
                'elec': [I18n.resource.energyManagement.projectBenchmark.ELECTRICITY, '#71D360'],
                'gas': [I18n.resource.energyManagement.projectBenchmark.GAS, '#FFD428']
            };
            var arr = [];
            var typeSeries = {};
