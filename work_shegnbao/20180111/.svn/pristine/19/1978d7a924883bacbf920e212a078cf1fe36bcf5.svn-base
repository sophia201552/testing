...
(function(root, factory) {
    ...
    class SketchpadChild {
        ...
        constructor(opt, type = 'modular', parentId = '') {
            this.opt = opt; //原始数据
            this.id = (type == 'input' || type == 'output' || type == 'group')? parentId : opt['_id'];
            this.type = type; //类型(策略模板/输入参数/输出参数)
            this.parents = []; //输出参数
            this.parent = undefined; //策略模板(作为输入参数)
            this.parentMap = new Map();//策略模板数组(作为输入参数) 为兼容一个输出可以同时作为多个模块的输入
            this.children = []; //输入参数
        ...
        addParents(parents) {
            let ids = new Set(this.parents.map(parent => parent.id));
        ...
        setNextSlibing(sibling) {
            this.nextSibling = sibling;
            sibling.prevSibling = this;
        }

        addChildren(children) {
            let ids = new Set(this.children.map(child => child.id));
        ...
        setLoc(loc) {
            Object.assign(this.opt.loc, loc);
        }
        ...
        hide() {
            this.isHide = true;
            this.sketchpad._createObstacle();
        }
        ...
        destroy() {
            ...
            switch (block.type) {
                ...
                case 'modular':
                    ...
                    let outputNames = block.parents.map(output => output.opt.name);
                    outputNames.forEach(outputName => {
                        let inputs = this.sketchpad.find({ 'opt.name': outputName, 'child.id': block.id });
                        inputs.forEach(input => {
                            for (let value of input.parentMap.values()) {
                                let i = value.children.findIndex(child => child.id == input.id);
                                value.children.splice(i, 1);
                                let v = value.children.find(child => child.id == input.id);
                                v&&v.remove();
                            }
        ...
        toBeInput(parentId) {
            ...
            // if (this.child.parents.length == 1) {
            //     this.child.setNextSlibing(parent);
            //     this.hide();
            // }
        ...
        getOneLines() {
            return this.sketchpad.getOneLines(this);
        }      
    ...
    class Sketchpad {
        constructor(props) {
            ...
            this.INPUTWIDTH = 132;
            this.INPUTHEIGHT = 40;
            this.MODULEWIDTH = 155;
            this.MODULEHEIGHT = 72;
            this.GROUPWIDTH = 170;
            this.GROUPHEIGHT = 60;
            this.maxStep = 1000;
            this.tools = {
        ...
        static intersectionByPort(points = [], blocks = []) { //区域内是否有障碍 intersectionByRect速度优化版 只返回一个障碍
            let tag = false;
            let r = undefined;
            blocks.forEach((block) => {
                if(tag){
                ...
                block.points.forEach((point) => {
                    ...
                    let hasIntersection = Sketchpad.intersectionByPoint(point, { points: points });
                    if (hasIntersection) {
                        tag = true;
                        r = {
                            minX:block.minX,
                ...
                points.forEach((point) => {
                    ...
                    let hasIntersection = Sketchpad.intersectionByPoint(point, block);
                    if (hasIntersection) {
                        tag = true;
                        r = {
                            minX:block.minX,
        ...
        resize(props) {
            // this.clear();
            const {width = this.width, height = this.height, scale = this.scale, layer = this.layer} = props;
        ...
        add(blocks = []) { //添加画板元素
            ...
            blocks.forEach((block) => {
                let idArr = block['id'].split('_');
                let cognateId = idArr[1];
                if(idArr.length>2){
                    cognateId += ('_'+idArr[2]);
                }
                let cognateBlock;
        ...
        repaintOneLine(child) { //重新计算
            let id = child.id,
                type = child.type,
                block,
                oneLine,
                idArr = id.split('_'),
                line = this.lines.find(o=>o.id==(idArr.length<3?idArr[1]:(idArr[1]+'_'+idArr[2])));
            this._createObstacle();
            
            let dtIn = ()=>{

            };
            let ctIn = (block, child, oneLine, id, line)=>{
                child.line = [{ //起始点
                ...
                if (points.length > 0) {
                    points.push(block.opt.loc.x - 1, block.opt.loc.y + block.opt.loc.h / 2); //入口
                    points.push(block.inPoint.x + this.PADDING / 2, block.inPoint.y - this.PADDING / 2, block.inPoint.x + this.PADDING - 1, block.inPoint.y, block.inPoint.x + this.PADDING / 2, block.inPoint.y + this.PADDING / 2);
                }
                if(oneLine){
                    oneLine.points = points;
                }else{
                    line.lines.push({
                        points: points,
                        id: 'inline_'+id
            ...
            let ctOut = (block, parent, oneLine, id, line)=>{
                parent.inLine = [{
            ...
            if(child.child && child.parent){
                child.parentMap.forEach(parent=>{
                    let line = this.lines.find(o=>o.id==parent.opt._id);
                    oneLine = line.lines.find(o=>o.id=='inline_'+child.opt._id+'_'+parent.opt._id);
                    ctIn(parent, child, oneLine, id, line);
                });
            }
            if(child.opt.refOutputId){
                let c = this.find({
                    'opt._id': child.opt.refOutputId
                })[0];
                let line = this.lines.find(o=>o.id==child.id.split('_')[1]);
                oneLine = line.lines.find(o=>o.id=='inline_'+child.id);
                ctIn(c.parent, child, oneLine, id, line);
            }
            switch(type){
                case 'input':
                    oneLine = line.lines.find(o=>o.id=='inline_'+id);
                    ctIn(child.parent, child, oneLine, id, line);
                    break;
                case 'output':
                    oneLine = line.lines.find(o=>o.id=='outline_'+id);
                    ctOut(child.child, child, oneLine, id, line);
                    break;
            }
        ...
        findBlockById(id, blocks) { //根据id查找策略模板
            return this._findBlockById(id, blocks).block;
        }

        findBlockIndexById(id, blocks) { //根据id查找策略模板在数组中的index
            return this._findBlockById(id, blocks).index;
        }
        ...
        createSortInfo(block, num = 0, type = 'input') {
            ...
            let _this = this;
            let offset = this.layer?this.layer.parent.offset():{x:0,y:0};
            let WIDTH,HEIGHT;
            switch(type){
                case 'input':
                case 'output':
                    ...
                    break;
                case 'group':
                    WIDTH = _this.GROUPWIDTH;
                    HEIGHT = _this.GROUPHEIGHT;
                    break;
            }
            let W = WIDTH + 2 * _this.PADDING, //输入/输出参数的宽(包含padding)
                H = HEIGHT + 2 * _this.PADDING, //输入/输出参数的高(包含padding)
                MINSPAN = _this.PADDING*2, //块与块之间排列的间隔
                MAXSPAN = (num - 1) * MINSPAN,
                maxW = W + MINSPAN,
                maxH = H + MINSPAN;
            
            let funArr;
            switch (type) { //根据类型选择方向优先级
                case 'input':
                    funArr = [left];
                    break;
                case 'output':
            ...
            function left(span){
                ...
                h = num * H + MAXSPAN;
                w = maxW;
                x = block.minX - maxW - span;
                y = block.minY - (h - block.maxH) / 2;
                points = Sketchpad.rectToPoints(x, y, w, h);
                intersection = Sketchpad.intersectionByPort(points, _this.obstacle);
                y = y + _this.PADDING;
                if (intersection == undefined) {
                    let finArr = [];
                    for (let i = 0; i < num; i++) {
                        finArr.push({ x: x, y: y + i * maxH, w: WIDTH, h: HEIGHT });
                    }
                    return finArr;
                } else {
                    return left(block.minX - intersection.minX + 1);
                }
            }
            ...
            function right(span){
                ...
                if (intersection == undefined) {
                    ...
                    return right(intersection.maxX - block.maxX + 1);
                }
            }

            // function left(span) {
            //     h = num * H + MAXSPAN;
            //     // if (block.minX - offset.x > maxW + span && h <= _this.HEIGHT) {
            //         w = maxW;
            //         x = block.minX - span;
            //         y = block.minY - (h - block.maxH) / 2;
            //         // y = Math.min(y, (_this.HEIGHT - h));
            //         points = Sketchpad.rectToPoints(x, y, w, h);
            ...
            // function top(span) {
            //     w = num * W + MAXSPAN;
            //     if (block.minY - offset.y > maxH + span && w <= _this.WIDTH) {
            //         h = maxH;
            //         x = block.minX - (w - block.maxW) / 2;
            //         y = block.minY - span;
            //         x = Math.min(x, (_this.WIDTH - w));
            //         points = Sketchpad.rectToPoints(x, y, w, h);
            //         intersection = Sketchpad.intersectionByRect(points, _this.obstacle);
            //         x = x + _this.PADDING;
            //         if (intersection.length < 1) {
            ...
            //     w = num * W + MAXSPAN;
            //     if (_this.HEIGHT > block.maxY - offset.y + maxH + span && w <= _this.WIDTH) {
            //         h = maxH;
            //         x = block.minX - (w - block.maxW) / 2;
            //         y = block.maxY + span;
            //         x = Math.min(x, (_this.WIDTH - w));
            ...
            // function right(span) {
            //     h = num * H + MAXSPAN;
            //     // if (_this.WIDTH > block.maxX - offset.x + maxW + span && h <= _this.HEIGHT) {
            //         w = maxW;
            //         x = block.maxX + span;
            //         y = block.minY - (h - block.maxH) / 2;
            //         // y = Math.min(y, (_this.HEIGHT - h));
        ...
        _addBlock(block) { //添加策略模板
            let blockIds = new Set(this.blocks.map(block => block.id));
        ...
        _createObstacle() { //创建障碍物
            let obstacle = [],
                inputs = [],
                outputs = [];
            let create = function(blocks, idsSet = new Set()) {
                blocks.forEach((block) => {
                    if (!idsSet.has(block.id) && !block.isHide) { //隐藏的不能当做障碍物
                        obstacle.push(block);
                        idsSet.add(block.id);
                    }
                    if (block.children) {
                        inputs = inputs.concat(block.children);
                        create(block.children, idsSet); //添加输入参数
                    }
                    if (block.parents) {
                        outputs = outputs.concat(block.parents);
                        create(block.parents, idsSet); //添加输出参数
                    }
                });
            };
        ...
        _paintLine() {
            ...
            var arr1 = [];
            var _this = this;
            var isOK = function(num, num2) {
                num = num.toFixed(2);
                num2 = num2.toFixed(2);
                if (Math.abs(num - num2) <= 0.1) {
                    return true;
                } else {
                    return false;
                }
            ...
            this.blocks.forEach((block, i) => {
                ...
                block.children.forEach((child, index) => {
                    ...
                    for (let i = 0; i < child.line.length; i++) {
                        var data = this._step(child.line[i], block, block.inPoint);
                        // if (data && (!isOK(data.point.x, block.inPoint.x) || !isOK(data.point.y, block.inPoint.y))) {
                        //     child.line.push(data);
                        // } else {
                        //     child.line.push({
                        //         point: block.inPoint,
                ...
                //blocks之间的线
                if (block.nextSibling != undefined) {
                    block.line = [{ //起始点
                        point: block.outPoint,
                        targetPoint: block.nextSibling.inPoint
                    }];
                    for (let i = 0; i < block.line.length; i++) {
                        var data = this._step(block.line[i], block.nextSibling, block.nextSibling.inPoint);
                        if (data && (data.point.x !== block.nextSibling.inPoint.x || data.point.y != block.nextSibling.inPoint.y)) {
                            block.line.push(data);
                        } else {
                            block.line.push({
                                point: block.nextSibling.inPoint,
                                targetPoint: undefined
                            });
                            break;
                        }
                    ...
                    block.line.length > 0 && block.line.unshift({ //出口
                        point: {
                            x: block.opt.loc.x + block.opt.loc.w,
                            y: block.opt.loc.y + block.opt.loc.h / 2
                        },
                        targetPoint: block.outPoint
                    })
                    var points = [];
                    block.line.forEach((v) => {
                        points.push(v.point.x, v.point.y);
                    });
                    if (points.length > 0) {
                        points.push(block.nextSibling.opt.loc.x - 1, block.nextSibling.opt.loc.y + block.nextSibling.opt.loc.h / 2); //入口
                        //箭头
                        points.push(block.nextSibling.inPoint.x + this.PADDING / 2, block.nextSibling.inPoint.y - this.PADDING / 2, block.nextSibling.inPoint.x + this.PADDING - 1, block.nextSibling.inPoint.y, block.nextSibling.inPoint.x + this.PADDING / 2, block.nextSibling.inPoint.y + this.PADDING / 2);
                    }
                    arr2 = [{ id: 'siblingline_' + block.id + '_' + block.nextSibling.id, points: points }];
                    arr1.push({ id: block.id + '_' + block.nextSibling.id, color: color, lines: arr2 });
                }
            });
            return arr1;
        }
