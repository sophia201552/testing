...
var PatrolReport = (function(){
    ...
    PatrolReport.prototype.show = function(){
        ...
        WebAPI.get('/static/app/Patrol/views/patrolReport.html').done(function(resultHtml) {
            ...
            $btnTaskReport.on('click',function(){
                _this.reportType = "task";
                _this.init();
            });
            $btnPeopleReport.on('click',function(){
                _this.reportType = "people";
                _this.peopleInit();
            });
            //日期控件前进按钮
            ...
            //报表下载
            $("#btnReportDownload").off("click").on("click",function(){
                var opt,loadData,startTime,endTime;
                if(_this.loadType === "week"){
                    var inputTime = new Date($inputPatrolTime.val());
                    startTime = _this.screen.getMonday(inputTime);
                    endTime = _this.screen.getSunday(inputTime);
                    loadData = _this.reportData;  
                    opt = {
                        startTime: startTime,
                        endTime: endTime,
                        data: loadData,
                        type: _this.reportType
                    }
                    _this.getDownloadReportExcel(opt);                 
                }else{
                    startTime = $inputPatrolTime.val().slice(0,-2) + "01";
                    var lastDay = (new Date($inputPatrolTime.val().split("-")[0],$inputPatrolTime.val().split("-")[1],0)).getDate();
                    endTime = $inputPatrolTime.val().slice(0,-2) + lastDay;
                    var url;
                    if(_this.reportType === "task"){
                        url = '/patrol/report/getBaseOnMission/';
                    }else{
                        url = '/patrol/report/getBaseOnMan/';
                    }
                    WebAPI.get(url + AppConfig.projectId + '/'+ startTime + '/' + endTime).done(function(result){
                        opt = {
                            startTime: startTime,
                            endTime: endTime,
                            data: _this.reportType === "task"?result:result.data,
                            type: _this.reportType
                        }
                        _this.getDownloadReportExcel(opt); 
                    })
            ...
            //下载报表的类型切换
            $("#reportTypeDownload").off("change").on("change",function(){
                _this.loadType = $(this).val();
            })
            //日期控件点击事件
    ...
    PatrolReport.prototype.getDownloadReportExcel = function(opt){
        var postData = {
        ...
        if(opt.type === "task"){
            ...
            postData.head = ["巡更人员","部门","完成率"];
        }
        postData.head = postData.head.concat(opt.data.time);
        var hasToday = false;
        if(Date.parse(new Date()) <= Date.parse(opt.endTime)){
            hasToday = true;
        }
        opt.data.data.forEach(function(row) {
            var item;
            if(opt.type === "task"){
                item = [row.name];               
            }else{
                item = [row.name,row.partment,row.percent|| "-"];
            }
            var lastday = row.data.length - 1;
            row.data.forEach(function(day,i){
                if(hasToday && i === lastday){
                    item.push("▃");
                }else{
        ...
        WebAPI.post("/logistics/export/excel/",postData).done(function(result){
            var aTag = document.createElement('a');
            aTag.download = '巡更报表-' + opt.startTime + '--' + opt.endTime + '.xls';
            aTag.href = result;
            ...
            for(var i = 0;i < result.data.length;i++){
                ...
                if(result.data[i].startTime) {
                    ...
                    for (var j = 0; j < result.data[i].data.length - 1; j++) {
                        ...
                        }else if(result.data[i].data[j].error === -2){
                        //不作处理
                        }else{
                            var Na = i;
                            break;
                    ...
                    }else if(result.data[i].data[result.data[i].data.length - 1].error === -2){
                        //不作处理
                    } else{
                        var Na = i;
                    }
