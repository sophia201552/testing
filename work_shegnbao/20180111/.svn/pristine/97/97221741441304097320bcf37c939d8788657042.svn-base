...
var CheckTimeOut = function() {
    return $.ajax({
        url: AppConfig.host + '/appTemperature/checkTimeOut',
        type: 'GET',
        timeout: 10000,
...
function setAppConfig() {
    ...
    if (typeof cordova != 'undefined') {
        ...
        AppConfig.host = location.origin;
    }
...
function setFileConfig() {
    FileAPI = new FileStorage('BeopTemperature');
}
...
var IndexScreen = (function() {
    ...
    IndexScreen.prototype = {
            ...
            CheckTimeOut().done(function() {
                ...
                WebAPI.post("/login", {
                    ...
                    if (navigator.connection && navigator.connection.type == 'none') {
                        ...
                        window.plugins && window.plugins.toast.show(I18n.resource.login.ERROR_1, 'short', 'center');
                        _this.loginLocal();
                    }
                    //localStorage.removeItem('userInfo');
                    //router.empty().to({typeClass:IndexScreen});
        ...
        loginLocal: function() {
            ...
            try {
                curRoom = JSON.parse(localStorage.getItem('lastOpenRoom'));
                AppConfig.roomId = curRoom._id
            } catch (e) {
                curRoom = null;
                AppConfig.roomId = null;
