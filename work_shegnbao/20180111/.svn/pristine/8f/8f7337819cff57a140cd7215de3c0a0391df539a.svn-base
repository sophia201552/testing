...
class PointTable:
    ...
    def import_data_to_db(self, data, source_type):
        ...
        try:
            for data_item in data:
                ...
                data_item['value'] = self.replace_invalid_chars_in_point_name(data_item.get('value'))
    ...
    def get_original_points_map(self, source_type, point_list):
        ...
        cursor = None
        try:
            cursor = self.db.find({'projId': self.project_id, 'type': source_type, 'params.mapping.point': {'$in': point_list}},
                                  projection={'groupId': False, 'type': False})
    ...
    @classmethod
    def is_conflict(self, ptName):
        apiList = self.get_api_name_list()
        if isinstance(ptName, list):
            f = lambda x: True if x in apiList else False
            return list(map(f, ptName))
        else:
    ...
    def edit_point(self, point_id, model):
        ...
        if int(model['params']['flag']) == 2:
            self.updateRtBufferCalcPointInfo(model.get('projId'), old_name, model.get('value'), model['params']['flag'])
        return self.db.update_one({'_id': ObjectId(point_id)}, {'$set': model})

    def updateRtBufferCalcPointInfo(self, projId, oldName, newName, flag):
        json_data = {
            'orginName': oldName,
            'projId': projId,
            "newName": newName,
    ...
    def replace_invalid_chars_in_point_name(self, point_name):
        if not point_name:
            return None
    ...
    def sync_cloud_points(self, user_id):
        ...
        for point in real_time_points:
            ...
            if not Utils.CLOUD_POINT_NAME_VALID.match(point_name):
                point_name_cloud = self.replace_invalid_chars_in_point_name(point_name_cloud)
    ...
    def find_calc_point(self, projId, pointname):
        ...
        try:
            ...
            for item in cursor:
                ...
                #     tempLine = []
                #     line = line.replace('\'', '"')
                #     tempLine = self.findSubString(tempLine, line, '"%s"' % pointname)
                #     if tempLine:
    ...
    def find_diag_point(self, projId, pointname):
        ...
        try:
            ...
            for item in cursor:
                ...
                # for index, line in enumerate(code):
                # tempLine = []
                # tempLine = self.findSubString(tempLine, line, pointname)
                # if tempLine:
                #     apiPos.update({index: tempLine})
                # count += len(tempLine)
                # code = '\n'.join(code)
    ...
    # 过滤掉未出现查询点的报表章节
    def filterFacReport(self, reportIds, springLayout, projId, pointList, pointname, reportName, pageId):
        rt = []
        total = 0
        projName = self.getProjNameCn(projId)
        for index, springLayout in enumerate(springLayout):
            points, jsList, htmlList = [], [], []
            ...
            if num:
                ...
                rt.append({
                    ...
                    'url': '/observer#page=FacReportWrapScreen&options=' + urllib.parse.quote(
                        '{"id":"%s"}' % str(pageId)) + '&container=' + urllib.parse.quote(
                        "indexMain") + '&projectId=%d' % (int(projId),)
                })
            '''
            self.getComplexContent(springLayout, points, jsList, htmlList)
            point, js, html = 0, 0, 0
            for p in pointList:
                if p in points:
                    point += 1
                js += ''.join(jsList).count(p)
                html += ''.join(htmlList).count(p)
            point += points.count(pointname)
            js += ''.join(jsList).count(pointname)
            html += ''.join(jsList).count(pointname)
            total = total + html + js + point
            if js or html or point:
                pageName = self.menu_info_db.find_one({"_id": ObjectId(str(pageId))}).get('text')
                rt.append({reportIds[index]: {'html': html, 'js': js, 'points': point}, 'reportName': "{}/{}/{}".format(projName, pageName, reportName[index])})
            '''

        return rt, total

    def getContentAndPoints(self, item, points, js, html):
        if item.get('html'):
    ...
    def getComplexContent(self, layout, points, js, html):  # TODO
        if isinstance(layout, dict):
            for k, v in layout.items():
                if isinstance(v, dict):
                    self.getContentAndPoints(v, points, js, html)
                if isinstance(v, list) or isinstance(v, dict):
                    self.getComplexContent(v, points, js, html)
        elif isinstance(layout, list):
            for item in layout:
                if isinstance(item, dict):
                    self.getContentAndPoints(item, points, js, html)
        ...
        for item in layout:
            if isinstance(item, list):
                for lay in item:
                    if lay.get('layout'):
                        #说明是最外层
                        self.getComplexContent(lay.get('layout'), L)
                    elif lay.get('modal', {}).get('option', {}):
                        #说明还有下层
                        if lay['modal']['option'].get('layouts'):
                            #继续遍历
                            son = lay['modal']['option'].get('layouts')
                            self.getComplexContent(son, L)
                        else:
                            L.append((lay['modal']['option'].get('html'), lay['modal']['option'].get('js')))
                    else:
                        L.append((lay.get('html'), lay.get('js')))
            else:
                lay = item
                if lay.get('layout'):
                    # 说明是最外层
                    ...
                    for lay in layout:
                        ...
                        if rv:
                            for index, modals in enumerate(lay):
                                ...
                                # numCloudHtml = sum(option.get('html').count("@%s|%s" % (projId, pointId)) for pointId in pointList) if option.get(
                                #     'html') else 0
                                numPoints = sum(
                                    modal.get('points').count(pointId) for pointId in pointList) if modal.get(
                                    'points') else 0
                                # numCloud = sum(modal.get('points').count("@%s|%s" % (projId, pointId)) for pointId in pointList) if modal.get(
                                #    'points') else 0
                                if numId or numName:
