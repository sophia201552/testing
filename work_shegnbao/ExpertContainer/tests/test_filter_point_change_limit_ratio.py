...
@pytest.mark.p0
@pytest.mark.parametrize(('projId','pointName','fLimitPerHour','acttime','expected'),[
...
def test_filter_point_change_limit_ratio(projId,pointName,fLimitPerHour,acttime,expected):
    lb = LogicBase(projId,acttime ,nMode=LogicBase.ONLINE_TEST_REALTIME)
    assert lb, "create instance failed"
    rt = lb.filter_point_change_limit_ratio(projId,pointName,fLimitPerHour)
