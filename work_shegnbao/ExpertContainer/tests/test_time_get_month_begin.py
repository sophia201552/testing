...
def test_get_time():
    ...
    assert lb.time_get_month_begin() == "2017-10-01 00:00:00",\
        "time_get_month_begin error:input time is not equal to instance time"
...
def test_get_time_with_str_wrong():
    ...
    lb = LogicBase(49, test_time_str)
    assert lb, "create instance LogicBase failed"
    assert lb.time_get_month_begin() == None,\
        "time_get_month_begin error:input wrong format of time is not equal to None"