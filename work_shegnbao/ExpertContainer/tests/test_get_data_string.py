...
@pytest.mark.parametrize(('projId', 'pointList', 'expectedList'),[
    (293,['Accum_ChGroup_GroupEnergyM', 'L11S1_VAVC2_DmprComd'], ['str', 'str']),
    (293,['Accum_ChGroup_GroupEnergyM', 'L11S1_VAVC2_DmprComd', 'MustBeNone'], ['str', 'str', 'NoneType']),
])
def test_get_data_string_real(projId, pointList, expectedList):
    lb = LogicBase(projId, datetime.now(), nMode = LogicBase.REALTIME)
    if lb:
        lb.before_actlogic()
        rt = lb.get_data_string(projId, pointList)
        lb.after_actlogic()
...
@pytest.mark.p0
@pytest.mark.parametrize(('projId', 'pointList', 'expectedList'),[
    (293,['Accum_ChGroup_GroupEnergyM', 'L11S1_VAVC2_DmprComd'], ['41067.41666666666', '100.0']),
    (293,['Accum_ChGroup_GroupEnergyM', 'L11S1_VAVC2_DmprComd', 'MustBeNone'], ['41067.41666666666', '100.0', None]),
])
def test_get_data_string_his(projId, pointList, expectedList):
