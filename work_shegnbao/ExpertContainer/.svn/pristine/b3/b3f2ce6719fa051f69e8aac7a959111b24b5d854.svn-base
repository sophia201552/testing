...
@pytest.mark.parametrize(('initTime', 'expectedTime'),[
    (test_time,test_time.strftime('%Y-%m-%d %H:00:00')),
    (test_time.strftime('%Y-%m-%d %H:%M:%S'),test_time.strftime('%Y-%m-%d %H:00:00')),
])
def test_get_time(initTime,expectedTime):
    lb = LogicBase(49, initTime)
    assert lb, "create instance LogicBase failed"
    assert lb.time_get_hour_begin() == expectedTime,\
...
def test_get_time_with_str_wrong():
    ...
    assert lb, "create instance LogicBase failed"
    assert lb.time_get_hour_begin() == None,\
        "time_get_hour_begin error:input wrong format of time is not equal to None"