...
def test_get_data_float_real(projId, pointList, expectedList):
    lb = LogicBase(projId, datetime.now(), nMode = LogicBase.REALTIME)
    if lb:
        lb.before_actlogic()
        rt = lb.get_data_float(projId, pointList)
        lb.after_actlogic()
...
def test_get_data_float_his(projId, pointList, expectedList):
