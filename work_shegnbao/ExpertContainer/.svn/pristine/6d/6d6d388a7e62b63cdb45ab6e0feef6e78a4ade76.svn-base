...
// 日志
var stream = fs.open('debug'+args[1]+'.log', 'w');
var result = fs.open('result'+args[1]+'.log', 'w');
// 计时器
var timer = null;
// 每英寸点数
var dpi = 100;
...
page.onConsoleMessage = function(msg) {
    if (msg === 'phantom - render summary complete') {
        ...
        page.evaluate(function(arrRequests, arrResponses) {
            console.log('1cookies:' + document.cookie);
            var beginEnable = false;
            var beginTime = new Date();
            var delayEnable = false;
    ...
    //else if (msg.length <= 50 || msg[0] == '1' || msg == 'convert echarts') {
...
page.open(host + '/login', settings_login, function(status) {
    page.evaluate(function() {
        localStorage.isUserSelectedLanguage = 'en';
    });//设置登录语言为英文
    if (status == 'success') {
