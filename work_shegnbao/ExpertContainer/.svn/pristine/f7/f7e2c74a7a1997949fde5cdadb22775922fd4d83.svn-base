...
def test_correct(projId, pointList, testTime, valueList, mode):
    ...
    rt = lb.set_data_algorithm(projId, pointList, valueList)
    assert rt, 'set_data_algorithm: return value is False'
    # 判断buffer数据库有没有更新
...
def test_wrong(projId, pointList, testTime, valueList, mode):
    ...
    assert lb, "create instance failed"
    rt = lb.set_data_algorithm(projId, pointList, valueList)
    assert not rt, 'set_data_algorithm: return value is not False'
